...
begin
   ...
   -- raise_application_error(-20000,v_user_id);
    ...
    commit;
     ...
     insert into nci_ds_rslt (hdr_id, item_id, ver_nr,  rule_desc)
                    ...
                    admin_item where regexp_replace(upper(item_nm),'\(|\)|\;|\-|\_',' ') = v_entty_nm and admin_item_typ_id = 53;
                    commit;
   ...
   /* insert into nci_ds_rslt (hdr_id, item_id, ver_nr,  rule_desc)
     ...
     insert into nci_ds_rslt (hdr_id, item_id, ver_nr,  rule_desc)
                    ...
                    vw_cncpt where regexp_replace(upper(item_nm),'\(|\)|\;|\-|\_',' ') = v_entty_nm and CNTXT_NM_DN = 'NCIP'
     ...
     insert into nci_ds_rslt (hdr_id, item_id, ver_nr,  rule_desc)
                    ...
                    admin_item where regexp_replace(upper(item_nm),'\(|\)|\;|\-|\_',' ') like '%' || v_entty_nm || '%'  and admin_item_typ_id = 53
                    and v_hdr_id not in (select hdr_id from nci_ds_rslt);
                    commit;
                    
                    insert into nci_ds_rslt (hdr_id, item_id, ver_nr,  rule_desc)
                    select distinct  v_hdr_id , item_id, ver_nr, 'Concept Like Match' from 
                    vw_cncpt where regexp_replace(upper(item_nm),'\(|\)|\;|\-|\_',' ') like '%' ||  v_entty_nm || '%' and CNTXT_NM_DN = 'NCIP'
                    and v_hdr_id not in (select hdr_id from nci_ds_rslt);
                    commit;
        ...
        commit;
              ...
              v_sql := ' insert into nci_ds_rslt_dtl (hdr_id, item_id, ver_nr, perm_val_nm,  rule_id, score, rule_desc)
                ...
                execute immediate v_sql;
                    ...
                    v_sql := ' insert into nci_ds_rslt_dtl (hdr_id, item_id, ver_nr,  perm_val_nm, rule_id, score, rule_desc)
                    select distinct ' ||  cur.hdr_id || ', r.item_id, r.ver_nr, ''NA'', 6, 100, ''Alternate Name Like Match'' from alt_nms r, vw_de de
                    where (instr(upper(r.nm_desc), ' || v_entty_nm || ',1) > 0)
                    and de.item_id = r.item_id and de.ver_nr = r.ver_nr and de.currnt_ver_ind = 1 and de.val_dom_typ_id = 17
                    and (' || cur.hdr_id || ', r.item_id, r.ver_nr) not  in (select hdr_id, item_id, ver_nr from nci_ds_rslt_dtl) and ' || v_flt_str;
                     execute immediate v_sql;
