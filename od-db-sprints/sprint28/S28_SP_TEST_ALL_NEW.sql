...
((select count(*) from onedata_wa.NCI_ADMIN_ITEM_REL_ALT_KEY where rel_typ_id = 63)-(select count(*) from sbrext.quest_contents_ext where qtl_name = 'QUESTION' and P_MOD_IDSEQ is not null)) DIFF_CNT,
(select count(*) from sbrext.quest_contents_ext where qtl_name = 'QUESTION' and P_MOD_IDSEQ is not null) caDSR_CNT,NULL
from dual;
commit;
...
((select count(*) from onedata_wa.NCI_QUEST_VALID_VALUE )-(select count(*) from sbrext.quest_contents_ext where qtl_name = 'VALID_VALUE' and P_QST_IDSEQ is not null)) DIFF_CNT,
(select count(*) from sbrext.quest_contents_ext where qtl_name = 'VALID_VALUE' and P_QST_IDSEQ is not null) caDSR_CNT,NULL
from dual;
commit;
