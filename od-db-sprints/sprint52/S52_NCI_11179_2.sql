...
if (upper(hookinput.originalrowset.tablename) like '%CART%') then
    ...
    for i in 1 .. hookInput.originalRowset.Rowset.count loop
        ...
        v_tab_admin_item_nm.extend();
         select decode(ai.VAL_DOM_TYP_ID,17,'Enumerated','Non-enumerated') || ' VD: ' || ai.item_id || '-' || ai.item_nm into  vd_id_nm from vw_value_dom ai, de where ai.item_id = de.val_dom_item_id and ai.ver_nr = de.val_dom_ver_nr and de.item_id =ihook.getColumnValue(row_cur,'ITEM_ID')
        and de.ver_nr = ihook.getColumnValue(row_cur,'VER_NR');
   ...
   --     v_tab_admin_item_nm(v_tab_admin_item_nm.count) := 'CDE:' || ihook.getColumnValue(row_cur,'ITEM_ID') || '-' || substr(ihook.getColumnValue(row_cur,'ITEM_NM'),1,75) || chr(13) || 'VD:' ||  substr(vd_id_nm,1,75) ;
        v_tab_admin_item_nm(v_tab_admin_item_nm.count) := 'CDE:' || ihook.getColumnValue(row_cur,'ITEM_ID') || '-' || ihook.getColumnValue(row_cur,'VER_NR') || '-' || substr(ihook.getColumnValue(row_cur,'ITEM_NM'),1,100) || chr(13) ||  substr(vd_id_nm,1,100) ;
     end if;
    end loop;
  end if;
  if (v_item_typ_id = 3) then --Value Domain
    ...
    for i in 1 .. hookInput.originalRowset.Rowset.count loop
     ...
     if (ihook.getColumnValue(row_cur, 'ADMIN_ITEM_TYP_ID') = v_item_typ_id) then
      ...
      v_tab_admin_item_nm.extend();
   select decode(ai.VAL_DOM_TYP_ID,17,'Enumerated','Non-enumerated') || ' VD: ' || ai.item_id || '-' ||  ihook.getColumnValue(row_cur,'VER_NR')  || '-' ||  ai.item_nm into  vd_id_nm from vw_value_dom ai
...
--raise_application_error(-20000,'Here');
    for i in 1 .. input_rows.count loop
      ...
      v_tab_admin_item_nm.extend();
   select decode(ai.VAL_DOM_TYP_ID,17,'Enumerated','Non-enumerated') || ' ' || ai.item_id || '-' ||  ihook.getColumnValue(row_cur,'VER_NR')  || '-' ||  ai.item_nm into  vd_id_nm from vw_value_dom ai
       where ai.item_id = ihook.getColumnValue(row_cur,'ITEM_ID') and ai.ver_nr = ihook.getColumnValue(row_cur,'VER_NR') ;
        ...
        for j in 1 .. input_rows.count loop
           row_cur := input_rows(j);
            ...
            if (v_src_alt_nm_typ is not null and v_src_alt_nm_typ > 0 and k=1) then
             for rec in   (select max(nm_desc) nm_Desc   from perm_val a, nci_admin_item_Ext ext, alt_nms an
            where nci_val_mean_item_id=ext.item_id  and nci_val_mean_ver_nr = ext.ver_nr and ext.cncpt_concat = v_tab_val_mean_cd(i)
            and a.val_dom_item_id=ihook.getColumnValue(row_cur,'ITEM_ID')
            and a.val_dom_ver_nr= ihook.getColumnValue(row_cur,'VER_NR') and a.fld_delete=0
            and an.nm_typ_id = v_src_alt_nm_typ and an.item_id =nci_val_mean_item_id and an.ver_nr = nci_val_mean_ver_nr) loop
         ...
         --      raise_application_error(-20000,rec.nm_desc);
            ...
            and a.val_dom_item_id=ihook.getColumnValue(row_cur,'ITEM_ID')
            and a.val_dom_ver_nr= ihook.getColumnValue(row_cur,'VER_NR') and a.fld_delete=0
            and an.nm_typ_id = v_tgt_alt_nm_typ and an.item_id =nci_val_mean_item_id and an.ver_nr = nci_val_mean_ver_nr) loop
