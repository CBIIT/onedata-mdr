...
AS
 ...
 --          raise_application_error(-20000, 'Here 1');
    ...
    IF HOOKINPUT.ANSWERID = 2 or Hookinput.answerid = 4 THEN  -- Validate using drop-down
      ...
      if (ihook.getColumnValue(rowvd, 'DTTYPE_ID') is not null )  then --- Tracker 667
           ihook.setColumnValue(rowvd, 'NCI_STD_DTTYPE_ID', nci_11179_2.getStdDataType(ihook.getColumnValue(rowvd, 'DTTYPE_ID')));
...
if (v_cncpt_src ='STRING') then
           ...
           --             j := i+1;
            ...
            if (i = cnt) then
                      j :=1;
                        ...
                        ihook.setColumnValue(rowform, 'CNCPT_' || idx  ||'_ITEM_ID_' || j,'');
                        ihook.setColumnValue(rowform, 'CNCPT_' || idx || '_VER_NR_' || j, '');
                        for cur in(select item_id, item_nm , item_long_nm, item_desc from admin_item where admin_item_typ_id = 49 and upper(item_long_nm) = upper(trim(v_cncpt_nm)) and admin_item.admin_stus_nm_dn = 'RELEASED') loop
                                ihook.setColumnValue(rowform, 'CNCPT_' || idx  ||'_ITEM_ID_' || j,cur.item_id);
                                ihook.setColumnValue(rowform, 'CNCPT_' || idx || '_VER_NR_' || j, 1);
                               -- v_dec_nm := trim(v_dec_nm || ' ' || cur.item_nm) ;
                                v_long_nm_suf := trim(v_long_nm_suf || ':' || cur.item_long_nm);
  ...
  if (v_invalid_concepts is not null) then
            ihook.setColumnValue(rowform, 'CTL_VAL_MSG', 'Warning: Invalid concept codes: ' ||  v_invalid_concepts);
            end if;
end if;
end;
END;
