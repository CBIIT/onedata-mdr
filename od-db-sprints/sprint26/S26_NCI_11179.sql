...
begin
    ...
    if hookInput.invocationNumber = 0 then  ---First invocation of the hook - prompt for user name
            ...
            ANSWERS.EXTEND;
            ANSWERS(ANSWERS.LAST) := ANSWER;
            QUESTION               := T_QUESTION('Specify a name that is easily remembered to store selections in your user cart.  Please use the same cart name each time you add items during your daily use.  Guest user carts and their contents are deleted at the end of the day.' , ANSWERS);
            HOOKOUTPUT.QUESTION    := QUESTION;
            forms                  := t_forms();
            form1                  := t_form('User Cart (Hook)', 2,1);
            forms.extend;
