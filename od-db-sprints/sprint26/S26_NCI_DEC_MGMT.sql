...
if (v_cncpt_src ='STRING') then
      if (ihook.getColumnValue(rowform, 'CNCPT_CONCAT_STR_' || idx) is not null) then
            ...
            ihook.setColumnValue(rowform, 'CTL_VAL_MSG', ihook.getColumnValue(rowform, 'CTL_VAL_MSG') ||  ' WARNING: Invalid Concepts: ' ||  v_invalid_concepts || ' Retired Concepts: ' || v_retired_concepts);
            end if;
end if;
