...
from admin_item rt, data_typ dt, value_dom vd , uom where vd.REP_CLS_ITEM_ID= rt.item_id and vd.REP_CLS_VER_NR = rt.ver_Nr and 
...
from admin_item rt, data_typ dt, value_dom vd, uom,
...
vd.item_id = pv.val_dom_item_id (+) and vd.ver_nr = pv.val_dom_ver_nr (+) and vd.uom_id = uom.uom_id (+))
...
decode(VAL_DOM_HIGH_VAL_NUM,null,'','/Max Val:' ||VAL_DOM_HIGH_VAL_NUM) || decode(VAL_DOM_LOW_VAL_NUM,null,'','/Min Val:' ||VAL_DOM_LOW_VAL_NUM  )
|| ' (' || nvl(pv.pv_cnt,0) || ' Values)',1,255)
from admin_item rt, data_typ dt, value_dom vd, uom, vw_val_dom_ref_term vdrt,
