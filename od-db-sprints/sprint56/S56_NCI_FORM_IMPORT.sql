...
begin
  ...
  row := t_row();
       ...
       -- raise_application_error(-20000,cur.src_quest_typ);
            ...
            elsif (LOWER(cur.src_question_id) = 'footer') then
  ...
  ihook.setColumnValue(row, 'ADMIN_STUS_ID', 66);
    ...
    ihook.setColumnValue(row, 'CNTXT_VER_NR', ihook.getColumnValue(row_ori,'CNTXT_VER_NR'));
      ...
      -- jira 2906: create new, empty module when field type = descriptive
      if (LOWER(cur.src_quest_typ) = 'descriptive' and (lower(cur.src_question_id) <> 'footer') and cur.BTCH_SEQ_NBR <> 1) then
       -- raise_application_error(-20000,cur.src_quest_typ);
         /*   if (cur.BTCH_SEQ_NBR = 1) then
            f_row := t_row();
            ihook.setColumnValue(f_row, 'ITEM_ID', v_form_id);
            ihook.setColumnValue(f_row, 'CURRNT_VER_IND', 1);
            ihook.setColumnValue(f_row, 'HDR_INSTR', cur.SRC_QUEST_LBL);
            f_rows:= t_rows();
            f_rows.extend;
            f_rows(f_rows.last) := f_row;
                action             := t_actionrowset(f_rows, 'Form AI', 2, 1,'update');
                actions.extend;
                actions(actions.last) := action;
        --    v_desc_txt_cnt := v_desc_txt_cnt + 1;
            v_disp_ord := v_disp_ord + 1;
        --    raise_application_error(-20000,v_desc_txt_cnt);
        elsif (LOWER(cur.src_question_id) = 'footer') then
            f_row := t_row();
            ihook.setColumnValue(f_row, 'ITEM_ID', v_form_id);
            ihook.setColumnValue(f_row, 'CURRNT_VER_IND', 1);
            ihook.setColumnValue(f_row, 'FTR_INSTR', cur.SRC_QUEST_LBL);
            f_rows:= t_rows();
            f_rows.extend;
            f_rows(f_rows.last) := f_row;
                action             := t_actionrowset(f_rows, 'Form AI', 2, 1,'update');
        ...
        if (LOWER(cur.src_question_id) like 'image%') then
            ihook.setColumnValue(row, 'ITEM_NM', 'Image');
            ihook.setColumnValue(row, 'INSTR', cur.SRC_QUEST_LBL);
