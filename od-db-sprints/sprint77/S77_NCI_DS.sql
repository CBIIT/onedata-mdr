create or replace PACKAGE            nci_ds AS
  ...
  procedure spCDEAIRun ( v_data_in in clob, v_data_out out clob, v_user_id in varchar2);
...
function getDSSysMsg (v_hdr_id in number, v_op in varchar2, v_mtch_lmt in number) return varchar2;
...
procedure spBatchDelete (v_data_in in clob, v_data_out out clob, v_usr_id in varchar2);
...
procedure spDSRunTest (v_data_in in clob, v_data_out out clob, v_user_id in varchar2);
...
procedure spDSPostMatch(v_hdr_id in number, v_user_id in varchar2,v_src_val_id in number, v_mtch_lmt in number);
...
procedure spDSMatchFlow(v_hdr_id in number, v_user_id in varchar2, v_mtch_nm in varchar2, v_filter_str in varchar2, v_src_val_id in number, v_mtch_typ in varchar2, v_mtch_lmt in number, v_cncpt_ind in number);
...
function getDSExecutionMsg return t_question;
function getDSMtchLmt (v_hdr_id in number) return number;
function setprefCDEIndicator (v_hdr_id in number, v_cde_id in number, v_ver_nr in number) return varchar2;
...
procedure spVMMatchBulk (v_data_in in clob, v_data_out out clob, v_user_id in varchar2);
function getVMConcMatchForm (v_rowset in t_rowset) return t_forms;
function getQuestionVMConcParameters return t_question;
END;
/
...
--    ANSWERS(ANSWERS.LAST) := ANSWER;
    ...
    QUESTION               := T_QUESTION('(OPTIONAL) Specify Match parameters below, then select ''Run Match''. <BR><BR> <i>Matches will only include selected values.</i><BR><i>TEST and Training contexts, Retired WFS excluded from all results by default unless selected below.</i>' , ANSWERS);

return question;
end;


function getQuestionVMConcParameters return t_question is
  question t_question;
  answer t_answer;
  answers t_answers;
begin
    ...
    QUESTION               := T_QUESTION('(OPTIONAL) Specify Match parameters below, then select ''Run Match''. <BR><BR> <i>Entire Batch will be processed.<BR>If unspecified, matching will default to <strong>Unrestricted</strong> and <strong>All VMs/Concepts</strong>.</i>' , ANSWERS);

return question;
end;

function getMatchConfirmation return t_question is
  question t_question;
  answer t_answer;
  answers t_answers;
begin
    ...
    ANSWERS                    := T_ANSWERS();
    ANSWER                     := T_ANSWER(1, 1, '<button onclick="window.close()">Close This Window</button>');
    ANSWERS.EXTEND;
    ANSWERS(ANSWERS.LAST) := ANSWER;
...
--    ANSWERS(ANSWERS.LAST) := ANSWER;
    ...
    QUESTION               := T_QUESTION('Your match is running. You may close this window.' , ANSWERS);

return question;
end;

function getDSExecutionMsg return t_question is
  question t_question;
  answer t_answer;
  answers t_answers;
begin
    ...
    ANSWERS                    := T_ANSWERS();
     ANSWER                     := T_ANSWER(1, 1, 'Test');
    ANSWERS.EXTEND;
    ANSWERS(ANSWERS.LAST) := ANSWER;

    QUESTION               := T_QUESTION('Your match is running!', ANSWERS);

return question;
end;

function getDSParameterEditForm (v_rowset in t_rowset, v_mode in varchar2) return t_forms is
  forms t_forms;
  form1 t_form;
  x_rowset t_rowset;
  row t_row;
...
begin
    ...
    elsif v_mode = 'IS' then
        form1                  := t_form('DS Entity to Match Parameters IS (for popup)', 2,1);
    else
        form1                   := t_form('DS Entity to Match Parameters Batch', 2,1);
    end if;

    row := v_rowset.rowset(1);
    rows := t_rows();
    ihook.setColumnValue(row,'PRMTR_ID',0);
    ihook.setColumnValue(row,'MTCH_LMT',10);
    ihook.setColumnValue(row,'SRC_VAL_ID',247);
    ihook.setColumnValue(row,'HDR_ID',194361);

      rows := t_rows(); rows.extend;          rows(rows.last) := row;
                 x_rowset := t_rowset(rows, 'DS Entity to Match Parameters', 1, 'NCI_DS_PRMTR');
    form1.rowset           := x_rowset;
    forms.extend;    forms(forms.last) := form1;
  return forms;
end;

function getVMConcMatchForm (v_rowset in t_rowset) return t_forms is
  forms t_forms;
  form1 t_form;
begin
    forms                  := t_forms();
    form1                  := t_form('VM/Concept Match Bulk Load (for popup)', 2,1);
    form1.rowset           := v_rowset;
    forms.extend;    forms(forms.last) := form1;
  return forms;
end;

function getCDEAIParameterEditForm (v_rowset in t_rowset) return t_forms is
  forms t_forms;
  form1 t_form;
begin
    forms                  := t_forms();
...
begin
    ...
    if (v_admin_stus_id <> 0) then
        ...
        v_out_str := v_out_str || ' and de.ADMIN_STUS_ID not in (77,81,95,96,80)' ;
    end if;
...
--set preferred cde if it is only match and it is exact
  if (v_mtch_cnt = 1) then
    ...
    if (v_str like '%Exact Match%') then
        ...
        commit;
        update nci_ds_rslt set CDE_PREF_IND = 1 where item_id = tmp_item_id and ver_nr = tmp_ver_nr and hdr_id = v_hdr_id;
        commit;
    end if;
  end if;

    --set pref cde indicator
    select count(*) into v_temp from nci_ds_hdr where hdr_id = v_hdr_id and cde_item_id is not null;
    if (v_temp = 1) then
        select cde_item_id, cde_ver_nr into tmp_item_id, tmp_ver_nr from nci_ds_hdr where hdr_id = v_hdr_id;
        v_pref_msg := setprefCDEIndicator(v_hdr_id, tmp_item_id, tmp_ver_nr);
    end if;
--update system message
    v_sys_msg := getDSSysMsg(v_hdr_id, v_mode, v_mtch_lmt);
...
BEGIN
    --shared conditional statement for all dynamic queries, some conditions moved to getDSFilterString
    --v_univ_cond := 'de.currnt_ver_ind = 1 and de.ADMIN_STUS_NM_DN not like ''%RETIRED%'' and de.CNTXT_NM_DN not in (''TEST'', ''Training'') ';
   -- v_univ_cond := 'de.currnt_ver_ind = 1 and de.CNTXT_NM_DN not in (''TEST'', ''Training'') ';
    v_univ_cond := 'de.currnt_ver_ind = 1 ';
...
BEGIN
    ...
    commit;
    --IS updates
    insert into onedata_ra.nci_ds_prmtr
    select * from nci_ds_prmtr wa where wa.prmtr_id not in (select ra.prmtr_id from onedata_ra.nci_ds_prmtr ra);
    commit;
...
END;
/*
spDSRunTest is currently used for IS. It should be renamed to spDSRun and the original spDSRun should be renamed to spDSRunTest. The original spDSRun is used for testing and should not be used in production.
*/
procedure spDSRunTest (v_data_in in clob, v_data_out out clob, v_user_id in varchar2)
AS
    hookInput t_hookInput;
...
begin
    ...
    if (hookinput.invocationnumber = 0) then   -- First invocation
        ...
        hookoutput.forms := getDSParameterEditForm(rowset, 'IS');
        ...
        --Set variables from form input
           ...
           -- v_cncpt_ind := nvl(ihook.getColumnValue(row_sel, 'CNCPT_IND'), 0);
            for j in 1..ihook.getmsColumnValue(row_sel,'MS_CNTXT').count loop
                v_temp_str := REGEXP_SUBSTR(ihook.getmsColumnValue(row_sel, 'MS_CNTXT')(j), '[^v]+', 1, 1);
                select cntxt_nm_dn into v_temp_str from vw_cntxt_ds where item_id = v_temp_str;
                v_cntxt_str := v_temp_str || ',' || v_cntxt_str;
                v_cnt := v_cnt + 1;
            end loop;
            v_cntxt_str := rtrim(v_cntxt_str, ',');
  --          v_error_msg := v_cntxt_str || ' ' || v_error_msg;
--raise_application_error(-20001, v_cntxt_str);
        --Use Variables to Build Filter String
          --  v_flt_str := getDSFilterString(nvl(v_reg_stus_id,0), nvl(v_admin_stus_id,0), nvl(v_cntxt_item_id,0), nvl(v_cntxt_ver_nr,0), nvl(v_csi_item_id,0), nvl(v_csi_ver_nr,0), nvl(v_val_dom_typ,0));

        --Loop through all rows and insert into new object that triggers IS service on insert
            ...
            --set row values for insert
            select seq_ds_prmtr_id.nextval into v_id from dual;
                ihook.setColumnValue(row, 'PRMTR_ID', v_id);
                ihook.setColumnValue(row, 'HDR_ID', v_hdr_id);
                --raise_application_error(-20001, ihook.getColumnValue(row, 'HDR_ID'));
                ihook.setColumnValue(row, 'ENTTY_NM', v_entty_nm);
                ihook.setColumnValue(row, 'SRC_VAL_ID', v_src_val_id);
               -- if nvl(v_cntxt_str,'') <> '' then
                    ihook.setColumnValue(row, 'MS_CNTXT', v_cntxt_str);
              --  end if;
               if nvl(v_reg_stus_id,0) <> 0 then
                    ihook.setColumnValue(row, 'REG_STUS_ID', v_reg_stus_id);
                end if;
                if nvl(v_admin_stus_id,0) <> 0 then
                    ihook.setColumnValue(row, 'ADMIN_STUS_ID',v_admin_stus_id);
                end if;
                /**if nvl(v_cntxt_item_id,0) <> 0 THEN
                    ihook.setColumnValue(row, 'CNTXT_ID', v_cntxt_item_id);
                end if;
                if nvl(v_cntxt_ver_nr, 0) <> 0 then
                ...
                if nvl(v_csi_ver_nr, 0) <> 0 then
                    ihook.setColumnValue(row, 'CS_VER_NR', v_csi_ver_nr);
                end if;
                if nvl(v_val_dom_typ, 0) <> 0 then
                    ihook.setColumnValue(row, 'VAL_DOM_TYP_ID', v_val_dom_typ);
                end if;

                ihook.setColumnValue(row, 'MTCH_LMT', v_mtch_lmt);
            ...
            --raise_application_error(-20001, to_char(hookinput.invocationnumber));
...
EXCEPTION
    WHEN OTHERS THEN
    raise_application_error(-20001, v_error_msg || 'There was an internal processing error. Please report this to your OneData Administrator.');
    --nci_util.debugHook('GENERAL', v_data_out);
end;
...
AS
    ...
    v_mode varchar2(32);
    v_id number;
    v_btch_usr_nm varchar2(100);
    v_btch_nm varchar2(50);
    v_cncpt_ind number;
    v_cntxt_str varchar2(1000);
...
begin
    ...
    if (hookinput.invocationnumber = 0) then   -- First invocation
        ...
        hookoutput.forms := getDSParameterEditForm(rowset, 'BTCH');
        hookOutput.question := nci_ds.getQuestionDSParameters;
        V_DATA_OUT := IHOOK.GETHOOKOUTPUT (HOOKOUTPUT);
        --nci_util.debugHook('GENERAL',v_data_out);
	else
  ...
  --      if (hookinput.answerid = 1) then
            forms              := hookinput.forms;
            form1              := forms(1);
   --raise_application_error(-20000,form1.rowset.rowset.count);
            row_sel            := form1.rowset.rowset(1);
        ...
        --Set variables from form input
            ...
            v_val_dom_typ := ihook.getColumnValue(row_sel, 'VAL_DOM_TYP_ID'); 
            v_mtch_lmt := nvl(ihook.getColumnValue(row_sel, 'MTCH_LMT'), v_mtch_lmt);
           -- v_btch_usr_nm := ihook.getColumnValue(row_sel, 'BTCH_USR_NM');
          --  v_btch_nm := ihook.getColumnValue(row_sel, 'BTCH_NM');
            v_cncpt_ind := nvl(ihook.getColumnValue(row_sel, 'CNCPT_IND'), 0);
            for j in 1..ihook.getmsColumnValue(row_sel,'MS_CNTXT').count loop
                v_temp_str := REGEXP_SUBSTR(ihook.getmsColumnValue(row_sel, 'MS_CNTXT')(j), '[^v]+', 1, 1);
                select cntxt_nm_dn into v_temp_str from vw_cntxt_ds where item_id = v_temp_str;
     ...
     --v_hdr_id := 194361;
            ...
            select seq_ds_prmtr_id.nextval into v_id from dual;
               ...
               -- ihook.setColumnValue(row, 'HDR_ID', v_hdr_id);
                ...
                ihook.setColumnValue(row, 'ENTTY_NM', v_entty_nm);
                ihook.setColumnValue(row, 'SRC_VAL_ID', v_src_val_id);
               -- ihook.setColumnValue(row, 'BTCH_USR_NM', v_user_id);
               if nvl(v_reg_stus_id,0) <> 0 then
                    ihook.setColumnValue(row, 'REG_STUS_ID', v_reg_stus_id);
                end if;
                if nvl(v_admin_stus_id,0) <> 0 then
...
--raise_application_error(-20000,ihook.getcolumnvalue(row_ori, 'BTCH_USR_NM') || '   ' ||ihook.getcolumnvalue(row_ori, 'BTCH_NM'));
                ihook.setColumnValue(row, 'MTCH_LMT', v_mtch_lmt);
                ihook.setColumnValue(row, 'BTCH_USR_NM', ihook.getcolumnvalue(row_ori, 'BTCH_USR_NM'));
                ihook.setColumnValue(row, 'BTCH_NM', ihook.getcolumnvalue(row_ori, 'BTCH_NM'));
                ihook.setColumnValue(row, 'CNCPT_IND', v_cncpt_ind);
                ihook.setColumnValue(row, 'MS_CNTXT', v_cntxt_str);
                --
            rows.extend(); rows(rows.last) := row;

            --end loop;
            action := t_actionrowset(rows, 'DS Entity to Match Parameters Batch (for insert)', 2,1,'insert');

            actions.extend;
            actions(actions.last) := action;
   action := t_actionrowset(rows, 'DS Batch Header', 2,1,'update');
...
begin
    ...
    if (hookinput.invocationnumber = 0) then   -- First invocation
        ...
        hookoutput.forms := getCDEAIParameterEditForm(rowset);
        hookOutput.question := nci_ds.getQuestionDSParameters;
        V_DATA_OUT := IHOOK.GETHOOKOUTPUT (HOOKOUTPUT);
--nci_util.debugHook('GENERAL',v_data_out);
    else
...
--nci_util.debugHook('GENERAL', v_data_out);
        --Set variables from form input
           ...
           -- raise_application_error(-20001, to_char(v_cde_thresh) || ' ' || to_char(v_pv_thresh));
            ...
            v_cntxt_str := rtrim(v_cntxt_str, ',');
         /**       for j in 1..ihook.getmscolumnvalue(row_sel, 'CDE_MDL_VARIANTS').count loop

                    --v_cde_var := ihook.getMSColumnValue(row_sel, 'CDE_MDL_VARIANTS');
        ...
        --Loop through all rows and insert into new object that triggers IS service on insert
            ...
            --set row to match and hdr_id
                row_ori := hookInput.originalRowset.rowset(i);
                v_hdr_id := ihook.getColumnValue(row_ori, 'HDR_ID');
       /*     if (getDSSrcValCnt(v_hdr_id, 0) <> 0) then
                --v_pv_var := nvl(ihook.getColumnValue(row_sel, 'VARIANT_2'),266);
                v_pv_var := nvl(v_pv_var_init, 266);
                else
                v_pv_var := 269; --None, cde embedding only
            end if;
        */
        v_pv_var := v_pv_var_init;
            --set entity name to match against: user tip overrides
                v_entty_nm := nvl(ihook.getColumnValue(row_ori, 'ENTTY_NM_USR'), ihook.getColumnValue(row_ori, 'ENTTY_NM'));
            ...
            select seq_ds_prmtr_id.nextval into v_id from dual;
                ...
                if nvl(v_cde_thresh, 0) <> 0 then
                    ihook.setColumnValue(row, 'THRESHOLD_1', v_cde_thresh);
                end if;
                if nvl(v_pv_thresh, 0) <> 0 then
                    ihook.setColumnValue(row, 'THRESHOLD_2', v_pv_thresh);
                end if;
           /*     if nvl(v_cde_var, 0) <> 0 then
                    ihook.setColumnValue(row, 'VARIANT_1', v_cde_var);
                ...
                -- hard code CDEMeta-SapBERT model
                    select obj_key_desc into v_cde_var_nm from obj_key where obj_key_id = 253;
                    ihook.setColumnValue(row, 'VARIANT_1_NM', v_cde_var_nm);
                ...
                if nvl(v_pv_var, 0) <> 0 then
                    ...
                    ihook.setColumnValue(row, 'VARIANT_2_NM',v_pv_var_nm);

                end if;
                ihook.setColumnValue(row, 'MTCH_LMT', v_mtch_lmt);
                ihook.setColumnValue(row, 'DT_LAST_MODIFIED', TO_CHAR(SYSDATE, 'MM/DD/YY HH24:MI:SS'));
                ihook.setColumnValue(row, 'SRC_MTCH_ENGN', 'CDE AI Match');
                --for multi-select UPDATE: hard-code CDEMeta-SapBERT as it is only allowable option
                ihook.setColumnValue(row, 'VARIANT_1_LIST', '253');
                ihook.setColumnValue(row, 'MS_CNTXT', nvl(v_cntxt_str, ''));
               -- raise_application_error(-20001, v_cntxt_str);


            rows.extend(); rows(rows.last) := row;
            end loop;
            action := t_actionrowset(rows, 'DS Entity to Match CDE AI Parameters (for insert)', 2,1,'insert');
...
begin
    ...
    if (hookinput.invocationnumber = 0) then   -- First invocation
        ...
        rowset := hookinput.originalrowset;
        row_ori := hookinput.originalRowset.rowset(1);
        --ihook.setColumnValue(row_ori, 'MTCH_LMT', 10);

        hookoutput.forms := getDSParameterEditForm(rowset, 'ORI');
        hookOutput.question := nci_ds.getQuestionDSParameters;
        V_DATA_OUT := IHOOK.GETHOOKOUTPUT (HOOKOUTPUT);
       --nci_util.debugHook('GENERAL',v_data_out);
	else
        ...
        --Set variables from form input
            ...
            v_val_dom_typ := ihook.getColumnValue(row_sel, 'VAL_DOM_TYP_ID'); 
            v_mtch_lmt := nvl(ihook.getColumnValue(row_sel, 'MTCH_LMT'), v_mtch_lmt);
        --jira 9379: make concept matching optional, default to no
            v_cncpt_ind := nvl(ihook.getColumnValue(row_sel, 'CNCPT_IND'),0);
        --Use Variables to Build Filter String
        ...
        --Loop through all rows and run match
           ...
           --  raise_application_Error(-20001, v_mtch_lmt); 
                spDSMatchFlow(v_hdr_id, v_user_id, v_entty_nm, v_flt_str, v_src_val_id, 'CDE', v_mtch_lmt, v_cncpt_ind);

            --run post match operations
                spDSPostMatch(v_hdr_id, v_user_id, v_src_val_id, v_mtch_lmt);
...
--regex cleansing/formatting   
   ...
   select '''' || regexp_replace(upper(regexp_replace(v_mtch_nm, '''', '''''' )),v_reg_str_ds,'') || '''' into v_entty_nm from dual;
  --  select '''' || upper(regexp_replace(v_mtch_nm, '''', '''''' )) || '''' into v_entty_nm from dual;
...
--        end if;
    ...
    commit;
    v_sys_msg := getDSSysMsg(v_hdr_id, 'DS', 0);
    update nci_ds_hdr set sys_msg = v_sys_msg where hdr_id = v_hdr_id;
    commit;
  end loop;
  commit;
...
--    end if;
--end if;
    v_sys_msg := getDSSysMsg(v_hdr_id, 'DEC', 0);
...
---procedure spDSMatchNonEnum ( v_hdr_id in number, v_usr_id in varchar2,v_flt_str in varchar2, v_mtch_typ in varchar2, v_mtch_nm in varchar2);
  ...
  if (v_temp = 1) then
    ...
    update nci_ds_hdr set LST_RUN_TYP = 'CDE Match (' || v_mode || ')', LST_RUN_DT = systimestamp where hdr_id = v_hdr_id;
    commit;
    v_sys_msg := getDSSysMsg(v_hdr_id, v_mode, 0);
    update nci_ds_hdr set sys_msg = v_sys_msg where hdr_id = v_hdr_id;
    commit;
  end loop;
  commit;
...
BEGIN
    ...
    delete from nci_ds_rslt where hdr_id = v_hdr_id and mtch_typ = 'CDE' and cde_pref_ind <> 1;
    commit;
...
BEGIN
    ...
    for cur in (select hdr_id,(count(*) - v_pv_cnt) cnt from nci_ds_dtl d where hdr_id = v_hdr_id group by hdr_id) loop
        ...
        v_sql := 'insert into nci_ds_rslt ( HDR_ID,ITEM_ID,VER_NR, NUM_PV_IN_SRC, NUM_PV_MTCH, RULE_DESC, MTCH_TYP, mtch_desc_txt, SCORE) select distinct * from (select ' || cur.hdr_id || ', de_item_id item_id, de_ver_nr ver_nr, ' || cur.cnt || ', count(*), dtl.rule_Desc || '' (and PV Matched)'', ' 
            ...
            'dtl.hdr_id = ' || cur.hdr_id || ' and dtl.item_id = de_item_id and dtl.ver_nr = de_ver_nr and (v.perm_val_nm) in (select distinct (' || v_src_dtl || ') from nci_ds_dtl where hdr_id = ' || cur.hdr_id
     ...
     --   raise_application_error(-20001, v_sql);
        ...
        if  (sql%rowcount = 0) then
            v_sql := 'insert into nci_ds_rslt ( HDR_ID,ITEM_ID,VER_NR, NUM_PV_IN_SRC, NUM_PV_MTCH, RULE_DESC, MTCH_TYP, mtch_desc_txt, SCORE) select distinct * from (select ' || cur.hdr_id || ', de_item_id item_id, de_ver_nr ver_nr, ' || cur.cnt || ', count(*), dtl.rule_Desc || '' (and VM Matched)'', ' 
                || '''' || 'CDE' || '''' || ', max(dtl.mtch_desc_txt), dtl.SCORE from vw_nci_de_pv_lean v, admin_item ai, nci_ds_rslt_dtl dtl where nvl(ai.currnt_ver_ind,0) = 1 and v.de_item_id = ai.item_id and v.de_ver_nr = ai.ver_nr and ' ||
                'dtl.hdr_id = ' || cur.hdr_id || ' and dtl.item_id = de_item_id and dtl.ver_nr = de_ver_nr and (v.item_nm) in (select distinct (' || v_src_dtl || ') from nci_ds_dtl where hdr_id = ' || cur.hdr_id 
                || ' and excl_pv_ind = 0) and (' || v_hdr_id || ', dtl.item_id, dtl.ver_nr) not in (select hdr_id, item_id, ver_nr from nci_ds_rslt) group by de_item_id, de_ver_nr, dtl.rule_Desc, dtl.SCORE having count(*) = ' || cur.cnt || ' ' || v_order_by || ') WHERE ROWNUM <= ' || v_mtch_lmt;
            execute immediate v_sql;
        end if;
...
--jira 9296: return any enumerated matches that have any pv matches in order to correctly show matched pv count
       -- if  (sql%rowcount = 0) then
       v_lmt := v_mtch_lmt - sql%rowcount;
            v_sql := 'insert into nci_ds_rslt ( HDR_ID,ITEM_ID,VER_NR, NUM_PV_IN_SRC, NUM_PV_MTCH, RULE_DESC, MTCH_TYP, mtch_desc_txt, SCORE) select distinct * from (select ' || cur.hdr_id || ', de_item_id item_id, de_ver_nr ver_nr, ' || cur.cnt || ', count(*), dtl.rule_Desc || '' (and PV Matched)'', ' 
                || '''' || 'CDE' || '''' || ', max(dtl.mtch_desc_txt), (dtl.SCORE-0.02) from vw_nci_de_pv_lean v, admin_item ai, nci_ds_rslt_dtl dtl where nvl(ai.currnt_ver_ind,0) = 1 and v.de_item_id = ai.item_id and v.de_ver_nr = ai.ver_nr and ' ||
                'dtl.hdr_id = ' || cur.hdr_id || ' and dtl.item_id = de_item_id and dtl.ver_nr = de_ver_nr and (v.perm_val_nm) in (select distinct (' || v_src_dtl || ') from nci_ds_dtl where hdr_id = ' || cur.hdr_id 
                || ' and excl_pv_ind = 0) and (' || v_hdr_id || ', dtl.item_id, dtl.ver_nr) not in (select hdr_id, item_id, ver_nr from nci_ds_rslt) group by de_item_id, de_ver_nr, dtl.rule_Desc, dtl.SCORE having count(*) > ' || 0 || ' ' || v_order_by || ') WHERE ROWNUM <= ' || v_lmt;
            execute immediate v_sql;

       --- end if;
       -- if  (sql%rowcount = 0) then
       v_lmt := v_lmt - sql%rowcount;
            v_sql := 'insert into nci_ds_rslt ( HDR_ID,ITEM_ID,VER_NR, NUM_PV_IN_SRC, NUM_PV_MTCH, RULE_DESC, MTCH_TYP, mtch_desc_txt, SCORE) select distinct * from (select ' || cur.hdr_id || ', de_item_id item_id, de_ver_nr ver_nr, ' || cur.cnt || ', count(*), dtl.rule_Desc || '' (and VM Matched)'', ' 
                || '''' || 'CDE' || '''' || ', max(dtl.mtch_desc_txt), (dtl.SCORE-0.02) from vw_nci_de_pv_lean v, admin_item ai, nci_ds_rslt_dtl dtl where nvl(ai.currnt_ver_ind,0) = 1 and v.de_item_id = ai.item_id and v.de_ver_nr = ai.ver_nr and ' ||
                'dtl.hdr_id = ' || cur.hdr_id || ' and dtl.item_id = de_item_id and dtl.ver_nr = de_ver_nr and (v.item_nm) in (select distinct (' || v_src_dtl || ') from nci_ds_dtl where hdr_id = ' || cur.hdr_id 
                || ' and excl_pv_ind = 0) and (' || v_hdr_id || ', dtl.item_id, dtl.ver_nr) not in (select hdr_id, item_id, ver_nr from nci_ds_rslt) group by de_item_id, de_ver_nr, dtl.rule_Desc, dtl.SCORE having count(*) > ' || 0 || ' ' || v_order_by || ') WHERE ROWNUM <= ' || v_lmt;
...
BEGIN
 ...
 rows := t_rows();
row := t_row();
--jira 9313: need to manually set the preferred cde id and version so that its there when system message gets generated (hookoutput action is too slow)
update nci_ds_hdr set 
    cde_item_id = ihook.getColumnValue(row_ori, 'ITEM_ID'), 
...
commit;
update nci_ds_rslt set CDE_PREF_IND = 0 where HDR_ID = ihook.getColumnValue(row_ori, 'HDR_ID');
commit;
update nci_ds_rslt set CDE_PREF_IND =1 where HDR_ID = ihook.getColumnValue(row_ori, 'HDR_ID') and ITEM_ID = ihook.getColumnValue(row_ori, 'ITEM_ID') and VER_NR = ihook.getColumnValue(row_ori, 'VER_NR');
   commit;    
       ihook.setColumnValue(row, 'HDR_ID', ihook.getColumnValue(row_ori, 'HDR_ID'));
        ...
        ihook.setColumnValue(row, 'SYS_MSG', getDSSysMsg(ihook.getColumnValue(row_ori, 'HDR_ID'), 'CDE', getDSMtchLmt(ihook.getColumnValue(row_ori, 'HDR_ID'))));
...
row := t_row();
 ...
 for cur in (Select * from admin_item where admin_item_typ_id in (2) and item_id = ihook.getColumnValue(row_ori, 'ITEM_ID') and ver_nr = ihook.getColumnValue(row_ori, 'VER_NR')) loop
            ...
            rows.extend;
            rows(rows.last) := row;
            v_out_str := 'Selected DEC set to ' || ihook.getColumnValue(row_ori, 'ITEM_ID') || 'v' || ihook.getColumnValue(row_ori, 'VER_NR');
            --ihook.setColumnValue(row, 'SYS_MSG',getDSSysMsg(ihook.getColumnValue(row_ori, 'HDR_ID'), 'DEC', getDSMtchLmt(ihook.getColumnValue(row_ori, 'HDR_ID'))));
            hookoutput.message := v_out_str;
        --    raise_application_error(-20000,cur.item_id);
            action             := t_actionrowset(rows, 'DS Entity To Match', 2, 0,'update');
            actions.extend;
            actions(actions.last) := action;
...
--command that kicks off VM Match bulk process
procedure spVMMatchBulk (v_data_in in clob, v_data_out out clob, v_user_id in varchar2)
AS
    hookInput t_hookInput;
    hookOutput t_hookOutput := t_hookOutput();
    ...
    forms t_forms;
    form1 t_form;
    v_mtch_rstrct varchar(16);
    v_mtch_pref varchar(16);
    v_btch_nm varchar(255);
    v_btch_usr_nm varchar2(255);
    v_cnt number;
    v_entty_nm varchar2(255);
    v_entty_nm_like varchar2(255);
    v_hdr_id number;
...
BEGIN
    ...
    rows := t_rows();
    row_ori :=  hookInput.originalRowset.rowset(1);
    v_btch_usr_nm := ihook.getColumnValue(row_ori, 'BTCH_USR_NM');
    ...
    if (hookinput.invocationnumber = 0) then   -- First invocation
        ...
        hookoutput.forms := getVMConcMatchForm(rowset);
        hookOutput.question := nci_ds.getQuestionVMConcParameters;
        V_DATA_OUT := IHOOK.GETHOOKOUTPUT (HOOKOUTPUT);
       --nci_util.debugHook('GENERAL',v_data_out);
	else
        if (hookinput.ANSWERID = 1) then -- parameters entered, Run Match
           -- raise_application_error(-20001, 'here');
            ...
            if ihook.getColumnValue(row_sel, 'MTCH_RSTRCT') <> 0 then --set restricted
                v_mtch_rstrct := 'R';
            else
                v_mtch_rstrct := 'U';
            end if;

            if ihook.getColumnValue(row_sel, 'MTCH_PREF') is not null then
                if (ihook.getColumnValue(row_sel, 'MTCH_PREF') = 270) then --set to VMs only
                    v_mtch_pref := 'V';
                ELSE -- set to concepts only
                    v_mtch_pref := 'C';
                end if;
            else --no preference, set to All
                v_mtch_pref := 'A';
            end if;
    --loop through all rows for imported batch
            v_cnt := 0;
            for cur in (select hdr_id, entty_nm, entty_nm_usr from nci_ds_hdr where btch_nm = v_btch_nm and btch_usr_nm = v_btch_usr_nm) loop
                v_hdr_id := cur.hdr_id;
            --  raise_application_error(-20001, to_char(v_hdr_id));
                v_entty_nm := regexp_replace(upper(nvl(cur.entty_nm_usr ,cur.entty_nm)),v_reg_str_adv,'');
                v_entty_nm_with_space := upper(nvl(cur.entty_nm_usr ,cur.entty_nm));

                delete from nci_ds_rslt where hdr_id =v_hdr_id;
                commit;

                -- VMMatchFlow(v_hdr_id, v_entty_nm,v_entty_nm_with_space,v_typ, v_pref, v_ext);
                VMMatchFlow(v_hdr_id, v_entty_nm, v_entty_nm_with_space,v_mtch_rstrct,v_mtch_pref,'NA');
                v_cnt := v_cnt + 1;
        select count(*) into v_temp from nci_ds_rslt where hdr_id = v_hdr_id;
        if (v_temp = 1) then
        select rule_desc into v_rule_desc from nci_ds_rslt where hdr_id=v_hdr_id;
...
procedure spDSResultsPostHook (v_data_in in clob, v_data_out out clob, v_usr_id in varchar2) as
    hookInput t_hookInput;
    hookOutput t_hookOutput := t_hookOutput();
    ...
    row_ori t_row;
    v_hdr_id integer;
    v_num_val_mtch number;
    v_num_src_val number;
BEGIN
    hookinput := ihook.gethookinput (v_data_in);
    hookoutput.invocationnumber  := hookinput.invocationnumber;
    hookoutput.originalrowset    := hookinput.originalrowset;
        ...
        row_ori := hookInput.originalRowset.rowset(1);
        v_hdr_id := ihook.getColumnValue(row_ori, 'HDR_ID');
        v_num_val_mtch := ihook.getColumnValue(row_ori, 'NUM_PV_MTCH');
        v_num_src_val := ihook.getColumnValue(row_ori, 'NUM_PV_IN_SRC');
raise_application_error(-20001, v_num_val_mtch);
    --jira 9296
    if (v_num_val_mtch > v_num_src_val) then
    raise_application_error(-20001, 'here');
        update nci_ds_rslt set num_pv_mtch = v_num_src_val where hdr_id = v_hdr_id and item_id = ihook.getColumnValue(row_ori, 'ITEM_ID') and ver_nr = ihook.getColumnValue(row_ori, 'VER_NR');
        commit; 
...
begin
 ...
 --   for i in 1..hookinput.originalrowset.rowset.count loop  --- Iterate through all the selected rows
        ...
        if (v_pref_cde is null and v_pref_cde_old is not null) then --clear system message
            v_sys_msg := getDSSysMsg(v_hdr_id, 'P', getDSMtchLmt(v_hdr_id));
        ...
        v_pref_msg := setprefCDEIndicator(v_hdr_id, v_pref_cde, ihook.getColumnValue(row_ori, 'CDE_VER_NR'));
        --raise_application_error(-20001, v_pref_cde);
 --   end loop;
    V_DATA_OUT := IHOOK.GETHOOKOUTPUT (HOOKOUTPUT);
end;
...
is
BEGIN
    update nci_ds_rslt set CDE_PREF_IND = 0 where HDR_ID = v_hdr_id;
    commit;
    update nci_ds_rslt set CDE_PREF_IND = 1 where HDR_ID = v_hdr_id and ITEM_ID = v_cde_id and VER_NR = v_ver_nr;
    commit;
    return 'success';
...
begin
 ...
 --   for i in 1..hookinput.originalrowset.rowset.count loop  --- Iterate through all the selected rows
    ...
    if (ihook.getColumnValue(row_ori, 'USR_CMNTS') is not null) then
        ...
        --jira 9316
           -- v_desc_col := ihook.getColumnValue(row_ori, 'ENTTY_NM') || ' | ' || ihook.getColumnValue(row_ori, 'BTCH_NM');
          --  update nci_ds_hdr set DESC_COL = v_desc_col where HDR_ID = ihook.getColumnValue(row_ori, "HDR_ID");
          --  commit;
...
begin
    ...
    select count(*) into v_temp from nci_ds_dtl where hdr_id = v_hdr_id and excl_pv_ind = 0;
    update nci_ds_hdr set num_pv = v_temp where hdr_id = v_hdr_id;
    commit;
    update onedata_ra.nci_ds_hdr set num_pv = v_temp where hdr_id = v_hdr_id;
    commit;
...
procedure spBatchDelete (v_data_in in clob, v_data_out out clob, v_usr_id in varchar2) AS
    hookInput t_hookInput;
    hookOutput t_hookOutput := t_hookOutput();
    ...
    v_err_msg varchar2(512);
      showRowset     t_showableRowset;
  v_fltr_str varchar2(4000);
  v_sql varchar2(4000);
  v_obj_nm varchar2(1000);
BEGIN
    hookinput := ihook.gethookinput (v_data_in);
    hookoutput.invocationnumber  := hookinput.invocationnumber;
    hookoutput.originalrowset    := hookinput.originalrowset;

--raise_application_error(-20000,hookinput.ta
     row_ori := hookInput.originalRowset.rowset(1);
     -- DS Batch Header or DS Entity To Match
    v_obj_nm := hookinput.originalrowset.objectname;
    if (hookinput.invocationnumber = 0 and v_obj_nm = 'DS Entity To Match') then
    rows := t_rows();
    row := t_row();


       ihook.setColumnValue(row, 'Deletion Type', 'All rows with Batch User: ' || ihook.getColumnValue(row_ori,'BTCH_USR_NM'));
       ihook.setColumnValue(row, 'Deletion ID', 1);
     rows.extend; rows(rows.last) := row;
     ihook.setColumnValue(row, 'Deletion Type', 'All rows with Batch Name: ' || ihook.getColumnValue(row_ori,'BTCH_NM'));
       ihook.setColumnValue(row, 'Deletion ID', 2);
     rows.extend; rows(rows.last) := row;
     ihook.setColumnValue(row, 'Deletion Type', 'All rows with Batch Name/Batch User: ' || ihook.getColumnValue(row_ori,'BTCH_NM') || '/' ||  ihook.getColumnValue(row_ori,'BTCH_USR_NM'));
       ihook.setColumnValue(row, 'Deletion ID', 3);
     rows.extend; rows(rows.last) := row;
        showrowset := t_showablerowset (rows, 'Model Selection', 4, 'single');
       	 hookoutput.showrowset := showrowset;
         hookoutput.question := nci_11179.getQuestionGeneric('Please select option.', 'Delete');

     end if;
     if (hookinput.invocationnumber = 0 and v_obj_nm = 'DS Batch Header') then
           hookoutput.question := nci_11179.getQuestionGeneric('Please confirm deletion for Batch Name/Batch User: ' || ihook.getColumnValue(row_ori,'BTCH_NM') || '/' ||  ihook.getColumnValue(row_ori,'BTCH_USR_NM'), 'Delete');

     end if;
    if (hookinput.invocationnumber = 1) then
    if (v_obj_nm <> 'DS Batch Header') then
      row_sel := hookInput.selectedRowset.rowset(1);
      if (ihook.getColumnValue(row_sel,'Deletion ID') = 2) then -- batch name
        v_fltr_str := ' where btch_nm = ''' || ihook.getColumnValue(row_ori,'BTCH_NM') || '''';
    end if;
     if (ihook.getColumnValue(row_sel,'Deletion ID') = 1) then -- batch user
        v_fltr_str := ' where btch_usr_nm = ''' || ihook.getColumnValue(row_ori,'BTCH_USR_NM') || '''' ;
    end if;
      if (ihook.getColumnValue(row_sel,'Deletion ID') = 3 ) then -- batch user/name
        v_fltr_str := ' where btch_nm = ''' || ihook.getColumnValue(row_ori,'BTCH_NM') || ''' and btch_usr_nm = ''' || ihook.getColumnValue(row_ori,'BTCH_USR_NM') || '''' ;
    end if;
   else
        v_fltr_str := ' where btch_nm = ''' || ihook.getColumnValue(row_ori,'BTCH_NM') || ''' and btch_usr_nm = ''' || ihook.getColumnValue(row_ori,'BTCH_USR_NM') || '''' ;
   end if;
    v_fltr_str := v_fltr_str || ' and MTCH_TYP_NM = ''' || nvl(ihook.getColumnValue(row_ori,'MTCH_TYP_NM'),'CDE') || '''';

     v_sql := 'DELETE FROM nci_ds_cntxt_sel WHERE hdr_id  in (Select hdr_id from nci_ds_hdr  ' || v_fltr_str || ')';
   -- raise_application_error (-20000, v_sql);
    execute immediate v_sql;
     v_sql := 'DELETE FROM nci_ds_rslt_dtl WHERE hdr_id  in (Select hdr_id from nci_ds_hdr  ' || v_fltr_str || ')';
    execute immediate v_sql;
     v_sql := 'DELETE FROM nci_ds_rslt WHERE hdr_id  in (Select hdr_id from nci_ds_hdr  ' || v_fltr_str || ')';
    execute immediate v_sql;
    v_sql := 'DELETE FROM nci_ds_prmtr WHERE hdr_id  in (Select hdr_id from nci_ds_hdr  ' || v_fltr_str || ')';
    execute immediate v_sql;
    v_sql := 'DELETE FROM nci_ds_dtl WHERE hdr_id  in (Select hdr_id from nci_ds_hdr  ' || v_fltr_str || ')';
    execute immediate v_sql;
    v_sql := 'DELETE FROM nci_ds_hdr   ' || v_fltr_str ;
    execute immediate v_sql;
...
IS
    v_lmt number;
BEGIN
   select * into v_lmt from ( select mtch_lmt from nci_ds_prmtr where hdr_id = v_hdr_id order by dt_lst_modified desc) where rownum = 1;
   return nvl(v_lmt, 0);
    EXCEPTION
        WHEN OTHERS THEN
        return nvl(v_lmt, 0);
...
IS
    ...
    v_lmt_str varchar2(100) := ' ';
BEGIN
    --  0. Get enumerated value
    select count(*) into v_temp from nci_ds_dtl where hdr_id = v_hdr_id;
    if (v_temp = 0) then v_val_dom_typ := 18 ; else v_val_dom_typ := 17; end if;

    -- 1/2 Get Match limit string if not called from DEC
    if (v_op <> 'DEC') then
        v_lmt_str := '(limit: ' || v_mtch_lmt || ') ';
    end if;
    ...
    if (v_temp = 1) then -- match has been run
        ...
        v_sys_msg := 'Last Run: ' || v_lst_run_typ || v_lmt_str || to_char(v_lst_run_dt, v_dt_fmt) || '<BR>';
    end if;

    -- 2. System Message returns preferred CDE details if set
    select count(*) into v_temp from nci_ds_hdr where hdr_id = v_hdr_id and cde_item_id is not null;
    --raise_application_error(-20000, v_temp);
    if (v_temp = 1 and v_op <> 'DEC') then -- preferred cde set, not called from post-hook
        ...
        --if (v_temp = 1) then
            ...
            if (v_val_dom_typ = 18) then -- non-enumerated > no pv information should be returned
                v_sys_msg := v_sys_msg || 'Selected CDE set to ' || v_item_id || 'v' || v_ver_nr || ' <BR>';
            else
                v_sys_msg := v_sys_msg || 'Selected CDE set to ' || v_item_id || 'v' || v_ver_nr || ' <BR>' ||
                    'Source PVs ' || v_pv_src || ' - Matched PVs ' || v_pv_mtch || ' - CDE PVs ' || v_pv_cde || '<BR>';
            end if;
        --end if;
    end if;
    ...
    if (v_temp = 1 and v_op <> 'DECS') then -- selected dec set, not called from post-hook
        select de_conc_item_id, de_conc_ver_nr  into v_item_id, v_ver_nr
            from nci_ds_hdr d
            where d.hdr_id = v_hdr_id;
            v_sys_msg := v_sys_msg || 'Selected DEC set to ' || v_item_id || 'v' || v_ver_nr || ' <BR>';
    end if;
