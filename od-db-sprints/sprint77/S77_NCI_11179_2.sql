...
PROCEDURE spShowVersionDependency (v_data_in in clob, v_data_out out clob, v_admin_item_typ in out varchar2);
...
if (v_admin_item_typ ='UNK') then -- called from ADMIN_ITEM detect type of first row.
        row_cur := hookInput.originalRowset.Rowset(1);
    case ihook.getColumnValue(row_cur, 'ADMIN_ITEM_TYP_ID') 
    when 2 then v_admin_item_typ :='DEC';
    when 3 then v_admin_item_typ := 'VD';
    when 4 then v_admin_item_typ := 'CDE';
    else v_admin_item_typ := 'CDE';
    end case;
    hookoutput.message := ' First item selected is used to determine dependency type. Default is CDE.';
end if;
--raise_application_error(-20000,hookinput.originalrowset.objectName);
if (v_admin_item_typ ='DEC') then
    ...
    for i in 1 .. hookInput.originalRowset.Rowset.count loop
        ...
        ihook.setColumnValue(row,'DE_CONC_ITEM_ID', nvl(ihook.getColumnValue(row_cur,'DE_CONC_ITEM_ID'),ihook.getColumnValue(row_cur,'ITEM_ID')));
        ihook.setColumnValue(row,'DE_CONC_VER_NR', nvl(ihook.getColumnValue(row_cur,'DE_CONC_VER_NR'),ihook.getColumnValue(row_cur,'VER_NR')));
  ...
  --  showRowset := t_showableRowset(rows, 'Data Element Concepts',2, 'unselectable');
    ...
    hookOutput.message := 'Data Element Concept Dependency' || chr(13) || hookoutput.message;
end if;
...
if (v_admin_item_typ ='VD') then
  ...
  --  showRowset := t_showableRowset(rows, 'Data Element Concepts',2, 'unselectable');
    ...
    hookOutput.message := 'Value Domain Dependency' || hookoutput.message;
end if;
...
if (v_admin_item_typ ='CDE') then
    ...
    for i in 1 .. hookInput.originalRowset.Rowset.count loop
        row := t_row();
        row_cur := hookInput.originalRowset.Rowset(i);
        if (hookinput.originalrowset.objectName = 'CDE Update') then
        ihook.setColumnValue(row,'CDE_ITEM_ID', ihook.getColumnValue(row_cur,'CREAT_CDE_ID') );
        ihook.setColumnValue(row,'CDE_VER_NR', ihook.getColumnValue(row_cur,'UOM_ID'));

        elsif hookinput.originalrowset.objectName = 'CDE Bulk Create Version' then
        ihook.setColumnValue(row,'CDE_ITEM_ID', ihook.getColumnValue(row_cur,'IMP_CDE_ITEM_ID') );
        ihook.setColumnValue(row,'CDE_VER_NR', ihook.getColumnValue(row_cur,'IMP_CDE_VER_NR'));

        else
        ihook.setColumnValue(row,'CDE_ITEM_ID', ihook.getColumnValue(row_cur,'ITEM_ID') );
        ihook.setColumnValue(row,'CDE_VER_NR', ihook.getColumnValue(row_cur,'VER_NR'));
        end if  ;
  ...
  --  showRowset := t_showableRowset(rows, 'Data Element Concepts',2, 'unselectable');
    ...
    hookOutput.message := 'Data Element Dependency.' || hookoutput.message;
end if;
...
/*
            ...
            **/
            ...
            if (v_mode = 'CDE') then -- if called from CDE Match
               -- SELECT lst_run_typ INTO v_run_typ FROM nci_ds_hdr WHERE hdr_id = v_hdr_id;
                SELECT src_val_id,lst_run_typ INTO v_src_val_id,v_lst_run_typ from nci_ds_hdr where hdr_id = v_hdr_id;
                v_rule_desc := ihook.getColumnValue(row_ori, 'RULE_DESC');
                if (instr(v_rule_desc, 'PV') <> 0 OR instr(v_lst_run_typ, 'CDE AI') <> 0) then
                        ...
                        for cur1 in (select perm_val_nm,entty_nm_usr from NCI_DS_DTL where hdr_id = v_hdr_id and excl_pv_ind = 0 and nvl(fld_delete,0) <> 1) loop
                           -- if (upper(nvl(cur1.entty_nm_usr,cur1.perm_Val_nm)) = upper(v_tab_val_mean_nm(i))  or upper(nvl(cur1.entty_nm_usr,cur1.perm_Val_nm)) = upper(substr(v_perm_val_nm,2))) then
                            if (nvl(cur1.entty_nm_usr,cur1.perm_Val_nm) = v_tab_cde_pv(i)  or nvl(cur1.entty_nm_usr,cur1.perm_Val_nm) = substr(v_perm_val_nm,2)) then
                                ihook.setColumnValue(row, v_src_lbl, nvl(cur1.entty_nm_usr,cur1.perm_Val_nm));
                               -- ihook.setColumnValue(row, v_src_lbl, 'test');
                              --  ihook.setColumnValue(row, v_src_lbl, upper(v_tab_val_mean_nm(i)));
                                v_tab_perm_val_nm.extend();
...
if (rows.count > 0) then
    ...
    if (v_mode = 'CDE') then
        ...
        hookoutput.message := SUBSTR('<p style="text-align:left"><strong>Last Run:</strong> CDE Match ' || hookoutput.message, 0, 4000);
    end if;
else
...
if (upper(hookinput.originalrowset.tablename) like '%CART%') then
    ...
    for i in 1 .. hookInput.originalRowset.Rowset.count loop
        ...
        for rec in (  select nci_val_mean_item_id, nci_val_mean_ver_nr, --cncpt_concat, 
        cncpt_concat_nm, cncpt_concat_with_int cncpt_concat, ai.item_nm from perm_val pv, nci_admin_item_ext ext, de, admin_item ai
  ...
  if (v_item_typ_id = 3) then --Value Domain
    ...
    for i in 1 .. hookInput.originalRowset.Rowset.count loop
     ...
     if (ihook.getColumnValue(row_cur, 'ADMIN_ITEM_TYP_ID') = v_item_typ_id) then
        ...
        for rec in (  select nci_val_mean_item_id, nci_val_mean_ver_nr, --cncpt_concat, 
        cncpt_concat_nm, cncpt_concat_with_int cncpt_concat, ai.item_nm from perm_val pv, nci_admin_item_ext ext, admin_item ai
   ...
   select decode(ai.VAL_DOM_TYP_ID,17,'Enumerated',18,'Non-enumerated',16, 'Enumerated by Reference','') || ' VD: ' || ai.item_id || '-' ||  ihook.getColumnValue(row_cur,'VER_NR')  || '-' ||  ai.item_nm into  vd_id_nm from vw_value_dom ai
    ...
    k := 1;
        ...
        if (ihook.getColumnValue(row_cur, 'ADMIN_ITEM_TYP_ID') = v_item_typ_id or v_cart) then
            ...
            v_perm_val_nm := '';
            for rec in   (select perm_val_nm   from perm_val a, nci_admin_item_Ext ext, de
            where nci_val_mean_item_id=ext.item_id  and nci_val_mean_ver_nr = ext.ver_nr and ext.cncpt_concat_with_int = v_tab_val_mean_cd(i)
            and a.val_dom_item_id=de.val_dom_item_id and de.item_id = ihook.getColumnValue(row_cur,'ITEM_ID')
            and a.val_dom_ver_nr=de.val_dom_ver_nr and de.ver_nr = ihook.getColumnValue(row_cur,'VER_NR') and a.fld_delete=0) loop
                v_perm_val_nm :=  v_perm_val_nm || '|' || rec.perm_val_nm ;
            end loop;
