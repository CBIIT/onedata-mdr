...
insert into nci_admin_item_xmap (item_id, ver_nr, xmap_cd,xmap_desc, evs_src_id, evs_Src_ver_Nr)
select distinct c.item_id, c.ver_nr, TARGET_CODE,TARGET_TERM,v_evs_Src_id, 
TARGET_TERMINOLOGY_VERSION from 
...
insert into nci_admin_item_xmap (item_id, ver_nr, xmap_cd,xmap_desc, evs_src_id, evs_Src_ver_Nr)
select distinct c.item_id, c.ver_nr, i.SOURCE_ATOM_CODE,i.SOURCE_ATOM_NAME,o.obj_key_Id, 
1 from 
admin_item c, (select nci_code, source, max(source_atom_code) SOURCE_ATOM_CODE, max(source_atom_name) SOURCE_ATOM_NAME from sag_load_mt
group by nci_code, source) i, obj_Key o where c.admin_item_typ_id = 49 and i.nci_CODe = c.item_long_nm
and i.source = o.OBJ_KEY_CMNTS and o.obj_typ_id = 23
and (c.item_id, c.ver_nr, i.SOURCE_ATOM_CODE, o.obj_key_Id) not in (select item_id, ver_nr, xmap_cd,evs_Src_id
from nci_admin_item_xmap w);
commit;
