...
and ai.item_id = e.item_id and ai.ver_nr = e.ver_nr and ai.regstr_stus_nm_dn not like '%RETIRED%' and ai.admin_stus_nm_dn not like '%RETIRED%' and upper(ai.CNTXT_NM_DN) not in ('TEST','TRAINING')
and csix.ITEM_ID=csi.ITEM_ID and csix.VER_NR =csi.VER_NR and csix.admin_stus_nm_dn ='RELEASED'
--and nvl(ai.CURRNT_VER_IND,0) = 1 
union
SELECT distinct ai.CREAT_DT,
...
commit;
  ...
  CREATE OR REPLACE FORCE EDITIONABLE VIEW VW_NCI_MDL_MAP_FOR_VIEW ("MDL_ITEM_ID", "MDL_VER_NR", "MDL_MAP_ITEM_ID", "MDL_MAP_VER_NR", "LVL_TYP", "SRC_MDL_ITEM_ID", "SRC_MDL_VER_NR", "SRC_MDL_ITEM_NM", "CREAT_DT", "CREAT_USR_ID", "LST_UPD_USR_ID", "FLD_DELETE", "LST_DEL_DT", "S2P_TRN_DT", "LST_UPD_DT", "TGT_MDL_ITEM_ID", "TGT_MDL_VER_NR", "TGT_MDL_ITEM_NM") DEFAULT COLLATION "USING_NLS_COMP"  AS 
...
alter table de enable all triggers;
 ...
 --      max(decode(upper(obj_key_Desc),'CODING INSTRUCTIONS',ref_desc) ) CODE_INSTR_REF_DESC,
       ...
       max(decode(upper(obj_key_Desc),'EXAMPLE', ref_desc) ) EXAMPL
       FROM REF, OBJ_KEY WHERE REF_TYP_ID = OBJ_KEY.OBJ_KEY_ID   and obj_key.obj_typ_id = 1 and nvl(ref.fld_delete,0) = 0
           AND UPPER(OBJ_KEY_DESC) in ('INSTRUCTIONS', 'CODING INSTRUCTIONS','EXAMPLE')  and NCI_CNTXT_ITEM_ID = 12119072 --pediatric cancer context
           group by item_id ,ver_nr)  CODE_INSTR,
             (  SELECT item_id,ver_nr, max(nm_desc) nm_desc FROM ALT_NMS, OBJ_KEY WHERE NM_TYP_ID = OBJ_KEY.OBJ_KEY_ID   and obj_key.obj_typ_id =  11 
             AND UPPER(OBJ_KEY_DESC)='PCDC ALT NAME' group by item_id, ver_nr)  PCDC_NM,
           (  SELECT item_id, ver_nr, max(def_desc) def_desc FROM ALT_DEF, OBJ_KEY 
           WHERE NCI_DEF_TYP_ID = OBJ_KEY.OBJ_KEY_ID   AND UPPER(OBJ_KEY_DESC)='PCDC DEFINITION'  and obj_key.obj_typ_id =  15 group by item_id, ver_nr)  PCDC_DEF,
           (SELECT item_id, ver_nr, ref_desc PREF_QUEST_TXT
              FROM REF
             WHERE ref_typ_id = 80) REF,
           (  SELECT item_id,
