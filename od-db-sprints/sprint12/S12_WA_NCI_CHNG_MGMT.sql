...
procedure createAIWithoutConcept(rowform in out t_row, idx in integer, v_item_typ_id in integer, v_long_nm in varchar2, actions in out t_actions);
...
procedure createAIWithoutConcept(rowform in out t_row, idx in integer, v_item_typ_id in integer, v_long_nm in varchar2, actions in out t_actions) as
v_def  varchar2(4000);
row t_row;
rows t_rows;
 action t_actionRowset;
...
begin
    ...
    and upper(cncpt_concat_nm) = upper(v_long_nm) and ai.admin_item_typ_id = v_item_typ_id) loop
        ...
        ihook.setColumnValue(row,'ITEM_DESC',v_long_nm);
