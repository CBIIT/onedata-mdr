...
procedure spDelRef (v_data_in in clob, v_data_out out clob, v_user_id in varchar2);
...
function getProceedQuestion (v_cmd in varchar2, v_hdr in varchar2) return t_question;
PROCEDURE spCopyModuleUsingID  (    v_data_in IN CLOB,    v_data_out OUT CLOB,    v_user_id in varchar2) ;


procedure spDeleteAI  ( v_data_in IN CLOB, v_data_out OUT CLOB, v_user_id  IN varchar2);
END;
/
...
BEGIN
 ...
 rowsblob := t_rows();
 if (nci_form_mgmt.isUserAuth(ihook.getColumnValue(row_ori, 'ITEM_ID'), ihook.getColumnValue(row_ori,'VER_NR'), v_user_id) = false) then
 raise_application_error(-20000,'You are not authorized to delete Reference Documents from this form.');
 end if;
    ...
    for i in 1..hookinput.originalrowset.rowset.count loop  --- Iterate through all the selected rows
        row_ori := hookInput.originalRowset.rowset(i);
            ...
            delete from sbr.reference_documents where rd_idseq = ihook.getColumnValue(row_ori,'NCI_IDSEQ');
            
            for curblob in (select * from ref_doc where nci_ref_id = ihook.getColumnValue(row_ori, 'REF_ID')) loop
              rowblob := t_row();
              ihook.setColumnValue(rowblob,'REF_DOC_ID',curblob.REF_DOC_ID);
              delete from sbr.reference_blobs where rd_idseq = curblob.nci_idseq;

            rowsblob.extend;
            rowsblob(rowsblob.last) := rowblob;
              
            end loop;
            
    end loop;
    
            action             := t_actionrowset(rowsblob, 'Reference Document Blobs (Form CO)', 2, 1,'delete');
            actions.extend;
            actions(actions.last) := action;
            action             := t_actionrowset(rowsblob, 'Reference Document Blobs (Form CO)', 2, 2,'purge');
            actions.extend;
            actions(actions.last) := action;
    
            action             := t_actionrowset(rows, 'Reference Documents (Form CO)', 2, 5,'delete');
            actions.extend;
            actions(actions.last) := action;
            action             := t_actionrowset(rows, 'Reference Documents (Form CO)', 2, 6,'purge');
            actions.extend;
            actions(actions.last) := action;
    hookoutput.actions    := actions;
    hookoutput.message := 'Reference Document(s) deleted successfully.';
   commit;
  V_DATA_OUT := IHOOK.GETHOOKOUTPUT (HOOKOUTPUT);
END;
...
if (v_temp = 0) then
select item_nm into v_prot_name from admin_item where item_Id = ihook.getColumnValue(rowform, 'P_ITEM_ID') and ver_nr = ihook.getColumnValue(rowform, 'P_ITEM_VER_NR');
...
values( ihook.getColumnValue(row_ori, 'ITEM_ID') , ihook.getColumnValue(row_ori, 'VER_NR') , 'X', 'Form-Protocol',ihook.getColumnValue(rowform, 'P_ITEM_ID') , 
'FORM-PROTOCOL',
v_user_id,v_user_id, sysdate , sysdate,
ihook.getColumnValue(rowform, 'P_ITEM_ID'), ihook.getColumnValue(rowform, 'P_ITEM_VER_NR'), v_prot_name);

insert into nci_admin_item_rel (p_item_id, p_item_ver_nr,c_item_id, c_item_ver_nr, rel_typ_id ,rep_no,CREAT_USR_ID,
LST_UPD_USR_ID)
values (ihook.getColumnValue(rowform, 'P_ITEM_ID'),ihook.getColumnValue(rowform, 'P_ITEM_VER_NR'), ihook.getColumnValue(row_ori, 'ITEM_ID') ,ihook.getColumnValue(row_ori, 'VER_NR'),
60,0,v_user_id, v_user_id);
insert into onedata_ra.nci_admin_item_rel (p_item_id, p_item_ver_nr,c_item_id, c_item_ver_nr, rel_typ_id ,rep_no,CREAT_USR_ID,
LST_UPD_USR_ID)
values (ihook.getColumnValue(rowform, 'P_ITEM_ID'),ihook.getColumnValue(rowform, 'P_ITEM_VER_NR'), ihook.getColumnValue(row_ori, 'ITEM_ID') ,ihook.getColumnValue(row_ori, 'VER_NR'),
60,0, v_user_id, v_user_id);
...
--else
 ...
 hookoutput.question := getProceedQuestion ('Submit', 'Confirm. Modules will be deleted and recopied: ' ||substr(v_msg_str,2));
 else
for i in 1..hookinput.originalrowset.rowset.count loop
 row_ori :=  hookInput.originalRowset.rowset(i);
 ...
 -1,1,cur1.p_item_id, cur1.p_item_ver_nr, cur1.disp_ord,'MR',cur1.cntxt_item_id, cur1.cntxt_ver_nr, v_user_id);
...
--else
 ...
 hookoutput.question := getProceedQuestion ('Submit', 'Confirm. Modules will be added to forms with following modules: ' ||substr(v_msg_str,2));
 else
for i in 1..hookinput.originalrowset.rowset.count loop
 row_ori :=  hookInput.originalRowset.rowset(i);
...
nci_11179.spCopyModuleNCI(actions, ihook.getColumnValue(row_ori,'CPY_MOD_ITEM_ID'), ihook.getColumnValue(row_ori, 'CPY_MOD_VER_NR'),
 -1,1,cur1.p_item_id, cur1.p_item_ver_nr, v_disp_ord,'MA',cur1.cntxt_item_id, cur1.cntxt_ver_nr, v_user_id);
...
--else
 ...
 hookoutput.question := getProceedQuestion ('Submit','Permanently Delete Question(s): ' ||substr(v_msg_str,2));
 else
for i in 1..hookinput.originalrowset.rowset.count loop
 row_ori :=  hookInput.originalRowset.rowset(i);
 ...
 nci_form_mgmt.updFormModuleQuestion(cur1.nci_pub_id, cur1.nci_ver_nr,v_user_id);
...
BEGIN
 ...
 hookoutput.question := getProceedQuestion ('Submit', 'Permanently Delete Module(s): ' || substr(v_msg_str,2));
 else
for i in 1..hookinput.originalrowset.rowset.count loop
 row_ori :=  hookInput.originalRowset.rowset(i);
...
if (rows.count >0) then
   ...
   update ADMIN_ITEM set LST_UPD_DT = sysdate, LST_UPD_USR_ID = v_user_id where ITEM_ID = ihook.getColumnValue(row_ori, 'P_ITEM_ID') and VER_NR = ihook.getColumnValue(row_ori, 'P_ITEM_VER_NR');
commit;
end if;
...
BEGIN
 ...
 if (hookinput.invocationNUmber = 0) then
 hookoutput.question := getProceedQuestion ('Submit', 'Permanently Delete Form: ' || v_id || 'v' || v_ver_nr);
 else
...
delete from onedata_ra.admin_item where item_id = v_id and ver_nr = v_ver_nr;
commit;
end if;
...
ACTION_TYP, LVL, LVL_PK, ATTR_NM,  CREAT_USR_ID_AUDT,
LST_UPD_USR_ID_AUDT, CREAT_DT_AUDT, LST_UPD_DT_AUDT)
select v_id,v_ver_nr, 'M', 'CS/CSI', v_id, 
'CS/CSI',
--decode(TRNS_TYP, 'U', UPD_COL_NM, 'REF_NM'), 
--decode(TRNS_TYP, 'U', UPD_COL_NEW_VAL,REF_NM), 
v_user_id,v_user_id, sysdate , sysdate
...
delete from onedata_ra.clsfctn_schm where item_id = v_id and ver_nr = v_ver_nr;
when 51 then
delete from NCI_CLSFCTN_SCHM_ITEM where item_id = v_id and ver_nr = v_ver_nr;
delete from onedata_ra.NCI_CLSFCTN_SCHM_ITEM where item_id = v_id and ver_nr = v_ver_nr;
end case;
...
delete from onedata_Ra.ref_doc where nci_ref_id in (Select ref_id from onedata_ra.ref where item_id = v_id and ver_nr = v_ver_nr);
delete from ref where item_id = v_id and ver_nr = v_ver_nr;
delete from onedata_Ra.ref where item_id = v_id and ver_nr = v_ver_nr;
delete from nci_admin_item_rel where rel_typ_id = 65 and c_item_id = v_id and c_item_ver_nr = v_ver_nr; 
delete from onedata_Ra.nci_admin_item_rel where rel_typ_id = 65 and c_item_id = v_id and c_item_ver_nr = v_ver_nr; 
...
delete from onedata_ra.nci_admin_item_rel where c_item_id = v_id and c_item_ver_nr = v_ver_nr;

DeleteCommonChildren(v_id, v_ver_nr, v_idseq);
delete from nci_module where item_id = v_id and ver_nr = v_ver_nr;
delete from onedata_ra.nci_module where item_id = v_id and ver_nr = v_ver_nr;
...
select max(nvl(retain_ind,0)) into v_retain_ind from nci_usr_cart where cart_nm = v_deflt_cart_nm and cntct_secu_id = v_user_id and upper(guest_usr_nm) = 'NONE';
 forms              := hookInput.forms;
        form1              := forms(1);
...
-- not stand-along modules

for cur1 in (select p_item_id, p_item_ver_nr from nci_admin_item_rel r, admin_item ai where c_item_id = cur.item_id and c_item_ver_nr = cur.ver_nr and rel_typ_id = 61
and ai.item_id = r.p_item_id and ai.ver_nr = r.p_item_ver_nr and ai.currnt_ver_ind = 1 and p_item_id <> -1) loop
  v_added := v_added + 1;
    ...
    ihook.getColumnValue(row_ori, 'CNTXT_ITEM_ID'),ihook.getColumnValue(row_ori, 'CNTXT_VER_NR'), v_user_id);
                               nci_11179_2.AddItemToCart(cur.item_id, cur.ver_nr, v_user_id, v_deflt_cart_nm,v_retain_ind, rowscart);

    v_disp_ord := v_disp_ord + 1;
    end loop;

-- stand-along module
for cur1 in (select p_item_id, p_item_ver_nr from nci_admin_item_rel r where c_item_id = cur.item_id and c_item_ver_nr = cur.ver_nr and rel_typ_id = 61
 and p_item_id = -1) loop
   v_added := v_added + 1;
    nci_11179.spCopyModuleNCI (actions, cur.item_id,cur.ver_nr,
    cur1.p_item_id, cur1.p_item_ver_nr,
    ihook.getColumnValue(row_ori,'ITEM_ID'), ihook.getColumnValue(row_ori,'VER_NR'), v_disp_ord,'C',
