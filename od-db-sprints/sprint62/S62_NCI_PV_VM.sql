...
procedure spPVVMImportOLD ( row_ori in out t_row, actions in out t_actions, rowais in out t_rows, rowiucs in out t_rows, rowcdvms in out t_rows, rowpvs in out t_rows, rowaltnms in out t_rows);
function parsePVVMRefStr (v_str in varchar2, v_delim in varchar2, row in out t_row) return integer;
...
function parsePVVMRefStr (v_str in varchar2, v_delim in varchar2, row in out t_row) return integer --returns either the referenced vm id or 0
    IS
        V_OUT integer;
        v_ref_str varchar2(4000);
        v_ref_cnt integer;
        v_del varchar2(32);
        v_id number;
        startIndex number;
    ...
    BEGIN
        startIndex := 1;
        endIndex := INSTR(v_str, v_delim,1,1);
        v_ref_str := v_str;
        v_del := v_delim;
        v_ref_cnt := nci_11179.getWordCountDelim(v_ref_str, v_del);
        
        for i in 1..v_ref_cnt loop
            if (endIndex = 0) then
                v_seq_id := to_number(v_ref_str);
                select nvl(item_1_id,0) into v_id from nci_stg_pv_vm_import where btch_seq_nbr = v_seq_id and btch_nm = ihook.getColumnValue(row, 'BTCH_NM');
                if (v_id <> 0) then
                    return v_id;
                end if;
            else
                v_seq_id := to_number(substr(v_ref_str, startIndex, endIndex-1));
            --select count(*) into v_temp from nci_stg_pv_vm_import where btch_seq_nbr 
                select nvl(item_1_id,0) into v_id from nci_stg_pv_vm_import where btch_seq_nbr = v_seq_id and btch_nm = ihook.getColumnValue(row, 'BTCH_NM');
                if (v_id <> 0) then
                    return v_id;
                end if;
                v_ref_str := substr(v_ref_str, endIndex+1, length(v_ref_str)-endIndex);
                endIndex := INSTR(v_ref_str, v_delim,1,1);
            end if;
        end loop;
...
--        nci_dec_mgmt.createValAIWithConcept(row_ori , 1,v_item_typ_glb ,'C','DROP-DOWN',actions); -- Vm
        ...
        --parse ref_btch_str for referenced seq ids, then iterate over each seq id and check if item_1_id is set
            ...
            v_vm_item_id := parsePVVMRefStr(ihook.getColumnValue(row_ori, 'REF_BTCH_STR'), ',', row_ori);
