...
begin
  ...
  if (   ihook.getColumnValue(rowform, 'ITEM_1_ID') is not null and ihook.getColumnValue(rowform, 'ITEM_2_ID') is not null) then
     ...
     --    raise_application_error(-20000, 'Here');
                    ihook.setColumnValue(rowform, 'CTL_VAL_MSG', 'Warning: DUPLICATE DEC found in other context:' || cur.item_id || 'v' || cur.ver_nr);
                                     -- v_val_ind:= false;
...
if (v_cncpt_src ='STRING') then
      if (ihook.getColumnValue(rowform, 'CNCPT_CONCAT_STR_' || idx) is not null) then
                ...
                for i in  1..cnt loop
                        ...
                        v_interim := false;
                        for cur in(select item_id, item_nm , item_long_nm, item_desc from admin_item where admin_item_typ_id = 49 and upper(item_long_nm) = upper(trim(v_cncpt_nm))
