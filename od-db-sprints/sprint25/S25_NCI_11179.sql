...
v_to_form_ver number, v_disp_ord number, v_src in varchar2, v_cntxt_item_id in number, v_cntxt_ver_nr in number);
...
/*
EXECUTE immediate 'DBMS_MVIEW.REFRESH(''VW_CLSFCTN_SCHM'')';
execute immediate 'EXECUTE DBMS_MVIEW.REFRESH(''VW_CLSFCTN_SCHM_ITEM'');';

EXECUTE immediate 'EXECUTE DBMS_MVIEW.REFRESH(''VW_CNTXT'');';
EXECUTE immediate 'EXECUTE DBMS_MVIEW.REFRESH(''VW_CONC_DOM'');';
EXECUTE immediate 'EXECUTE DBMS_MVIEW.REFRESH(''VW_CNCPT'');';
*/
...
/* Work in Progress */
procedure spCopyQuestionDirect ( v_from_module_id in integer, v_from_module_ver in integer, v_to_module_id in integer, v_to_module_ver in integer, v_disp_ord number, v_src in varchar2) as
v_id integer;
v_found boolean;
v_itemid integer;
row  t_row;
...
--raise_application_error(-20000, 'Here' || v_id);
  ...
  cur.item_nm, 63, cur.disp_ord, cur.edit_ind, cur.req_ind, cur.instr, cur.deflt_val);
  commit;
  ...
  v_itemid, 1, cur1.INSTR, cur1.disp_ord, cur1.NCI_PUB_ID);
...
v_from_form_id in number, v_from_form_ver number, v_to_form_id number, v_to_form_ver number, v_disp_ord number, v_src in varchar2, v_cntxt_item_id in number, v_cntxt_ver_nr in number) as
...
-- Questions
 if (v_src = 'C') then  -- on Copy, rep number is 0
  ...
  if (v_src = 'V') then  -- on Copy, rep number is 0
   spCopyQuestionDirect(v_from_module_id, v_from_module_ver, v_to_module_id, v_to_form_ver,v_disp_ord, v_src);
 end if;
...
/*  If Form  */
    ...
    -- Add Modules
     ...
     and nvl(fld_delete,0) = 0) loop
        nci_11179.spCopyModuleNCI(actions, cur2.c_item_id, cur2.c_item_ver_nr, v_admin_item.item_id, v_admin_item.item_id, v_admin_item.item_id, v_version, cur2.disp_ord, 'V', null, null);
     end loop;
     -- Add protocols
