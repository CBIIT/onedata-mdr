...
procedure spCreateDloadProfile_v2 (v_data_in in clob, v_data_out out clob, v_usr_id  IN varchar2);
function getDloadObjNm (v_fmt_id in integer) return varchar2;
END;
/
...
*/
function getDloadObjNm (v_fmt_id in integer) return varchar2 is
begin
case v_fmt_id
    when 90 then return 'ALS Specific';
    when 110 then return 'ALS Specific for Form';
    when 223 then return 'NIH Submission Template Specific';
    when 224 then return 'Custom CDE Subtype';
    when 230 then return 'Custom Data Collection Excel';
    when 245 then return 'Custom DEC Subtype';
    when 246 then return 'Custom VD Subtype';
    when 249 then return 'CDE Model Import Template';
    else return 'NA';
end case;
end;
...
begin
    ...
    if (hookinput.invocationnumber = 1) then -- show create form
        ...
        --extension rowset
        v_dload_ext := getDloadObjNm(v_coll_typ);
        rowsetext := t_rowset(rows, v_dload_ext, 1, 'NCI_DLOAD_HDR');

        
        hookOutput.forms := getALSCreateForm(rowsethdr, rowsetext, v_coll_typ);
    ...
    -- Non-eitable lookup values are lost. So workaround.
     --   v_coll_typ := ihook.getColumnValue(rowhdr, 'DLOAD_FMT_ID');
        ...
        ihook.setColumnValue (rowhdr, 'DLOAD_FMT_ID', v_coll_typ);
    v_dload_ext := getDloadObjNm(v_coll_typ);
    if (v_dload_ext <> 'NA') then
        form1 := forms(2);
        --rowext := form1.rowset.rowset(1);
    end if;
        rowext := form1.rowset.rowset(1);
        ...
        ihook.setColumnValue(rowhdr, 'HDR_ID', v_id);
        ihook.setColumnValue(rowext, 'HDR_ID', v_id);
...
if (v_err_str is not null) then
    ...
    actions.extend;
    actions(actions.last) := action;
        v_dload_ext := getDloadObjNm(ihook.getColumnValue(rowhdr, 'DLOAD_FMT_ID'));

    if (ihook.getColumnValue(rowhdr, 'DLOAD_FMT_ID' ) in (90, 110, 223, 224, 230, 245, 246, 249)) then
        rows := t_rows();
        rows.extend;    rows(rows.last) := rowext;

        action := t_actionrowset(rows, v_dload_ext, 2, 2, 'insert');
        actions.extend;
        actions(actions.last) := action;
    end if;
    ...
    if (ihook.getColumnValue(rowhdr, 'DLOAD_FMT_ID') = 230) then
       if (ihook.getColumnValue(rowext, 'ALT_NM_TYP_ID') is null and ihook.getColumnValue(rowext, 'COL_ID') = 91) then
           raise_application_error(-20000, 'Alternate Name Type must be selected when column heading is set to CDE Alternate Name.');
           return;
       end if;
    end if;
...
BEGIN
    ...
    if (ihook.getColumnValue(row_ori,'DLOAD_FMT_ID') not in (229, 244)) then
...
v_item_typ_id := nci_11179_2.getDloadItemTyp (ihook.getColumnValue(row_ori,'DLOAD_FMT_ID'));
v_fmt_id := ihook.getColumnValue(row_ori, 'DLOAD_FMT_ID');
    if (hookinput.invocationnumber = 0) then   -- First invocation
         forms                  := t_forms();
...
select max(nvl(retain_ind,0)) into v_retain_ind from nci_usr_cart where cart_nm = v_deflt_cart_nm and cntct_secu_id = v_usr_id and upper(guest_usr_nm) = 'NONE';
        ...
        -- Only add if item is of the right type and not currently in collection.
                ...
                rows.extend;
                            rows (rows.last) := row;
                            v_found := false;
                            if (v_item_typ_id = 58 and v_fmt_id <> 244) then 
...
--            actions.extend;
--            actions(actions.last) := action;           
            if (v_item_typ_id = 58 and v_fmt_id <> 244) then -- insert into mdl map rules details            
...
---raise_application_error(-20000, v_item_typ_id);
        ...
        -- If Item needs to be added.
        if (rows.count > 0) then
            if (v_item_typ_id = 58 and v_fmt_id <> 244) then --model map insert
                action := t_actionrowset(rows_mm, 'Model Mapping Detail (New)', 2,0,'insert');
                actions.extend;
                actions(actions.last) := action;
            else --all other download types
...
begin
    ...
    elsif (v_coll_typ = 245) then
        form1 := t_form('Custom DEC Subtype', 2, 1);
        form1.rowset := v_rowset2;
        forms.extend;   forms(forms.last) := form1;
        
    elsif (v_coll_typ = 246) then
        form1 := t_form('Custom VD Subtype', 2, 1);
        form1.rowset := v_rowset2;
        forms.extend;   forms(forms.last) := form1;
    elsif (v_coll_typ = 249 ) then
        form1 := t_form('CDE Model Import Template', 2, 1);
        form1.rowset := v_rowset2;
        forms.extend;  forms(forms.last) := form1;
    end if;
  return forms;
...
BEGIN
    v_hdr_id := hdr_id;
    j := 2;
    if (v_mode like '%DEC%') then
        select count(*) into v_cnt from nci_dload_cstm_col_dtl_dec where hdr_id = v_hdr_id;
    elsif (v_mode like '%VD%') then
        select count(*) into v_cnt from nci_dload_cstm_col_dtl_vd where hdr_id = v_hdr_id;
    else
        select count(*) into v_cnt from nci_dload_cstm_col_dtl where hdr_id = v_hdr_id;
    end if;
   -- raise_application_error(-20000, v_cnt);
    if (v_cnt > 0) then
        if (v_mode = 'I') then --cde insert
            for i in (select col_id from nci_dload_cstm_col_dtl where hdr_id = v_hdr_id and nvl(fld_delete, 0) <> 1) LOOP
                update nci_dload_cstm_col_dtl set col_pos = j where hdr_id = v_hdr_id and col_id = i.col_id;
                update onedata_ra.nci_dload_cstm_col_dtl set col_pos = j where hdr_id = v_hdr_id and col_id = i.col_id;
                j := j+1;
            END LOOP;
        elsif (v_mode = 'DECI') then --dec insert
            for i in (select col_id from nci_dload_cstm_col_dtl_dec where hdr_id = v_hdr_id and nvl(fld_delete, 0) <> 1) LOOP
                update nci_dload_cstm_col_dtl_dec set col_pos = j where hdr_id = v_hdr_id and col_id = i.col_id;
                update onedata_ra.nci_dload_cstm_col_dtl_dec set col_pos = j where hdr_id = v_hdr_id and col_id = i.col_id;
                j := j+1;
            END LOOP;
        elsif (v_mode = 'VDI') then --vd insert
            for i in (select col_id from nci_dload_cstm_col_dtl_vd where hdr_id = v_hdr_id and nvl(fld_delete, 0) <> 1) LOOP
                update nci_dload_cstm_col_dtl_vd set col_pos = j where hdr_id = v_hdr_id and col_id = i.col_id;
                update onedata_ra.nci_dload_cstm_col_dtl_vd set col_pos = j where hdr_id = v_hdr_id and col_id = i.col_id;
                j := j+1;
            END LOOP;        
        elsif (v_mode = 'RCDE') then --remove cde gaps
       --raise_application_error(-20000, v_hdr_id);
            j := 1;
            for i in (select col_id from nci_dload_cstm_col_dtl where hdr_id = v_hdr_id and nvl(fld_delete, 0) <> 1 order by col_pos asc) LOOP
                update nci_dload_cstm_col_dtl set col_pos = j where hdr_id = v_hdr_id and col_id = i.col_id;
                update onedata_ra.nci_dload_cstm_col_dtl set col_pos = j where hdr_id = v_hdr_id and col_id = i.col_id;
              --  raise_application_error(-20000, v_hdr_id);
                j := j+1;
            END LOOP;
        elsif (v_mode = 'RDEC') then --remove dec gaps
--raise_application_error(-20000, v_mode);
            j := 1;
            for i in (select col_id from nci_dload_cstm_col_dtl_dec where hdr_id = v_hdr_id and nvl(fld_delete, 0) <> 1 order by col_pos asc) LOOP
                update nci_dload_cstm_col_dtl_dec set col_pos = j where hdr_id = v_hdr_id and col_id = i.col_id;
                update onedata_ra.nci_dload_cstm_col_dtl_dec set col_pos = j where hdr_id = v_hdr_id and col_id = i.col_id;
              --  raise_application_error(-20000, v_hdr_id);
                j := j+1;
            END LOOP;
        elsif (v_mode = 'RVD') then --remove vd gaps
     --   raise_application_error(-20000, v_mode);
            j := 1;
            for i in (select col_id from nci_dload_cstm_col_dtl_vd where hdr_id = v_hdr_id and nvl(fld_delete, 0) <> 1 order by col_pos asc) LOOP
                update nci_dload_cstm_col_dtl_vd set col_pos = j where hdr_id = v_hdr_id and col_id = i.col_id;
                update onedata_ra.nci_dload_cstm_col_dtl_vd set col_pos = j where hdr_id = v_hdr_id and col_id = i.col_id;
              --  raise_application_error(-20000, v_hdr_id);
                j := j+1;
...
answers     t_answers;
 forms t_forms;
    ...
    v_max_disp_ord number;
     rowform t_row;
     v_sql  varchar2(4000);
     v_dtl_tbl varchar2(64);
     v_dtl_obj varchar2(128);
     v_hdr_id number;
     v_dload_fmt number;
     v_mode varchar2(32);
BEGIN
 ...
 select dload_fmt_id into v_dload_fmt from nci_dload_hdr where hdr_id = v_hdr_id;
    if (v_dload_fmt = 245) then
        v_mode := 'DEC';
    elsif (v_dload_fmt = 246) then
        v_mode :='VD';
    else
        v_mode := 'CDE';
    end if;
...
-- end if;
    if (v_mode = 'DEC') then
        v_dtl_tbl := 'NCI_DLOAD_CSTM_COL_DTL_DEC';
        v_dtl_obj := 'Custom DEC Download - Column Relationship';
    elsif (v_mode = 'VD') then
        v_dtl_tbl := 'NCI_DLOAD_CSTM_COL_DTL_VD';
        v_dtl_obj := 'Custom VD Download - Column Relationship';
    else
        v_dtl_tbl := 'NCI_DLOAD_CSTM_COL_DTL';
        v_dtl_obj := 'Custom CDE Download - Column Relationship';
    end if;
   ...
   if hookInput.invocationNumber = 0 then  -- If first invocation, prompt for version number
    ...
    if (v_mode = 'DEC') then
        select least(max(col_pos), count(col_pos))  into v_max_disp_ord from nci_dload_cstm_col_dtl_dec where hdr_id = ihook.getColumnValue(row_ori, 'HDR_ID') and nvl(fld_Delete,0) = 0;
    elsif (v_mode = 'VD') then
        select least(max(col_pos), count(col_pos))  into v_max_disp_ord from nci_dload_cstm_col_dtl_vd where hdr_id = ihook.getColumnValue(row_ori, 'HDR_ID') and nvl(fld_Delete,0) = 0;
    else
            ...
            if (v_cur_disp_ord > v_new_disp_ord) then
             ...
             --       v_sql := 'select hdr_id, fmt_nm, col_id, col_pos, dt_last_modified, creat_dt, creat_usr, retain_ind, creat_usr_id, lst_upd_usr_id, fld_delete, lst_del_dt, s2p_trn_dt, lst_upd_dt from nci_dload_cstm_col_dtl where nvl(fld_delete,0) = 0 and hdr_id = ' ||  ihook.getColumnValue(row_ori, 'HDR_ID') || ' and col_pos = ' || i ;
                   -- v_sql := 'select * from nci_dload_cstm_col_dtl where nvl(fld_delete,0) = 0 and hdr_id = ' ||  ihook.getColumnValue(row_ori, 'HDR_ID') || ' and col_pos = ' || i ; 
                    v_sql := 'select * from ' || v_dtl_tbl || ' where nvl(fld_delete,0) = 0 and hdr_id = ' ||  ihook.getColumnValue(row_ori, 'HDR_ID') || ' and col_pos = ' || i ; 
                   -- raise_application_error(-20000, v_sql);
               --     nci_11179.ReturnRow(v_sql, 'NCI_DLOAD_CSTM_COL_DTL', row);
                    nci_11179.ReturnRow(v_sql, v_dtl_tbl, row);
...
--            nci_11179.ReturnRow(v_sql, 'NCI_DLOAD_CSTM_COL_DTL', row);
            v_sql := 'select * from ' || v_dtl_tbl || ' where nvl(fld_delete,0) = 0 and hdr_id = ' ||  ihook.getColumnValue(row_ori, 'HDR_ID') ||  ' and col_pos = ' || v_cur_disp_ord ;
            nci_11179.ReturnRow(v_sql, v_dtl_tbl, row);
          ...
          --  action := t_actionRowset(rows, 'Custom CDE Download - Column Relationship', 2, 1000, 'update');
            action := t_actionRowset(rows, v_dtl_obj, 2, 1000, 'update');
            actions.extend; actions(actions.last) := action;
            hookoutput.actions:= actions;
    end if;
     v_data_out := ihook.getHookOutput(hookOutput);
...
v_hdr_id := ihook.getColumnValue(row_ori, 'HDR_ID');
select dload_fmt_id into v_dload_fmt from nci_dload_hdr where hdr_id = v_hdr_id;
--raise_application_error(-20000, v_dload_fmt);
if (v_dload_fmt = 224) then
    v_op := 'RCDE';
elsif (v_dload_fmt = 245) then
    v_op := 'RDEC';
elsif (v_dload_fmt = 246) then
    v_op := 'RVD';
end if;
--raise_application_error(-20000,v_op);
spInitCstmColOrder(v_hdr_id, v_op);
