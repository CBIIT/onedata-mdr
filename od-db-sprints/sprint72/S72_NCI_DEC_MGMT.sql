...
procedure createValAIWithConceptRow(rowform in out t_row, idx in integer,v_item_typ_id in integer, 
v_mode in varchar2,v_cncpt_src in varchar2,  rowais in out t_rows, rowiucs in out t_rows);
procedure createSingleConceptAI(v_cncpt_id number, v_cncpt_ver_nr number, v_item_typ_id in integer,v_ai_id out number, v_ai_ver_nr out number,actions in out t_actions);
...
procedure spDECValCreateImport ( rowform in out t_row,  v_op  in varchar2, actions in out t_actions, v_val_ind in out boolean, v_usr_id in varchar2);
procedure createVersionDECImport (rowform in out t_row, actions in out t_actions,v_item_id in number, v_ver_nr in number, v_usr_id in varchar2);
-- main proc to detect if the concept combination exists or not.
...
--procedure CncptCombExistsVM (rowform in out t_row, v_item_nm in varchar2, v_item_def in varchar2, v_item_typ in integer, v_idx in number, v_item_id out number, v_item_ver_nr out number);
...
-- Final procedure to create DEC - after all the validations
...
-- Integer concept hard-coded 
v_int_cncpt_id  number := 2433736;

-- Bulk updae Workflos Status, Registration Status, Context and/or Conceptual Domain
...
BEGIN
  ...
  if hookInput.invocationNumber = 0  then -- first invocation show the empty form. Item_id/Ver_nr is defaulted to -1 as dummy values
     	 answers := t_answers();
        ...
        -- Default values for the form that shows up
        rows := t_rows();  row := t_row();
        ihook.setColumnValue(row, 'ITEM_ID', -1); -- Dummy value
        ihook.setColumnValue(row, 'VER_NR',-1); -- Dummy value
  ...
  elsif hookInput.invocationNumber = 1 then -- Second iteration of the hook go thru all selected rows and update non-null values
        ...
        for i in 1..hookinput.originalrowset.rowset.count loop  -- loop thru the selected rows when the hook was triggered.
                row_ori :=  hookInput.originalRowset.rowset(i);
                if (ihook.getColumnValue(row_ori, 'ADMIN_ITEM_TYP_ID') = 2) then -- only update DEC
                    v_item_id :=ihook.getColumnValue(row_ori, 'ITEM_ID');
                    v_ver_nr :=ihook.getColumnValue(row_ori, 'VER_NR');
                    -- workflow status, registration status, context are in the ADMIN_ITEM table. Rows is for ADMIN_ITEM
                      ...
                      -- Conceptual Domain is in DE_CONC table. So separate rowscd to assimilate the changes
                      if (ihook.getColumnValue(rowform, 'CONC_DOM_ITEM_ID') is not null ) then
                            row := t_row();
                            -- Get current DE_CONC row values. 
                 ...
                 end if;  -- end if Conceptual Domain
        end loop; -- End loop selected rows

        if (rows.count > 0) then -- if Status or Context to update.
                action := t_actionrowset(rows, 'Administered Item (No Sequence)', 2,1,'update');
                actions.extend;
                actions(actions.last) := action;
        end if;

        if (rowscd.count > 0) then  -- if any Conceptual Domain to update
   ...
   end if;  -- end if second iteration
...
-- There are 2 sections in the Create DEC screen. Admin_ITEM and then the Concept section
-- Default values for both section have to be setup for a new DEC

    -- Change default values for Admin_item section
    row := row_ori;
    ihook.setColumnValue(row, 'ITEM_ID', -1);  -- Default the Item ID
    nci_11179_2.setStdAttr(row);  -- Default Workflow, Admin status
    ...
    -- Change default values for Concepts section
    ...
    -- Common procedure called from Create New and Create from existing
...
begin
   ...
   -- functionality provided to also have concept strings that they can use to manipulate    
   ...
   ihook.setColumnValue(row, 'CNCPT_CONCAT_STR_2_ORI', v_str);
    ...
    spDECCommonEdit(rowsetai,rowsetcncpt,  'e',3, hookinput, hookoutput);
...
-- v_init is the initial rowset to populate. v_init_cncpt is the default row for the second section
-- v_op is insert or update
-- v_src 1 - create new, 2 - Create from Existing; 3 - Edit
PROCEDURE       spDECCommon ( v_init in t_rowset,  v_init_cncpt in t_rowset, v_op  in varchar2, v_src in integer, hookInput in t_hookInput, hookOutput in out t_hookOutput)
AS
rowform t_row;
forms t_forms;
...
is_valid boolean;
begin
    if hookInput.invocationNumber = 0 then  -- First invocation is the only time the default values are used
        ...
        ihook.setColumnValue(rowform, 'CNTXT_ITEM_ID', ihook.getColumnValue(rowai, 'CNTXT_ITEM_ID'));
         ihook.setColumnValue(rowform, 'CNTXT_VER_NR', ihook.getColumnValue(rowai, 'CNTXT_VER_NR'));
...
-- This section is not used. Separate procedure for Edit. Not deleting for avoiding regression testing.
--- NOT USED.
...
v_answer integer;
is_valid boolean;
v_tmp_short_nm varchar2(255);
--jira 4007
v_op_alt varchar2(32);
begin
            row_ori :=  hookInput.originalRowset.rowset(1);
...
or ihook.getColumnValue(rowform,'CNCPT_CONCAT_STR_2') = ihook.getColumnValue(rowform,'CNCPT_CONCAT_STR_2_ORI'))) then
    v_answer := 2;  -- Validate using dd
elsif (hookinput.answerid = 5 and (ihook.getColumnValue(rowform, 'CNCPT_CONCAT_STR_1') is null or ihook.getColumnValue(rowform, 'CNCTP_CONCAT_STR_2') is null)) then
    v_answer := 2; --Validate using DD
elsif(hookinput.answerid = 6 and ihook.getColumnValue(rowform,'CNCPT_CONCAT_STR_1') is not null
and (ihook.getColumnValue(rowform,'CNCPT_CONCAT_STR_1') <> ihook.getColumnValue(rowform,'CNCPT_CONCAT_STR_1_ORI')
or ihook.getColumnValue(rowform,'CNCPT_CONCAT_STR_2') <> ihook.getColumnValue(rowform,'CNCPT_CONCAT_STR_2_ORI'))) then
...
--jira 4007: only update from Edit DEC if hookinput answer is update
...
--raise_application_error(-20000, hookinput.answerid || v_answer);
  ...
  -- If update, then need to get the Item_id to exclude from duplicate check
       if (lower(v_op) = 'update' or v_op_alt = 'update') then
            row_ori :=  hookInput.originalRowset.rowset(1);
            v_item_id := ihook.getColumnValue(row_ori,'ITEM_ID');
            v_ver_nr := ihook.getColumnValue(row_ori,'VER_NR');
       end if;
    ...
    --    nci_11179_2.stdCncptRowValidation(rowform, 1,is_valid, v_err_str );
     --   nci_11179_2.stdCncptRowValidation(rowform, 2,is_valid, v_err_str );
        if (v_err_str is not null) then
         ...
         -- If any of the test fails or if the user has triggered validation, then go back to the screen.
        if (is_valid=false or v_answer = 1 or v_answer = 2) then
--raise_application_error(-20000,'here 2');
                hookoutput.message := ihook.getColumnValue(rowform, 'CTL_VAL_MSG');
                ihook.setColumnValue(rowai, 'ITEM_NM', ihook.getColumnValue(rowform, 'GEN_STR'));
       --         ihook.setColumnValue(rowform, 'CNTXT_VER_NR', ihook.getColumnValue(rowai, 'CNTXT_VER_MR'));
                ihook.setColumnValue(rowai, 'ADMIN_ITEM_TYP_ID', 2);
    ...
    if (upper(v_op) = 'INSERT') then
        ...
        createDEC(rowai, rowform, actions, v_item_id);
        hookoutput.message := 'DEC Created Successfully with ID ' || v_item_id ;
    elsif (lower(v_op) = 'update' or v_op_alt = 'update') then
    -- Update DEC. Get the selected row, update name, definition, context.
        row := t_row();
        row_ori :=  hookInput.originalRowset.rowset(1);
    ...
    if (ihook.getColumnValue(rowai, 'ITEM_NM_CURATED') is not null) then
       ...
       -- regenerating short name: jira 3810-only regenerate if was generated before. if manually updated, do not regenerate

       if (nci_11179_2.isDECnmSysGenNew(ihook.getColumnValue(rowai, 'ITEM_LONG_NM')) or upper(ihook.getColumnValue(rowai, 'ITEM_LONG_NM')) = 'SYSGEN') then
      -- raise_application_error(-20000, ihook.getColumnValue(rowai, 'ITEM_LONG_NM'));
            v_tmp_short_nm := nci_11179_2.getStdShortName(ihook.getColumnValue(rowform,'ITEM_1_ID'), ihook.getColumnValue(rowform,'ITEM_1_VER_NR')) || ':' || 
                nci_11179_2.getStdShortName(ihook.getColumnValue(rowform,'ITEM_2_ID'), ihook.getColumnValue(rowform,'ITEM_2_VER_NR'));
            ihook.setColumnValue(row,'ITEM_LONG_NM',v_tmp_short_nm);
        else
            --manually entered short name
...
-- userd if single concept AI needs to be created. 
procedure createSingleConceptAI(v_cncpt_id number, v_cncpt_ver_nr number, v_item_typ_id in integer,v_ai_id out number, v_ai_ver_nr out number,actions in out t_actions)
as
v_cncpt_code  varchar2(255);
row t_row;
rows t_rows;
v_obj_nm varchar2(255);
action t_actionrowset;

begin


select item_long_nm into v_cncpt_code from admin_item where  item_id = v_cncpt_id and ver_nr = v_cncpt_ver_nr;

-- Admin Item found
                for cur in (select ai.item_id, ai.ver_nr from admin_item ai, nci_admin_item_ext e where ai.item_id = e.item_id and
                ai.ver_nr = E.ver_nr and e.cncpt_concat = v_cncpt_code and ai.admin_item_typ_id = v_item_typ_id) loop
                  v_ai_id := cur.item_id;
                  v_ai_ver_nr := cur.ver_nr;
                  return;
                  end loop;
                  -- no item found then create
    for cur in (select item_nm, item_long_nm, item_desc from admin_item where item_id = v_cncpt_id and ver_nr = v_cncpt_ver_nr) loop
    row := t_row();
    rows := t_rows();
        v_ai_id := nci_11179.getItemId;
        v_ai_ver_nr := 1;    

        ihook.setColumnValue(row,'ITEM_ID', v_ai_id);
        ihook.setColumnValue(row,'ADMIN_ITEM_TYP_ID', v_item_typ_id);
        nci_11179_2.setStdAttr(row);
        nci_11179_2.setItemLongNm (row, v_ai_id);
     ...
     ihook.setColumnValue(row,'CNCPT_CONCAT', cur.item_long_nm);
        ihook.setColumnValue(row,'CNCPT_CONCAT_DEF',cur.item_desc);
        ihook.setColumnValue(row,'CNCPT_CONCAT_NM', cur.item_nm);
        ihook.setColumnValue(row,'CNCPT_CONCAT_NM_WITH_INT', cur.item_long_nm);
...
if (   ihook.getColumnValue(rowform, 'ITEM_1_ID') is not null and ihook.getColumnValue(rowform, 'ITEM_2_ID') is not null) then
  ...
  --  ihook.setColumnValue(rowform, 'CTL_VAL_MSG', 'WARNING: Duplicate DEC found in same context: ' || cur.item_id || 'v' || cur.ver_nr ||  ' '||  cur.cntxt_nm_dn || '. Run Update CDE.' || chr(13));
     ...
     if (ihook.getColumnValue(rowform, 'CNCPT_1_ITEM_ID_1') is null or ihook.getColumnValue(rowform, 'CNCPT_2_ITEM_ID_1') is null) then
        ...
        */
        if (v_stus like '%RETIRED%') then
            ihook.setColumnValue(rowform, 'CTL_VAL_MSG', ihook.getColumnValue(rowform, 'CTL_VAL_MSG') || 'WARNING: DEC is Retired.' || chr(13));
        elsif (v_stus like '%RELEASED%') then --jira 3284
            ihook.setColumnValue(rowform, 'CTL_VAL_MSG', ihook.getColumnValue(rowform, 'CTL_VAL_MSG') || 'WARNING: DEC is RELEASED.' || chr(13));
        end if;
...
--raise_application_error(-20000, 'here 1');
        --createDECImportOld(rowform, actions);
            createDECImport(rowform, actions); --jira 3149
         --   raise_application_error(-20000, 'here');
        end if;
        ...
        if (v_val_ind = true and v_op = 'N') THEN

            createValAIWithConcept(rowform , 1,5,'C','DROP-DOWN',actions); -- OC
            createValAIWithConcept(rowform , 2,6,'C','DROP-DOWN',actions); -- Property
            createVersionDECImport(rowform, actions, v_item_id, v_ver_nr, v_usr_id); 
        end if;
       --   raise_application_error(-20000,'Inside');
...
if (nvl(v_mode,'C') <> 'O' or v_mode = 'V') then
    ...
    and upper(cncpt_concat_nm) = upper(v_long_nm) and ai.admin_item_typ_id = v_item_typ_id and ai.admin_stus_nm_dn not like '%RETIRED%') loop
        ihook.setColumnValue(rowform,'ITEM_' || idx || '_ID',cur.item_id);
        ihook.setColumnValue(rowform,'ITEM_' || idx || '_VER_NR',cur.ver_nr);
    --     ihook.setColumnValue(rowform,'CTL_VAL_MSG', 'WARNING: Reusing Existing VM: ' || cur.item_id || chr(13));
        return;
    end loop;
end if;
if (v_mode = 'V') then
...
-- v_mode : V - Validate, C - Validation and Create;  v_cncpt_src:  STRING: String; DROP-DOWN: Drop-downs
procedure createValAIWithConceptRow(rowform in out t_row, idx in integer,v_item_typ_id in integer, v_mode in varchar2,v_cncpt_src in varchar2,  
rowais in out t_rows,  rowiucs in out t_rows) as
...
begin
   ...
   /*     v_long_nm := trim(ihook.getColumnValue(rowform, 'ITEM_1_LONG_NM_INT')  || ' ' || ihook.getColumnValue(rowform, 'ITEM_2_LONG_NM_INT'));
       ...
       -- ihook.setColumnValue(row,'ITEM_DESC',substr(substr(ihook.getColumnValue(rowform, 'ITEM_1_DEF')  || ':' || ihook.getColumnValue(rowform, 'ITEM_2_DEF'),1,4000),2));
 ihook.setColumnValue(row,'ITEM_DESC',substr(ihook.getColumnValue(rowform, 'ITEM_1_DEF')  || ':' || ihook.getColumnValue(rowform, 'ITEM_2_DEF'),1,4000));
      ...
      --  raise_application_error (-20000, ihook.getColumnValue(rowform,'CONC_DOM_ITEM_ID')  || ihook.getColumnValue(rowform, 'ITEM_1_ID') || ihook.getColumnValue(rowform, 'ITEM_2_ID'));
       ...
       ihook.setColumnValue(row,'CONC_DOM_ITEM_ID',nvl(ihook.getColumnValue(rowform,'CONC_DOM_ITEM_ID'),1) );
        ...
        if (ihook.getColumnValue(rowform,'ORIGIN_ID') is not null) then
            ihook.setColumnValue(row,'ORIGIN_ID',nvl(ihook.getColumnValue(rowform,'ORIGIN_ID'),1) );
        end if;
...
procedure createVersionDECImport (rowform in out t_row, actions in out t_actions,v_item_id in number, v_ver_nr in number, v_usr_id in varchar2) as
v_nm  varchar2(255);
v_long_nm varchar2(255);
v_def  varchar2(4000);
v_short_nm varchar2(30);
...
begin
  ...
  nci_11179_2.resetAuditAttr(row, v_usr_id);
      ...
      --  ihook.setColumnValue(row,'CNTXT_VER_NR', ihook.getColumnValue(rowform,'CNTXT_VER_NR'));
            ...
            ihook.setColumnValue(row,'ADMIN_STUS_ID',65); --draft new
       ...
       -- raise_application_error(-20000,ihook.getColumnValue(row,'ADMIN_NOTES'));
        
          ihook.setColumnValue(row,'ADMIN_NOTES',ihook.getColumnValue(rowform,'ADMIN_NOTES') || ' ' || ihook.getColumnValue(row, 'ADMIN_NOTES') );

        ihook.setColumnValue(row,'LST_UPD_DT',sysdate );
         ihook.setColumnValue(row,'CREAT_DT',sysdate );
        rows.extend;
        rows(rows.last) := row;
--raise_application_error(-20000, 'Deep' || ihook.getColumnValue(row,'CNTXT_ITEM_ID') || 'ggg'|| ihook.getColumnValue(row,'ADMIN_STUS_ID') || 'GGGG' || ihook.getColumnValue(row,'ITEM_ID'));
       ...
       ihook.setColumnValue(row,'CURRNT_VER_IND', 0);
        ihook.setColumnValue(row,'ADMIN_ITEM_TYP_ID', 2);
            ...
            ihook.setColumnValue(rowform, 'CTL_VAL_MSG','DEC Version '||  ihook.getColumnValue(rowform,'NEW_VER_NR') || ' Created Successfully with ID ' || v_item_id|| chr(13)) ;
...
begin
   ...
   row := t_row();
        ...
        ihook.setColumnValue(row,'ADMIN_ITEM_TYP_ID', 2);
            ...
            select item_long_nm into v_tmp_short_nm from admin_item where item_id = v_item_id and ver_nr = v_ver_nr;
            if (upper(ihook.getColumnValue(rowform,'DEC_ITEM_LONG_NM')) = 'SYSGEN' or nci_11179_2.isDECnmSysGenNew(trim(ihook.getColumnValue(rowform, 'DEC_ITEM_LONG_NM')))) then
             v_short_nm := nci_11179_2.getStdShortName(ihook.getColumnValue(rowform, 'ITEM_1_ID'), ihook.getColumnValue(rowform, 'ITEM_1_VER_NR')) || ':' ||
             nci_11179_2.getStdShortName(ihook.getColumnValue(rowform, 'ITEM_2_ID'), ihook.getColumnValue(rowform, 'ITEM_2_VER_NR'));
             ihook.setColumnValue(row,'ITEM_LONG_NM', v_short_nm);
            elsif (ihook.getColumnValue(rowform, 'DEC_ITEM_LONG_NM') is not null) then
--                select item_long_nm into v_tmp_short_nm from admin_item where item_id = v_item_id and ver_nr = v_ver_nr;
--                if (nci_11179_2.isDECnmSysGenNew(v_tmp_short_nm)) then
                    v_short_nm := ihook.getColumnValue(rowform, 'DEC_ITEM_LONG_NM');
                    ihook.setColumnValue(row,'ITEM_LONG_NM', v_short_nm);
--                end if;
            elsif (nci_11179_2.isDECnmSysGenNew(v_tmp_short_nm) and ihook.getColumnValue(rowform, 'DEC_ITEM_LONG_NM') is null) then
                v_short_nm := nci_11179_2.getStdShortName(ihook.getColumnValue(rowform, 'ITEM_1_ID'), ihook.getColumnValue(rowform, 'ITEM_1_VER_NR')) || ':' ||
                nci_11179_2.getStdShortName(ihook.getColumnValue(rowform, 'ITEM_2_ID'), ihook.getColumnValue(rowform, 'ITEM_2_VER_NR'));
                ihook.setColumnValue(row,'ITEM_LONG_NM', v_short_nm);
            end if;
       ...
       -- raise_application_error(-20000, ihook.getColumnValue(rowform, 'ADMIN_NOTES') );
        if (ihook.getColumnValue(rowform, 'ADMIN_NOTES') is not null) then
            for cur in ( select nvl(admin_notes,'') admin_notes from admin_item where item_id = v_item_id and ver_nr = v_ver_nr) loop
                ihook.setColumnValue(row,'ADMIN_NOTES', ihook.getColumnValue(rowform, 'ADMIN_NOTES') || ' ' || cur.admin_notes);
            end loop;
        end if;
...
procedure createDECImportOld (rowform in out t_row, actions in out t_actions) as
v_nm  varchar2(255);
v_long_nm varchar2(255);
v_def  varchar2(4000);
v_short_nm varchar2(30);
