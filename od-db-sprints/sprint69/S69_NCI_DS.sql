...
BEGIN
    ...
    if (v_enum) then
        ...
        v_enum_cond := 'de.val_dom_typ_id = 17';
        v_stop_cond := '(' || v_hdr_id || ',de.item_id, de.ver_nr) not in (select hdr_id, item_id, ver_nr from nci_ds_rslt_dtl)';
    else
        v_tbl := 'nci_ds_rslt';
        v_enum_cond := 'de.val_dom_typ_id = 18';
       -- v_enum_cond := '1=1';
       -- v_stop_cond := '(' || v_hdr_id || ') not in (select hdr_id from nci_ds_rslt)';
        v_stop_cond := '(' || v_hdr_id || ',de.item_id, de.ver_nr) not in (select hdr_id, item_id, ver_nr from nci_ds_rslt) and ROWNUM <= ' || v_mtch_lmt;
        --v_stop_cond := '1=1';
...
--Rule ID 1: Preferred Name Exact Match
    if (v_mode = 'E' and v_term = 'P') then
        ...
        v_group_cond := 'de.item_id, de.ver_nr';
    end if;
...
--Rule ID 4: Concept Exact Match
    if (v_mode = 'E' and v_term = 'C') then
        ...
        v_mtch_cond := v_entty_nm || ' = c.MTCH_TERM and c.item_id = r.cncpt_item_id and c.ver_nr = r.cncpt_ver_nr and r.item_id = de.item_id and r.ver_nr = de.ver_nr';
        v_group_cond := 'de.item_id, de.ver_nr';
    end if;
...
--Rule ID 5: Synonym Exact Match    
    if (v_mode = 'E' and v_term = 'S') then
        ...
        v_max_word := 'max(de.item_nm)';
        v_frm_tbls := 'vw_cncpt c, vw_de de, vw_nci_de_cncpt r';
        v_mtch_cond := v_entty_nm || ' = c.SYN_MTCH_TERM and c.item_id = r.cncpt_item_id and c.ver_nr = r.cncpt_ver_nr and r.item_id = de.item_id and r.ver_nr = de.ver_nr';
        v_group_cond := 'de.item_id, de.ver_nr';
    end if;
...
--Rule ID 9: Concepts Like Match
    if (v_mode = 'L' and v_term = 'C') then
        ...
        v_max_word := 'max(de.item_nm)';
        v_frm_tbls := 'vw_cncpt c, vw_de de, vw_nci_de_cncpt r';
        v_mtch_cond := '((instr(c.MTCH_TERM, ' || v_entty_nm || ',1) > 0 ) or (instr(' || v_entty_nm || ', c.MTCH_TERM,1) > 0 )) and length(c.mtch_term) > ' || v_mtch_min_len || ' and c.item_id = r.cncpt_item_id and c.ver_nr = r.cncpt_ver_nr and r.item_id = de.item_id and r.ver_nr = de.ver_nr';
        v_group_cond := 'de.item_id, de.ver_nr';
    end if;
...
--Rule ID 10: Synonym Like Match
    if (v_mode = 'L' and v_term = 'S') then
        ...
        v_max_word := 'max(de.item_nm)';
        v_frm_tbls := 'vw_cncpt c, vw_de de, vw_nci_de_cncpt r';
        v_mtch_cond := '((instr(c.SYN_MTCH_TERM, ' || v_entty_nm || ',1) > 0 ) or (instr(' || v_entty_nm || ', c.SYN_MTCH_TERM,1) > 0 )) and length(c.syn_mtch_term) > ' || v_mtch_min_len || ' and c.item_id = r.cncpt_item_id and c.ver_nr = r.cncpt_ver_nr and r.item_id = de.item_id and r.ver_nr = de.ver_nr';
        v_group_cond := 'de.item_id, de.ver_nr';
    end if;
...
--build parts of the query
    ...
    v_select := ' select ' || v_hdr_id || ', ''' || v_mtch_typ || ''', de.item_id, de.ver_nr, ''NA'',' || v_rule_id || ',' || v_score || ',''' || v_rule_desc || ''', ' || v_max_word || ',0';
...
-- dynamic sql
--spDSSubDynmc(v_hdr_id, v_user_id, v_entty_nm, v_mtch_typ, v_src_val_id, v_filter_str, v_enum,         v_mode,         v_term)
--spDSSubDynmc(v_hdr_id, v_user_id, v_entty_nm, v_mtch_typ, [0,247,248], v_filter_str, [true,false], ['E','L','R'], ['P','Q','A','C','S']);
...
BEGIN
    v_order_by := 'order by count(*) DESC';
