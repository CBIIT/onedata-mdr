...
/* Work in Progress */
procedure spCopyQuestionDirect (actions in out t_actions, v_from_module_id in integer, v_from_module_ver in integer, v_to_module_id in integer, v_to_module_ver in integer, v_disp_ord number, v_src in varchar2) as
v_id integer;
v_found boolean;
v_itemid integer;
row  t_row;
...
-- Questions
 if (v_src = 'C') then  -- on Copy, rep number is 0
  ...
  if (v_src = 'V') then  -- on Copy, rep number is 0
   spCopyQuestionDirect(actions, v_from_module_id, v_from_module_ver, v_to_module_id, v_to_form_ver,v_disp_ord, v_src);
 end if;
