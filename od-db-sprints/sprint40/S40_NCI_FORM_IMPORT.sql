...
for cur in (select * from NCI_STG_FORM_QUEST_IMPORT where FORM_IMP_ID = ihook.getColumnValue(row_ori, 'FORM_IMP_ID')) loop
  ...
  --  raise_application_error(-20000,substr(v_cde_nm, 1, instr(v_cde_nm,'v')-2));
    --    ihook.setColumnValue(row,'CDE_ITEM_ID',substr(v_cde_nm, 1, instr(v_cde_nm,'v')-2));
        v_cde_nm := replace(substr(v_cde_nm, instr(v_cde_nm,'v')+1),'_','.');
    --    ihook.setColumnValue(row,'CDE_VER_NR',v_cde_nm);
  ...
  for curq in (select * from NCI_STG_FORM_QUEST_IMPORT where form_imp_id = ihook.getColumnValue(row_ori, 'FORM_IMP_ID')) loop
        ...
        -- Validate question - Question text has to be the name of the CDE or an alternate name
                ...
                if (v_temp = 0) then
                    ...
                    v_ctl_val_msg := 'WARNING: Question label does not match. CDE will not be used.';
                end if;
        ...
        -- for each VV, make sure PV exists. If it does, then VM Name is either the VM name or alternate name
                for curvv in (Select * from  NCI_STG_FORM_VV_IMPORT where quest_imp_id = curq.quest_imp_id) loop
                    ...
                    if (v_temp=1) then
                        ...
                        update  NCI_STG_FORM_VV_IMPORT  set  CTL_VAL_STUS = 'WARNING', CTL_VAL_MSG = 'WARNING: PV/VM not found. CDE will not be used.' where VAL_IMP_ID=curvv.val_imp_id;
                        commit;
...
v_req_ind := 1;
  ...
  decode(cur.CTL_VAL_STUS, 'ERROR', null, cur.cde_item_id), 
  ...
  --cur.req_ind, 
  decode(cur.CTL_VAL_STUS, 'ERROR',  nvl(cur.SRC_QUEST_INSTR,'; ') || 
  '  CDE ' || cur.cde_item_id || ' v' || cur.cde_ver_nr || ' was not attached due to Question Text not matching or Valid Values not matching.',cur.SRC_QUEST_INSTR), 
 ...
 v_id, 63, v_quest_disp_ord, v_req_ind,
  --cur.req_ind, 
  decode(cur.CTL_VAL_STUS, 'ERROR',  nvl(cur.SRC_QUEST_INSTR ,'')|| ' CDE selected was not attached due to errors',cur.SRC_QUEST_INSTR),  sysdate, v_usr_id, sysdate, v_usr_id );
--  commit;
