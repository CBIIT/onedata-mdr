...
begin
   ...
   -- chkPVVMActionValid ( row_ori, v_usr_id );
    ...
    if ihook.getColumNValue(rowform,'P_ITEM_ID') is null then
    raise_application_error(-20000, 'VM is not  single-concept');
    return;
    end if;
    ...
    select max(disp_ord)+1 into v_disp_ord from nci_quest_Valid_value where q_pub_id = ihook.getColumnValue(row_ori,'Q_PUB_ID') and 
