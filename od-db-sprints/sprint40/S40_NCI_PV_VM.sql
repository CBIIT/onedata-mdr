...
procedure createPVVMFromRow (row_ori in t_row, row in out t_row, answerid in integer,actions in out t_actions);  -- create pv/vm from parent
    ...
    if (hookinput.invocationnumber = 1) then
        ...
        if (hookinput.answerid = 1) then -- Create PV/VM with Parent only
            ...
            v_answerid := 51;
              ...
              createPVVMFromRow(row_ori, rowform, v_answerid, actions);
...
j := 1;
     ...
     --raise_application_error(-20000,ihook.getColumnValue(rowform,'C_ITEM_ID'));
        ...
        createPVVMFromRow(row_ori, rowform, hookinput.answerid, actions);
    end loop;
    if (actions.count > 0) then
        hookoutput.actions := actions;
...
procedure createPVVMFromRow (row_ori in t_row, row in out t_row, answerid in integer,  actions in out t_actions)
as
i int;
  v_pv varchar2(4000);
  v_item_id number;
