...
--raise_application_error(-20000,v_sql);
                ...
                --    if (usr_tips_ind = 1) then
                    update nci_ds_rslt set rule_desc = rule_desc || ' (' || v_suffix || ')' where hdr_id = v_hdr_id;
                    commit;
                  --  end if;
...
insert into nci_ds_rslt ( HDR_ID,ITEM_ID,VER_NR, NUM_PV_IN_SRC, NUM_PV_MTCH, RULE_DESC)
select cur.quest_imp_id, de_item_id item_id, de_ver_nr ver_nr, cur.cnt, count(*) , dtl.rule_desc from vw_nci_de_pv_lean v, admin_item ai, nci_ds_rslt_dtl dtl
where nvl(ai.currnt_ver_ind,0) = 1 and v.de_item_id = ai.item_id and v.de_ver_NR = ai.VER_NR
...
--if  (sql%rowcount = 0) then
insert into nci_ds_rslt ( HDR_ID,ITEM_ID,VER_NR, NUM_PV_IN_SRC, NUM_PV_MTCH, RULE_DESC)
select cur.quest_imp_id, de_item_id item_id, de_ver_nr ver_nr, cur.cnt, count(*) , dtl.rule_desc from vw_nci_de_pv v, admin_item ai, nci_ds_rslt_dtl dtl
where nvl(ai.currnt_ver_ind,0) = 1 and v.de_item_id = ai.item_id and v.de_ver_NR = ai.VER_NR
