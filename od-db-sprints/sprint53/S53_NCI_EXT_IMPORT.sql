...
for cur in (select c.item_id, c.ver_nr, x.xmap_cd, x.xmap_desc from admin_item c, SAG_LOAD_ICDo i,
nci_admin_item_xmap x where c.admin_item_typ_id = 49 and i.CONCEPT_CODe = c.item_long_nm
and i.TARGET_CODE = x.xmap_cd and upper(i.target_term) = upper(x.xmap_desc) and x.evs_src_id = v_evs_src_id and  x.item_id =0) loop
update nci_admin_item_xmap set item_id = cur.item_id, ver_nr = cur.ver_nr where 
item_id = 0 and ver_nr = 1 and xmap_cd = cur.xmap_cd and xmap_desc = cur.xmap_desc and evs_src_id = v_evs_src_id;
