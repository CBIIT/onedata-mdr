...
decode(ai.CURRNT_VER_IND,1,'Yes',0,'No') CURRNT_VER_IND, rel.instr,
            ...
            ai.CREAT_USR_ID, ai.CREAT_DT, ai.LST_UPD_DT,ai.LST_UPD_USR_ID ,decode(nvl(rel.fld_delete,0),0,'No',1,'Yes') FLD_DELETE from admin_item ai, admin_item c, NCI_ADMIN_ITEM_REL rel, admin_item frm
...
for cur in (select * from nci_admin_item_rel_alt_key where lst_upd_dt >= sysdate -vHours/24 ) loop
...
update sbrext.quest_contents_ext set ( display_order, latest_version_ind,
            long_name,
            date_modified, modified_by, deleted_ind)=
(select  curqst.disp_ord,curqst.CURRNT_VER_IND,curqst.ITEM_LONG_NM,  curqst.LST_UPD_DT,curqst.LST_UPD_USR_ID ,curqst.fld_delete from dual)  where qc_idseq = cur.nci_idseq;
...
for currep in (select * from nci_quest_vv_rep where lst_upd_dt >= sysdate - vHours/24) loop
...
BEGIN
            ...
            for curvv in (SELECT vv.NCI_IDSEQ,      q.p_mod_idseq,
                 ...
                 and   vv.q_ver_nr = q.version
                       AND vv.lst_upd_dt >= sysdate - vHours/24) loop
...
-- Update for enumerated
/*

update sbrext.QUEST_ATTRIBUTES_EXT set vv_idseq = (select 
