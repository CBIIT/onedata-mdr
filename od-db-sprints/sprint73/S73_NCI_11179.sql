...
FUNCTION get_name_from_concepts(v_item_id in number, v_ver_nr in number) return varchar2;
...
procedure spGetAddToCart  ( hookoutput in out t_hookoutput,v_typ in char, v_usr_id in varchar2);
function getSelectedCartName (hookinput in t_hookinput) return varchar2;
...
procedure spRemoveFromGuestCart (v_data_in in clob, v_data_out out clob, v_user_id varchar2);
...
procedure spCreateCommonChildrenNCI (actions in out t_actions, v_from_item_id in number, v_from_ver_nr in number, v_to_item_id in number, v_to_ver_nr in number, v_user_id varchar2);
procedure spCreateCDEDervComp (actions in out t_actions,v_from_item_id in number, v_from_ver_nr in number, v_to_item_id in number, v_to_ver_nr in number, v_user_id varchar2) ;
...
procedure spCreateSAModuleVerNCI (v_data_in in clob, v_data_out out clob, v_user_id varchar2);
...
function getSubTypeTableName(v_type in integer) return varchar2;
...
function getLongName(v_nm in varchar2) return varchar
 is
 begin
    if lengthb(v_nm) >= 255 then
        return substr(v_nm,1,240) || ' ...';
    else
        return nvl(v_nm,' ');
    end if;
 end;
...
procedure spGetAddToCart  ( hookoutput in out t_hookoutput, v_typ in char, v_usr_id in varchar2)  -- v_typ C - Cart only, P - Pin only, B - Both, I - Initial
as
 question t_question;
  answer t_answer;
  answers t_answers;
...
begin
    ...
    if (v_typ = 'C') then -- cart
        ...
        form1                  := t_form('User Cart Selection Form (Hook)', 2,1);
    end if;
    row := t_row();  
  --  raise_application_error(-20000, v_usr_id);
    ...
    hookoutput.message := 'Cart Name: If not specified or selected, Default user cart will be used.';
...
for cur in (select * from VW_LIST_USR_CART_NM where CNTCT_SECU_ID = v_usr_id and cart_nm <> v_Deflt_cart_nm) loop
  ...
  if (v_found) then
       	 showrowset := t_showablerowset (rows, 'User Cart List', 2, 'single');
...
procedure getCartNameSelectionFormMultiple  ( hookOutput in out t_hookoutput, v_usr_id in varchar2) 
as
rows t_rows;
row t_row;
v_found boolean;   showrowset	t_showablerowset;
...
begin
        rows := t_rows();
        v_found := false;
for cur in (select * from VW_LIST_USR_CART_NM where CNTCT_SECU_ID = v_usr_id ) loop
		   row := t_row();
	   	   iHook.setcolumnvalue (ROW, 'CART_NM', cur.CART_NM);
		   iHook.setcolumnvalue (ROW, 'CNTCT_SECU_ID', v_usr_id);
		   rows.extend;
...
procedure getGuestCartDownload  (hookOutput in out t_hookoutput, row_ori in t_row, v_usr_id in varchar2, v_item_typ in int) 
as
rows t_rows;
row t_row;
v_found boolean;   showrowset	t_showablerowset;
...
begin
        ...
        if (v_item_typ = 4) then
       	 showrowset := t_showablerowset (rows, 'User Cart List (for Download)', 2, 'single');
        end if;
        if (v_item_typ = 54) then
       	 showrowset := t_showablerowset (rows, 'User Cart List (for Form Download)', 2, 'single');
        end if;
...
begin
     ...
     forms              := hookInput.forms;
        ...
        if (ihook.getColumnValue(row,'CART_NM_SPEC') is not null) then
          return ihook.getColumnValue(row,'CART_NM_SPEC');
        else
        for cur in (select cart_nm from VW_LIST_USR_CART_NM where CART_USR_NM= ihook.getColumnValue(row,'CART_USR_NM')) loop
         return cur.cart_nm;
         end loop;
        end if;
   return v_deflt_cart_nm;
end;
...
FUNCTION getMinWordLen (v_nm IN varchar2) RETURN integer IS
   ...
   numwords integer;
  BEGIN
  numwords := getWordCount(v_nm);
  for i in 1..numwords loop
  curlen := length(getWord(v_nm,i,numwords));
   -- raise_application_error(-20000,curlen || ' ' || minlen);

   if minlen > curlen or minlen = 0 then
  minlen := curlen;
 end if;
  end loop;
...
begin
  ...
  if (v_idx = 1 and v_idx = v_max)then
    ...
    v_word := substr(v_nm_new, 1, instr(v_nm_new, ' ',1,1)-1);
...
function getMinWordLenght(v_nm in varchar2) return integer is
i integer;
numwords integer;
curlen integer;
v_word varchar2(255);
...
function get_name_from_concepts(v_item_id in number, v_ver_nr in number) return varchar2 is
cursor con is
select c.item_nm, cai.NCI_CNCPT_VAL
from cncpt_admin_item cai, admin_item c
where cai.item_id = v_item_id and cai.ver_nr = v_ver_nr
...
spGetAddToCart(hookoutput, 'C', v_usr_id);
else

v_cart_nm := nvl(GetSelectedCartName(hookinput), v_deflt_cart_nm);

select max(nvl(retain_ind,0)) into v_retain_ind from nci_usr_cart where cart_nm = v_cart_nm and cntct_secu_id = v_usr_id and upper(guest_usr_nm) = 'NONE';
--v_src := 'AI';
--raise_application_error(-20000, v_cart_nm);
    ...
    for i in 1..hookinput.originalrowset.rowset.count loop  --- Iterate through all the selected rows
        ...
        if (v_temp = 0) then
            v_add := v_add + 1;
            nci_11179_2.AddItemToCart(v_item_id, v_ver_nr, v_usr_id, v_cart_nm, v_retain_ind, rows);
            -- Add DDE components
            if (nci_11179_2.isDDE(v_item_id, v_ver_nr) = true) then
                    ...
                    and p_item_id = v_item_id and p_item_ver_nr = v_ver_nr and c_item_id <> p_Item_id order by disp_ord ) loop
                                ...
                                where item_id  = v_c_item_id and ver_nr = v_c_ver_nr and cntct_secu_id = v_usr_id  and cart_nm = v_cart_nm;
    ...
    --     action := t_actionrowset(rows, 'NCI_USR_CART', 1,1,'insert');
     actions.extend;
        ...
        hookoutput.actions := actions;
        hookoutput.message := rows.count|| ' item(s) added successfully to cart. ' || v_already || ' item(s) selected already in your cart';
    else
...
if ( hookinput.invocationnumber = 0) then
nci_11179.getCartNameSelectionFormMultiple(hookOutput, v_usr_id);
end if;

if (hookinput.invocationnumber = 1) then

    rows := t_rows();
    for i in 1..hookinput.selectedrowset.rowset.count loop  --- Iterate through all the selected rows
        row_ori := hookInput.selectedRowset.rowset(i);
       -- raise_application_error(-20000,ihook.getcolumnvalue(row_ori,'CNTCT_SECU_ID')  || ' ' || ihook.getcolumnvalue(row_ori,'CART_NM'));
       update nci_usr_cart set Retain_ind = 1 where CNTCT_SECU_ID=ihook.getcolumnvalue(row_ori,'CNTCT_SECU_ID')  and CART_NM=ihook.getcolumnvalue(row_ori,'CART_NM') and 	upper(GUEST_USR_NM)='NONE';
       commit;
    end loop;
...
begin
   ...
   -- raise_application_error(-20000,v_user_id);
    ...
    for cur in (select * from admin_item where item_id = ihook.getColumnValue(row_ori,'ITEM_ID') and ver_nr = ihook.getColumnValue(row_ori,'VER_NR')) loop
    v_item_typ := cur.admin_item_typ_id;
    end loop;



nci_11179.getGuestCartDownload(hookOutput,row_ori, v_usr_id, v_item_typ);
...
begin
    ...
    hookoutput.originalrowset    := hookinput.originalrowset;
   -- raise_application_error(-20000,v_user_id);
       update nci_usr_cart set Retain_ind = 1 where CNTCT_SECU_ID=v_usr_id  and CART_NM='Default' and 	upper(GUEST_USR_NM)='NONE';
       commit;
hookoutput.message := 'Default Cart will be retained unless cart is completely empty.';
    V_DATA_OUT := IHOOK.GETHOOKOUTPUT (HOOKOUTPUT);
 --      nci_util.debugHook('GENERAL',v_data_out);
...
begin
    ...
    if hookInput.invocationNumber = 0 then  ---First invocation of the hook - prompt for user name
            ...
            ANSWERS.EXTEND;
            ANSWERS(ANSWERS.LAST):= ANSWER;
            QUESTION             := T_QUESTION('Specify a name that is easily remembered to store selections in your user cart. Please use the same cart name each time you want to add items. Cart items will be deleted on a rolling 7 day basis.' , ANSWERS);            HOOKOUTPUT.QUESTION    := QUESTION;
            forms                := t_forms();
            ...
            for i in 1..hookinput.originalrowset.rowset.count loop -- Iterate throu all selected rows.
                ...
                if (v_found = false) then
                    ...
                    and p_item_ver_nr = v_ver_nr and c_item_id <> p_item_id  order by disp_ord) loop
                                ...
                                v_c_ver_nr := curint.c_item_ver_nr;
                                select count(*) into v_temp from nci_usr_cart 
                                where item_id  = v_c_item_id and ver_nr = v_c_ver_nr and  guest_usr_nm = v_user_nm;
                   v_found := false;
                                    for k in 1..rows.count loop
                                        row_sub := rows(k);
                                        if (ihook.getColumnValue(row_sub, 'ITEM_ID') = curint.c_item_id and ihook.getColumnValue(row_sub, 'VER_NR') = curint.c_item_ver_nr) then
                                        v_found := true;
                                        end if;
                                    end loop;
                                    if (v_temp = 0) and v_found = false then
                                        row := t_row();
                                        v_add := v_add + 1;
                                        ihook.setColumnValue(row,'ITEM_ID', v_c_item_id);
                                        ihook.setColumnValue(row,'VER_NR', v_c_ver_nr);
                                        ihook.setColumnValue(row,'CNTCT_SECU_ID', 'GUEST');
                                        ihook.setColumnValue(row,'GUEST_USR_NM', v_user_nm);
                                        ihook.setColumnValue(row,'CART_NM', v_user_nm);
                                        rows.extend;    rows(rows.last) := row;
...
begin
    ...
    for i in 1..hookinput.originalrowset.rowset.count loop -- Iterate throu selected rows
      ...
      -- raise_application_error(-20000,ihook.getColumnValue(row_ori,'CART_NM')  || v_user_id);
        delete from nci_usr_cart where cart_nm =  ihook.getColumnValue(row_ori,'CART_NM') 
        and CNTCT_SECU_ID = v_user_id and item_id = ihook.getColumnValue(row_ori,'ITEM_ID') and ver_nr = ihook.getColumnValue(row_ori,'VER_NR')
        and guest_usr_nm = 'NONE';
        delete from onedata_ra.nci_usr_cart where cart_nm =  ihook.getColumnValue(row_ori,'CART_NM') 
        and CNTCT_SECU_ID = v_user_id and item_id = ihook.getColumnValue(row_ori,'ITEM_ID') and ver_nr = ihook.getColumnValue(row_ori,'VER_NR')
     and guest_usr_nm = 'NONE';
        commit;
    end loop;
...
PROCEDURE spRemoveFromGuestCart  (
    v_data_in IN CLOB,
    v_data_out OUT CLOB,
    v_user_id  varchar2)
AS
...
-- Eleiminate insert of duplicates if the user is clicking numtiple times. Will only work for where CDE is attached.

if (cur.c_item_id is not null) then
select count(*) into v_src_cnt from nci_admin_item_rel_alt_key where c_item_id = cur.c_item_id and c_item_ver_nr = cur.c_item_ver_nr
and  p_item_id = v_from_module_id and
        p_item_ver_nr = v_from_module_ver;
select count(*) into v_dest_cnt from nci_admin_item_rel_alt_key where c_item_id = cur.c_item_id and c_item_ver_nr = cur.c_item_ver_nr
and  p_item_id = v_to_module_id and
        p_item_ver_nr = v_to_module_ver;
end if;

if (v_dest_cnt < v_src_cnt) then
...
--raise_application_error(-20000,v_src);
-- Module
-- Module Rel
--Questions
-- Question VV
...
v_table_name := 'ADMIN_ITEM';
 ...
 v_meta_col_cnt := TEMPLATE_11179.getColumnCount(v_table_name);
 if (v_src = 'C' or v_src = 'MA' or v_src = 'MR') then -- C - copy, MR - SA Migration Replace, MA - SA Migration Add
 v_to_module_id := nci_11179.getItemId;
end if;
--reuse module id
 if (v_src = 'V') then -- V - versioning
 ...
 dbms_sql.close_cursor(v_cur);
  ...
  --  actions.extend; actions(actions.last) := action;
    ...
    if (v_src = 'C' ) then
      ...
      if (v_src = 'MR' ) then -- SA migration
      ihook.setColumnValue(row, 'CPY_MOD_ITEM_ID',v_from_module_id);
      ihook.setColumnValue(row, 'CPY_MOD_VER_NR',v_from_module_ver);
      ihook.setColumnValue(row, 'CPY_MIG_UPD_DT',to_char(sysdate, DEFAULT_TS_FORMAT) );
      ihook.setColumnValue(row, 'CPY_MIG_TYP','Replace');
      
    end if;
      if (v_src = 'MA' ) then -- SA migration
      ihook.setColumnValue(row, 'CPY_MOD_ITEM_ID',v_from_module_id);
      ihook.setColumnValue(row, 'CPY_MOD_VER_NR',v_from_module_ver);
      ihook.setColumnValue(row, 'CPY_MIG_UPD_DT',to_char(sysdate, DEFAULT_TS_FORMAT));
      ihook.setColumnValue(row, 'CPY_MIG_TYP','Add');
...
procedure spCreateCommonChildrenNCI (actions in out t_actions, v_from_item_id in number, v_from_ver_nr in number, v_to_item_id in number, v_to_ver_nr in number, v_user_id varchar2) as
    action           t_actionRowset;
    action_rows              t_rows := t_rows();
    action_rows_csi              t_rows := t_rows();
...
/*  Copy Alternate Names */
    ...
    item_id = v_from_item_id and ver_nr = v_from_ver_nr and nvl(fld_delete,0) = 0) loop
        ...
        nci_11179_2.resetAuditAttr(row, v_user_id);
...
begin
        ...
        v_table_name := getSubTypeTableName(v_type);
    v_meta_col_cnt := TEMPLATE_11179.getColumnCount(v_table_name);
...
procedure spReturnSubtypeSpecifiedRow (v_item_id in number, v_ver_nr in number, v_type in number, row in out t_row, rowform in t_row) as
    action           t_actionRowset;
    action_rows              t_rows := t_rows();
...
begin
    ...
    for i in 1..v_meta_col_cnt loop
       -- dbms_sql.column_value(v_cur, i, v_col_val);
        if (ihook.getColumnValue(rowform, v_meta_desc_tab(i).col_name) is not null) then
        ihook.setColumnValue(row, v_meta_desc_tab(i).col_name, ihook.getColumnValue(rowform, v_meta_desc_tab(i).col_name));
        end if;
    end loop;
    ihook.setColumnValue(row,'ITEM_ID', v_item_id);
    ihook.setColumnValue(row,'VER_NR', v_ver_nr);
...
--        actions.extend; actions(actions.last) := action;
insert into perm_val (val_dom_item_id, val_dom_Ver_nr, perm_val_beg_dt, perm_val_end_Dt, perm_val_nm, perm_val_desc_txt, nci_val_mean_item_id, nci_val_mean_ver_nr,
prnt_cncpt_item_id, prnt_cncpt_ver_nr, nci_origin_id, nci_origin, creat_usr_id, lst_upd_usr_id)
select  v_to_id,v_to_ver_nr, perm_val_beg_dt, perm_val_end_Dt, perm_val_nm, perm_val_desc_txt, nci_val_mean_item_id, nci_val_mean_ver_nr,
prnt_cncpt_item_id, prnt_cncpt_ver_nr, nci_origin_id, nci_origin, creat_usr_id, lst_upd_usr_id
...
procedure spCreateCDEDervComp (actions in out t_actions,v_from_item_id in number, v_from_ver_nr in number, v_to_item_id in number, v_to_ver_nr in number, v_user_id varchar2) as
    action           t_actionRowset;
    action_rows              t_rows := t_rows();
    row          t_row;
    v_found boolean;
...
begin
    ...
    /*  DE then copy Derived Components */
        ...
        P_item_id = v_from_item_id and p_ITEM_ver_nr = v_from_ver_nr and rel_typ_id = 66 and nvl(fld_Delete,0) = 0) loop
            ...
            ihook.setColumnValue(row, 'P_ITEM_VER_NR', v_to_ver_nr);
            action_rows.extend; action_rows(action_rows.last) := row;
        end loop;
...
begin
    if v_admin_item.admin_item_typ_id = 1 then
        ...
        v_table_name := 'NCI_FORM';
    elsif v_admin_item.admin_item_typ_id = 57 then
        v_table_name := 'NCI_MDL';
        elsif v_admin_item.admin_item_typ_id = 58 then
        v_table_name := 'NCI_MDL_MAP';
    end if;
...
/* Model Map */
 if v_admin_item.admin_item_typ_id = 58 then -- Add Mapping and Value Mapping
        action_rows := t_rows();
        nci_codemap_mm.spCopyMM(v_admin_item.item_id, v_admin_item.ver_nr, v_admin_item.item_id, v_version,v_user_id, 0);
        nci_codemap_vm.spCopyVM(v_admin_item.item_id, v_admin_item.ver_nr, v_admin_item.item_id, v_version,v_user_id, 0);
...
/*  if model, copy model element, char, rel*/
    if v_admin_item.admin_item_typ_id = 57 then
       action_rows := t_rows();
     
     -- Copy all Model Elements
        for curme in ( select * from nci_mdl_elmnt where mdl_item_id = v_admin_item.item_id and mdl_item_ver_nr = v_admin_item.ver_nr) loop
            row := t_row();
            v_sql := 'select * from NCI_MDL_ELMNT where ITEM_ID = ' || curme.item_id || ' and ver_nr = ' || curme.ver_nr ;
            ReturnRow(v_sql, 'NCI_MDL_ELMNT',row);
            ihook.setColumnValue(row, 'MDL_ITEM_VER_NR', v_version);
            ihook.setColumnValue(row, 'VER_NR', v_version);
            action_rows.extend; action_rows(action_rows.last) := row;
        end loop;
    
        action := t_actionRowset(action_rows, 'Model Element (No Sequence)',2, 10, 'insert');
        actions.extend; actions(actions.last) := action;
       
        -- Copy all model element characteristics
        action_rows := t_rows();
        for curmec in ( select c.* from nci_mdl_elmnt_char c,nci_mdl_elmnt e where e.item_id = c.mdl_elmnt_item_id and e.ver_nr = c.mdl_elmnt_ver_nr
        and mdl_item_id = v_admin_item.item_id and mdl_item_ver_nr = v_admin_item.ver_nr) loop
            row := t_row();
        ...
        action := t_actionRowset(action_rows, 'Model Element Characteristics',2, 12, 'insert');
        actions.extend; actions(actions.last) := action;
    
        -- Copy all model element relationships
        action_rows := t_rows();
        for currel in ( select c.* from NCI_MDL_ELMNT_REL c,nci_mdl_elmnt e where e.item_id = c.prnt_item_id and e.ver_nr = c.prnt_item_ver_nr
        and mdl_item_id = v_admin_item.item_id and mdl_item_ver_nr = v_admin_item.ver_nr) loop
            row := t_row();
            v_sql := 'select * from NCI_MDL_ELMNT_REL where REL_ID = ' || currel.REL_id  ;
            ReturnRow(v_sql, 'NCI_MDL_ELMNT_REL',row);
            ihook.setColumnValue(row, 'PRNT_ITEM_VER_NR', v_version);
            ihook.setColumnValue(row, 'CHLD_ITEM_VER_NR', v_version);
            ihook.setColumnValue(row, 'REL_ID', -1);
            action_rows.extend; action_rows(action_rows.last) := row;
        end loop;
        action := t_actionRowset(action_rows, 'Model Element Relationship',2, 15, 'insert');
        actions.extend; actions(actions.last) := action;
...
begin
    ...
    for update nowait;
     ...
     if (v_admin_item.admin_item_typ_id not in (2,3,4,54,57,58)) then -- changed based on new rules from DW 2/3/2021
     ...
     if (v_admin_item.admin_item_typ_id in (2,3,4,54,57,58) and v_admin_item.admin_stus_nm_dn not like '%RELEASED%') then -- changed based on new rules from DW 3/1/2026
        hookOutput.message := 'Administered Item has to be RELEASED or RELEASED-NON-CMPLNT to be versioned.';
        v_data_out := ihook.getHookOutput(hookOutput);
        return;
    end if;
    ...
    if hookInput.invocationNumber = 0 then  -- If first invocation, prompt for version number
        ...
        if (v_admin_item.admin_item_typ_id = 54) then -- form
          ...
          --  ihook.setColumnValue(row, 'REGSTR_STUS_ID',9 );
            if (ihook.getColumnValue(rowform,'CHNG_DESC_TXT') is not null) then -- append to existing 
            ihook.setColumnValue(row,'CHNG_DESC_TXT', substr(ihook.getColumnValue(row,'CHNG_DESC_TXT') ||' ' ||  ihook.getColumnValue(rowform,'CHNG_DESC_TXT'),1,2000));
            end if;
            ...
            ihook.setColumnValue(row, 'NCI_IDSEQ', '');
            --jira 2301 add output message for second invocation
                    if (v_admin_item.admin_item_typ_id = 54) then -- form
          select count(*) into v_temp from vw_nci_MODULE_DE WHERE frm_item_id = v_admin_item.item_id and frm_ver_nr = v_admin_item.ver_nr and DE_ITEM_ID is not null
             and (DE_ADMIN_STUS_NM_DN like '%RETIRED%'); --CDE_CURRNT_VER_IND = 0);
        ...
        -- end block 2301
            ...
            ihook.setColumnValue(row, 'CHNG_DESC_TXT', 'New version created. ' || ihook.getColumnValue(row_ori, 'CHNG_DESC_TXT'));
            ...
            spCreateCommonChildrenNCI(actions, v_admin_item.item_id,v_admin_item.ver_nr, v_admin_item.item_id, v_version, v_user_id);
            ...
            hookOutput.message := hookOutput.message || chr(13) || 'Version created successfully.';
        end if;
    end if;
...
if (nvl(ihook.getColumnValue (rowform,  'IND_TYP_1') ,0) = 1)  then -- copy CSI
        ...
        cs_item_id = v_admin_item.item_id and cs_item_ver_nr = v_admin_item.ver_nr) loop
            ...
            if (ihook.getColumnValue(rowst, 'P_ITEM_ID') is not null) then
            -- set parent CSI version number
            ihook.setColumnValue(rowst, 'P_ITEM_VER_NR', v_version);
            end if;
...
/*  Command Hook - Create Version */
procedure spCreateSAMOduleVerNCI (v_data_in in clob, v_data_out out clob, v_user_id varchar2) as
...
v_mod_id := ihook.getColumnValue(row_ori,'C_ITEM_ID');
v_mod_ver_nr := ihook.getColumnValue(row_ori,'C_ITEM_VER_NR');

for cur in (Select * from admin_item where item_id = v_mod_id and ver_nr = v_mod_ver_nr and currnt_ver_ind <> 1) loop
        hookOutput.message := 'Cannot create version if the Stand-alone modules is not the latest version.';
        v_data_out := ihook.getHookOutput(hookOutput);
        return;
    ...
    if hookInput.invocationNumber = 0 then  -- If first invocation, prompt for version number
        ...
        ANSWERS.EXTEND;
        ANSWERS(ANSWERS.LAST) := ANSWER;
        QUESTION               := T_QUESTION('Specify New Version, Current Version is: ' || trim(to_char(ihook.getColumnValue(row_ori,'C_ITEM_VER_NR' ), '9999.99')) , ANSWERS);
        HOOKOUTPUT.QUESTION    := QUESTION;
        forms                  := t_forms();
        form1                  := t_form('Version Creation', 2,1);
        forms.extend;
        ...
        hookOutput.forms := forms;
	      ...
	      forms              := hookInput.forms;
            ...
            spReturnAIRow ( v_mod_id, v_mod_ver_nr, row);
            ...
            /* set latest version to 0 for current row */
            rowupd := t_row();
            ihook.setColumnValue(rowupd, 'ITEM_ID', v_mod_id);
            ihook.setColumnValue(rowupd, 'VER_NR', v_mod_ver_nr);
            ihook.setColumnValue(rowupd, 'CURRNT_VER_IND', 0);
      ihook.setColumnValue(rowupd, 'CHNG_DESC_TXT', 'New version created. ' || nvl(ihook.getColumnValue(row, 'CHNG_DESC_TXT'),''));
     

            ai_update_action_rows.extend; ai_update_action_rows(ai_update_action_rows.last) := rowupd;
      ...
      --      action := t_actionRowset(ai_insert_action_rows, 'Administered Item (No Sequence)',2, 1, 'insert');
       --     actions.extend; actions(actions.last) := action;
           ...
           -- procedure spCopyModuleNCI (actions in out t_actions, v_from_module_id in number,v_from_module_ver in number,  v_from_form_id in number, v_from_form_ver number, v_to_form_id number,
            --v_to_form_ver number, v_disp_ord number, v_src in varchar2, v_cntxt_item_id in number, v_cntxt_ver_nr in number, v_user_id in varchar2);
            nci_11179.spCopyModuleNCI(actions, v_mod_id, v_mod_ver_nr, -1, 1,
           -1, v_version,0, 'V', null, null, v_user_id);
  
            /* Call sub-type creation function */
         --   spCreateSubtypeVerNCI(actions, v_mod_id, v_version, v_user_id);
