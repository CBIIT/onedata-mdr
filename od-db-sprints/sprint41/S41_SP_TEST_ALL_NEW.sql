...
INSERT INTO OD_CADSR_TABLES_TEST_RESULTS (TABLE_NAME, WA_CNT, DIFF_CNT, CADSR_CNT,COMMENTS)
SELECT 'Form FOOTER Instruction', 
...
((SELECT COUNT(*) FROM ONEDATA_WA.ADMIN_ITEM CSIAI, ONEDATA_WA.NCI_CLSFCTN_SCHM_ITEM CSI, ONEDATA_WA.ADMIN_ITEM CS WHERE
 ...
 AND CSI.ITEM_ID = CSIAI.ITEM_ID AND CSI.VER_NR = CSIAI.VER_NR  AND NVL(CSI.FLD_DELETE,0) = 0 and CSIAI.ADMIN_STUS_NM_DN NOT LIKE 'RETIRED%'
 and CS.ADMIN_STUS_NM_DN NOT LIKE 'RETIRED%')) WA_CNT,
NULL DIFF_CNT,
(SELECT COUNT(*) FROM SBR.CS_CSI r,SBR.ADMINISTERED_COMPONENTS csi,SBR.ADMINISTERED_COMPONENTS cs
where csi.ac_idseq=csi_idseq and csi.ACTL_NAME='CS_ITEM' and csi.ASL_NAME NOT LIKE 'RETIRED%' and cs.ASL_NAME NOT LIKE 'RETIRED%'
and cs.ac_idseq=cs_idseq) CADSR_CNT,NULL
FROM DUAL;
...
CASE 
    WHEN DIFF_CNT>0
    THEN 'THE TABLE '||TABLE_NAME||' HAS FOR '||DIFF_CNT||' RECORDS MORE IN OD'
    WHEN DIFF_CNT<0
    THEN 'THE TABLE '||TABLE_NAME||' HAS FOR '||-1*(DIFF_CNT)||' RECORDS MORE IN CADSR'
    END 
    WHERE DIFF_CNT<>0;
    COMMIT;
END;
