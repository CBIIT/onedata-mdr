-- Tracker 1965
alter table NCI_STG_PV_VM_IMPORT add (DT_LAST_MODIFIED VARCHAR2(32) DEFAULT sysdate);
commit;
CREATE OR REPLACE EDITIONABLE TRIGGER "ONEDATA_WA"."TR_PV_VM_IMPORT_AUD_TS" 
  BEFORE UPDATE
...
AND AIR.C_ITEM_VER_NR = DE.VER_NR;
  ...
  SELECT   DE.ITEM_ID  DE_ITEM_ID, 
	...
	PV.VAL_DOM_ITEM_ID  VAL_DOM_ITEM_ID, PV.VAL_DOM_VER_NR VAL_DOM_VER_NR ,
		PV.CREAT_DT, AN.CREAT_USR_ID, AN.LST_UPD_USR_ID, greatest(nvl(AN.FLD_DELETE,0),nvl(pv.fld_delete,0)) fld_delete, PV.LST_DEL_DT, PV.S2P_TRN_DT, AN.LST_UPD_DT,
  ...
  SELECT   DE.ITEM_ID  DE_ITEM_ID, 
		DE.VER_NR DE_VER_NR, PV.NCI_VAL_MEAN_ITEM_ID VAL_MEAN_ITEM_ID, PV.NCI_VAL_MEAN_VER_NR VAL_MEAN_VER_NR, vm.ITEM_NM VAL_MEAN_LONG_NM, 
	PV.VAL_DOM_ITEM_ID  VAL_DOM_ITEM_ID, PV.VAL_DOM_VER_NR VAL_DOM_VER_NR ,
		PV.CREAT_DT, PV.CREAT_USR_ID, PV.LST_UPD_USR_ID, greatest(nvl(PV.FLD_DELETE,0), nvl(an.fld_Delete,0)) FLD_DELETE, PV.LST_DEL_DT, PV.S2P_TRN_DT, PV.LST_UPD_DT,
