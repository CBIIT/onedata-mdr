   ...
   for k in 1..rows.count loop
    ...
    if (v_val_ind = true) then
        if (ihook.getColumnValue(row_to_comp, 'VAL_DOM_ITEM_ID') = ihook.getColumnValue(row_ori, 'VAL_DOM_ITEM_ID') and ihook.getColumnValue(row_to_comp, 'VAL_DOM_VER_NR') = ihook.getColumnValue(row_ori, 'VAL_DOM_VER_NR')
            ...
            and ihook.getColumnValue(row_to_comp, 'VM_STR_TYP') = ihook.getColumnValue(row_ori, 'VM_STR_TYP') and ihook.getColumnValue(row_ori, 'BTCH_SEQ_NBR') <> ihook.getColumnValue(row_to_comp, 'BTCH_SEQ_NBR')) then
                ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', 'Warning: PV shares VM with Batch Number: ' || ihook.getColumnValue(row_to_comp, 'BTCH_SEQ_NBR') || chr(13) || 'Run Create PV/VM for Seq ' || ihook.getColumnValue(row_to_comp, 'BTCH_SEQ_NBR') || ', then Validate again.'  );
...
-- ITEM_2_ID : Value Domain public Id; ITEM_2_VER_NR: Value Domain Version; ITEM_2_LONG_NM: Used to see if Concept or String - VM String Type
  ...
  -- raise_application_error(-20000, v_str);
                ...
                for i in  1..cnt loop
                        ...
                        if (v_count = 0) then
                            ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', ihook.getColumnValue(row_ori, 'CTL_VAL_MSG') || 'WARNING: Invalid Concept: ' || v_cncpt_nm || chr(13));
                        end if; 
...
-- jira 1939 - only checking the first concept in the drop-down, what if user clears the first one in the drop down from the ui
    ...
    and val_dom_ver_nr = ihook.getColumnValue(row_ori, 'VAL_DOM_VER_NR') and upper(perm_val_nm) = upper(ihook.getColumnValue(row_ori, 'PERM_VAL_NM')) and ihook.getColumnValue(row_ori, 'ITEM_1_ID') = nci_val_mean_item_id and ihook.getColumnValue(row_ori, 'ITEM_1_VER_NR') = nci_val_mean_ver_nr) loop
       ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', ihook.getColumnValue(row_ori, 'CTL_VAL_MSG') ||  'PV/VM combination already exists in the VD' || chr(13));                      
        v_val_ind := false;
    end loop;
   ...
   -- select ASL_NAME into vd_asl_name from value_domains_view where vd_id = ihook.getColumnValue(row_ori, 'VAL_DOM_ITEM_ID');
        if (v_val_ind = false) then 
         ...
         if (vd_valdom_id is null) then
            ...
            select conc_dom_item_id, conc_dom_ver_nr into cd_id, cd_ver_nr from value_dom where vd_valdom_id = item_id and ihook.getColumnValue(row_ori, 'VAL_DOM_VER_NR') = ver_nr;
           -- raise_application_error(-20000, cd_id || ' ' || cd_ver_nr);
            for cur_cd in (select admin_stus_nm_dn from admin_item where admin_item_typ_id = 1 and cd_id = item_id and cd_ver_nr = ver_nr) loop
                cd_status := cur_cd.admin_stus_nm_dn;
            -- jira 1981
        ...
        if (vd_status = 'RETIRED ARCHIVED' ) then
         ...
         if (ihook.getcolumnValue(row_ori, 'ITEM_1_ID') is not null) then
              -- Jira 1889 Do not remove values from ITEM_1_ID and ITEM_1_VER_NR - always have VM ID in that column  
                ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', 'Existing VM Found' || chr(13) || ihook.getColumnValue(row_ori, 'CTL_VAL_MSG') );
        ...
        --raise_application_error(-20000, ihook.getColumnValue(row_ori, 'VM_ALT_NM_TYP_ID'));
            select count(*) into v_temp from alt_nms where item_id = ihook.getColumnValue(row_ori, 'ITEM_ID') and ver_nr = ihook.getColumnValue(row_ori, 'VER_NR')  
            and nm_desc = ihook.getColumnValue(row_ori, 'VM_ALT_NM') and nm_typ_id = ihook.getColumnValue(row_ori, 'VM_ALT_NM_TYP_ID') and cntxt_item_id  = ihook.getColumnValue(row_ori, 'VM_ALT_NM_CNTXT_ITEM_ID')
