...
begin
   ...
   -- raise_application_error(-20000,v_user_id);
    ...
    for i in 1..hookinput.originalrowset.rowset.count loop  --- Iterate through all the selected rows
        ...
        v_entty_nm := regexp_replace(upper(nvl(ihook.getColumnValue(row_ori,'ENTTY_NM_USR') ,ihook.getColumnValue(row_ori,'ENTTY_NM'))),'\(|\)|\;','');
    ...
    commit;
     ...
     insert into nci_ds_rslt (hdr_id, item_id, ver_nr,  rule_desc)
                    ...
                    admin_item where regexp_replace(upper(item_nm),'\(|\)|\;','') = v_entty_nm and admin_item_typ_id = 53;
                    commit;
    
    
     insert into nci_ds_rslt (hdr_id, item_id, ver_nr,  rule_desc)
                    select distinct  v_hdr_id , item_id, ver_nr, 'Long Name Exact Match' from 
                    vw_cncpt where regexp_replace(upper(item_nm),'\(|\)|\;','') = v_entty_nm and CNTXT_NM_DN = 'NCIP'
                    and v_hdr_id not in (select hdr_id from nci_ds_rslt);
                    commit;
   
     insert into nci_ds_rslt (hdr_id, item_id, ver_nr,  rule_desc)
                    select distinct  v_hdr_id , r.item_id, r.ver_nr, 'Synonym Exact Match' from 
                    vw_cncpt c, alt_nms r where regexp_replace(upper(nm_desc),'\(|\)|\;','') = v_entty_nm
    ...
    for i in 1..hookinput.originalrowset.rowset.count loop  --- Iterate through all the selected rows
     ...
     for i in 1..v_cnt loop
        ...
        if (v_temp = 0) then        -- Non enumerated section
           ...
           select '''' || replace(upper(nvl(cur.entty_nm_usr, cur.entty_nm)),'''','') || '''' into v_entty_nm from dual;
              select '''' || replace(upper(nvl(cur.entty_nm_usr, cur.entty_nm)),'''','') || '''' into v_entty_nm_like from dual;
    ...
    --     raise_application_error(-20000, v_sql);
       ...
       ' || v_entty_nm || '  = upper(r.nm_desc)
        ...
        commit;
              ...
              v_sql := ' insert into nci_ds_rslt_dtl (hdr_id, item_id, ver_nr, perm_val_nm,  rule_id, score, rule_desc)
                    select distinct ' ||  cur.hdr_id || ', r.item_id, r.ver_nr,  ''NA'', 5, 100, ''Question Text Like Match'' from ref r, obj_key ok, vw_de de
                    where (instr(upper(r.ref_desc),' || v_entty_nm || ', 1) > 0  ) and
                    r.ref_typ_id = ok.obj_key_id and upper(obj_key_desc) like ''%QUESTION%'' and r.ref_nm != ''%'' and de.item_id = r.item_id and de.ver_nr = r.ver_nr and de.currnt_ver_ind = 1
                    and (' || cur.hdr_id || ', r.item_id, r.ver_nr) not in (select  hdr_id, item_id, ver_nr from nci_ds_rslt_dtl) and de.val_dom_typ_id = 17 and ' || v_flt_str;
                --      raise_application_error(-20000, v_sql);
                execute immediate v_sql;
                    ...
                    v_sql := ' insert into nci_ds_rslt_dtl (hdr_id, item_id, ver_nr,  perm_val_nm, rule_id, score, rule_desc)
                    select distinct ' ||  v_hdr_id || ', r.item_id, r.ver_nr, ''NA'', 6, 100, ''Alternate Name Like Match'' from alt_nms r, vw_de de
                    where (instr(upper(r.nm_desc), ' || v_entty_nm || ',1) > 0)
                    and de.item_id = r.item_id and de.ver_nr = r.ver_nr and de.currnt_ver_ind = 1 and de.val_dom_typ_id = 17
