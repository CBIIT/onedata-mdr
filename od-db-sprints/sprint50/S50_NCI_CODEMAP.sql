...
--  raise_application_error(-20000,v_mdl_hdr_id);
 ...
 BEGIN
  ...
  if (ihook.getColumnValue(row_ori,'CTL_VAL_STUS') = 'CREATED') then
  raise_application_error(-20000, 'Model already created. You can update CDE and DEC associated with the Characteristics using Update Model.');
  end if;
...
getModelElementAction ( row_ori ,actions , v_mdl_hdr_id , v_mdl_elmnt_id );
 ...
 BEGIN
  ...
  ihook.setcolumnValue(row_ori, 'MECM_ID', ihook.getColumnValue(row_ori,'STG_MECM_ID'));
  v_add := true;
  for cur in (Select * from nci_mec_map where mecm_id = ihook.getcolumnValue(row_ori,'MECM_ID')) loop
    v_add := false;
  end loop;
  if (v_add = true) then
  ...
  for curmodel in (select * from nci_mdl where item_id = ihook.getColumnValue(row_ori,'ITEM_ID') and ver_nr = ihook.getColumnValue(row_ori,'VER_NR')) loop
   ...
   hookoutput.message :='Alternate Name Type not set.';
    ...
    row := t_row();
     ...
     select count(*) into v_temp from alt_nms where nm_typ_id = curmodel.assoc_NM_TYP_ID and upper(nm_desc) = upper(curchar.nm_desc)
...
--raise_application_error(-20000,v_temp || curchar.cde_item_id);
     ...
     ihook.setColumnValue(row,'NM_DESC',upper(curchar.nm_DESC));
...
= (select CDE_ITEM_ID, CDE_VER_NR, DE_CONC_ITEM_ID, DE_CONC_VER_NR, VAL_DOM_ITEM_Id, VAL_DOM_VER_NR from nci_stg_mdl_elmnt_char s where 
...
and s.de_conc_item_id is not null)
...
--end loop;
 ...
 if (ihook.getColumnValue(row_ori, 'CTL_VAL_STUS') = 'CREATED' or ihook.getColumnValue(row_ori, 'CTL_VAL_STUS') = 'UPDATED') then
  ...
  -- Derive Primary Language and Model type from source
  
  for cur in (select * from obj_key where obj_typ_id = 40  and upper(obj_key_desc) = nvl(upper(ihook.getColumnValue(row_ori, 'SRC_PRMRY_LANG')),'XX')) loop
    ihook.setColumnValue(row_ori, 'PRMRY_MDL_LANG_ID', cur.obj_key_id);
end loop;
  for cur in (select * from obj_key where obj_typ_id = 44  and upper(obj_key_desc) = nvl(upper(ihook.getColumnValue(row_ori, 'SRC_MDL_TYP')),'XX')) loop
    ihook.setColumnValue(row_ori, 'MDL_TYP_ID', cur.obj_key_id);
end loop;

 -- validate Primary Language and Model Type is set.
/*
if (ihook.getColumnValue(row_ori, 'ASSOC_NM_TYP_ID') is null) then
    v_valid := false;
    v_val_stus_msg := v_val_stus_msg || 'Model: Alternate Name Type has to be specified.' || chr(13);
end if;
*/
/*
...
*/
 -- validate ME type id else default Semantic Model - Class. Physical Model - Table
  ...
  commit;
     for cur1 in (select * from obj_key where obj_typ_id = 49 and upper(cur.imp_grp_nm) = upper(obj_key_desc) and cur.imp_grp_nm is not null) loop
    update nci_stg_mdl_elmnt set me_grp_id =cur1.obj_Key_id where mdl_imp_id = ihook.getColumnValue(row_ori, 'MDL_IMP_ID') and ITEM_LONG_NM = cur.ITEM_LONG_NM;
    commit;
    end loop;
 ...
 if ( nvl(ihook.getColumnValue(row_ori, 'CTL_VAL_STUS'),'IMPORTED') = 'IMPORTED') then
...
if ( ihook.getColumnValue(row_ori, 'SRC_ME_PHY_NM') is not null and  ihook.getColumnValue(row_ori, 'SRC_MEC_PHY_NM') is not null) then 
    select count(*) into v_temp from vw_mdl_flat f, nci_mdl_map mm where f."Model Public ID" = mm.src_mdl_item_id and f."Model Version" = mm.src_mdl_ver_Nr
    and mm.item_id  = ihook.getColumnValue(row_ori, 'MDL_MAP_ITEM_ID')
    and mm.ver_nr = ihook.getColumnValue(row_ori, 'MDL_MAP_VER_NR') and  "Element Physical Name" =ihook.getColumnValue(row_ori, 'SRC_ME_PHY_NM')
    and "Characteristics Physical Name" =ihook.getColumnValue(row_ori, 'SRC_MEC_PHY_NM');
    if (v_temp = 0) then 
        v_valid := false;
...
if ( ihook.getColumnValue(row_ori, 'TGT_ME_PHY_NM') is not null and  ihook.getColumnValue(row_ori, 'TGT_MEC_PHY_NM') is not null) then 
    select count(*) into v_temp from vw_mdl_flat f, nci_mdl_map mm where f."Model Public ID" = mm.tgt_mdl_item_id and f."Model Version" = mm.tgt_mdl_ver_Nr
    and mm.item_id  = ihook.getColumnValue(row_ori, 'MDL_MAP_ITEM_ID')
    and mm.ver_nr = ihook.getColumnValue(row_ori, 'MDL_MAP_VER_NR') and  upper("Element Physical Name") =upper(ihook.getColumnValue(row_ori, 'TGT_ME_PHY_NM'))
    and upper("Characteristics Physical Name") =upper(ihook.getColumnValue(row_ori, 'TGT_MEC_PHY_NM'));
    if (v_temp = 0) then 
        v_valid := false;
