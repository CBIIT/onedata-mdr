...
if (v_mode in ('C','U')) then
    for i in 1..hookinput.originalRowset.rowset.count loop
        row_ori := hookInput.originalRowset.rowset(i);
        if (ihook.getColumnValue(row_ori, 'CTL_VAL_STUS') <> 'VALIDATED') then
            raise_application_error(-20000, 'Please validate selection before creating/updating.');
            --raise_application_error(-20000,'Please validate before creating/updating.' || chr(13) || 'Batch Name: ' || ihook.getColumnValue(row_ori, 'BTCH_NM') || '. Seq ID: ' || ihook.getColumnValue(row_ori, 'BTCH_SEQ_NBR'));
        end if;
        if (v_mode = 'C' and ihook.getColumnValue(row_ori, 'DE_CONC_ITEM_ID') is not null) then
            raise_application_error(-20000, 'DEC specified. Please check selection and run Update CDE.'); -- Batch Name: ' || ihook.getColumnValue(row_ori, 'BTCH_NM') || ' Seq ID: ' || ihook.getColumnValue(row_ori, 'SEQ_ID')); 
        end if;
        if (v_mode ='U' and ihook.getColumnValue(row_ori, 'DE_CONC_ITEM_ID') is null) then
            raise_application_error(-20000, 'DEC ID/Ver not specified. Please check selection and run Create CDE.');
        end if;
    end loop;
end if;
...
-- Parsefirst
   ...
   -- raise_application_error(-20000,'here');
    ...
    if (ihook.getColumnValue(row_ori, 'DE_CONC_ITEM_ID') is null) then --only check for oc/prop missing if dec is not specified
    ...
    elsif (ihook.getColumnValue(row_ori, 'DE_CONC_ITEM_ID') is not null) then
        if (ihook.getColumnValue(row_ori, 'CNCPT_1_ITEM_ID_1') is null and ihook.getColumnValue(row_ori, 'CNCPT_2_ITEM_ID_1') is null) then --populate the oc/prop concepts
            raise_application_error(-20000,'here');
        end if;
    end if;

            if (   ihook.getColumnValue(row_ori, 'CONC_DOM_ITEM_ID') is null and ihook.getColumnValue(row_ori, 'DE_CONC_ITEM_ID') is null ) then --dec id specifies update
...
if (ihook.getColumnValue(row_ori, 'CTL_VAL_STUS') <> 'PROCESSED') then
    ...
    if (ihook.getColumnValue(row_ori, 'CREAT_CDE_ID') is null or ihook.getColumnValue(row_ori, 'IMP_UPD_DEC_VER') is null) then
        ihook.setColumnValue(row_ori, 'CTL_VAL_STUS', 'ERRORS');
        v_val_ind := false;
        ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', 'CDE ID or Version missing.');   
    else
        v_item_id := ihook.getColumnValue(row_ori, 'CREAT_CDE_ID');
        v_ver_nr := ihook.getColumnValue(row_ori, 'IMP_UPD_DEC_VER');
        ...
        elsif (v_cde_fnd = 1) then --cde found, auto-populate null fields
        
            select ai.admin_notes, ai.item_desc, ai.item_nm, ai.origin, ai.item_long_nm, de.de_conc_item_id, de.de_conc_ver_nr, de.pref_quest_txt, de.val_dom_item_id, de.val_dom_ver_nr, de.derv_mthd, de.derv_rul, de.derv_typ_id, de.derv_rul_item_id, de.derv_rul_ver_nr, de.concat_char
            into v_admin_notes, v_desc, v_item_nm, v_orig, v_item_long_nm, v_de_conc_id, v_de_conc_ver_nr, v_pqt, v_vd_id, v_vd_ver_nr, v_derv_mthd, v_derv_rul, v_derv_typ, v_derv_rul_id, v_derv_rul_ver_nr, v_concat_char
    ...
    if (ihook.getColumnValue(row_ori, 'CTL_VAL_STUS') <> 'PROCESSED') then
        if (ihook.getColumnValue(row_ori, 'ITEM_ID')  is null or ihook.getColumnValue(row_ori, 'VER_NR')  is null or ihook.getColumnValue(row_ori, 'ADMIN_ITEM_TYP_ID') is null) then
            ...
            if (v_temp < 1) then
                ...
                if (ihook.getColumnValue(row_ori, 'IMP_DEF') is null or ihook.getColumnValue(row_ori, 'IMP_DEF') = '') then
                    v_val_ind := false;
...
for i in 1..hookinput.originalRowset.rowset.count loop
    ...
    if (ihook.getColumnValue(row_ori, 'CTL_VAL_STUS')<> 'PROCESSED') then
        ...
        if ( v_val_ind = true and v_mode = 'V') then
            nci_chng_mgmt.spCDEValUpdate(row_ori, 'V', actions, v_val_ind, v_item_id, v_ver_nr);
            if (v_val_ind = true) then
                iHook.setColumnValue(row_ori, 'CTL_VAL_STUS', 'VALIDATED');
            end if;
        end if;
        
        if (v_item_id is not null and  v_mode = 'C') then  
            nci_chng_mgmt.spCDEValUpdate(row_ori, 'C', actions, v_val_ind, v_item_id, v_ver_nr);
            if (v_val_ind = true) then
                iHook.setColumnValue(row_ori, 'CTL_VAL_STUS', 'PROCESSED');
            end if;
        end if;
        
        if (v_val_ind = false and v_mode = 'V') then
            iHook.setColumnValue(row_ori, 'CTL_VAL_STUS', 'ERRORS');
        end if;
