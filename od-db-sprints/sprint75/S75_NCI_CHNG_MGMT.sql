...
PROCEDURE spRefreshNmDef   ( v_data_in IN CLOB, v_data_out OUT CLOB, v_usr_id  IN varchar2);
...
procedure spCDEValUpdate (rowform in out t_row, v_op in varchar2, actions in out t_actions, v_val_ind in out boolean, v_item_id in number, v_ver_nr in number, v_usr_id in varchar2);
function get_AI_id(P_ID NUMBER,P_VER in number) RETURN VARCHAR2;
function getCSAccessLvlQuestion(v_from in number) return t_question;
procedure spUpdateAccessLvl (v_data_in IN CLOB, v_data_out OUT CLOB, v_usr_id in varchar2);
END;
/
...
BEGIN
  ...
  if (ihook.getColumnValue(row_ori,'LVL') <> 99) then
    raise_application_error(-20000, 'Please select a CS to edit.');
    return;
  end if;

  if hookinput.invocationNumber = 0 then
    HOOKOUTPUT.QUESTION := nci_chng_mgmt.getCSAccessLvlQuestion(1);
    v_item_id := ihook.getColumnValue(row_ori,'ITEM_ID');
    v_ver_nr := ihook.getColumnValue(row_ori,'VER_NR');
  ...
  if hookinput.invocationNumber = 1 then --user made selection
    row := t_row();
    ihook.setColumnValue(row, 'ITEM_ID', v_item_id);
    ihook.setColumnValue(row, 'VER_NR', v_ver_nr);

    if hookinput.answerid = 1 then --public 250
    --  ihook.setColumnValue(row, 'ACCESS_LVL', 250);
      update CLSFCTN_SCHM set ACCESS_LVL = 250 where item_id = v_item_id and ver_nr = v_ver_nr;
      commit;
      hookoutput.message := 'CS Access Level set to Public. <i>(run ''Refresh Views'' to see changes)</i>';
    elsif hookinput.answerid = 2 then --internal 251
      update CLSFCTN_SCHM set ACCESS_LVL = 251 where item_id = v_item_id and ver_nr = v_ver_nr;
      commit;
      hookoutput.message := 'CS Access Level set to Internal. <i>(run ''Refresh Views'' to see changes)</i>';
     -- ihook.setColumnValue(row, 'ACCESS_LVL', 251);
    end if;

  --  rows := t_rows(); rows.extend;          rows(rows.last) := row;
    --rowsetcs := t_rowset(rows, 'CS', 1, 'CLSFCTN_SCHM');

  --  action := t_actionrowset(rows, 'CS', 2,2,'update');
 --   actions.extend;    actions(actions.last) := action;
...
*/
 ...
 --       if (v_op = 'V') then  --- check if CDE is a duplicate
    ...
    --  raise_application_error(-20000, v_err_Str);
        ...
        ihook.setColumnValue(rowform, 'CTL_VAL_STUS', 'ERRORS');
          ...
          if (ihook.getColumnValue(rowform, 'CDE_ITEM_NM') is not null) THEN
            ihook.setColumnValue(rowform, 'CURRNT_VAL_ITEM_NM', ihook.getColumnValue(rowform, 'CDE_ITEM_NM'));
          else
            ihook.setColumnValue(rowform, 'CURRNT_VAL_ITEM_NM', substr(v_dec_item_nm || ' ' || v_vd_item_nm, 1, 255));
          end if;
        end if;
   ...
   --     raise_application_error(-20000, 'Front' || v_dec_item_id);             select item_nm, item_desc into v_dec_item_nm, v_dec_item_def from admin_item where item_id = v_dec_item_id and ver_nr = v_dec_ver_nr;
        ...
        --jira 9374: long name processing moved to validation step - set to whatever the current value is from validation step
        ihook.setColumnValue(row, 'ITEM_NM', ihook.getColumnValue(rowform, 'CURRNT_VAL_ITEM_NM'));
  ...
  if (v_origin_id is not null) then
     ihook.setColumnValue(row,'ORIGIN_ID',v_origin_id);
       ...
       ihook.setColumnValue(row,'ORIGIN',ihook.getColumnValue(rowform,'CDE_ORIGIN'));
        ...
        ihook.setColumnValue(row,'ITEM_ID', v_id);
    ihook.setColumnValue(row,'VER_NR', 1.00);
     --jira 3594 ihook.setColumnValue(row,'REF_NM', substr(ihook.getColumnValue(rowform, 'PREF_QUEST_TXT'),1,255));
     ihook.setColumnValue(row,'REF_NM', 'PQT');
...
function getCSAccessLvlQuestion(v_from in number) return t_question is
  question t_question;
  answer t_answer;
  answers t_answers;
begin
 ...
 ANSWERS                    := T_ANSWERS();
 if (v_from = 1) then -- CS Access Level
    ANSWER                     := T_ANSWER(1, 1, 'Set to Public');
    ANSWERS.EXTEND;
    ANSWERS(ANSWERS.LAST) := ANSWER;

    ANSWER                     := T_ANSWER(2, 2, 'Set to Internal');
    ANSWERS.EXTEND;
    ANSWERS(ANSWERS.LAST) := ANSWER;
    QUESTION               := T_QUESTION('Mark the CS for Public or Internal Use. <BR> <i>Child CSI''s are updated automatically. Run ''Refresh Views'' command to see changes.</i> ', ANSWERS);
  end if;
  return question;
end;
...
BEGIN
  ...
  hookoutput.originalrowset    := hookinput.originalrowset;
       ...
       for i in 1..hookinput.originalrowset.rowset.count loop
          ...
          select substrb(dec.item_nm || ' ' || vd.item_nm,1,255) ,substrb(dec.item_desc || ':' || vd.item_desc,  1, 4000) into v_item_nm, v_item_def 
          from admin_item dec, admin_item vd, de
            where dec.item_id = de.DE_CONC_ITEM_ID and dec.ver_nr =  de.de_conc_ver_nr
            and vd.item_id =  de.VAL_DOM_ITEM_ID and vd.ver_nr = de.VAL_DOM_VER_NR and 
            de.item_id =  ihook.getColumnValue(row_ori, 'ITEM_ID') and de.ver_nr = ihook.getColumnValue(row_ori, 'VER_NR');
    ihook.setColumnValue(row, 'ITEM_NM', v_item_nm);
        ...
        hookoutput.message := 'Name/Definition refreshed.';
...
--- called from CDE Update
procedure spCDEValUpdate (rowform in out t_row, v_op in varchar2, actions in out t_actions, v_val_ind in out boolean, v_item_id in number, v_ver_nr in number, v_usr_id in varchar2) as
c_ver_suffix varchar2(5) := 'v1.00';
    row t_row;
    rows  t_rows;
...
BEGIN
    ...
    ihook.setColumnValue(row,'ITEM_NM', nvl(ihook.getColumnValue(rowform, 'ITEM_NM'), ihook.getColumnValue(rowform, 'CURRNT_VAL_ITEM_NM')));
      ...
      ihook.setColumnValue(row,'VAL_DOM_ITEM_ID', ihook.getColumnValue(rowform, 'VAL_DOM_ITEM_ID'));
               ihook.setColumnValue(row,'VAL_DOM_VER_NR', ihook.getColumnValue(rowform, 'VAL_DOM_VER_NR'));

        if (ihook.getColumnValue(rowform,'CNTXT_ITEM_ID') is not null and v_op <> 'N') then
            ihook.setColumnValue(row,'CNTXT_ITEM_ID', ihook.getColumnValue(rowform,'CNTXT_ITEM_ID'));
        end if;
        if (ihook.getColumnValue(rowform,'CNTXT_VER_NR') is not null and v_op <> 'N') then
            ihook.setColumnValue(row,'CNTXT_VER_NR', ihook.getColumnValue(rowform,'CNTXT_VER_NR'));
        end if;
  ...
  if (v_origin_id is not null) then
     ihook.setColumnValue(row,'ORIGIN_ID',v_origin_id);
       ...
       ihook.setColumnValue(row,'ORIGIN',ihook.getColumnValue(rowform,'CDE_ORIGIN'));
        ...
        if (ihook.getColumnValue(rowform, 'DTTYPE_ID') is not null) then
            ihook.setColumnValue(row, 'DERV_TYP_ID', ihook.getColumnValue(rowform, 'DTTYPE_ID'));
            ihook.setColumnValue(row, 'DERV_DE_IND', 1);
        end if;
        if (ihook.getColumnValue(rowform, 'ITEM_3_DEF') is not null) then
            ihook.setColumnValue(row, 'DERV_MTHD', ihook.getColumnValue(rowform, 'ITEM_3_DEF'));
        end if;
        if (ihook.getColumnValue(rowform, 'EFF_DT') is not null) then
            ihook.setColumnValue(row, 'EFF_DT', ihook.getColumnValue(rowform, 'EFF_DT'));
        end if;
   ...
   ihook.setColumnValue(row,'REF_NM', 'PQT');
     ihook.setColumnValue(row,'REF_DESC', ihook.getColumnValue(rowform, 'PREF_QUEST_TXT'));
     ihook.setColumnValue(row,'LANG_ID', 1000);
     ihook.setColumnValue(row,'NCI_CNTXT_ITEM_ID',ihook.getColumnValue(row, 'CNTXT_ITEM_ID')  );
     ihook.setColumnValue(row,'NCI_CNTXT_VER_NR',ihook.getColumnValue(row, 'CNTXT_VER_NR')  );
     ihook.setColumnValue(row,'REF_TYP_ID', 80);
     ihook.setColumnValue(row,'REF_ID', -1);   
        rows.extend;
        rows(rows.last) := row;
        if (v_op = 'U') then 
            ...
            action := t_actionrowset(rows, 'Data Element', 2,11,'update');
            actions.extend;
            actions(actions.last) := action;
    ...
    if ( ihook.getColumnValue(rowform, 'IMP_PREF_QUEST_TXT') is not null) then -- if null then copying from existing version which will be done in the copy children
            ...
            ihook.setColumnValue(rowform, 'CTL_VAL_MSG',  'CDE Version ' ||ihook.getColumnValue(rowform,'NEW_VER_NR') || ' Created Successfully' ||  chr(13)) ;
