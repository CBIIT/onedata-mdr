...
procedure spAssociateCDE (v_data_in in clob, v_data_out out clob, v_usr_id in varchar2);
...
procedure spCopyQuestionWithinForm (v_data_in in clob, v_data_out out clob,  v_user_id in varchar2);
...
procedure createVVFromRow (row in out t_row, row_ori in t_row,  answerid in integer, v_disp_ord in integer, actions in out t_actions);
PROCEDURE spCreateVVFromParent ( v_data_in IN CLOB, v_data_out OUT CLOB, v_usr_id  IN varchar2);
procedure spCopySingleQuestion (actions in out t_actions, v_from_question_id in number, v_from_question_ver in number, v_to_module_id in number, v_to_module_ver in number, v_disp_ord number, v_rep number) ;
...
function getVVQuestionFromParentStep2  return t_question
is
  question t_question;
  answer t_answer;
  answers t_answers;
...
-- There is no way for a hook step to know what the previous selection was. I am using AnswerId to determine if 50+ then use Name. Else, use code.
 ANSWERS                    := T_ANSWERS();
    ANSWER                     := T_ANSWER(1, 1, 'Create with Selected');
    ANSWERS.EXTEND;
    ANSWERS(ANSWERS.LAST) := ANSWER;

  QUESTION               := T_QUESTION('Select Concepts to create VV. ', ANSWERS);
...
v_disp_ord integer;
v_temp integer;
v_max_disp_ord integer;
v_cur_disp_ord integer;
begin
    -- Standard header
   ...
   -- chkPVVMActionValid ( row_ori, v_usr_id );
    ...
    rowform := t_Row();
    Select count(*) into v_temp from cncpt_admin_item where item_id = ihook.getColumnValue(row_ori,'VAL_MEAN_ITEM_ID')
    and ver_nr = ihook.getColumnValue(row_ori,'VAL_MEAN_VER_NR') ;


    if v_temp = 0 then
    raise_application_error(-20000, 'VM is not associated with Concept.');
    return;
    end if;
    if v_temp > 1 then
    raise_application_error(-20000, 'Cannot create VVs from VM with multiple concepts.');
    return;
    end if;
     for cur in (Select cncpt_item_id, cncpt_ver_nr from cncpt_admin_item where item_id = ihook.getColumnValue(row_ori,'VAL_MEAN_ITEM_ID')
    and ver_nr = ihook.getColumnValue(row_ori,'VAL_MEAN_VER_NR') group by cncpt_item_id, cncpt_ver_nr having count(*) = 1) loop
        ihook.setColumnValue(rowform,'P_ITEM_ID',cur.cncpt_item_id);
        ihook.setColumnValue(rowform,'P_ITEM_VER_NR',cur.cncpt_ver_nr);
    end loop;
        hookoutput.question := getVVQuestionFromParentStep2 ;
        rows := t_rows();
         rows.extend;
    ...
    v_disp_ord := ihook.getColumnValue(row_ori, 'DISP_ORD')+1;
   -- select max(disp_ord) into v_max_disp_ord from nci_quest_Valid_value where q_pub_id = ihook.getColumnValue(row_ori,'Q_PUB_ID') and 
   -- q_ver_nr = ihook.getColumnValue(row_ori, 'Q_VER_NR');
 ...
 if (hookinput.selectedRowset is not null) then  
     ...
     for i in 1..hookinput.selectedrowset.rowset.count  loop
        ...
        createVVFromRow( rowform,row_ori, hookinput.answerid,v_disp_ord, actions);
        v_disp_ord := v_disp_ord + 1;
   -- raise_application_error(-20000,'Step 2' || v_disp_ord);
    end loop;

    v_cur_disp_ord := ihook.getColumnValue(row_ori, 'DISP_ORD')+1;
    rows := t_rows();
    for cur in (select * from nci_quest_valid_Value where q_pub_id = ihook.getColumnValue(row_ori,'Q_PUB_ID') and 
    q_ver_nr = ihook.getColumnValue(row_ori, 'Q_VER_NR') and disp_ord >= v_cur_disp_ord order by disp_ord) loop
    ...
    ihook.setColumnValue (row, 'DISP_ORD', v_disp_ord);
     ...
     action             := t_actionrowset(rows, 'Question Valid Values (Hook)', 2, 10,'update');
            actions.extend;
...
begin
  ...
  /* for curcd in (Select * from value_dom where item_id = ihook.getColumnValue(row_ori, 'ITEM_ID') and ver_nr = ihook.getColumnValue(row_ori,'VER_NR')) loop
     ...
     ihook.setColumnValue(row,'CNCPT_1_ITEM_ID_1',ihook.getColumnValue(row,'C_ITEM_ID'));
      ihook.setColumnValue(row,'CNCPT_1_VER_NR_1',ihook.getColumnValue(row,'C_ITEM_VER_NR'));
       nci_dec_mgmt.createValAIWithConcept(row , 1,53 ,'C','DROP-DOWN',actions); -- Vm
  ...
  --raise_application_error(-20000,v_pv);
      ...
      if (v_pv is not null) then
        select count(*) into v_temp from NCI_QUEST_VALID_VALUE where upper(Value) = upper(v_pv) and VAL_MEAN_ITEM_ID = v_item_id and VAL_MEAN_VER_NR = v_ver_nr
      and q_pub_id = ihook.getColumnValue(row_ori,'Q_PUB_ID') and q_ver_nr = ihook.getCOlumnValue(row_ori,'Q_VER_NR');
     --      raise_application_error(-20000,v_temp);
        ...
        select cncpt_concat into v_prnt_lnm from nci_admin_item_ext where item_id = ihook.getcolumnvalue(row_ori,'VAL_MEAN_ITEM_ID') and ver_nr = ihook.getColumnValue(row_ori,'VAL_MEAN_VER_NR');
         for curpv in (select * from admin_item where item_id =  ihook.getColumnValue(row,'C_ITEM_ID') and ver_nr = ihook.getColumnValue(row,'C_ITEM_VER_NR')) loop
                ...
                ihook.setColumnValue (rowq, 'Q_VER_NR', ihook.getColumnValue(row_ori, 'Q_VER_NR'));
            ...
            --    ihook.setColumnValue (rowq, 'CMNTS', curpv.ITEM_LONG_NM);
                ...
                ihook.setColumnValue (rowq, 'VAL_MEAN_ITEM_ID', v_item_id);
                ihook.setColumnValue (rowq, 'VAL_MEAN_VER_NR', v_ver_nr);
                v_item_id := nci_11179.getItemId;
                ihook.setColumnValue (rowq, 'NCI_PUB_ID', v_item_id);
                ihook.setColumnValue (rowq, 'NCI_VER_NR', 1);
                ihook.setColumnValue (rowq, 'DISP_ORD', v_disp_ord);

                    rows.extend;
                    rows (rows.last) := rowq;
  action             := t_actionrowset(rows, 'Question Valid Values (Hook)', 2, 5,'insert');
            actions.extend;
            actions(actions.last) := action;
...
begin
select nci_11179.getLongName(nvl(LISTAGG(DE_ITEM_ID || 'v' || DE_VER_NR, ','),'None')) into v_retired_cdes from VW_NCI_MODULE_DE WHERE frm_item_id = v_item_id and frm_ver_nr = v_ver_nr and DE_ITEM_ID is not null
    and DE_ADMIN_STUS_NM_DN like '%RETIRED%';
    --raise_application_error(-20000, v_retired_cdes);

select nci_11179.getLongName(nvl(LISTAGG(DE_ITEM_ID || 'v' || DE_VER_NR, ','),'None')) into v_not_latest from VW_NCI_MODULE_DE WHERE frm_item_id = v_item_id and frm_ver_nr = v_ver_nr and DE_ITEM_ID is not null    
    and DE_CURRNT_VER_IND = 0 and DE_ADMIN_STUS_NM_DN not like '%RETIRED%';
    --raise_application_error(-20000, v_not_latest);
    return 'WARNING: CDEs on Form Retired: ' || v_retired_cdes || '  Not Latest Version: ' || v_not_latest || chr(13); 
end;
...
begin
                                    ...
                                    ai.item_id =v_item_id and ai.ver_nr = v_ver_nr and ai.item_id = de.item_id  and ai.ver_nr = de.ver_nr )  loop
                                         ...
                                         --   v_ver_nr := cur.ver_nr;
                                            ...
                                            j := 0;
                                            if (nvl(ihook.getColumnValue(row_sel, 'ITEM_ID'),0) = 0) then  -- Boolean tag to see if VV should be created
                                          --  raise_application_error(-20000,'Create vv');
                                          ...
                                          v_vvid := nci_11179.getItemId;
                                            ...
                                            end if; ---DO NOT create VV
                                      --      raise_application_error(-20000,'DO NOT Create vv');
...
begin
  ...
  if (ihook.getColumnValue(row_ori,'P_ITEM_ID') is null) then  -- calling from flattenerd view
    ihook.setColumnValue(row_ori,'P_ITEM_ID', ihook.getColumnValue(row_ori, 'FRM_ITEM_ID'));
    ihook.setColumnValue(row_ori,'P_ITEM_VER_NR', ihook.getColumnValue(row_ori, 'FRM_VER_NR'));
    ihook.setColumnValue(row_ori,'C_ITEM_ID', ihook.getColumnValue(row_ori, 'MOD_ITEM_ID'));
    ihook.setColumnValue(row_ori,'C_ITEM_VER_NR', ihook.getColumnValue(row_ori, 'MOD_VER_NR'));
 ...
 raise_application_error(-20000,'You are not authorized to add a question to this module.');
    ...
    if (hookinput.invocationnumber = 0) then   -- First invocation
       ...
       -- form1                  := t_form('Add Item to Collection (Hook)', 2,1);
        form1                  := t_form('Add Question Using ID', 2,1);
            ...
            for i in  1..cnt loop
                  IF (VALIDATE_CONVERSION(nci_11179.getWord(v_str, i, cnt) AS NUMBER) = 1) THEN
                    ...
                    -- Only add if item is of the right type and not currently in collection.
                           select 1 , ver_nr into v_temp,  v_ver_nr from admin_item where item_id = v_item_id and currnt_ver_ind = 1 and admin_item_typ_id = v_item_typ_id
...
--raise_application_error(-20000,actions.count);
            ...
            v_cnt_already :=  v_cnt_valid_type - nvl(rows.count,0) ;
                 hookoutput.message := 'Total Items: ' || cnt ||  ';    Invalid Format/Type/Status: ' || v_invalid_typ || ';      Items added: ' || nvl(rows.count,0) || ';  DDE Components added: ' || v_cmp_add ;
...
BEGIN
 ...
 select r.p_item_id, r.p_item_ver_nr into v_frm_id, v_frm_ver_nr from nci_admin_item_rel r, nci_admin_item_rel_alt_key q where
q.nci_pub_id = ihook.getColumnValue(row_ori, 'Q_PUB_ID') and q.nci_ver_nr = ihook.getColumnValue(row_ori, 'Q_VER_NR') and q.p_item_id = r.c_item_id and q.p_item_ver_nr = r.c_item_ver_nr and r.rel_typ_id =61;
 if (nci_form_mgmt.isUserAuth(v_frm_id, v_frm_ver_nr, v_usr_id) = false) then
...
v_usr_id,v_usr_id, sysdate , sysdate,
ai.item_id, ai.ver_nr,  ai.item_nm
from NCI_ADMIN_ITEM_REL an, ADMIN_ITEM ai where 
an.REL_TYP_ID = 60 and an.c_item_id =ihook.getColumnValue(row_ori,'C_ITEM_ID') and an.c_item_ver_nr = ihook.getColumnValue(row_ori,'C_ITEM_VER_NR')  
and ai.item_id = an.P_ITEM_ID and ai.ver_nr = an.P_ITEM_VER_NR
and ai.item_id=ihook.getColumnValue(row_ori,'P_ITEM_ID') and  ai.ver_nr=ihook.getColumnValue(row_ori,'P_ITEM_VER_NR');
commit;
...
nci_pub_id = ihook.getColumnValue(row_ori, 'Q_PUB_ID') and nci_ver_nr = ihook.getColumnValue(row_ori, 'Q_VER_NR');
 ...
 -- if current position is less than new position
    ...
    updFormModuleQuestion (ihook.getColumnValue(row_ori, 'Q_PUB_ID'), ihook.getColumnValue(row_ori, 'Q_VER_NR'), v_user_id);
...
----
    ...
    -- Tracker 2505
        ihook.setColumnValue(action_row,'ADMIN_NOTES','');
        ihook.setColumnValue(action_row,'CHNG_DESC_TXT','');
  ...
  ELSE -- Second invocation
   ...
   --    raise_application_error(-20000, 'yyy' || nvl(ihook.getColumnValue(rowform, 'P_ITEM_VER_NR'),'1') || 'kkk');
    /*   if (nvl(ihook.getColumnValue(rowform, 'P_ITEM_VER_NR'),0) = 0) then
        ...
        rows.extend;
        rows(rows.last) := row;
        -- jira 2301 added block to reinsert output message
            select count(*) into v_retired_cde from VW_NCI_MODULE_DE WHERE frm_item_id = v_from_item_id and frm_ver_nr = v_from_ver_nr and DE_ITEM_ID is not null
    and (DE_ADMIN_STUS_NM_DN like '%RETIRED%' or DE_CURRNT_VER_IND = 0);
 ...
 raise_application_error(-20000,'You are not authorized to edit any question in this form.');
        ...
        for i in 1..hookinput.originalRowset.rowset.count loop
         ...
         row := t_row();
           ihook.setColumnValue(row, 'NCI_PUB_ID', ihook.getColumnValue(row_ori,'NCI_PUB_ID'));
         ihook.setColumnValue(row, 'NCI_VER_NR', ihook.getColumnValue(row_ori,'NCI_VER_NR'));
         ihook.setColumnValue(row, 'QUEST_VV_REP_ID', ihook.getColumnValue(row_ori,'QUEST_VV_REP_ID'));


           ihook.setColumnValue(row, 'DEFLT_VAL_ID', '');
             ihook.setColumnValue(row, 'DEFLT_VAL', '');
               ihook.setColumnValue(row, 'VAL', '');
            rows.extend;
            rows(rows.last) := row;
        end loop;
...
if (ihook.getcolumnValue(row_ori, 'C_ITEM_ID') is null) then
raise_application_error(-20000, 'No CDE attached to question.');
return;
end if;
...
v_found boolean;
  ...
  msg varchar2(4000);
    ...
    type t_quest_txt is table of admin_item.item_desc%type;

    v_tab_item_id  t_item_id := t_item_id();
    v_tab_ver_nr  t_ver_nr := t_ver_nr();
    v_tab_quest_txt  t_quest_txt := t_quest_txt();
rep integer;
v_tmp_fnd boolean;
v_isDDE boolean;
v_dde_nm varchar2(4000);
BEGIN
  hookinput                    := Ihook.gethookinput (v_data_in);
  hookoutput.invocationnumber  := hookinput.invocationnumber;
  hookoutput.originalrowset    := hookinput.originalrowset;
   ...
   if (hookinput.invocationNumber = 0 and v_src = 'N') then 
    ...
    if ((hookInput.invocationNumber = 0 and v_src = 'D') or (hookinput.invocationnumber = 1 and v_src = 'N')) then
	  ...
	  if (v_found) then
       	 ...
       	 answers := t_answers();
  	   	 answer := t_answer(1, 1, 'Add with Valid Values');
  	   	 answers.extend; answers(answers.last) := answer;
	   	 answer := t_answer(3, 3, 'Add without Valid Values');
  	   	 answers.extend; answers(answers.last) := answer;

    if (v_src = 'D') then
	   	 answer := t_answer(2, 2, 'Add All with Valid Values');
  	   	 answers.extend; answers(answers.last) := answer;
     	 answer := t_answer(4, 4, 'Add All without Valid Values');
  	   	 answers.extend; answers(answers.last) := answer;
  end if;
 ...
 if ((hookInput.invocationNumber = 1 and v_src = 'D') or (hookinput.invocationnumber = 2 and v_src = 'N')) then
        ...
        if (hookinput.answerid in ( 1,3)) then -- Selected CDEs
            --jira 3663 process DDEs first
            for h in 1..hookInput.selectedRowset.rowset.count loop
                row_sel := hookInput.selectedRowset.rowset(h);
                v_isDDE := nci_11179_2.isDDEparent(ihook.getColumnValue(row_sel, 'ITEM_ID'),ihook.getColumnValue(row_sel, 'VER_NR'));
                if (v_isDDE = true) then --process DDE
                    ...
                    --jira 3663 need to prepend dde name to question text for component cdes
                    v_tab_quest_txt.extend();
                    select nvl(r.ref_desc, ai.item_long_nm) 
                    --select ai.item_nm
                    into v_dde_nm from admin_item ai, ref r where ai.item_id = ihook.getColumnValue(row_sel, 'ITEM_ID') and ai.ver_nr = ihook.getColumnValue(row_sel, 'VER_NR')
                    and ai.item_id = r.item_id (+) and ai.ver_nr = r.ver_nr (+) and r.ref_typ_id (+) = 80;
                    v_tab_quest_txt(v_tab_quest_txt.count) := v_dde_nm;

                    --add component cdes
                    for cur in (select c_item_id, c_item_ver_nr, item_nm  from nci_admin_item_rel, admin_item where rel_typ_id in (65,66) and p_item_id = ihook.getColumnValue(row_sel,'ITEM_ID')
                    and p_item_ver_nr = ihook.getColumnValue(row_sel,'VER_NR') and p_item_id = admin_item.item_id and p_item_ver_nr = admin_item.ver_nr order by disp_ord) loop
                        v_tab_item_id.extend();
                        v_tab_ver_nr.extend();
            ...
            -- end 3663 changes
            -- process rest of selection (non DDEs)
            for i in 1..hookInput.selectedRowset.rowset.count loop
                row_sel := hookInput.selectedRowset.rowset(i);
                v_tab_item_id.extend();
                v_tab_ver_nr.extend();
                v_tab_quest_txt.extend();
                v_tmp_fnd := false;
                for m in 1..v_tab_item_id.count loop
                    if (ihook.getColumnValue(row_sel, 'ITEM_ID') = v_tab_item_id(m) and ihook.getColumnValue(row_sel, 'VER_NR') = v_tab_ver_nr(m)) then
                        v_tmp_fnd := true;
                    end if;
                end loop;
                if (v_tmp_fnd = false) then
                    v_tab_item_id(v_tab_item_id.count) := ihook.getColumnValue(row_sel, 'ITEM_ID');
                    v_tab_ver_nr(v_tab_ver_nr.count) := ihook.getColumnValue(row_sel, 'VER_NR');
                    v_tab_quest_txt(v_tab_quest_txt.count) := '';
                end if;
               -- v_tab_item_id(v_tab_item_id.count) := ihook.getColumnValue(row_sel,'ITEM_ID');
              --  v_tab_ver_nr(v_tab_ver_nr.count) := ihook.getColumnValue(row_sel,'VER_NR');
                -- Add component Item id
                -- jira 3663: add rel_typ_id 66
--                    for cur in (select c_item_id, c_item_ver_nr from nci_admin_item_rel where rel_typ_id in (65,66) and p_item_id = ihook.getColumnValue(row_sel,'ITEM_ID')
--                    and p_item_ver_nr = ihook.getColumnValue(row_sel,'VER_NR') order by disp_ord) loop
--                        v_tmp_fnd := false;
--                        for j in 1..v_tab_item_id.count loop
--                            if (cur.c_item_id = v_tab_item_id(j)) then
--                                v_tmp_fnd := true;
--                            end if;
--                        end loop;
--                        if (v_tmp_fnd = false) then        
...
--                    end loop;
        ...
        if (hookinput.answerid in ( 2,4)) then -- Add CDEs from Cart
      --  raise_application_error(-20000, v_cart_nm);
      --jira 3663 process DDEs first
      --raise_application_error(-20000, 'here');
            for h in (select c.item_id, c.ver_nr from NCI_USR_CART c, admin_item ai where c.fld_delete= 0 and cntct_secu_id = v_user_id and ai.item_id = c.item_id and ai.ver_nr = c.ver_nr and
        ai.admin_item_typ_id = 4 and c.CART_NM = v_cart_nm ) loop
                --row_sel := hookInput.selectedRowset.rowset(h);
                v_isDDE := nci_11179_2.isDDEparent(h.item_id,h.ver_nr);
                if (v_isDDE = true) then --process DDE
                   -- raise_application_error(-20000, ihook.getColumnValue(row_sel, 'ITEM_ID'));
                    v_tab_item_id.extend();
                    v_tab_ver_nr.extend();
                    v_tab_item_id(v_tab_item_id.count) := h.item_id;
                    v_tab_ver_nr(v_tab_ver_nr.count) := h.ver_nr;
                    --jira 3663 need to prepend dde name to question text for component cdes
                    v_tab_quest_txt.extend();
--                    select item_nm into v_dde_nm from admin_item where item_id = h.item_id and ver_nr = h.ver_nr;
--                    v_tab_quest_txt(v_tab_quest_txt.count) := v_dde_nm;

                    select nvl(r.ref_desc, ai.item_long_nm) 
                    --select ai.item_nm
                    into v_dde_nm from admin_item ai, ref r where ai.item_id = h.item_id and ai.ver_nr = h.ver_nr
                    and ai.item_id = r.item_id (+) and ai.ver_nr = r.ver_nr (+) and r.ref_typ_id (+) = 80;
                    v_tab_quest_txt(v_tab_quest_txt.count) := v_dde_nm;

                    --add component cdes
                    for cur in (select c_item_id, c_item_ver_nr from nci_admin_item_rel where rel_typ_id in (65,66) and p_item_id = h.item_id
                    and p_item_ver_nr = h.ver_nr order by disp_ord) loop
                        v_tab_item_id.extend();
                        v_tab_ver_nr.extend();
                        v_tab_item_id(v_tab_item_id.count) := cur.c_item_id;
                        v_tab_ver_nr(v_tab_ver_nr.count) := cur.c_item_ver_nr;
        ...
        ai.admin_item_typ_id = 4 and c.CART_NM = v_cart_nm ) loop
                ...
                v_tmp_fnd := false;
                for m in 1..v_tab_item_id.count loop
                    if (cur1.item_id = v_tab_item_id(m)) then
                        v_tmp_fnd := true;
                    end if;
                end loop;
                if (v_tmp_fnd = false) then
                ...
                -- Add component Item id
                -- jira 3663: add rel_typ_id 66
--                    for cur in (select c_item_id, c_item_ver_nr from nci_admin_item_rel where rel_typ_id in (65, 66) and p_item_id = cur1.item_id
--                    and p_item_ver_nr = cur1.ver_nr order by disp_ord) loop
--                        v_tmp_fnd := false;
--                        for k in 1..v_tab_item_id.count loop
--                            if (cur.c_item_id = v_tab_item_id(k)) then
--                                v_tmp_fnd := true;
--                            end if;
--                        end loop;
--                        if (v_tmp_fnd = false) then        
...
--                    end loop;
    ...
    --      raise_application_error(-20000, 'herere ' || v_tab_item_id.count);
             ...
             --   raise_application_error(-20000, 'herere ' || cur.item_id);
                        ...
                        v_isDDE := nci_11179_2.isDDEchild(cur.item_id, cur.ver_nr);
                        if (v_isDDE = true) then
                            for par in (select p_item_id, item_nm from admin_item ai, nci_admin_item_rel ar 
                            where ai.item_id = ar.p_item_id and ai.ver_nr = ar.p_item_ver_nr and ar.c_item_id = cur.item_id and ar.c_item_ver_nr = cur.ver_nr and ar.rel_typ_id = 66) loop
                                for n in 1..v_tab_item_id.count loop
                                    if (par.p_item_id = v_tab_item_id(n)) then
                                        ihook.setColumnValue(row, 'ITEM_LONG_NM', v_tab_quest_txt(i) || ':' || cur.QUEST_TEXT);
                                        --ihook.setColumnValue(row, 'ITEM_LONG_NM', par.item_nm || ':' || cur.QUEST_TEXT);
                                    end if;
                                end loop;
                            end loop;
--                            select item_nm into v_dde_nm from admin_item ai, nci_admin_item_rel ar 
--                            where ai.item_id = ar.p_item_id and ai.ver_nr = ar.p_item_ver_nr and ar.c_item_id = cur.item_id and ar.c_item_ver_nr = cur.ver_nr and ar.rel_typ_id = 66;
--                            ihook.setColumnValue(row, 'ITEM_LONG_NM', v_dde_nm || ':' || cur.QUEST_TEXT);  
--                        else
--                            ihook.setColumnValue (row, 'ITEM_LONG_NM', cur.QUEST_TEXT );
                        end if;
                         ihook.setColumnValue (row, 'ITEM_NM', cur.ITEM_LONG_NM );
                        ...
                        j := 0;
                        if (hookinput.answerid in (1,2)) then
                for cur1 in (select * from  VW_NCI_DE_PV where de_item_id = cur.item_id and de_ver_nr = cur.ver_nr and nvl(fld_delete,0) = 0 order by PERM_VAL_NM) loop
                  row := t_row();
                ihook.setColumnValue (row, 'Q_PUB_ID', v_id);
                ihook.setColumnValue (row, 'Q_VER_NR', 1);
...
PROCEDURE spCopyQuestionWithinForm
  (
    v_data_in IN CLOB,
    v_data_out OUT CLOB,
    v_user_id in varchar2)
...
BEGIN
  ...
  hookoutput.originalrowset    := hookinput.originalrowset;
  row_ori :=  hookInput.originalRowset.rowset(1);
  v_mod_item_id := ihook.getColumnValue(row_ori,'C_ITEM_ID');
  v_mod_ver_nr:= ihook.getColumnValue(row_ori,'C_ITEM_VER_NR');
  
    rows := t_rows();
if (nci_form_mgmt.isUserAuth(ihook.getColumnValue(row_ori, 'P_ITEM_ID'), ihook.getColumnValue(row_ori,'P_ITEM_VER_NR'), v_user_id) = false) then
 raise_application_error(-20000,'You are not authorized to copy question.');
 end if;
    ...
    if (hookInput.invocationNumber = 0 ) then
	   ...
	   for cur in (select * from nci_admin_item_rel where p_item_id = ihook.getColumnValue(row_ori,'P_ITEM_ID') and p_item_ver_nr = ihook.getColumnValue(row_ori,'P_ITEM_VER_NR')
       and rel_typ_id =61 and c_item_id <> v_mod_item_id) loop
		   row := t_row();
	  ...
	  if (v_found) then
       	 showrowset := t_showablerowset (rows, 'Form-Module Relationship', 2, 'single');
       	 hookoutput.showrowset := showrowset;

       	 answers := t_answers();
  	   	 answer := t_answer(1, 1, 'Select Module to Copy Questions From');
  	   	 answers.extend; answers(answers.last) := answer;

	   	 question := t_question('Select Module to Copy Questions From', answers);
       	 hookOutput.question := question;
	   end if;

	end if;

 if (hookInput.invocationNumber = 1) then -- show questions
          row_sel := hookInput.selectedRowset.rowset(1);
          rows := t_rows();
           row := t_row();
	   	   iHook.setcolumnvalue (ROW, 'P_ITEM_ID', ihook.getColumnValue(row_sel,'C_ITEM_ID'));
		   iHook.setcolumnvalue (ROW, 'P_ITEM_VER_NR', ihook.getColumnValue(row_sel,'C_ITEM_VER_NR'));
		   rows.extend;
		   rows (rows.last) := row;
		   
       	 showrowset := t_showablerowset (rows, 'Questions (Edit)', 2, 'multi');
       	 hookoutput.showrowset := showrowset;

       	 answers := t_answers();
  	   	 answer := t_answer(1, 1, 'Select Questions');
  	   	 answers.extend; answers(answers.last) := answer;

	   	 question := t_question('Select Questions to Copy', answers);
       	 hookOutput.question := question;
    end if;
   -- if (hookInput.invocationNumber = 2 or v_mod_specified = true) then -- copy module
    if (hookInput.invocationNumber = 2 ) then
 select nvl(max(DISP_ORD),-1) into v_disp_ord from NCI_ADMIN_ITEM_REL_ALT_KEY where p_item_id= v_mod_item_id and p_item_ver_nr = v_mod_ver_nr;
   
 for i in 1..hookinput.selectedRowset.rowset.count loop

	row_sel := hookInput.selectedRowset.rowset(i);
    
   -- raise_application_error(-20000,ihook.getColumnValue(row_ori,'REP_NO'));
    spCopySingleQuestion (actions, ihook.getColumnValue(row_sel,'NCI_PUB_ID'),ihook.getColumnValue(row_sel,'NCI_VER_NR'),
    v_mod_item_id, v_mod_ver_nr, v_disp_ord + 1,ihook.getColumnValue(row_ori,'REP_NO'));
    v_disp_ord := v_disp_ord + 1;
 end loop;

    hookoutput.actions := actions;
    hookoutput.message := 'Question copied successfully.';
    --hookoutput.message := v_add || ' concepts added.';
    end if;
...
procedure spCopySingleQuestion (actions in out t_actions, v_from_question_id in number, v_from_question_ver in number, v_to_module_id in number, v_to_module_ver in number, v_disp_ord number, v_rep number) as
v_id integer;
v_found boolean;
v_itemid integer;
row  t_row;
...
begin
 ...
 rowsrep := t_rows();
 rows:= t_rows();
for cur in (select * from nci_admin_item_rel_alt_key where nci_pub_id = v_from_question_id and
        nci_ver_nr = v_from_question_ver and rel_typ_id = 63 and nvl(fld_delete,0) = 0) loop
    rowq := t_row();
...
if (v_rep > 0) then
  -- Question Repetition
   ...
   for i in 1..v_rep loop
    ...
    ihook.setColumnValue (row, 'REP_SEQ', j);
    ihook.setColumnValue (row, 'EDIT_IND', 1);
...
if (ihook.getColumnVAlue(row_ori, 'DEFLT_VAL_ID') is not null) then

raise_application_error(-20000,'Please remove the Question Default Value before remove CDE Association.');
return;
end if;

for cur in (select * from nci_quest_vv_Rep where quest_pub_id=  ihook.getColumnValue(row_ori, 'NCI_PUB_ID') and quest_Ver_nr = ihook.getColumnValue(row_ori, 'NCI_VER_NR')
and deflt_val_id is not null) loop
 raise_application_error(-20000,'This is used as a default on repetition. Please remove default value association first.');
end loop;
...
PROCEDURE spAssociateCDE ( v_data_in IN CLOB, v_data_out OUT CLOB, v_usr_id  IN varchar2) as
    hookInput t_hookInput;
    hookOutput t_hookOutput := t_hookOutput();
    actions t_actions := t_actions();
    action t_actionRowset;
...
if (hookinput.invocationnumber = 0) then
    -- if no DE already, give an error message
    if (ihook.getColumnValue(row_ori, 'C_ITEM_ID') is not null) then
        raise_application_error(-20000,'CDE already associate. Please delete and add new question.');
        return;
    end if;
...
--  raise_application_error(-20000, v_ver_nr);
    ...
    if (isUserAuth(v_item_id, v_ver_nr, v_usr_id) = false) then
     ...
     forms                  := t_forms();
        form1                  := t_form('Select CDE (Hook)', 2,1);
        forms.extend;    forms(forms.last) := form1;
        hookoutput.forms := forms;
       	 hookOutput.question := getAddComponentCreateQuestion;

-- form for CDE selection.
end if;
if (hookinput.invocationnumber = 1) then
            ...
            row_sel := form1.rowset.rowset(1);
                         for cur in (select ai.item_id, ai.ver_nr, ai.cntxt_item_id, ai.cntxt_ver_nr, item_long_nm item_long_nm, de.pref_quest_txt QUEST_TEXT, item_nm
                                    from admin_item ai, de where
                                    ai.item_id =ihook.getColumnValue(row_sel,'ITEM_ID') and ai.ver_nr = ihook.getColumnValue(row_sel,'VER_NR') and ai.item_id = de.item_id  and ai.ver_nr = de.ver_nr )  loop

                                            ihook.setColumnValue (row_ori, 'ITEM_LONG_NM', cur.QUEST_TEXT );
                                             ihook.setColumnValue (row_ori, 'ITEM_NM', cur.ITEM_LONG_NM );
                            end loop;    
   ihook.setColumnValue(row_ori,'C_ITEM_ID',ihook.getColumnValue(row_sel,'ITEM_ID'));
   ihook.setColumnValue(row_ori,'C_ITEM_VER_NR',ihook.getColumnValue(row_sel,'VER_NR'));
-- Tracker 2985 Default Value should be removed, "Answer is Editable" is checked, "Answer is Mandatory" is unchecked.
   ihook.setColumnValue(row_ori,'EDIT_IND',1);
   ihook.setColumnValue(row_ori,'REQ_IND',0);
...
rows:=t_rows();
for cur in (select * from  NCI_QUEST_VV_rep where quest_pub_id = ihook.getcolumnValue(row_ori,'NCI_PUB_ID') 
                                            and quest_ver_nr = ihook.getColumnValue(row_ori,'NCI_VER_NR') ) loop
                     row := t_row();
                     ihook.setColumnValue(row,'QUEST_VV_REP_ID', cur.QUEST_VV_REP_ID);
                     ihook.setColumnValue(row,'EDIT_IND', 1);
                      ihook.setColumnValue(row,'DEFLT_VAL_ID','');
ihook.setColumnValue(row,'VAL','');
            ...
            action             := t_actionrowset(rows, 'Question Repetition', 2, 2,'update');
       actions.extend;
        actions(actions.last) := action;

-- delete current valid values       
rows:=t_rows();
  for cur in (select * from  NCI_QUEST_VALID_VALUE where q_pub_id = ihook.getcolumnValue(row_ori,'NCI_PUB_ID') 
                     ...
                     nci_form_curator.DeleteVV(cur.NCI_IDSEQ,v_usr_id);
                     row := t_row();
                     ihook.setColumnValue(row,'NCI_PUB_ID', cur.NCI_PUB_ID);
                     ihook.setColumnValue(row,'NCI_VER_NR', cur.NCI_VER_NR);
 ...
 --updFormModuleQuestion (ihook.getColumnValue(row_ori, 'Q_PUB_ID'), ihook.getColumnValue(row_ori, 'Q_VER_NR'), v_usr_id);
                                            ...
                                            j := 0;
                                            rowsvv := t_rows();
                                   for cur1 in (select * from  VW_NCI_DE_PV where de_item_id = ihook.getcolumnValue(row_sel,'ITEM_ID') 
                                            and de_ver_nr = ihook.getColumnValue(row_sel,'VER_NR') and nvl(fld_delete,0) = 0 order by PERM_VAL_NM) loop
                                                      row := t_row();
                                                    ihook.setColumnValue (row, 'Q_PUB_ID', ihook.getcolumnvalue(row_ori,'NCI_PUB_ID'));
                                                    ihook.setColumnValue (row, 'Q_VER_NR',  ihook.getcolumnvalue(row_ori,'NCI_VER_NR'));
                                                    ihook.setColumnValue (row, 'VM_NM', cur1.item_nm);
        ...
        if (rowsvv.count > 0) then
        action := t_actionrowset(rowsvv, 'Question Valid Values (Hook)', 2,4,'insert');
        actions.extend;
        actions(actions.last) := action;
        end if;
