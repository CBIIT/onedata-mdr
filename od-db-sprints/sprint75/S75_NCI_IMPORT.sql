    ...
    for i in 1..hookinput.originalRowset.rowset.count loop
        ...
        if (ihook.getColumnValue (row_ori, 'CTL_VAL_STUS') <> 'PROCESSED') then
          ...
          if (v_mode in ('N','NV') and ihook.getColumnValue(row_ori, 'NEW_VER_NR') is not null and ihook.getColumnValue(row_ori, 'NEW_VER_NR') <= nvl(ihook.getColumnValue(row_ori, 'DE_CONC_VER_NR'),1)) then
              ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', ihook.getColumnValue(row_ori,'CTL_VAL_MSG') ||  'New version should be greater than current version.' || chr(13));
                v_val_ind := false;
            end if;
...
begin
  if ( ihook.getColumnValue(row_ori, 'NEW_VER_NR') is null) then
           ...
           and v_typ = 3) then
                v_val_ind := false;
                ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', 'ERROR: New Version Number has to be greater than current version.' || chr(13));
           end if;
        ...
        if (v_cnt =0 ) then
            v_val_ind := false;
            ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', ihook.getColumnValue(row_ori, 'CTL_VAL_MSG')  || 'ERROR: Invalid Item/Ver combination or Item/Ver is not the latest version.' || chr(13));
        end if;
    ...
    if  ihook.getColumnValue(row_ori, 'NEW_VER_NR') is  not null then
        ...
        if (v_cnt =1 ) then
            v_val_ind := false;
            ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', ihook.getColumnValue(row_ori, 'CTL_VAL_MSG')  || 'ERROR: Specified new version already exists.' || chr(13));
        end if;
    end if;
end;
