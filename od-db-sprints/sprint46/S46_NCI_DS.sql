create or replace PACKAGE            nci_ds AS
 ...
 procedure DECMatchSub ( v_hdr_id in number,  v_entty_nm in varchar2, v_entty_nm_with_space  in varchar2, v_typ in varchar2);
...
--raise_application_error(-20000,'Test' || ihook.getColumnValue(row_ori,'MATCH_TYP'));
  if (ihook.getColumnValue(row_ori,'MTCH_TYP_NM') = 'VM') then
      ...
      DECMatchSub(v_hdr_id, v_entty_nm,v_entty_nm_with_space,v_typ);
    end if;
...
procedure DECMatchSub ( v_hdr_id in number,  v_entty_nm in varchar2, v_entty_nm_with_space  in varchar2, v_typ in varchar2) AS
v_minlen integer;
v_cnt integer := 0;
v_cur_word  varchar2(4000);
v_sel_word varchar2(4000);
...
begin
 ...
 -- raise_application_error(-20000,v_entty_nm);
   insert into nci_ds_rslt (hdr_id, item_id, ver_nr,  rule_desc, MTCH_TYP)
                    select distinct  v_hdr_id , item_id, ver_nr, '1. DEC Exact Match','DEC' from 
                    ADMIN_ITEM where MTCH_TERM_ADV = v_entty_nm and admin_item_typ_id = 2 and upper(admin_stus_nm_dn) not like '%RETIRED%';
    ...
    commit;
     ...
     insert into nci_ds_rslt (hdr_id, item_id, ver_nr,  rule_desc, MTCH_TYP)
                    select distinct  v_hdr_id , item_id, ver_nr, '2. DEC Like Match' , 'DEC' from 
                    ADMIN_ITEM where admin_item_typ_id = 2 and upper(admin_stus_nm_dn) not like '%RETIRED%'  and ((MTCH_TERM_ADV like '%' || v_entty_nm || '%' )  or
                    (v_entty_nm like '%' || MTCH_TERM_ADV || '%' and length(mtch_term) >= v_minlen)) and (v_hdr_id, item_id, ver_nr) not in (select hdr_id, item_id, ver_nr from nci_ds_rslt where hdr_id = v_hdr_id); 
    v_cnt := v_cnt +  SQL%ROWCOUNT;
 commit;
 ...
 --raise_application_error(-20000, 'Hre' || v_sel_word);
     ...
     insert into nci_ds_rslt (hdr_id, item_id, ver_nr,  rule_desc, MTCH_TYP)
                    select distinct  v_hdr_id , item_id, ver_nr, '3. DEC Like Match Largest word', 'DEC' from 
                    ADMIN_ITEM where admin_item_typ_id = 2 and upper(admin_stus_nm_dn) not like '%RETIRED%' and ((MTCH_TERM_ADV like '%' || v_sel_word || '%' )  or
                    (v_sel_word like '%' || MTCH_TERM_ADV || '%'))  and length(mtch_term_adv) >= length(v_sel_word) and (v_hdr_id, item_id, ver_nr) not in (select hdr_id, item_id, ver_nr from nci_ds_rslt);
     commit;               
...
begin
  ...
  DECMatchSub(v_hdr_id, v_entty_nm,v_entty_nm_with_space,'X');
...
--raise_application_error(-20000,v_sql);
                    ...
                    where instr('|| concat(v_entty_nm,' ')  || ',de.MTCH_TERM , 1) > 0 
                     and currnt_ver_ind = 1 and de.val_dom_typ_id = 18 and de.ADMIN_STUS_NM_DN not like ''%RETIRED%''
                     and (' || v_hdr_id || ') not in (select hdr_id from nci_ds_rslt) and ' || v_flt_str;
                   --  raise_application_error(-20000, v_sql);
                     execute immediate v_sql;
                    ...
                    where instr(' || concat(v_entty_nm,' ')  || ',r.MTCH_TERM , 1) > 0                   and
                    r.ref_typ_id = ok.obj_key_id and upper(obj_key_desc) like ''%QUESTION%'' and r.ref_nm != ''%'' and de.item_id = r.item_id and de.ver_nr = r.ver_nr and de.currnt_ver_ind = 1
                    and (' || v_hdr_id || ') not in (select  hdr_id from nci_ds_rslt) and de.val_dom_typ_id = 18 and de.ADMIN_STUS_NM_DN not like ''%RETIRED%'' and ' || v_flt_str;
                   --  raise_application_error(-20000, v_sql);
                execute immediate v_sql;
                    ...
                    where instr( ' || concat(v_entty_nm,' ')  || ', r.MTCH_TERM, 1) > 0
                    and de.item_id = r.item_id and de.ver_nr = r.ver_nr and de.currnt_ver_ind = 1 and de.val_dom_typ_id = 18 and de.ADMIN_STUS_NM_DN not like ''%RETIRED%''
                    and (' || v_hdr_id || ') not  in (select hdr_id from nci_ds_rslt) and ' || v_flt_str;
--raise_application_error(-20000,v_sql);
                     execute immediate v_sql;
