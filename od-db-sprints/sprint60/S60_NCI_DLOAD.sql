...
BEGIN
 ...
 select creat_usr_id into v_usr_hdr from nci_dload_hdr where  hdr_id = ihook.getColumnValue(row_ori, 'HDR_ID');
    ...
    if (hookinput.invocationnumber = 1) then -- show create form
      ...
      v_coll_typ := ihook.getColumnValue(rowsel, 'DLOAD_FMT_ID');
        ...
        rows.extend;          rows(rows.last) := row;
          ...
          rowsetmdlmap := t_rowset(rows, 'Model Mapping Rules Specific', 1, 'NCI_DLOAD_HDR');
...
if (v_err_str is not null) then
 ...
 return;
    ...
    if (ihook.getColumnValue(rowhdr, 'DLOAD_FMT_ID') = 229) then
        ...
        action := t_actionrowset(rows, 'Model Mapping Rules Specific', 2, 2, 'insert');
        actions.extend;
        actions(actions.last) := action;
    end if;
...
begin
    ...
    form1 := t_form('Model Mapping Rules Specific', 2,1);
