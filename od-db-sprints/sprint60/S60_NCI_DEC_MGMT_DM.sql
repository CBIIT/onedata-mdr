...
--raise_application_error(-20000, hookinput.answerid || v_answer);
  ...
  -- If update, then need to get the Item_id to exclude from duplicate check
    ...
    if (ihook.getColumnValue(rowai, 'ITEM_NM_CURATED') is not null) then
       ...
       -- regenerating short name
       if (ihook.getColumnValue(row_ori,'ITEM_LONG_NM') = ihook.getColumnValue(rowai,'ITEM_LONG_NM') or upper(ihook.getColumnValue(rowai,'ITEM_LONG_NM')) = 'SYSGEN' ) then
...
-- both name and definition match
   for cur in (select ext.* from nci_admin_item_ext ext,admin_item a
    where nvl(a.fld_delete,0) = 0 and a.item_id = ext.item_id and a.ver_nr = ext.ver_nr and cncpt_concat_with_int = v_item_nm and a.admin_item_typ_id = v_item_typ
    and a.admin_stus_nm_dn='RELEASED' and upper(ext.cncpt_concat_def) = upper(v_def) and upper(ext.cncpt_concat_nm) = upper(v_nm)
    order by a.lst_upd_Dt desc) loop
    ...
    where nvl(a.fld_delete,0) = 0 and a.item_id = ext.item_id and a.ver_nr = ext.ver_nr and cncpt_concat_with_int = v_item_nm and a.admin_item_typ_id = v_item_typ
    and a.admin_stus_nm_dn='RELEASED' and upper(ext.cncpt_concat_def) = upper(v_def) and upper(ext.cncpt_concat_nm) <> upper(v_nm)
    order by a.lst_upd_Dt asc) loop
    ...
    and a.admin_stus_nm_dn='RELEASED' and upper(ext.cncpt_concat_def) <> upper(v_def) and upper(ext.cncpt_concat_nm) <> upper(v_nm)
               ...
               ihook.setColumnValue(rowform,'CTL_VAL_MSG',ihook.getColumnValue(rowform,'CTL_VAL_MSG') || 'ERROR VM Concepts matched but neither Name or Definition match.');
