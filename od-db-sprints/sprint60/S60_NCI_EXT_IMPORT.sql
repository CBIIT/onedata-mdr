...
for cur in (select obj_key_id evs_src_id, obj_key_Desc evs_src_desc from obj_key where obj_typ_id = 23 and OBJ_KEY_CMNTS is not null
order by obj_key_id) loop
for cur1 in (select obj_key_id to_evs_src_id, obj_key_Desc to_evs_src_desc from obj_key where obj_typ_id = 23 and OBJ_KEY_CMNTS is not null and obj_key_id <> cur.evs_src_id order by obj_key_id) loop
...
pref_ind = 0
and TERM_TYP = 'PT' and upper(trim(replace(source_atom_name,'NOS',''))) = upper(nci_meta_concept_name);
commit;
...
max(version),max(decode(pref_ind,2,0,pref_ind)), 'MT', term_typ from 
