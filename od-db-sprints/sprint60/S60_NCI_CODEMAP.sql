...
procedure spDeleteModelElement  ( v_data_in IN CLOB, v_data_out OUT CLOB, v_user_id  IN varchar2);
 ...
 procedure spDeleteValueMapRule (v_data_in IN CLOB, v_data_out OUT CLOB, v_user_id in varchar2);
...
row t_row;
 ...
 raise_application_error (-20000,'Work in progress');
   ...
   procedure spCopyModelMappingOld ( v_data_in in clob, v_data_out out clob, v_user_id in varchar2)
   as
   hookInput t_hookInput;
  hookOutput t_hookOutput := t_hookOutput();
    showRowset     t_showableRowset;
...
answers     t_answers;
 ...
 BEGIN
  ...
  rows := t_rows();
   v_item_id := ihook.getColumnValue(row_ori,'ITEM_ID');
   v_ver_nr := ihook.getColumnValue(row_ori,'VER_NR');
   for cur in (select ai.item_nm, ai.ver_nr, ai.item_id, decode(mm.src_mdl_ver_nr, ai.ver_nr, 'Current','') Curr_ind from nci_mdl_map mm, admin_item ai where mm.item_id = v_item_id and mm.ver_nr = v_ver_nr
   and mm.src_mdl_item_id = ai.item_id ) loop
   row := t_row();
  ...
  ihook.setColumnValue(row,'Version',cur.ver_nr);
   ...
   row := t_row();
     ihook.setColumnValue(row,'Version Selection','To');
     ihook.setColumnValue(row,'Model Name',cur.item_nm);
 ihook.setColumnValue(row,'Version',cur.ver_nr);
    ihook.setColumnValue(row,'Current Mapping',cur.curr_ind);
    ihook.setColumnValue(row,'Model Id',cur.item_id);
 rows.extend;
		   rows (rows.last) := row;
       	 ...
       	 showrowset := t_showablerowset (rows, 'Select Source/Target Version', 4, 'multi');
       	 hookoutput.showrowset := showrowset;

       	 answers := t_answers();
  	   	 answer := t_answer(1, 1, 'Copy Mapping');
  	   	 answers.extend; answers(answers.last) := answer;

	   	 question := t_question('Select Source and Target Model Version', answers);
       	 hookOutput.question := question;
...
BEGIN
  ...
  if (hookinput.invocationnumber = 0) then 
        hookoutput.question := nci_form_curator.getProceedQuestion('Confirm', 'Please confirm deletion of Value Mapping Rule(s).');
    end if;
if (hookinput.invocationnumber = 1) then
  
    for i in 1..hookInput.originalRowset.rowset.count loop
        row_ori :=  hookInput.originalRowset.rowset(i);
        v_item_id := ihook.getColumnValue(row_ori,'MECVM_ID');
        --v_ver_nr := ihook.getColumnValue(row_ori,'VER_NR');
        --v_mdl_id := ihook.getColumnValue(row_ori, 'MDL_ITEM_ID');
        --v_mdl_ver_nr := ihook.getColumnValue(row_ori, 'MDL_ITEM_VER_NR');
 

        delete from nci_mec_val_map where mecvm_id = v_item_id; 
        delete from onedata_ra.nci_mec_val_map where mecvm_id = v_item_id;


        commit;
    end loop;

hookoutput.message :=  'Value Mapping Rule(s) deleted successfully.';
end if;
