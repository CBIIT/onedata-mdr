...
procedure resetAuditAttr(row in out t_row, v_user_id in varchar2);
...
function isShortNmSysGen (v_in_str varchar2) return boolean;
...
begin
       ihook.setColumnValue(row,'CREAT_DT',to_char(sysdate, DEFAULT_TS_FORMAT) );
               ihook.setColumnValue(row,'LST_UPD_DT',to_char(sysdate, DEFAULT_TS_FORMAT) );
...
function isShortNmSysGen (v_in_str in varchar2) return boolean is
sysgen_ind boolean;
begin
