...
Y_ITEM_TYPE VARCHAR2) RETURN VARCHAR2
IS
vSQL VARCHAR2 (4000);
vCOND VARCHAR2 (8);
...
ITEM_DESC,ITEM_ID,ITEM_NM,VER_NR,CREAT_USR_ID,CREAT_DT,LST_UPD_DT,LST_UPD_USR_ID,ADMIN_STUS_NM_DN
 ...
 where ADMIN_ITEM_TYP_ID = '
            ...
            || ' and item_id not in (-20000,-20001,-20002,-20005,-20004,-20003)
                 and NCI_IDSEQ in (select CSI_IDSEQ from sbr_m.cs_CSI)
 UNION ALL
 select '
            || Y_IDSEQ_NAME
            || ' , '
...
FROM '
            || Y_TAB_NAME
            || ' AC ' ||
Y_CONDITION || vCOND || ' CSI_IDSEQ in (select CSI_IDSEQ from sbr_m.cs_CSI))
