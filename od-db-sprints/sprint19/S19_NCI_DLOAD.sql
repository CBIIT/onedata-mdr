...
function getALSCreateFormGuest (v_rowset1 in t_rowset, v_rowset2 in t_rowset) return t_forms;
...
begin
    ...
    if (hookinput.invocationnumber = 0) then -- show create form
        ...
        ihook.setColumnValue(row, 'DLOAD_FMT_ID',90 );
        rows.extend;          rows(rows.last) := row;
          rowsethdr := t_rowset(rows, 'Guest User Download Collection Header', 1, 'NCI_DLOAD_HDR');
        rows := t_rows();
        row := t_row();
            ihook.setColumnValue(row, 'HDR_ID', -1);
        rows.extend;          rows(rows.last) := row;
          ...
          hookOutput.forms := getALSCreateFormGuest(rowsethdr, rowsetals);
          HOOKOUTPUT.QUESTION    := getALSCreateQuestion;
    else --- Second invocation
         forms              := hookInput.forms;
...
function getALSCreateFormGuest (v_rowset1 in t_rowset, v_rowset2 in t_rowset) return t_forms is
  forms t_forms;
  form1 t_form;
begin
    forms                  := t_forms();
    form1                  := t_form('Guest User Download Collection Header', 2,1);
    form1.rowset :=v_rowset1;
    forms.extend;    forms(forms.last) := form1;
    form1                  := t_form('ALS Specific', 2,1);
