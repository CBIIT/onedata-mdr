...
-- There is no way for a hook step to know what the previous selection was. I am using AnswerId to determine if 50+ then use Name. Else, use code.
 ANSWERS                    := T_ANSWERS();
    ...
    ANSWERS.EXTEND;
    ANSWERS(ANSWERS.LAST) := ANSWER;
  QUESTION               := T_QUESTION('Select Concepts to create PV/VM. You HAVE to select atleast one Child. To create PV/VM with Parent only, used the standard command.', ANSWERS);
...
begin
    ...
    if (hookinput.invocationnumber = 1) then
       ...
       /* for cur in (select c_item_id, c_item_ver_nr from nci_cncpt_rel where p_item_id = ihook.getColumnValue(rowform,'ITEM_ID') and p_item_ver_nr = ihook.getColumnValue(rowform,'VER_NR')) loop
        row := t_row();
        ihook.setColumnValue(row,'ITEM_ID',cur.c_item_id);
        ihook.setColumnValue(row,'VER_NR',cur.c_item_ver_nr);
        rows.extend;
                    rows (rows.last) := row;
        end loop;
         	 showrowset := t_showablerowset (rows, 'Concepts', 2, 'multi'); */
...
if ((hookinput.answerid = 2 or hookinput.answerid = 52) and hookinput.selectedRowset is not null) then
j := 1;
end if;
     ...
     ihook.setColumnValue(row,'CNCPT_1_ITEM_ID_1',ihook.getColumnValue(rowform,'C_ITEM_ID'));
      ihook.setColumnValue(row,'CNCPT_1_VER_NR_1',ihook.getColumnValue(rowform,'C_ITEM_VER_NR'));
     ...
     for curpv in (select * from admin_item where item_id =  ihook.getColumnValue(rowform,'C_ITEM_ID') and ver_nr = ihook.getColumnValue(rowform,'C_ITEM_VER_NR')) loop
     if (hookinput.answerid > 50) then -- use name 
         v_pv := curpv.item_nm;
     else
   ...
   --  raise_application_error(-20000,v_pv);
      ...
      if (v_pv is not null) then
        ...
        ihook.setColumnValue(rowpv,'NCI_VAL_MEAN_VER_NR', 1);
         ihook.setColumnvalue(rowpv,'PRNT_CNCPT_ITEM_ID',ihook.getColumnValue(rowform, 'P_ITEM_ID'));
     ihook.setColumnvalue(rowpv,'PRNT_CNCPT_VER_NR',ihook.getColumnValue(rowform, 'P_ITEM_VER_NR'));
  
        ihook.setColumnValue(rowpv,'VAL_ID',-1);
            rows.extend;
            rows(rows.last) := rowpv;
...
--  If VM/CONC_DOM new
    ...
    -- VM Alternate Name
        ...
        and ihook.getColumnValue(row_ori, 'VM_ALT_NM_CNTXT_ITEM_ID') is not null and ihook.getColumnValue(row_ori, 'VM_ALT_NM_CNTXT_VER_NR') is not null ) then
