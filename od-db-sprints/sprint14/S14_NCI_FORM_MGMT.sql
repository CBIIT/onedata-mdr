...
nci_pub_id = ihook.getColumnValue(row_ori, 'Q_PUB_ID') and nci_ver_nr = ihook.getColumnValue(row_ori, 'Q_VER_NR');
 ...
 ELSE
        ...
        if (hookinput.answerId = 1) then
         ...
         select item_desc into v_ref from admin_item where item_id = ihook.getColumnValue(row_ori, 'NCI_VAL_MEAN_ITEM_ID') and ver_nr = ihook.getColumnValue(row_ori, 'NCI_VAL_MEAN_VER_NR');
