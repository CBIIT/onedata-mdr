...
BEGIN
 ...
 select item_long_nm, cntxt_item_id, cntxt_ver_nr, admin_item_typ_id  into 
v_item_nm, v_cntxt_id, v_cntxt_ver_nr, v_admin_item_typ 
 from admin_item where item_id = ihook.getColumnValue(row_ori, 'ITEM_ID') and 
ver_nr = ihook.getColumnValue(row_ori,'VER_NR');

          FOR cur
            IN (SELECT ai.item_id
                  FROM admin_item ai
                 WHERE   ai.ITEM_LONG_NM = v_item_nm
                        AND ai.item_id <> v_item_id
                        and ai.admin_item_typ_id = v_admin_item_typ
                       AND ai.cntxt_item_id = v_cntxt_id and ai.cntxt_ver_nr = 
v_cntxt_ver_nr)
        LOOP
            raise_application_error (-20000,
                                     'Duplicate Administered Item found. ' || 
cur.item_id);
            RETURN;
        END LOOP; 
end if;
