...
begin
   ...
   -- raise_application_error(-20000,v_user_id);
    ...
    commit;
     ...
     insert into nci_ds_rslt (hdr_id, item_id, ver_nr,  rule_desc)
                    ...
                    admin_item where regexp_replace(upper(item_nm),v_reg_str,'') = v_entty_nm and admin_item_typ_id = 53;
                    commit;
   ...
   /* insert into nci_ds_rslt (hdr_id, item_id, ver_nr,  rule_desc)
     ...
     insert into nci_ds_rslt (hdr_id, item_id, ver_nr,  rule_desc)
                    ...
                    vw_cncpt c, alt_nms r where regexp_replace(upper(nm_desc),v_reg_str,'') = v_entty_nm
                    and c.item_id = r.item_id and c.ver_nr = r.ver_nr and 
                    v_hdr_id not in (select hdr_id from nci_ds_rslt);
                    commit; 
     insert into nci_ds_rslt (hdr_id, item_id, ver_nr,  rule_desc)
                    select distinct  v_hdr_id , item_id, ver_nr, 'VM Like Match' from 
                    admin_item where regexp_replace(upper(item_nm),v_reg_Str,'') like '%' || v_entty_nm || '%'  and admin_item_typ_id = 53
                    and v_hdr_id not in (select hdr_id from nci_ds_rslt);
                    commit;
...
begin
        delete from nci_ds_rslt where hdr_id = v_hdr_id;
        commit;
           select '''' || replace(upper(v_mtch_nm),'''','') || '''' into v_entty_nm from dual;
              select '''' || replace(upper(v_mtch_nm),'''','') || '''' into v_entty_nm_like from dual;
                  ...
                  vw_de de where ' ||v_entty_nm || ' = upper(de.item_nm)    and currnt_ver_ind = 1             and de.ADMIN_STUS_NM_DN not like ''%RETIRED%''        and de.val_dom_typ_id = 18 and ' || v_flt_str ;
                    ...
                    and r.ref_typ_id = ok.obj_key_id and upper(obj_key_desc) like ''%QUESTION%'' and de.item_id = r.item_id and de.ver_nr = r.ver_nr and de.currnt_ver_ind = 1and de.ADMIN_STUS_NM_DN not like ''%RETIRED%''
...
begin
        ...
        v_hdr_id || ', ''' || v_mtch_typ || ''', de.item_id, de.ver_nr, ''NA'', 1, 100, ''Preferred Name Exact Match'' from   vw_de de where ' || v_entty_nm || '  = upper(de.item_nm)
    ...
    --     raise_application_error(-20000, v_sql);
        ...
        select distinct ' || v_hdr_id || ', ''' || v_mtch_typ || ''', r.item_id, r.ver_nr, ''NA'', 2, 100, ''Question Text Exact Match'' from ref r, obj_key ok, vw_de de  where ' || v_entty_nm || '  = upper(r.ref_desc)         and
       ...
       ' || v_entty_nm || '  = upper(r.nm_desc)
        ...
        commit;
              ...
              v_sql := ' insert into nci_ds_rslt_dtl (hdr_id, mtch_typ, item_id, ver_nr, perm_val_nm,  rule_id, score, rule_desc)
                    ...
                    where (instr(upper(r.ref_desc),' || v_entty_nm || ', 1) > 0  )  and de.ADMIN_STUS_NM_DN not like ''%RETIRED%'' and
                    r.ref_typ_id = ok.obj_key_id and upper(obj_key_desc) like ''%QUESTION%'' and r.ref_nm != ''%'' and de.item_id = r.item_id and de.ver_nr = r.ver_nr and de.currnt_ver_ind = 1
                    and (' || v_hdr_id || ', r.item_id, r.ver_nr) not in (select  hdr_id, item_id, ver_nr from nci_ds_rslt_dtl) and de.val_dom_typ_id = 17 and ' || v_flt_str;
                --      raise_application_error(-20000, v_sql);
                execute immediate v_sql;
