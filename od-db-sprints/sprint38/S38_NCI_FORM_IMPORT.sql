...
begin
  ...
  for curq in (select * from NCI_STG_FORM_QUEST_IMPORT where quest_imp_id = ihook.getColumnValue(row_ori, 'QUEST_IMP_ID')) loop
        ...
        nci_ds.spDSMatchNonEnum(curq.quest_imp_id, v_usr_id, '1=1', 'QUESTION_MATCH',curq.SRC_QUEST_LBL);
        else
        nci_ds.spDSMatchEnum(curq.quest_imp_id, v_usr_id, '1=1', 'QUESTION_MATCH',curq.SRC_QUEST_LBL);
        end if;
