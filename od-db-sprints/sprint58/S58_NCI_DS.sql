...
if (v_found = false) then
 ...
 for cur in (Select * from admin_item where admin_item_typ_id in (49) and item_id = ihook.getColumnValue(row_ori, 'ITEM_ID') and ver_nr = ihook.getColumnValue(row_ori, 'VER_NR')) loop
            ...
            pref_cncpt_concat_nm = trim(nvl( decode(VM_MTCH_ITEM_TYP, 'ID', '',pref_cncpt_concat_nm) ,'')|| ' '  || cur.item_nm),
                  pref_cncpt_concat_def = trim(nvl( decode(VM_MTCH_ITEM_TYP, 'ID', '',pref_cncpt_concat_def) ,'')|| '_'  || cur.item_desc),
...
if (v_pref='V' and v_typ='R') then
    ...
    if (v_typ = 'R') then
       if  (v_pref in ('V', 'C')) then
                ...
                || v_tbl || ' t where ''' || v_entty_nm || ''' = t.' || v_mtchTerm ||' and ' || v_flt_str;
                --raise_application_error (-20000,v_sql);
        else
            v_sql := 'insert into nci_ds_rslt (hdr_id, item_id, ver_nr, rule_desc) select distinct ' ||  v_hdr_id || ', t.item_id, t.ver_nr, ''' || v_ruleDesc || ''' from ' 
               || v_tbl || ' t where ((t.' || v_mtchTerm || ' like (''' || v_entty_nm || ''' || '' |%'') or t.' || v_mtchTerm || ' like (''%| '' || ''' || v_entty_nm || ''') or t.' || v_mtchTerm || ' like (''%| '' || ''' || v_entty_nm || ''' || '' |%'')) and (' || v_hdr_id ||', item_id, ver_nr) not in (select hdr_id, item_id, ver_nr from nci_ds_rslt))';
        -- raise_application_error (-20000,v_sql);
       end if;
        execute immediate v_sql;
        v_cnt := SQL%ROWCOUNT;
