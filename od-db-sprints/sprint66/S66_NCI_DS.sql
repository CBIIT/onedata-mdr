...
--        end if;
        update nci_ds_hdr set CDE_ITEM_ID = tmp_item_id, CDE_VER_NR = tmp_ver_nr where hdr_id = v_hdr_id;
        commit;
    end if;
  end if;
...
begin
    ...
    for i in 1..hookinput.originalrowset.rowset.count loop  --- Iterate through all the selected rows
     ...
     select count(*) into v_temp from nci_ds_cntxt_fltr where hdr_id = v_hdr_id;
     if v_temp > 0 then
        for cntxt in (select CNTXT_NM_DN from nci_ds_cntxt_fltr where hdr_id = v_hdr_id) loop
...
if (v_mode = 'VM') then
    v_src_dtl := 'VAL_MEAN_NM';
else
    v_src_dtl := 'PERM_VAL_NM';
end if;
    ...
    if (ihook.getColumnValue(row_ori, 'USR_CMNTS') is not null) then
        ...
        if v_cnt > 1 then
            for i in 1..v_cnt loop
                ...
                INSERT INTO nci_ds_cntxt_fltr (hdr_id, cntxt_nm_dn)
                VALUES (v_hdr_id, v_nxt);
                INSERT INTO onedata_ra.nci_ds_cntxt_fltr (hdr_id, cntxt_nm_dn)
                VALUES (v_hdr_id, v_nxt);
                commit;
...
BEGIN
    ...
    if (v_temp = 1 and v_op <> 'P') then -- preferred cde set, not called from post-hook
        ...
        --if (v_temp = 1) then
            ...
            select cde_item_id, cde_ver_nr, d.num_pv_in_src, d.num_pv_mtch, d.num_pv_in_cde into v_item_id, v_ver_nr, v_pv_src, v_pv_mtch, v_pv_cde
            from nci_ds_hdr d
