...
where   map.MAP_DEG in (86,87,120)
 ...
 ihook.setColumnValue(row, 'Characteristic Group', cur.mec_map_nm);
   ...
   rows.extend;     rows(rows.last) := row;
     ...
     -- spModelMapFuncValidation(v_item_id, v_ver_nr, v_open_paren, v_close_paren, rows);
if hookinput.invocationnumber = 0 then
showRowset := t_showableRowset(rows, 'Issues with Model Map Validation for: ' || ihook.getColumnValue(row_ori, 'ITEM_NM') || ';' || ihook.getColumnValue(row_ori,'ITEM_ID') || 'v' || ihook.getColumnValue(row_ori,'VER_NR'),4, 'unselectable');
