...
as
    ...
    v_perm_val_nm_list varchar2(1000);
...
--raise_application_error(-20000,v_mode || v_perm_val_nm);
            ...
            if (v_mode = 'CDE') then -- if called from CDE Match
                for cur1 in (select perm_val_nm from NCI_DS_DTL where hdr_id = v_hdr_id and excl_pv_ind = 0) loop
                    if upper(cur1.perm_Val_nm) = upper(v_tab_val_mean_nm(i)) or upper(cur1.perm_Val_nm) = upper(substr(v_perm_val_nm,2)) then
                        ihook.setColumnValue(row, 'Source PV', cur1.perm_val_nm);
                        v_tab_perm_val_nm.extend();
                        v_tab_perm_val_nm(v_tab_perm_val_nm.count) := upper(cur1.perm_val_nm);
                    end if;
                end loop; 
            end if;
...
if v_tab_perm_val_nm.count > 0 then
    for cur2 in (select perm_val_nm from NCI_DS_DTL where hdr_id = v_hdr_id and excl_pv_ind = 0) loop
        v_pv_mtchd := false;
        for p in 1..v_tab_perm_val_nm.count loop
            if upper(cur2.perm_val_nm) = v_tab_perm_val_nm(p) then
                v_pv_mtchd := true;
            end if;
        end loop;
        if not v_pv_mtchd then
            v_perm_val_nm_list := v_perm_val_nm_list || ', ' || cur2.perm_val_nm;
        end if;
    end loop;
...
if (rows.count > 0) then
    ...
    if (v_mode = 'CDE') then
        ...
        if (v_tmp = 0) then
            hookoutput.message := '<p style="text-align:left"><strong>Excluded PVs:</strong> ' || '<i>None</i>' || '<BR>' || '<strong>Unmatched PVs: </strong>' || nvl(LTRIM(v_perm_val_nm_list, ', '), '<i>None</i>') || '</p>';
        elsif (v_tmp > 0) then
            hookoutput.message := '<p style="text-align:left"><strong>Excluded PVs:</strong> ' || nci_ds.getPVExclusionList(v_hdr_id) || '<BR>' || '<strong>Unmatched PVs: </strong>' || nvl(LTRIM(v_perm_val_nm_list, ', '), '<i>None</i>') || '</p>';
