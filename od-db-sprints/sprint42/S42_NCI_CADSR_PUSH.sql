...
delete from sbr.administered_components where ac_idseq in ( select p_val_idseq  from sbrext.quest_contents_ext where qtl_name= 'VALUE_INSTR' and p_val_idseq not in (select qc_idseq from
 sbrext.quest_contents_ext where qtl_name= 'VALID_VALUE'));

--de
delete from sbrext.quest_contents_ext where qc_idseq in (select qc_idseq from  (select p_val_idseq qc_idseq  from sbrext.quest_contents_ext where qtl_name= 'VALUE_INSTR' and p_val_idseq not in (select qc_idseq from
 sbrext.quest_contents_ext where qtl_name= 'VALID_VALUE')));
...
update sbr.DESIGNATION_TYPES_LOV set (description,comments, date_modified, modified_by) =
    (select substr(obj_key_def,1,60), obj_key_cmnts, LST_UPD_DT,LST_UPD_USR_ID
    from obj_key
    where obj_typ_id = 11
    and   lst_upd_dt > sysdate - vHours/24
...
select nci_cd, substr(obj_key_def,1,60), obj_key_cmnts, CREAT_USR_ID, CREAT_DT, LST_UPD_DT,LST_UPD_USR_ID
from obj_key
where obj_typ_id = 11
and creat_dt > sysdate - vHours/24
...
update sbrext.definition_types_lov_ext set ( description,comments, date_modified, modified_by) =
    (select substr(obj_key_def,1,60), obj_key_cmnts, LST_UPD_DT,LST_UPD_USR_ID
    from obj_key
    where obj_typ_id = 15
    and   lst_upd_dt > sysdate - vHours/24
...
-- getshortdef added 8/8 due to production issue
--  Definitions
...
select ad.nci_idseq, ai.nci_idseq,  getShortDef(def_desc), c.nci_idseq, ad.CREAT_DT,ad.CREAT_USR_ID,  ad.LST_UPD_DT,ad.LST_UPD_USR_ID,decode(lang_id, 1000,'ENGLISH', 1007, 'Icelandic', 1004, 'Spanish'), ok.nci_cd
...
update sbr.definitions d set (DEFINITION, CONTE_IDSEQ, DATE_CREATED, CREATED_BY, DATE_MODIFIED, MODIFIED_BY, LAE_NAME, DEFL_NAME) =
    (select  getShortDef(def_desc), c.nci_idseq, ad.CREAT_DT,ad.CREAT_USR_ID,  ad.LST_UPD_DT,ad.LST_UPD_USR_ID,decode(lang_id, 1000,'ENGLISH', 1007, 'Icelandic', 1004, 'Spanish'), ok.nci_cd
    from alt_def ad, admin_item ai, admin_item c, obj_key ok
    where ad.item_id = ai.item_id
    and   ad.ver_nr = ai.ver_nr
    and   ad.cntxt_item_id = c.item_id
...
where  ad.item_id = ai.item_id
    and   ad.ver_nr = ai.ver_nr
    and   ad.nci_cntxt_item_id = c.item_id
...
select ad.nci_idseq, rd.ref_doc_id || ' - ' || file_nm, nci_doc_size, nci_charset, nci_doc_lst_upd_dt, blob_col, rd.CREAT_DT, rd.CREAT_USR_ID, rd.LST_UPD_DT,rd.LST_UPD_USR_ID
from ref ad, ref_doc rd
where ad.ref_id = rd.nci_ref_id
and   rd.ref_doc_id || ' - ' || rd.file_nm not in (select name from sbr.reference_blobs)
and nvl(rd.fld_delete,0) = 0
and rd.lst_upd_dt >= sysdate - 1/24;
commit;

for cur in (select r.nci_idseq, r.ref_id, rd.ref_doc_id, file_nm from ref_doc rd, ref r where rd.lst_upd_dt > rd.creat_dt and rd.lst_upd_dt >= sysdate - vHours/24 and r.ref_id = rd.nci_ref_id and 
r.nci_idseq in (select rd_idseq from sbr.reference_documents) and nvl(rd.fld_Delete,0) = 0) loop
update sbr.reference_blobs d set( DOC_SIZE, DAD_CHARSET, LAST_UPDATED, BLOB_CONTENT,DATE_MODIFIED, MODIFIED_BY, MIME_TYPe) =
    (select  nci_doc_size, nci_charset, nci_doc_lst_upd_dt, blob_col, rd.LST_UPD_DT,rd.LST_UPD_USR_ID, nci_mime_type
    from  ref_doc rd where
   rd.ref_doc_id = cur.ref_doc_id )
    where d.name = cur.ref_doc_id || ' - ' || cur.file_nm ;
end loop;
...
--Delete lingering V values - Tracker - 2349
...
and P_QST_IDSEQ in (select nci_idseq from onedata_wa.nci_admin_item_rel_alt_key where lst_upd_dt >= to_Date('11/01/2022','mm/dd/yyyy')));
commit;
...
--for curpv in (select pv_idseq into v_pv_idseq from admin_item ai, sbr.permissible_values pv where ai.item_id = cur.nci_val_mean_item_id and ai.ver_nr = cur.nci_val_mean_ver_nr
