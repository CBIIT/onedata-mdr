...
delete from nci_usr_cart where creat_dt < sysdate - 7;
commit;

delete from onedata_ra.nci_usr_cart where creat_dt < sysdate - 7;
commit;

-- rolling 30 day delete of download collection

delete from nci_dload_dtl where hdr_id in (select hdr_id from nci_dload_hdr where creat_dt < sysdate - 30);
commit;
...
hookInput        t_hookInput;
  ...
  BEGIN
    ...
    row_ori := hookInput.originalRowset.rowset (1);
   if (ihook.getColumnValue(row_ori, 'LEAD_ORG_ID') is not null and ihook.getcolumnValue(row_ori,'LEAD_ORG_ID') <> ihook.getColumnOldValue(row_ori, 'LEAD_ORG_ID')) then
