...
(select count(*) from sbr.data_elements where nvl(date_modified, date_created) >= sysdate - vHours/24) caDSR_CNT,
(select count(*) from sbr.administered_components where nvl(date_modified, date_created) >= sysdate - vHours/24 and actl_name = 'DATAELEMENT') RA_CNT
from dual;
commit;
...
(select count(*) from sbr.data_element_concepts where nvl(date_modified, date_created) >= sysdate - vHours/24) caDSR_CNT,
(select count(*) from sbr.administered_components where nvl(date_modified, date_created) >= sysdate - vHours/24 and actl_name = 'DE_CONCEPT') RA_CNT
from dual;
commit;
...
(select count(*) from sbrext.quest_vv_ext where nvl(date_modified, date_created) >= sysdate - vHours/24 ) caDSR_CNT
from dual;
commit;
...
(select count(*) from sbr.conceptual_domains where nvl(date_modified, date_created) >= sysdate - vHours/24) caDSR_CNT,
(select count(*) from sbr.administered_components where nvl(date_modified, date_created) >= sysdate - vHours/24 and actl_name = 'DE_CONCEPT') RA_CNT
from dual;
commit;
...
(select count(*) from sbrext.object_classes_ext where nvl(date_modified, date_created) >= sysdate - vHours/24) caDSR_CNT,
(select count(*) from sbr.administered_components where nvl(date_modified, date_created) >= sysdate - vHours/24 and actl_name = 'OBJECTCLASS') RA_CNT
from dual;
commit;
...
(select count(*) from sbrext.properties_ext where nvl(date_modified, date_created) >= sysdate - vHours/24) caDSR_CNT,
(select count(*) from sbr.administered_components where nvl(date_modified, date_created) >= sysdate - vHours/24 and actl_name = 'PROPERTY') RA_CNT
from dual;
commit;
...
(select count(*) from sbr.value_domains where nvl(date_modified, date_created) >= sysdate - vHours/24) caDSR_CNT,
(select count(*) from sbr.administered_components where nvl(date_modified, date_created) >= sysdate - vHours/24 and actl_name = 'VALUEDOMAIN') RA_CNT
from dual;
commit;
...
(select count(*) from sbrext.concepts_ext where nvl(date_modified, date_created) >= sysdate - vHours/24) caDSR_CNT,
(select count(*) from sbr.administered_components where nvl(date_modified, date_created) >= sysdate - vHours/24 and actl_name = 'DE_CONCEPT') RA_CNT
from dual;
commit;
...
(select count(*) from sbr.cs_items where nvl(date_modified, date_created) >= sysdate - vHours/24) caDSR_CNT,
(select count(*) from sbr.administered_components where nvl(date_modified, date_created) >= sysdate - vHours/24 and actl_name = 'CS_ITEM') RA_CNT
from dual;
commit;
...
(select count(*) from sbr.classification_schemes where nvl(date_modified, date_created) >= sysdate - vHours/24) caDSR_CNT,
(select count(*) from sbr.administered_components where nvl(date_modified, date_created) >= sysdate - vHours/24 and actl_name = 'CLASSIFICATION') RA_CNT
from dual;
commit;
...
(select count(*) from sbrext.representations_ext where nvl(date_modified, date_created) >= sysdate - vHours/24) caDSR_CNT,
(select count(*) from sbr.administered_components where nvl(date_modified, date_created) >= sysdate - vHours/24 and actl_name = 'REPRESENTATION') RA_CNT
from dual;
commit;
...
(select count(*) from sbr.value_meanings where nvl(date_modified, date_created) >= sysdate - vHours/24) caDSR_CNT,
(select count(*) from sbr.administered_components where nvl(date_modified, date_created) >= sysdate - vHours/24 and actl_name = 'VALUEMEANING') RA_CNT
from dual;
commit;
...
(select count(*) from sbrext.protocols_ext where nvl(date_modified, date_created) >= sysdate - vHours/24) caDSR_CNT,
(select count(*) from sbr.administered_components where nvl(date_modified, date_created) >= sysdate - vHours/24 and actl_name = 'PROTOCOL') RA_CNT
from dual;
commit;
...
(select count(*) from sbr.contexts where nvl(date_modified, date_created) >= sysdate - vHours/24) caDSR_CNT from dual;
commit;
...
(select count(*) from sbr.cd_vms where nvl(date_modified, date_created) >= sysdate - vHours/24) caDSR_CNT
from dual;
commit;
...
(select count(*) from sbr.vd_pvs where nvl(date_modified, date_created) >= sysdate - vHours/24) caDSR_CNT
from dual;
commit;
...
(select count(*) from sbrext.COMPONENT_CONCEPTS_EXT where nvl(date_modified, date_created) >= sysdate - vHours/24) caDSR_CNT
from dual;
commit;
...
(select count(*) from sbrext.quest_contents_ext where qtl_name in ( 'CRF','TEMPLATE') and nvl(date_modified, date_created) >= sysdate - vHours/24) caDSR_CNT,
(select count(*) from sbr.administered_components where nvl(date_modified, date_created) >= sysdate - vHours/24 and actl_name in ('CRF','TEMPLATE')) RA_CNT
from dual;
commit;

insert into TEST_RESULTS (TABLE_NAME, WA_CNT, caDSR_CNT)
select 'Administered Item - Module', 
(select count(*) cnt1 from onedata_wa.ADMIN_ITEM ai, onedata_wa.nci_admin_item_rel r 
where admin_item_typ_id = 52 and  nvl(ai.fld_delete,0) = 0 and ai.item_id = r.c_item_id and nvl(r.fld_Delete,0) = 0 and ai.ver_nr = r.c_item_ver_nr and r.rel_typ_id = 61 and greatest(ai.lst_upd_dt,r.lst_upd_dt) >= sysdate - vHours/24) WA_CNT,
(select count(*) from sbrext.quest_contents_ext where qtl_name = 'MODULE' and nvl(date_modified, date_created) >= sysdate - vHours/24) caDSR_CNT
from dual;
commit;
...
(select count(*) from sbrext.quest_contents_ext where qtl_name = 'QUESTION' and nvl(date_modified, date_created) >= sysdate - vHours/24) caDSR_CNT
from dual;
commit;
...
(select count(*) from sbrext.quest_contents_ext where qtl_name in ('FORM_INSTR') and nvl(date_modified, date_created) >= sysdate - vHours/24)
 caDSR_CNT
from dual;
commit;
...
(select count(*) from sbrext.quest_contents_ext where qtl_name in ('FOOTER') and nvl(date_modified, date_created) >= sysdate - vHours/24)
 caDSR_CNT
from dual;
commit;
...
(select count(*) from onedata_wa.NCI_ADMIN_ITEM_REL  where REL_TYP_ID=61 and   nvl(fld_delete,0) = 0 and lst_upd_dt >= sysdate - vHours/24 and instr is not null) WA_CNT,
(select count(*) from sbrext.quest_contents_ext where qtl_name in ('MODULE_INSTR') and nvl(date_modified, date_created) >= sysdate - vHours/24)
 caDSR_CNT
from dual;
commit;
...
(select count(*) from onedata_wa.NCI_ADMIN_ITEM_REL_ALT_KEY
  where REL_TYP_ID=63   and lst_upd_dt >= sysdate - vHours/24 and instr is not null and nvl(fld_delete,0) = 0) WA_CNT,
(select count(*) from sbrext.quest_contents_ext where qtl_name in ('QUESTION_INSTR') and nvl(date_modified, date_created) >= sysdate - vHours/24)
 caDSR_CNT
from dual;
commit;
