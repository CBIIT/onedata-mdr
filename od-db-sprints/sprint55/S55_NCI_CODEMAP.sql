...
decode(nvl(upper(cur.SRC_MAND_IND),'NO'),'YES',1,0),cur.FK_ELMNT_PHY_NM, cur.FK_ELMNT_CHAR_PHY_NM);
...
REQ_IND = decode(nvl(upper(cur.SRC_MAND_IND),'NO'),'YES',1,0),
...
getModelElementCharActionDirect ( row_ori, rowscharins , rowscharupd, v_mdl_elmnt_id, cur.item_long_nm);
...
begin
    ...
    and upper(SOURCE_DOMAIN_TYPE)='ENUMERATED' and upper(TARGET_DOMAIN_TYPE)='ENUMERATED' and upper(MAPPING_DEGREE)='SEMANTIC SIMILAR') loop
...
commit;
for cur in (Select * from nci_mec_map where tgt_mec_id is null and src_mec_id is not null and src_func_id is not null order by MEC_MAP_NM,MEC_SUB_GRP_NBR) loop
