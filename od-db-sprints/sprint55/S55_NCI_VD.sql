...
select item_nm into v_nm from vw_cncpt where item_id = ihook.getcolumnValue(rowvd,'CNCPT_2_ITEM_ID_1') and ver_nr = ihook.getcolumnValue(rowvd,'CNCPT_2_VER_NR_1');
v_nm := substr('Enum by Ref/' || v_nm || ' ' || v_dt,1,255);
...
select item_nm into v_nm from vw_cncpt where item_id = ihook.getcolumnValue(rowvd,'TERM_CNCPT_ITEM_ID') and ver_nr = ihook.getcolumnValue(rowvd,'TERM_CNCPT_VER_NR');
v_nm := substr('Enum by Ref/' || v_nm || ' ' || v_dt ||  ' (' || numpv || ' Values)',1,255);
...
-- Edit
   ...
   if (ihook.getColumnValue(rowvd,'CNCPT_2_ITEM_ID_1') is not null) then 
        select upper(item_nm) into v_temp_nm from admin_item where item_id = ihook.getColumnValue(rowvd,'CNCPT_2_ITEM_ID_1') and ver_nr =  ihook.getColumnValue(rowvd,'CNCPT_2_VER_NR_1');
