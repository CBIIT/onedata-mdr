...
BEGIN
  ...
  if (hookinput.originalRowset.tablename = 'ADMIN_ITEM') then
    ...
    THEN
        ...
        if ((ihook.getColumnValue (row_ori, 'DERV_TYP_ID') is not null and ihook.getColumnValue (row_ori, 'DERV_MTHD') is null) or 
        (ihook.getColumnValue (row_ori, 'DERV_TYP_ID') is null and ihook.getColumnValue (row_ori, 'DERV_MTHD') is not null)) then
            raise_application_error(-20000, 'Derivation Type or Derivation Method is missing. ');
            return;
        end if;       
    END IF;
