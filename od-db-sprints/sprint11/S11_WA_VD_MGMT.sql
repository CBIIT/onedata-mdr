create or replace PACKAGE            nci_vd AS
  ...
  PROCEDURE spVDCreateNew (v_data_in in clob, v_data_out out clob);
    procedure createValAIWithConcept(rowform in out t_row, rowai in t_row,  idx in integer,v_item_typ_id in integer, v_mode in varchar2,v_cncpt_src in varchar2,  actions in out t_actions);
 ...
 --          raise_application_error(-20000, 'Here 1');
    ...
    IF HOOKINPUT.ANSWERID = 1 or Hookinput.answerid = 3 THEN  -- Validate using string
           ...
           --     raise_application_error(-20000, 'Here');      
               createValAIWithConcept(rowvd ,rowai,  1,7,'V','STRING',actions);
     
    end if;
    
    IF HOOKINPUT.ANSWERID = 2 or Hookinput.answerid = 4 THEN  -- Validate using drop-down       
               createValAIWithConcept(rowvd , rowai, 1,7,'V','DROP-DOWN',actions);          
    end if;
  ...
  /*     if (   ihook.getColumnValue(rowvd, 'ITEM_1_ID') is not null ) then
    ...
    */ 
        ...
        and v_op = 'insert') then -- Status cannot be released if VD is enumerated
    ...
    IF HOOKINPUT.ANSWERID in ( 3,4)  and is_valid = true THEN  -- Create
        createValAIWithConcept(rowvd ,rowai,  1,7,'C','DROP-DOWN',actions); -- Rep
...
--   
   ...
   spVDCommon(rowsetai, rowsetst, 3, 'insert', hookinput, hookoutput);
...
procedure createValAIWithConcept(rowform in out t_row, rowai in t_row, idx in integer,v_item_typ_id in integer,v_mode in varchar2, v_cncpt_src in varchar2, actions in out t_actions) as
v_nm  varchar2(255);
v_long_nm varchar2(255);
v_def  varchar2(4000);
row t_row;
...
if (v_cncpt_src ='DROP-DOWN') then
        ...
        --- Primary Concept
             for cur in (select item_id, item_nm , item_long_nm from admin_item where admin_item_typ_id = 49 and item_id = ihook.getColumnValue(rowform, 'CNCPT_1_ITEM_ID_1')
             and ver_nr  = ihook.getColumnValue(rowform, 'CNCPT_1_VER_NR_1')) loop
                          --       v_dec_nm := trim(v_dec_nm || ' ' || cur.item_nm) ;
                              v_long_nm_suf := trim(v_long_nm_suf || ':' || cur.item_long_nm);
                                v_nm := trim(v_nm || ' ' || cur.item_nm);
