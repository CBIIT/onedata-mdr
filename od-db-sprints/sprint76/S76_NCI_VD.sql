create or replace PACKAGE            nci_vd AS
  ...
  procedure createVD(rowai in out t_row, rowvd in t_row, actions in out t_actions, v_id out  number);
...
procedure purgePV (v_item_id in number, v_ver_nr in number, actions in out t_actions);
...
procedure spVDValCreateImport ( rowform in out t_row , v_op  in varchar2, actions in out t_actions,  v_val_ind in out boolean);
 procedure spVDValUpdateImport ( rowform in out t_row , v_op  in varchar2, actions in out t_actions,  v_val_ind in out boolean);
 procedure spCreateVerValVDImport ( rowform in out t_row , v_op  in varchar2, actions in out t_actions,  v_val_ind in out boolean, v_usr_id in varchar2);
 procedure createVDImport(rowform in out t_row, actions in out t_actions);
 procedure updateVDImport(rowform in out t_row, actions in out t_actions);
 procedure createVerVDImport(rowform in out t_row, actions in out t_actions, v_usr_id in varchar2);
...
v_int_cncpt_id  number := 2433736;
 ...
 select 'Std DT: ' || dttype_nm into v_nm from data_typ where dttype_id = ihook.getColumnValue(rowvd, 'NCI_STD_DTTYPE_ID');


 select dttype_nm into v_temp_nm from data_typ where dttype_id = ihook.getColumnValue(rowvd, 'DTTYPE_ID');
 v_nm := v_nm ||  '/Source DT:' || v_temp_nm;


 if (ihook.getColumnValue(rowvd, 'NCI_DEC_PREC') is not null) then 
  v_nm := v_nm || '/Decimal:' || ihook.getColumnValue(rowvd, 'NCI_DEC_PREC');
 end if;

 if (ihook.getColumnValue(rowvd, 'UOM_ID') is not null) then 
 select uom_nm into v_uom from uom where uom_id = ihook.getColumnValue(rowvd, 'UOM_ID');
 v_nm := v_nm || '/UOM:' || v_uom;
 end if;

 if (ihook.getColumnValue(rowvd, 'VAL_DOM_FMT_ID') is not null) then 
 select fmt_nm into v_temp_nm from fmt where fmt_id = ihook.getColumnValue(rowvd, 'VAL_DOM_FMT_ID');
 v_nm := v_nm || '/Format:' || v_temp_nm;
 end if;
 if (ihook.getcolumnValue(rowvd,'VAL_DOM_MAX_CHAR') is not null) then
    v_nm := v_nm || '/Max Len:' ||ihook.getcolumnValue(rowvd,'VAL_DOM_MAX_CHAR');
 end if;
 if (ihook.getcolumnValue(rowvd,'VAL_DOM_MIN_CHAR') is not null) then
    v_nm := v_nm || '/Min Len:' ||ihook.getcolumnValue(rowvd,'VAL_DOM_MIN_CHAR');
 end if;
 if (ihook.getcolumnValue(rowvd,'VAL_DOM_HIGH_VAL_NUM') is not null) then
    v_nm := v_nm || '/Max Val:' ||ihook.getcolumnValue(rowvd,'VAL_DOM_HIGH_VAL_NUM');
 end if;
 if (ihook.getcolumnValue(rowvd,'VAL_DOM_LOW_VAL_NUM') is not null) then
    v_nm := v_nm || '/Min Val:' ||ihook.getcolumnValue(rowvd,'VAL_DOM_LOW_VAL_NUM');
 end if;

 if (ihook.getcolumnValue(rowvd,'VAL_DOM_TYP_ID')=18) then -- non enum
  v_nm := substr(ihook.getcolumnValue(rowvd,'ITEM_1_NM') || '/Non-enum/' || v_nm,1,255);
  end if;
 if (ihook.getcolumnValue(rowvd,'VAL_DOM_TYP_ID')=17) then -- enum
  v_nm := substr(ihook.getcolumnValue(rowvd,'ITEM_1_NM') ||'/Enum/' || v_nm || ' (' || numpv || ' Values)',1,255);
  end if;
...
ihook.setcolumnvalue(rowai,'ITEM_NM_CURATED', v_nm);
 end;
 ...
 select count(*) into numpv from perm_val where val_dom_item_id = ihook.getColumnValue(rowvd, 'ITEM_ID') and
 val_dom_ver_nr = ihook.getColumnValue(rowvd, 'VER_NR') and nvl(fld_delete,0) = 0;
 select item_nm into v_rt from admin_item where item_id = ihook.getColumnValue(rowvd, 'REP_CLS_ITEM_ID') and
 ver_nr= ihook.getColumnValue(rowvd, 'REP_CLS_VER_NR');
 ...
 if (ihook.getcolumnValue(rowvd,'VAL_DOM_TYP_ID')=18) then -- non enum
  v_nm := substr(v_rt || '/Non-enum/' || v_nm,1,255);
  end if;
 if (ihook.getcolumnValue(rowvd,'VAL_DOM_TYP_ID')=17) then -- enum
  v_nm := substr(v_rt||'/Enum/' || v_nm || ' (' || numpv || ' Values)',1,255);
  end if;
   if (ihook.getcolumnValue(rowvd,'VAL_DOM_TYP_ID')=16) then -- enum by ref
...
ihook.setcolumnvalue(row,'ITEM_NM_CURATED', v_nm);
 end;
...
procedure createVD (rowai in out t_row, rowvd in t_row, actions in out t_actions, v_id out  number) as
v_nm  varchar2(255);
...
-- if 16 and enumeraed by reference 
    ...
    -- Create New.
      ...
      /*  if (ihook.getColumnValue(rowvd,'VAL_DOM_TYP_ID') = 16) then
        select item_nm into v_pv_nm from admin_item where item_id = ihook.getColumnValue(rowvd,'CNCPT_2_ITEM_ID_1') and ver_nr =  ihook.getColumnValue(rowvd,'CNCPT_2_VER_NR_1');
        ihook.setColumnValue(row,'ITEM_NM', v_pv_nm || ' ' || ihook.getColumnValue(row, 'ITEM_NM')  );
   ...
   --     select count(*) into v_pv from perm_val where val_dom_item_id = v_item_id and val_dom_ver_nr = v_ver_nr;
                          getVdGenName (row, rowvd,0);
        rows.extend;
        rows(rows.last) := row;
...
--Rep Term (Hook Creation)
        ...
        ihook.setColumnValue(row,'TERM_CNCPT_ITEM_ID',  ihook.getColumnValue(rowvd,'CNCPT_2_ITEM_ID_1'));
        ihook.setColumnValue(row,'TERM_CNCPT_VER_NR',  ihook.getColumnValue(rowvd,'CNCPT_2_VER_NR_1'));
        ihook.setColumnValue(row,'TERM_USE_TYP',  ihook.getColumnValue(rowvd,'BTCH_SEQ_NBR'));
...
-- if reference by termnilogy and create node ia set
if (nvl(ihook.getColumnValue(rowvd,'CHAR_SET_ID'),0) = 1 and ihook.getColumnValue(rowvd, 'VAL_DOM_TYP_ID') = 16) then
  select item_nm into v_pv_nm from admin_item where item_id = ihook.getColumnValue(rowvd,'CNCPT_2_ITEM_ID_1') and ver_nr =  ihook.getColumnValue(rowvd,'CNCPT_2_VER_NR_1');
 -- raise_application_error(-20000,'Here;' || v_pv_nm);
      nci_dec_mgmt.createSingleConceptAI(ihook.getColumnValue(rowvd,'CNCPT_2_ITEM_ID_1'), ihook.getColumnValue(rowvd,'CNCPT_2_VER_NR_1'),53, v_vm_id , v_vm_ver_nr ,actions );
nci_pv_vm.createPVAtomic(v_id, 1, v_vm_id, v_vm_ver_nr, v_pv_nm, actions);
end if;
...
-- jira 1670.1 change status to Errors if validated indicator is false
    ...
    -- warning if ref term name not in specified or generated name
       if (   ihook.getColumnValue(rowform, 'VAL_DOM_TYP_ID') =16  ) then 
...
AS
    ...
    v_pv_nm varchar2(255);
    v_temp_nm varchar2(255);
    row_ori_vd  t_row := t_row();
 BEGIN

 if ( ihook.getColumNValue(rowform, 'CTL_VAL_STUS')<> 'PROCESSED') then --- only if Value Domain not specified

 v_item_id := ihook.getColumnValue(rowform, 'VAL_DOM_ITEM_ID');
      ...
      /*  if (   ihook.getColumnValue(rowform, 'VAL_DOM_TYP_ID') is null ) then
       ...
       -- end if;
        ...
        if (   ihook.getColumnValue(rowform, 'VAL_DOM_TYP_ID') in (17,18) and ihook.getColumnValue(row_ori_vd, 'VAL_DOM_TYP_ID') =18) then
                 ihook.setColumnValue(rowform, 'CTL_VAL_MSG', ihook.getColumnValue(rowform, 'CTL_VAL_MSG') || 'WARNING: Permissible Values need to be created.' || chr(13));
        end if;
        if (   (ihook.getColumnValue(rowform, 'VAL_DOM_TYP_ID')= 16 and ihook.getColumnValue(row_ori_vd, 'VAL_DOM_TYP_ID') =17) or 
        (ihook.getColumnValue(rowform, 'VAL_DOM_TYP_ID')= 17 and ihook.getColumnValue(row_ori_vd, 'VAL_DOM_TYP_ID') =16) ) then
                 ihook.setColumnValue(rowform, 'CTL_VAL_MSG', ihook.getColumnValue(rowform, 'CTL_VAL_MSG') || 'WARNING: PV/VMs need to be REVIEWED.' || chr(13));
        end if;
        -- changing to enumerated and no pv
     --    if (   ihook.getColumnValue(rowform, 'VAL_DOM_TYP_ID') =17 and ihook.getColumnValue(row_ori_vd, 'VAL_DOM_TYP_ID') <>1) then
     --            ihook.setColumnValue(rowform, 'CTL_VAL_MSG', ihook.getColumnValue(rowform, 'CTL_VAL_MSG') || 'WARNING: All Permissible Values will be deleted.' || chr(13));
      --  end if;
  /*    if (   ihook.getColumnValue(rowform, 'UOM_ID') is null ) then
                  ihook.setColumnValue(rowform, 'CTL_VAL_MSG', ihook.getColumnValue(rowform, 'CTL_VAL_MSG') || 'UOM is missing or invalid.' || chr(13));
...
-- jira 1670.1 change status to Errors if validated indicator is false
    ...
    if (v_val_ind = true and ihook.getColumnValue(rowform, 'CNCPT_3_ITEM_ID_1') is not null) then
    createValAIWithConcept(rowform , 3,7,'V','DROP-DOWN',actions); -- Rep
  ...
  /* if (ihook.getColumnValue(rowform,'VAL_DOM_TYP_ID') = 16 and v_val_ind = true) then
    ...
    IF v_op  = 'C'  and v_val_ind = true THEN  -- Create
--raise_application_error(-20000,'here');
if (ihook.getColumnValue(rowform, 'CNCPT_3_ITEM_ID_1') is not null )  then 
        createValAIWithConcept(rowform , 3,7,'C','DROP-DOWN',actions); -- Rep
...
-- append to existing change description
   ...
   if(ihook.getColumnValue(rowform, 'ADMIN_NOTES') is not null) then 
        ihook.setColumnValue (rowform,'ADMIN_NOTES', substr(ihook.getColumnValue(rowform, 'ADMIN_NOTES') || ' ' ||ihook.getColumnValue(row_ori_vd, 'ADMIN_NOTES'),1,4000));
    end if;

                updateVDImport(rowform, actions );
                -- only delete if target vd type is non-enumerated
                if (   ihook.getColumnValue(rowform, 'VAL_DOM_TYP_ID') =18 and ihook.getColumnValue(row_ori_vd, 'VAL_DOM_TYP_ID') in (16,17)) then
               -- raise_application_error(-20000,'purge pv');
                purgePV(v_item_id, v_ver_nr, actions);
        end if;
...
AS
 ...
 --clear validated VD name
 ihook.setColumnValue(rowform, 'CURRNT_VAL_ITEM_NM', '');

 nci_import.validateVersionRow(rowform, 3, v_item_id,v_ver_nr, v_val_ind);
           nci_11179.spReturnAIRow(v_item_id,v_ver_nr, row_ori_vd);
            nci_11179.spReturnSubtypeRow(v_item_id,v_ver_nr, 3, row_ori_vd);
...
if (ihook.getColumnValue(rowform, 'CURRNT_VAL_ITEM_NM') is null or ihook.getColumnValue(rowform, 'CURRNT_VAL_ITEM_NM') = '' or ihook.getColumnValue(rowform, 'CURRNT_VAL_ITEM_NM') <> nvl(ihook.getColumnValue(rowform, 'VAL_DOM_NM'), ihook.getColumnValue(rowform, 'ITEM_3_NM'))) then
    ihook.setColumnValue(rowform, 'CURRNT_VAL_ITEM_NM', nvl(ihook.getColumnValue(rowform, 'VAL_DOM_NM'), ihook.getColumnValue(rowform, 'ITEM_3_NM')));
end if;
if ((ihook.getColumnValue(rowform, 'CURRNT_VAL_ITEM_NM') is null or ihook.getColumnValue(rowform, 'CURRNT_VAL_ITEM_NM') = '') and v_val_ind = true) then
    select item_nm into v_temp_nm from admin_item where item_id = ihook.getColumnValue(rowform, 'VAL_DOM_ITEM_ID') and ver_nr = ihook.getColumnValue(rowform, 'VAL_DOM_VER_NR');
    ihook.setColumnValue(rowform, 'CURRNT_VAL_ITEM_NM', v_temp_nm);
end if;
--ihook.setColumnValue(rowform, 'CURRNT_VAL_ITEM_NM', nvl(ihook.getColumnValue(rowform, 'VAL_DOM_NM'), ihook.getColumnValue(rowform, 'ITEM_3_NM')));
  /* if (ihook.getColumnValue(rowform,'VAL_DOM_TYP_ID') = 16 and v_val_ind = true) then
        select item_nm into v_pv_nm from admin_item where item_id = ihook.getColumnValue(rowform,'TERM_CNCPT_ITEM_ID') and ver_nr =  ihook.getColumnValue(rowform,'TERM_CNCPT_VER_NR');
    ...
    IF v_op  = 'CV'  and v_val_ind = true THEN  -- Create
--raise_application_error(-20000,'here');
if (ihook.getColumnValue(rowform, 'CNCPT_3_ITEM_ID_1') is not null )  then 
        createValAIWithConcept(rowform , 3,7,'C','DROP-DOWN',actions); -- Rep
...
-- append to existing change description
   ...
   if(ihook.getColumnValue(rowform, 'ADMIN_NOTES') is not null) then 
        ihook.setColumnValue (rowform,'ADMIN_NOTES', substr(ihook.getColumnValue(rowform, 'ADMIN_NOTES') || ' ' || ihook.getColumnValue(row_ori_vd, 'ADMIN_NOTES'),1,4000));
                ...
                createVerVDImport(rowform, actions , v_usr_id);
                -- only delete if target vd type is non-enumerated
                --if (   ihook.getColumnValue(rowform, 'VAL_DOM_TYP_ID') =18 and ihook.getColumnValue(row_ori_vd, 'VAL_DOM_TYP_ID') in (16,17)) then
               -- raise_application_error(-20000,'purge pv');
               -- purgePV(v_item_id, v_ver_nr, actions);
...
if (ihook.getCOlumnValue(rowform,'ITEM_3_ID') is not null) then
   ...
   if (v_val_ind = false) then 
    ...
    --raise_application_error(-20000, ihook.getColumnValue(rowform, 'REP_CLS_ITEM_ID'));
...
if (ihook.getColumnValue(row,'VAL_DOM_TYP_ID') <> 16) then
...
-- if reference by termnilogy and create node ia set
if (nvl(ihook.getColumnValue(rowform,'CREAT_PV_VM'),0) = 1 and ihook.getColumnValue(rowform, 'VAL_DOM_TYP_ID') = 16) then
   select item_nm into v_pv_nm from admin_item where item_id = ihook.getColumnValue(rowform,'TERM_CNCPT_ITEM_ID') and ver_nr =  ihook.getColumnValue(rowform,'TERM_CNCPT_VER_NR');

nci_dec_mgmt.createSingleConceptAI(ihook.getColumnValue(rowform,'TERM_CNCPT_ITEM_ID'), ihook.getColumnValue(rowform,'TERM_CNCPT_VER_NR'),53, v_vm_id , v_vm_ver_nr ,actions );
nci_pv_vm.createPVAtomic(v_id, 1, v_vm_id, v_vm_ver_nr, v_pv_nm, actions);
end if;
...
begin
   ...
   v_ver_nr := ihook.getColumnValue(rowform,'VAL_DOM_VER_NR');
        ...
        nci_11179.spReturnSubtypeSpecifiedRow (v_item_id, v_ver_nr , 3, row , rowform );
         nci_11179.spReturnSubtypeSpecifiedRow (v_item_id, v_ver_nr , 0, row , rowform );
      --  raise_application_error(-20000,ihook.getColumnValue(row,'VAL_DOM_TYP_ID'));

      -- set status to DRAFT NEW
      ihook.setColumnValue(row,'ADMIN_STUS_ID',66);

    if (ihook.getColumnValue(rowform, 'VD_ITEM_LONG_NM') is not null) then
         ihook.setColumnValue(row,'ITEM_LONG_NM', ihook.getColumnValue(rowform, 'VD_ITEM_LONG_NM'));
    end if;
...
if (     ihook.getColumnValue(rowform, 'VAL_DOM_NM') is not null) then -- optional name specified
  ihook.setColumnValue(row,'ITEM_NM',  ihook.getColumnValue(rowform, 'VAL_DOM_NM')  );
end if;
...
--Rep Term (Hook Creation)
       ...
       actions(actions.last) := action;
       -- jira 1670.5 remove new vd id
   ihook.setColumnValue(rowform, 'CTL_VAL_MSG', ihook.getColumnValue(rowform, 'CTL_VAL_MSG') || 'VD Updated Successfully' || chr(13)) ;
...
nci_dec_mgmt.createSingleConceptAI(ihook.getColumnValue(rowform,'TERM_CNCPT_ITEM_ID'), ihook.getColumnValue(rowform,'TERM_CNCPT_VER_NR'),53, v_vm_id , v_vm_ver_nr ,actions );
nci_pv_vm.createPVAtomic(v_item_id, v_ver_nr, v_vm_id, v_vm_ver_nr, v_pv_nm, actions);
end if;
...
rows t_rows;
 ...
 v_ver_nr number(4,2);
 v_new_ver_nr number(4,2);
 v_ori_val_dom_typ integer;
 v_ori_short_nm varchar2(30);
begin
   rows := t_rows();
   ...
   v_new_ver_nr := ihook.getColumnValue(rowform,'NEW_VER_NR');
     -- nci_11179_2.setStdAttr(row);
        ...
        v_ori_val_dom_typ := ihook.getColumnValue(row, 'VAL_DOM_TYP_ID');
        v_ori_short_nm := ihook.getColumnValue(row, 'ITEM_LONG_NM');
        nci_11179.spReturnSubtypeSpecifiedRow (v_item_id, v_ver_nr , 3, row , rowform );
         nci_11179.spReturnSubtypeSpecifiedRow (v_item_id, v_ver_nr , 0, row , rowform );
         nci_11179_2.resetAuditAttr(row, v_usr_id);
    ...
    if (ihook.getColumnValue(rowform, 'VD_ITEM_LONG_NM') is not null) then
         ihook.setColumnValue(row,'ITEM_LONG_NM', ihook.getColumnValue(rowform, 'VD_ITEM_LONG_NM'));
    elsif nci_11179_2.isShortNmSysGen(v_ori_short_nm) = true then
         ihook.setColumnValue(row,'ITEM_LONG_NM',nci_11179_2.getStdShortName(v_item_id, v_new_ver_nr));
    end if;
     ihook.setColumnValue(row, 'ADMIN_ITEM_TYP_ID', 3);
     --   ihook.setColumnValue(row,'CONC_DOM_ITEM_ID',  nvl(ihook.getColumnValue(rowform,'VD_CONC_DOM_ITEM_ID'),ihook.getColumnValue(rowform,'CONC_DOM_ITEM_ID')));
      --  ihook.setColumnValue(row,'CONC_DOM_VER_NR',  nvl(ihook.getColumnValue(rowform,'VD_CONC_DOM_VER_NR'),ihook.getColumnValue(rowform,'CONC_DOM_VER_NR')));
...
--Rep Term (Hook Creation)
   ...
   nci_11179.spCreateCommonChildrenNCI (actions , v_item_id , v_ver_nr ,v_item_id ,v_new_ver_nr  , v_usr_id) ;

         if ( (ihook.getColumnValue(row, 'VAL_DOM_TYP_ID') = 17 and v_ori_val_dom_typ in (16, 17)) or  (ihook.getColumnValue(row, 'VAL_DOM_TYP_ID') = 16 and v_ori_val_dom_typ = 16) ) then 
            nci_11179.CopyPermVal (actions , v_item_id, v_ver_nr, v_item_id, v_new_ver_nr);
            end if;

--  if reference by termnilogy and create node ia set

if (nvl(ihook.getColumnValue(rowform,'CREAT_PV_VM'),0) = 1 and ihook.getColumnValue(row, 'VAL_DOM_TYP_ID') = 16 and v_ori_val_dom_typ <> 16) then
   select item_nm into v_pv_nm from admin_item where item_id = ihook.getColumnValue(row,'TERM_CNCPT_ITEM_ID') and ver_nr =  ihook.getColumnValue(row,'TERM_CNCPT_VER_NR');

nci_dec_mgmt.createSingleConceptAI(ihook.getColumnValue(rowform,'TERM_CNCPT_ITEM_ID'), ihook.getColumnValue(rowform,'TERM_CNCPT_VER_NR'),53, v_vm_id , v_vm_ver_nr ,actions );
nci_pv_vm.createPVAtomic(v_item_id, v_ver_nr, v_vm_id, v_vm_ver_nr, v_pv_nm, actions);
end if;

   ihook.setColumnValue(rowform, 'CTL_VAL_MSG', ihook.getColumnValue(rowform, 'CTL_VAL_MSG') || 'VD Version Created Successfully: ' ||v_new_ver_nr ||  chr(13)) ;
       ...
       ihook.setColumnValue(row,'CURRNT_VER_IND', 0);
        ihook.setColumnValue(row,'ADMIN_ITEM_TYP_ID', 3);
            ...
            actions(actions.last) := action;
            	...
            	ihook.setColumnValue(rowform, 'VAL_DOM_ITEM_ID_CREAT',v_item_id);
                ihook.setColumnValue(rowform, 'VAL_DOM_VER_NR_CREAT',v_new_ver_nr);
...
-- Create new
        if (   ihook.getColumnValue(rowai, 'ADMIN_STUS_ID') = 75 and  ihook.getColumnValue(rowvd, 'VAL_DOM_TYP_ID') in (16,  17)
        and v_op = 'insert' and v_from <> 2) then -- Status cannot be released if VD is enumerated
                  ihook.setColumnValue(rowvd, 'CTL_VAL_MSG', 'An Enumerated or Enumerated by Reference VD WFS cannot be Released if there are no permissible values.');
                  is_valid := false;
        end if;
...
-- Edit
   ...
   if  ihook.getColumnValue(rowvd, 'VAL_DOM_TYP_ID') = 16 then
   if (ihook.getColumnValue(rowvd,'CNCPT_2_ITEM_ID_1') is not null) then 
        select upper(nvl(nm_desc,item_nm)) into v_temp_nm from admin_item ai, alt_nms a, obj_key where ai.item_id = ihook.getColumnValue(rowvd,'CNCPT_2_ITEM_ID_1') 
        and ai.ver_nr =  ihook.getColumnValue(rowvd,'CNCPT_2_VER_NR_1')  and obj_typ_id = 11 and obj_key_desc = 'Ref Term Short Name' and a.nm_typ_id (+)= obj_key_id
        and a.item_id (+)= ihook.getColumnValue(rowvd,'CNCPT_2_ITEM_ID_1')  and a.ver_nr (+)= ihook.getColumnValue(rowvd,'CNCPT_2_VER_NR_1');
...
-- Create from Existing
         if (   ihook.getColumnValue(rowai, 'ADMIN_STUS_ID') = 75 and  ihook.getColumnValue(rowvd, 'VAL_DOM_TYP_ID') in ( 17)
        and v_op = 'insert' and v_from = 2) then -- Status cannot be released if VD is enumerated
         row_ori :=  hookInput.originalRowset.rowset(1);
 ...
 if (   ihook.getColumnValue(rowai, 'ADMIN_STUS_ID') = 75 and  ihook.getColumnValue(rowvd, 'VAL_DOM_TYP_ID') in ( 16)
        and v_op = 'insert' and v_from = 2) then -- Status cannot be released if VD is enumerated

                  ihook.setColumnValue(rowvd, 'CTL_VAL_MSG', 'An Enumerated By Reference VD WFS cannot be Released if there are no permissible values.');
                  is_valid := false;
        ...
        if (ihook.getColumnValue(rowvd, 'VAL_DOM_TYP_ID') <> 16 and  (ihook.getColumnValue(rowvd, 'BTCH_SEQ_NBR') is not null or  
        ihook.getColumnValue(rowvd, 'CNCPT_2_ITEM_ID_1') is not null)  )  then --- remove ref term and ref term data source if val type changed

            ihook.setColumnValue(rowvd,'BTCH_SEQ_NBR','');
            ihook.setColumnValue(rowvd, 'CNCPT_2_ITEM_ID_1','');
            ihook.setColumnValue(rowvd, 'CNCPT_2_VER_NR_1','');
        end if;
        ...
        if is_valid=false or v_answer in (1, 2) then
           ...
           if (v_op = 'update') then
                ...
                for i in 2..10 loop
                    ...
                    for cur in (select CNCPT_ORI_NM item_nm, item_desc from vw_cncpt  where item_id = ihook.getColumnValue(rowvd, 'CNCPT_1_ITEM_ID_1') and ver_nr = ihook.getColumnValue(rowvd, 'CNCPT_1_VER_NR_1')) loop
  ...
  -- raise_application_error(-20000, ihook.getColumnValue (rowai, 'ITEM_NM'));
              ...
              if (v_op = 'insert') then
                ...
                -- Update VD. Get the selected row, update name, definition, context.
                    ...
                    ihook.setColumnValue(row,'ITEM_NM', ihook.getColumnValue(rowai, 'ITEM_NM_CURATED')  );
                         ...
                         ihook.setColumnValue(row, 'ADMIN_ITEM_TYP_ID', 3);
                         select count(*) into v_pv from perm_val where val_dom_item_id = v_item_id and val_dom_ver_nr = v_ver_nr and nvl(fld_delete,0) = 0;
                          getVdGenName (row, rowvd,v_pv);
    ...
    ihook.setColumnValue(row,'TERM_CNCPT_ITEM_ID',  ihook.getColumnValue(rowvd,'CNCPT_2_ITEM_ID_1'));
       ...
       ihook.setColumnValue(row,'TERM_USE_TYP',  ihook.getColumnValue(rowvd,'BTCH_SEQ_NBR'));
        ...
        actions.extend;
        actions(actions.last) := action;
             -- delete pv if type changed from enum to non-enum
                        rowvdold := v_init_vd.rowset(1); -- only append if changed.
    if (ihook.getColumnValue(rowvd, 'VAL_DOM_TYP_ID') = 18 and ihook.getColumnValue(rowvdold, 'VAL_DOM_TYP_ID' ) <> 18) then
         purgePV (v_item_id,  v_ver_nr , actions );
   end if; 
        end if;
...
-- Tracker 2078 - do not copy PV if new VD is non-enumerated
        if (ihook.getcolumnValue(rowvd,'VAL_DOM_TYP_ID') in (17)) then -- enumerated only
...
rows t_rows;
 ...
 rows := t_rows();
  for cur in (select * from perm_val where val_dom_item_id = v_item_id and val_dom_Ver_nr = v_ver_nr) loop
      ...
      --  raise_application_error(-20000,'purge pv' || rows.count);
...
--
   ihook.setColumnValue(rowvd,'CNCPT_2_ITEM_ID_1',  ihook.getColumnValue(rowvd,'TERM_CNCPT_ITEM_ID'));
        ihook.setColumnValue(rowvd,'CNCPT_2_VER_NR_1',  ihook.getColumnValue(rowvd,'TERM_CNCPT_VER_NR'));
        ihook.setColumnValue(rowvd,'BTCH_SEQ_NBR',  ihook.getColumnValue(rowvd,'TERM_USE_TYP'));
...
--raise_application_error(-20000, v_cncpt_src);
...
if (v_cncpt_src ='DROP-DOWN') then
        ...
        v_long_nm_suf_int:= '';
                for i in 2..10 loop
                        ...
                        for cur in(select ai.item_id, item_nm , item_long_nm, item_desc from admin_item ai where 
                        admin_item_typ_id = 49 and ai.item_id = v_temp_id and ai.ver_nr = v_temp_ver) loop
                     --            v_def := substr( v_def || '_' ||cur.item_desc  ,1,4000);
                         v_def := substr( v_def || '_' ||cur.item_desc  ,1,4000);
        ...
        --- Primary Concept
            ...
            -- and ver_nr  = ihook.getColumnValue(rowform, 'CNCPT_' || idx || '_VER_NR_1')) loop
                          --       v_dec_nm := trim(v_dec_nm || ' ' || cur.item_nm) ;
             for cur in (select item_id, CNCPT_ORI_NM item_nm , item_long_nm, item_desc from vw_cncpt where  item_id = ihook.getColumnValue(rowform, 'CNCPT_' || idx || '_ITEM_ID_1')
            and ver_nr  = ihook.getColumnValue(rowform, 'CNCPT_' || idx || '_VER_NR_1')) loop
                              v_long_nm_suf := trim(v_long_nm_suf || ':' || cur.item_long_nm);
                                                  v_def := substr( v_def || '_' ||cur.item_desc  ,1,4000);
                               if (cur.item_id = v_int_cncpt_id and trim(ihook.getColumnValue(rowform,'CNCPT_INT_' || idx || '_' || i)) is not null) then --- integer concept
...
--raise_application_error(-20000, idx);
    --  raise_application_error(-20000,  '  Long name: ' || v_nm);
        ...
        --jira 9374
        --ihook.setColumnValue(rowform, 'CURRNT_VAL_ITEM_NM', v_nm);
       --                raise_application_error(-20000,v_nm);
...
--raise_application_error(-20000, ihook.getcolumnvalue(rowform, 'ITEM_1_NM'));
  ...
  --     v_def := '';
    ...
    --   raise_application_error(-20000, 'Test'  || i || v_cncpt_id);
          ...
          v_cncpt_id := ihook.getColumnValue(rowform, 'CNCPT_' || idx  ||'_ITEM_ID_1' );
           ...
           for cur in (select CNCPT_ORI_NM item_nm, item_long_nm, item_desc from vw_cncpt where item_id = v_cncpt_id and ver_nr = v_cncpt_ver_nr) loop
            row := t_row();
            ihook.setColumnValue(row,'ITEM_ID', v_id);
            ihook.setColumnValue(row,'VER_NR', 1);
            ihook.setColumnValue(row,'CNCPT_ITEM_ID',v_cncpt_id);
 ...
 --     v_nm := '';
  ...
  --     v_def := '';
       ...
       action := t_actionrowset(rows, 'Items under Concept (Hook)', 2,6,'insert');
        ...
        ihook.setColumnValue(row,'ITEM_ID', v_ai_id);
        ihook.setColumnValue(row,'VER_NR', v_ai_ver_nr);
        ihook.setColumnValue(row,'ITEM_LONG_NM', nci_11179_2.getStdShortName(v_ai_id, 1)); --jira 3995: use existing pub id
