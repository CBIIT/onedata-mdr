...
update admin_item set item_nm_curated = (select rt.item_nm || '/Non-enum/Std DT: ' || dt.dttype_nm || '/Source DT: ' ||sdt.dttype_nm || 
decode(vd.NCI_DEC_PREC,null,'','/Decimal: ' || vd.NCI_DEC_PREC ) ||
...
update admin_item set item_nm_curated = (select substr(rt.item_nm || '/Enum/Std DT: ' || dt.dttype_nm || '/Source DT: ' ||sdt.dttype_nm || 
decode(vd.NCI_DEC_PREC,null,'','/Decimal: ' || vd.NCI_DEC_PREC ) ||
decode(vd.UOM_ID,null,'','/UOM:' || uom.UOM_NM ) ||decode(vd.VAL_DoM_FMT_ID,null,'','/Format:' || fmt.fmt_nm ) ||
decode(VAL_DOM_MAX_CHAR,null,'','/Max Len:' ||VAL_DOM_MAX_CHAR) || decode(VAL_DOM_MIN_CHAR,null,'','/Min Len: ' ||VAL_DOM_MIN_CHAR ) ||
decode(VAL_DOM_HIGH_VAL_NUM,null,'','/Max Val:' ||VAL_DOM_HIGH_VAL_NUM) || decode(VAL_DOM_LOW_VAL_NUM,null,'','/Min Val:' ||VAL_DOM_LOW_VAL_NUM  )
...
-- Enum by reference
update admin_item set item_nm_curated = (select substr(rt.item_nm || '/Enum by Ref/' || vdrt.term_name || '/' || vdrt.use_name ||'/Std DT: ' || dt.dttype_nm || '/Source DT: ' ||sdt.dttype_nm || 
decode(vd.NCI_DEC_PREC,null,'','/Decimal: ' || vd.NCI_DEC_PREC ) ||
decode(vd.UOM_ID,null,'','/UOM:' || uom.UOM_NM ) ||decode(vd.VAL_DoM_FMT_ID,null,'','/Format:' || fmt.fmt_nm ) ||
decode(VAL_DOM_MAX_CHAR,null,'','/Max Len:' ||VAL_DOM_MAX_CHAR) || decode(VAL_DOM_MIN_CHAR,null,'','/Min Len: ' ||VAL_DOM_MIN_CHAR ) ||
decode(VAL_DOM_HIGH_VAL_NUM,null,'','/Max Val:' ||VAL_DOM_HIGH_VAL_NUM) || decode(VAL_DOM_LOW_VAL_NUM,null,'','/Min Val:' ||VAL_DOM_LOW_VAL_NUM  )
...
update admin_item set RVWR_CMNTS = (select rt.item_nm || '/Non-enum/Std DT: ' || dt.dttype_nm || '/Source DT: ' ||sdt.dttype_nm || 
decode(vd.NCI_DEC_PREC,null,'','/Decimal: ' || vd.NCI_DEC_PREC ) ||
decode(vd.UOM_ID,null,'','/UOM:' || uom.UOM_NM ) ||decode(vd.VAL_DoM_FMT_ID,null,'','/Format:' || fmt.fmt_nm ) ||
decode(VAL_DOM_MAX_CHAR,null,'','/Max Len:' ||VAL_DOM_MAX_CHAR  ) || decode(VAL_DOM_MIN_CHAR,null,'','/Min Len:' ||VAL_DOM_MIN_CHAR ) ||
decode(VAL_DOM_HIGH_VAL_NUM,null,'','/Max Val:' ||VAL_DOM_HIGH_VAL_NUM  ) || decode(VAL_DOM_LOW_VAL_NUM,null,'','/Min Val:' ||VAL_DOM_LOW_VAL_NUM  )
...
update admin_item set RVWR_CMNTS = (select substr(rt.item_nm || '/Enum/Std DT: ' || dt.dttype_nm || '/Source DT: ' ||sdt.dttype_nm || 
decode(vd.NCI_DEC_PREC,null,'','/Decimal: ' || vd.NCI_DEC_PREC ) ||
decode(vd.UOM_ID,null,'','/UOM:' || uom.UOM_NM ) ||decode(vd.VAL_DoM_FMT_ID,null,'','/Format:' || fmt.fmt_nm ) ||
decode(VAL_DOM_MAX_CHAR,null,'','/Max Len:' ||VAL_DOM_MAX_CHAR) || decode(VAL_DOM_MIN_CHAR,null,'','/Min Len: ' ||VAL_DOM_MIN_CHAR ) ||
decode(VAL_DOM_HIGH_VAL_NUM,null,'','/Max Val:' ||VAL_DOM_HIGH_VAL_NUM) || decode(VAL_DOM_LOW_VAL_NUM,null,'','/Min Val:' ||VAL_DOM_LOW_VAL_NUM  )
...
and vd.VAL_DOM_FMT_ID = fmt.fmt_id (+)
and vd.dttype_id = sdt.dttype_id)
where (admin_item.item_id,admin_item.ver_nr) in (Select item_id, ver_nr from value_dom where val_dom_typ_id = 18  and lst_upd_dt > sysdate - 2 )
and admin_item.lst_upd_dt > sysdate - 2;
commit;
...
vd.item_id = pv.val_dom_item_id (+) and vd.ver_nr = pv.val_dom_ver_nr (+) and vd.uom_id = uom.uom_id (+) and vd.VAL_DOM_FMT_ID = fmt.fmt_id (+)
and vd.dttype_id = sdt.dttype_id)
where (admin_item.item_id,admin_item.ver_nr) in (Select item_id, ver_nr from value_dom where val_dom_typ_id = 17 and lst_upd_dt > sysdate - 2)
and admin_item.lst_upd_dt > sysdate - 2;
commit;
...
and vd.dttype_id = sdt.dttype_id
and vd.item_id = vdrt.item_id and vd.ver_nr = vdrt.ver_nr)
where (admin_item.item_id,admin_item.ver_nr) in (Select item_id, ver_nr from value_dom where val_dom_typ_id = 16  and lst_upd_dt > sysdate - 2)
 and admin_item.lst_upd_dt > sysdate - 2;
commit;
