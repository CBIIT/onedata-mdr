...
procedure spModelMapCharValUpd ( v_data_in in clob, v_data_out out clob, v_user_id varchar2);
procedure spQuestRepUpd ( v_data_in in clob, v_data_out out clob, v_user_id varchar2);
procedure spUpdAdminItemAudit ( v_item_id in number, v_ver_nr in number, v_user_id in varchar2, actions in out t_actions);
...
procedure sp_postprocess ( v_data_in in clob, v_data_out out clob); -- hourly job
procedure spDailyAuditLoad ( v_data_in in clob, v_data_out out clob);
procedure spWeeklyProcess ( v_data_in in clob, v_data_out out clob);
procedure spRefreshConceptView ( v_data_in in clob, v_data_out out clob);
...
procedure spRefreshViewsHourly (v_data_in in clob, v_data_out out clob);
procedure spRefreshBasicViews (v_data_in in clob, v_data_out out clob);
...
procedure spPrePVVMImport(v_data_in in clob, v_data_out out clob, v_usr_id varchar2);
...
nci_data_audit.spLoadDataAudit(sysdate - 1.5, sysdate);
...
delete from nci_ds_prmtr_temp;
commit;
delete from onedata_Ra.nci_ds_prmtr_temp;
commit;
...
and ai.admin_item_typ_id in (1,2,3,5,6);
commit;
...
from  admin_item ai,
(SELECT cai.item_id, cai.ver_nr, LISTAGG(ai.item_long_nm, ':')WITHIN GROUP (ORDER by cai.nci_ord desc) as CNCPT_CD ,
LISTAGG(decode(ai.item_long_nm, 'C45255',decode( cai.nci_cncpt_val,null, ai.cncpt_ori_nm, ai.cncpt_ori_nm || '::' || cai.NCI_CNCPT_VAL),ai.cncpt_ori_nm), ' ')WITHIN GROUP (ORDER by cai.nci_ord desc) as CNCPT_NM ,
 LISTAGG(substr(decode(ai.item_long_nm, 'C45255',decode( cai.nci_cncpt_val,null, ai.item_desc, ai.item_desc || '::' || cai.NCI_CNCPT_VAL),ai.item_desc),1, 750), '_')WITHIN GROUP (ORDER by cai.nci_ord desc) as CNCPT_DEF,
 LISTAGG(decode(ai.item_long_nm, 'C45255',decode( cai.nci_cncpt_val,null, ai.item_long_nm, ai.item_long_nm || '::' || cai.NCI_CNCPT_VAL),ai.item_long_nm), ':')WITHIN GROUP (ORDER by cai.nci_ord desc) as CNCPT_CD_INT
from cncpt_admin_item cai, vw_cncpt_all_2 ai where ai.item_id = cai.cncpt_item_id and ai.ver_nr = cai.cncpt_ver_nr
group by cai.item_id, cai.ver_nr) b
where ai.item_id = b.item_id (+) and ai.ver_nr = b.ver_nr (+)
and ai.admin_item_typ_id in (7);
commit;
...
insert into onedata_ra.nci_admin_item_ext (item_id, ver_nr) select item_id, ver_nr from onedata_ra.admin_item minus select item_id, ver_nr from onedata_ra.nci_admin_item_ext
commit;
...
*/
--ENABLE after decision
nci_util.creategennameInc;
...
commit;
-- generate model map statistics


execute immediate 'alter table admin_item disable all triggers';

 for cur in (Select * from nci_mdl_map) loop
          nci_codemap_mm.updStatsModelMap(cur.item_id, cur.ver_nr);
end loop;

-- generate model statistics

 for cur in (Select * from nci_mdl) loop
          nci_codemap.updStatsModel(cur.item_id, cur.ver_nr);
end loop;
...
DBMS_MVIEW.REFRESH('MVW_CSI_FORM_NODE_DE_REL');
...
DBMS_MVIEW.REFRESH('VW_NCI_DE_HORT_EXPANDED');
...
DBMS_MVIEW.REFRESH('VW_PRMRY_OC_CNCPT');
DBMS_MVIEW.REFRESH('VW_PRMRY_OC_CNCPT_REL');
...
for cur in (select item_id, ver_nr from admin_item where admin_item_typ_id = 4 and lst_upd_dt > sysdate -1) loop
update NCI_ADMIN_ITEM_EXT e set ( USED_BY, CS_CONCAT, CSI_CONCAT)=
(select a.used_by, b.CSI,b.CSI
from  admin_item ai,
(SELECT item_id, ver_nr, LISTAGG(item_nm, ',') WITHIN GROUP (ORDER by ITEM_ID) AS USED_BY
FROM (select distinct ub.item_id,ub.ver_nr, c.item_nm from vw_nci_used_by ub, vw_cntxt c where ub.cntxt_item_id = c.item_id and ub.cntxt_ver_nr = c.ver_nr and ub.owned_by=0 
and ub.item_id = cur.item_id and ub.ver_nr = cur.ver_nr)
GROUP BY item_id, ver_nr) a,
(SELECT item_id, ver_nr, LISTAGG(item_nm, ',') WITHIN GROUP (ORDER by ITEM_ID) AS CSI
FROM (select distinct r.c_item_id item_id,r.c_item_ver_nr ver_nr, x.item_nm from vw_CLSFCTN_SCHM_ITEM x, NCI_ADMIN_ITEM_REL r
where  x.item_id = r.p_item_id and x.ver_nr = r.p_item_ver_nr and r.rel_typ_id = 65 and x.CS_item_id = 10466051 and r.c_item_id = cur.item_id and r.c_item_ver_nr = cur.ver_nr) group by item_id, ver_nr) b
where ai.item_id = a.item_id (+) and ai.ver_nr = a.ver_nr(+)
and  ai.item_id = b.item_id (+) and ai.ver_nr = b.ver_nr(+)
...
hookInput        t_hookInput;
  ...
  if (ihook.getColumnValue(row_ori,'REF_TYP_ID') = 80 or ihook.getColumnOldValue(row_ori,'REF_TYP_ID') = 80) then
    --jira 3612: allow super curator to update context for PQT
    if (ihook.getColumnValue(row_ori, 'NCI_CNTXT_ITEM_ID') != ihook.getColumnOldValue(row_ori,'NCI_CNTXT_ITEM_ID')) then
        --raise_application_error(-20000, ihook.getColumnValue(row_ori,'REF_ID'));
        update ref set nci_cntxt_item_id = ihook.getColumnValue(row_ori, 'NCI_CNTXT_ITEM_ID') where ref_id = ihook.getColumnValue(row_ori, 'REF_ID');
        update ref set nci_cntxt_ver_nr = ihook.getColumnValue(row_ori, 'NCI_CNTXT_VER_NR') where ref_id = ihook.getColumnValue(row_ori, 'REF_ID');
        commit;
    else
...
action t_actionRowset;
  ...
  BEGIN
    ...
    row_ori := hookInput.originalRowset.rowset (1);
/*
if (nvl(ihook.getColumnValue(row_ori, 'SRC_PV'),'xx') <> nvl(ihook.getColumnOldValue(row_ori, 'SRC_PV'),'xx')  or 
nvl(ihook.getColumnValue(row_ori, 'TGT_PV'),'xx') <> nvl(ihook.getColumnOldValue(row_ori, 'TGT_PV'),'xx') ) then 
    ihook.setColumnValue(row_ori, 'MAP_DEG', 120 );                     
end if;
*/
  
  rows := t_rows();rows.EXTEND;
            rows (rows.LAST) := row_ori;
            action :=                t_actionrowset (rows, 'Model Map Characteristic Values',  2,  0,   'update');
            actions.EXTEND;
            actions (actions.LAST) := action;
...
BEGIN
  ...
  if (hookinput.originalRowset.tablename = 'ADMIN_ITEM') then
    ...
    if (ihook.getColumnValue(row_ori, 'UNTL_DT') is not null and ihook.getColumnValue(row_ori, 'UNTL_DT') <= nvl(ihook.getColumnValue(row_ori, 'EFF_DT'), sysdate) ) then
         if (ihook.getColumnValue(row_ori, 'ADMIN_ITEM_TYP_ID') = 54) then
            raise_application_error(-20000, 'Create Date and End date cannot be the same. Please use "Delete Form" command or change the end date');
         else
            raise_application_error(-20000, 'Expiration date has to be the same or greater than Effective date.');
        end if;
         return;
...
-- Suspect code
 ...
 --   raise_application_error (-20000, 'here');
    ...
    /*
    j:= 0;msg :='ERROR: Model cannot be Released. Some Characteristics with Character/Text/String datatype do not have a required Max Length: ';
    ...
    and ihook.getColumnValue(row_ori, 'ADMIN_STUS_ID') <> ihook.getColumnOldValue(row_ori, 'ADMIN_STUS_ID')) then
    for cur in (select MEC_LONG_NM from nci_mdl_elmnt_char mec, nci_mdl_elmnt me where me.mdl_item_id = v_item_id and me.mdl_item_ver_nr = v_ver_nr 
    ...
    j:= j+1;
    msg := substr(msg || cur.MEC_LONG_NM || ',',1,4000);
    
    end loop;
            end if;
    
    if (j > 0) then
               raise_application_error(-20000, substr(msg, 1, length(msg)-1));
                return;
    end if;
    */
       if (ihook.getColumnValue (row_ori, 'ADMIN_ITEM_TYP_ID')= 57 and ihook.getColumnValue(row_ori, 'ADMIN_STUS_ID') = 75
...
-- Released DE cannot have a  DEC and VD
  ...
  --     raise_application_error(-20000,'Here');
    ...
    THEN
        ...
        THEN
        --jira 3810
            IF (nci_11179_2.isDECnmSysGen(v_item_id, v_ver_nr, ihook.getColumnOldValue (row_ori, 'OBJ_CLS_ITEM_ID'), ihook.getColumnOldValue (row_ori, 'OBJ_CLS_VER_NR'),ihook.getColumnOldValue (row_ori, 'PROP_ITEM_ID'), ihook.getColumnOldValue (row_ori, 'PROP_VER_NR')) = true) then
...
for cur in (select * from vw_cntxt where item_id = ihook.getColumnValue(row_ori, 'CNTXT_ITEM_ID')and ver_nr = ihook.getColumnValue(row_ori, 'CNTXT_VER_NR') and admin_stus_nm_dn like '%RETIRED%') loop
 raise_application_error(-20000, 'Cannot select Context that is retired.');
    return;
end loop;
   -- end if;
  V_DATA_OUT := IHOOK.GETHOOKOUTPUT (HOOKOUTPUT);
...
if (ihook.getColumnValue(row_ori, 'ITEM_ID') is not null and ihook.getColumnValue(row_ori, 'VER_NR') is not null) then
    select ITEM_NM, ADMIN_ITEM_TYP_ID into v_long_nm, v_item_typ from admin_item ai where ai.item_id = ihook.getColumnValue(row_ori, 'ITEM_ID') and ai.ver_nr = ihook.getColumnValue(row_ori, 'VER_NR');
    if (ihook.getColumnValue(row_ori, 'SPEC_LONG_NM') is null OR ihook.getColumnValue(row_ori, 'SPEC_LONG_NM') = '') then
    -- raise_application_error(-20000,'Ai Long name');
    ...
    if (ihook.getColumnValue(row_ori, 'ADMIN_ITEM_TYP_ID') is null) then
        ihook.setColumnValue(row_ori, 'ADMIN_ITEM_TYP_ID', v_item_typ);
    end if;
end if;
...
as
    ...
    --array array_t := array_t('PERM_VAL_NM','CNCPT_CONCAT_STR_1','BTCH_NM','BTCH_USR_NM','VW_SPEC_DEF', 'ITEM_1_ID','ITEM_1_NM', 'VM_ALT_NM', 'CMNTS_DESC_TXT');
    array array_t := array_t('PERM_VAL_NM');
BEGIN
    /* 
    columns to trim
    Batch Name  btch_nm
    Batch User  btch_user_nm
    VD ID
    VD Long Name
    PV  perm_val_nm
    Imported VM String  cncpt_concat_str_1
    VM String Type
    Generated VM Definition
    Alt Name
    comments
    vm defintion
    alt name lang
...
--        if (ihook.getColumnValue(row_ori, array(i)) is not null) then
--           -- ihook.setColumnValue(row_ori, array(i), LTrim(ihook.getColumnValue(row_ori, array(i))));
--           -- ihook.setColumnValue(row_ori, 'PERM_VAL_NM', Trim(ihook.getColumnValue(row_ori, 'PERM_VAL_NM')));
--           ihook.setColumnValue(row_ori, 'PERM_VAL_NM', REGEXP_REPLACE ( REGEXP_REPLACE ( ihook.getColumnValue(row_ori, 'PERM_VAL_NM')
--		    	 		, '^\s*(.+?)\s*$'
--					, '\1'
--					)
--		       , '\\t|\\n'
...
--        end if;
--    end loop;
            --  ihook.setColumnValue(row_ori, 'PERM_VAL_NM', replace(ihook.getColumnValue(row_ori, 'PERM_VAL_NM'), '\u001A', ''));
             -- ihook.setColumnValue(row_ori, 'PERM_VAL_NM', replace(ihook.getColumnValue(row_ori, 'PERM_VAL_NM'), '\u0010', ''));
              ihook.setColumnValue(row_ori, 'PERM_VAL_NM', replace(replace(ihook.getColumnValue(row_ori, 'PERM_VAL_NM'), chr(10),''),chr(13), ''));
            ihook.setColumnValue(row_ori, 'PERM_VAL_NM', REGEXP_REPLACE ( REGEXP_REPLACE ( ihook.getColumnValue(row_ori, 'PERM_VAL_NM')
		    	 		, '^\s*(.+?)\s*$'
					, '\1'
					)
  ...
  --  ihook.setColumnValue(row_ori, 'PERM_VAL_NM', 'TESTMN');
      rows := t_rows();

    rows.EXTEND;
    rows (rows.LAST) := row_ori;
    action := t_actionrowset (rows,
                                'PV VM Import',
                                2,
                                0,
                                'update');
    ...
    --ihook.setColumnValue(row_ori, 'PERM_VAL_NM', LTrim(ihook.getColumnValue(row_ori, 'PERM_VAL_NM')));
END;
END;
/