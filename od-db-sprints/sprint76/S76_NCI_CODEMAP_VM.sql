...
procedure spValidateValueMapImport ( v_data_in in clob, v_data_out out clob, v_user_id in varchar2);
 ...
 procedure spResyncValueMap ( v_data_in in clob, v_data_out out clob, v_user_id in varchar2); -- sync with new rule ids
...
procedure spResyncValueMap ( v_data_in in clob, v_data_out out clob, v_user_id in varchar2)
as
...
deleteValueMapSync(v_id, v_ver_Nr);      
updValueMapRuleStrMain (v_id , v_ver_nr );
hookoutput.message := 'Value Maps rules synchronised for '|| v_id || 'v'|| v_ver_nr;
...
begin
 ...
 --   raise_application_error(-20000,output_rows.count);
    ...
    v_temp := 0;
     ...
     decode(ihook.getColumnValue(output_rows(i),'1'), null, null, nvl(ihook.getColumnValue(output_rows(i),'LBL_1'),ihook.getColumnValue(output_rows(i),'Source Concept Name'))),
        decode(ihook.getColumnValue(output_rows(i),'2'), null, null, nvl(ihook.getColumnValue(output_rows(i),'LBL_2'),ihook.getColumnValue(output_rows(i),'Target Concept Name'))),
...
hookInput        t_hookInput;
  ...
  BEGIN
   ...
   if ( ihook.getColumnValue(row_ori, 'MAP_DEG')  in (86,87) and ihook.getColumnValue(row_ori, 'MAP_DEG') <> ihook.getColumnOldValue(row_ori, 'MAP_DEG') ) then
    ...
    --raise_application_error(-20000, 'Here' || ihook.getColumnValue(row_ori,'MDL_MAP_VER_NR'));
...
begin
  ...
  v_id := ihook.getColumnValue(row_ori,'ITEM_ID') ;
  v_ver_nr := ihook.getColumnValue(row_ori,'VER_NR') ;
  -- Tracker 3947 - only generate mapping for 
  -- enum-to-enum
  -- Enum-by-Ref --> Enum 
 ...
 hookoutput.question := nci_form_curator.getProceedQuestion ('Proceed', 'All rules except Value Mapping Types "Derived From" and "Ignore" will be regenerated. Please confirm value map generation for: ' || v_id || 'v' || v_ver_nr);
  ...
  --  and (upper(SOURCE_DOMAIN_TYPE)= 'ENUMERATED' or upper(TARGET_DOMAIN_TYPE)='ENUMERATED')and src_mec_id is not null and tgt_mec_id is not null) loop
    and upper(SOURCE_DOMAIN_TYPE) in ( 'ENUMERATED','ENUMERATED BY REFERENCE') and upper(TARGET_DOMAIN_TYPE) = 'ENUMERATED'
   -- and upper(TARGET_DOMAIN_TYPE)='ENUMERATED')
    and src_mec_id is not null and tgt_mec_id is not null and source_cde_item_id is not null and target_cde_item_id is not null
  ...
  --  raise_application_error(-20000, v_src_alt_nm_typ);
    ...
    v_temp := 0;
        ...
        decode(ihook.getColumnValue(output_rows(i),'1'), null, null, nvl(ihook.getColumnValue(output_rows(i),'LBL_1'),ihook.getColumnValue(output_rows(i),'Source Concept Name'))),
        decode(ihook.getColumnValue(output_rows(i),'2'), null, null, nvl(ihook.getColumnValue(output_rows(i),'LBL_2'),ihook.getColumnValue(output_rows(i),'Target Concept Name'))),
        --,ihook.getColumnValue(output_rows(i),'LBL_2'), 
        decode(ihook.getColumnValue(output_rows(i),'1'),null,130,decode(ihook.getColumnValue(output_rows(i),'2'),null,130,86)),
...
v_st_ts := systimestamp();
  ...
  and CTL_VAL_STUS in ( 'VALIDATED') and BTCH_SEQ_NBR = ihook.getColumnValue(row_ori, 'BTCH_SEQ_NBR') order by BTCH_SEQ_NBR) loop
     
        ihook.setcolumnValue(row_ori, 'MECVM_ID', nvl(cur1.MECVM_ID,cur1.STG_MECVM_ID));
    ...
    /*    ihook.setColumnValue( row_ori,  'SRC_CDE_ITEM_ID' ,ihook.getColumnValue(row_ori, 'SOURCE_CDE_ITEM_ID'));
        ...
        --ihook.setColumnValue(row_ori, 'IMP_SEQ_NBR', ihook.getColumnValue(row_ori, 'BTCH_SEQ_NBR'));
*/
        if (v_temp= 0) then
        v_add := v_add + 1;
          --  rowsadd.extend;   rowsadd(rowsadd.last) := row_ori;
            insert into nci_mec_val_map (MECVM_ID, SRC_MEC_ID, TGT_MEC_ID, MDL_MAP_ITEM_ID, MDL_MAP_VER_NR, SRC_PV, TGT_PV, VM_CNCPT_CD, VM_CNCPT_NM,
            PROV_ORG_ID, PROV_CNTCT_ID, PROV_RSN_TXT, PROV_TYP_RVW_TXT, PROV_RVW_DT, PROV_APRV_DT, SRC_LBL, TGT_LBL,
 ...
 --   raise_application_error(-20000,' Add');
      ...
      v_upd := v_upd + 1;
            update nci_mec_val_map set ( SRC_MEC_ID, TGT_MEC_ID, SRC_PV, TGT_PV, VM_CNCPT_CD, VM_CNCPT_NM,
            PROV_ORG_ID, PROV_CNTCT_ID, PROV_RSN_TXT, PROV_TYP_RVW_TXT, PROV_RVW_DT, PROV_APRV_DT, SRC_LBL, TGT_LBL,
            PROV_NOTES, MAP_DEG, MECM_ID, SRC_VM_CNCPT_CD, SRC_VM_CNCPT_NM, TGT_VM_CNCPT_CD, TGT_VM_CNCPT_NM, SRC_VD_ITEM_ID, SRC_VD_VER_NR, 
            TGT_VD_ITEM_ID, TGT_VD_VER_NR, SRC_CDE_ITEM_ID, SRC_CDE_VER_NR, TGT_CDE_ITEM_ID, TGT_CDE_VER_NR, MECM_ID_DESC, 
            CMNTS_DESC_TXT, SORT_ORD)=
            ...
            cur1.TARGET_VD_ITEM_ID, cur1.TARGET_VD_VER, cur1.SOURCE_CDE_ITEM_ID, cur1.SOURCE_CDE_VER, cur1.SOURCE_CDE_ITEM_ID, cur1.TARGET_CDE_VER, 
            cur1.MECM_ID_DESC, 
            cur1.CMNTS_DESC_TXT, cur1.SORT_ORD from dual) where mecvm_id = nvl(cur1.MECVM_ID,cur1.STG_MECVM_ID);
 --
  ...
  --  ihook.setColumnValue(row_ori,'CTL_VAL_STUS','UPDATED');
    update nci_stg_mec_val_map set CTL_VAL_STUS = 'UPDATED' where STG_MECVM_ID=cur1.STG_MECVM_ID;
...
*/
 ...
 hookoutput.message:= 'New mapping inserted: ' || v_add || ' Mapping updated: ' || v_upd;
    V_DATA_OUT := IHOOK.GETHOOKOUTPUT (HOOKOUTPUT);
 --nci_util.debugHook('GENERAL',v_data_out);
end;
   ...
   procedure spUpdateValueMapImportOld( v_data_in in clob, v_data_out out clob, v_user_id in varchar2)
   as
   hookInput t_hookInput;
  hookOutput t_hookOutput := t_hookOutput();
   actions t_actions := t_actions();
...
v_st_ts := systimestamp();
  for cur1 in (Select * from nci_stg_mec_val_map where mdl_map_item_id = v_mm_id and mdl_map_ver_nr = v_mm_ver 
  and BTCH_NM = ihook.getColumnValue(row_ori, 'BTCH_NM') and BTCH_USR_NM =  ihook.getColumnValue(row_ori, 'BTCH_USR_NM')
  and CTL_VAL_STUS in ( 'IMPORTED','ERROR','VALIDATED') and BTCH_SEQ_NBR = ihook.getColumnValue(row_ori, 'BTCH_SEQ_NBR') order by BTCH_SEQ_NBR) loop
      v_valid := true;
      v_val_stus_msg := '';
      ihook.setColumnValue(row_ori, 'MECVM_ID', cur1.MECVM_ID);
...
/*jira 4160: add more mapping type validations
130: not mapped yet
120: derived from
131: ignore
*/
    if (nvl(cur1.map_deg,0) not in  (120,131,130) ) then
        v_valid := false;
        v_val_stus_msg := v_val_stus_msg ||'Mapping Type can only be Derived From, Ignore, or Not Mapped Yet.'|| chr(13);
...
--        v_val_stus_msg := v_val_stus_msg || 'Both Source and Target Domain Types are enumerated.';
  --  elsif (nvl(cur1.map_deg,0) = 130) then
  --  raise_application_error(-20001, 'test mapping type: ' || ihook.getColumnValue(row_ori,'SRC_PV') || ' ' || ihook.getColumnValue(row_ori,'TGT_PV'));
    elsif (nvl(cur1.map_deg,0) = 130 and ihook.getColumnValue(row_ori,'SRC_PV') is not null and ihook.getColumnValue(row_ori,'TGT_PV') is not null) then
        v_valid := false;
      --  raise_application_error(-20001, 'test mapping type: ' || ihook.getColumnValue(row_ori,'SRC_PV') || ' ' || ihook.getColumnValue(row_ori,'TGT_PV'));
        v_val_stus_msg := v_val_stus_msg || 'Both Source and Target PVs are mapped. Please update mapping type.';
--    elsif (nvl(cur1.map_deg,0) = 130 and ihook.getColumnValue(row_ori,'SOURCE_DOMAIN_TYPE') not in (null, '') and ihook.getColumnValue(row_ori,'TARGET_DOMAIN_TYPE') not in (null, '')) then
...
--        v_valid := false;
--        v_val_stus_msg := v_val_stus_msg || 'Both Source and Target Domain Types are mapped.';
    end if;
    ...
    /*
     ...
     if (v_valid = true) then -- check for duplicate
        ...
        and  cur1.mecvm_id is null and src_pv = cur1.SRC_PV 
           ...
           v_valid := false;
            v_val_stus_msg := v_val_stus_msg ||'Duplicate  Value Map Rule ID: '|| cur.MECVM_ID ||  chr(13);
       end loop;
     end if;
*/
  ...
  if (v_Valid = true) then 
     ...
     -- raise_application_error(-20000,'1');
      ...
      for cur in (Select * from vw_nci_mec where mec_id = ihook.getColumnValue(row_ori,'SRC_MEC_ID') and  val_dom_typ_desc= 'Enumerated' and cur1.SRC_PV is not null) loop
    ...
    if (v_found = true and ihook.getColumnValue(row_ori, 'MECVM_ID') is null) then
        v_val_stus_msg := v_val_stus_msg ||'Source PV for enumerated Value Domain found. Duplicates cannot be inserted. Please import with Rule ID to update' ||  chr(13);
        v_valid := false;
    end if;
    if (v_found = true and ihook.getColumnValue(row_ori, 'MECVM_ID') is not null) then
    -- check if Rule ID is valid for Source MEC
    select count(*) into v_temp from nci_mec_val_map where mecvm_id = ihook.getColumnValue(row_ori, 'MECVM_ID') and src_mec_id = ihook.getColumnValue(row_ori, 'SRC_MEC_ID')
    and upper(src_pv) = upper(cur1.SRC_PV);
    if (v_temp = 0) then
        v_val_stus_msg := v_val_stus_msg ||' Source MEC/Target MEC/Source PV is found with imported rule ID, but the Rule ID does not match.' ||  chr(13);
        v_valid := false;
    else
        ihook.setColumnValue(row_ori, 'UPD_TYP','U');
        end if;
    end if;
      end loop;
      end if;
  ...
  if (v_Valid = true) then 
      ...
      and nvl(src_pv,0) = nvl(cur1.src_pv,0) and nvl(tgt_pv,0) = nvl(cur1.tgt_pv,0)
    --  and src_mec_id = ihook.getColumnValue(row_ori,'SRC_MEC_ID') and tgt_mec_id = ihook.getColumnValue(row_ori,'TGT_MEC_ID') 
      and src_mec_id = cur1.SRC_MEC_ID and tgt_mec_id = cur1.tgt_mec_id 
      and mecvm_id <> cur1.mecvm_id) loop
        v_val_stus_msg := v_val_stus_msg ||'Duplicate with Value Map Rule ID Test: ' ||  curint.mecvm_id ||  ihook.getColumnValue(row_ori,'MECVM_ID') || chr(13);
        v_valid := false;
 ...
 -- v_valid := true;
  ...
  for cur in (Select * from vw_nci_mec where mec_id = ihook.getColumnValue(row_ori,'TGT_MEC_ID') and val_dom_typ_desc = 'Enumerated' and cur1.TGT_PV is not null) loop
...
--     rows.extend;   rows(rows.last) := row_ori;
  ...
  update nci_stg_mec_val_map x set CTL_VAL_STUS= 'ERROR',
   CTL_VAL_MSG= 'Duplicate found in import batch.'
   where  mdl_map_item_id = v_mm_id and mdl_map_ver_nr = v_mm_ver 
  and BTCH_NM = ihook.getColumnValue(row_ori, 'BTCH_NM') and BTCH_USR_NM =  ihook.getColumnValue(row_ori, 'BTCH_USR_NM')
  and CTL_VAL_STUS in ( 'VALIDATED') and (SRC_MEC_ID, TGT_MEC_ID, SRC_PV, TGT_PV) in
  (select SRC_MEC_ID, TGT_MEC_ID, SRC_PV, TGT_PV from nci_stg_mec_val_map y where
   mdl_map_item_id = v_mm_id and mdl_map_ver_nr = v_mm_ver 
  and BTCH_NM = ihook.getColumnValue(row_ori, 'BTCH_NM') and BTCH_USR_NM =  ihook.getColumnValue(row_ori, 'BTCH_USR_NM')
  and  CTL_VAL_STUS in ( 'VALIDATED')
  and y.BTCH_SEQ_NBR < x.BTCH_SEQ_NBR);
  commit;
...
procedure deleteValueMapSync (v_mm_id number, v_mm_Ver number) as
i integer;
begin
delete from nci_mec_val_map where mdl_map_item_id = v_mm_id and mdl_map_ver_nr =v_mm_ver
and (src_mec_id, tgt_mec_id) not in (Select distinct src_mec_id, tgt_mec_id from nci_mec_map where 
mdl_map_item_id = v_mm_id and mdl_map_ver_nr =v_mm_ver and src_mec_id is not null and tgt_mec_id is not null);
commit;
delete from onedata_ra.nci_mec_val_map where mdl_map_item_id = v_mm_id and mdl_map_ver_nr =v_mm_ver
and (src_mec_id, tgt_mec_id) not in (Select distinct src_mec_id, tgt_mec_id from nci_mec_map where 
mdl_map_item_id = v_mm_id and mdl_map_ver_nr =v_mm_ver and src_mec_id is not null and tgt_mec_id is not null);
commit;
