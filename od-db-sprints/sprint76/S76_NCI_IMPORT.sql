...
procedure validateVersionRow (row_ori in out t_row, v_typ in integer, v_item_id in number, v_ver_nr in number, v_val_ind in out boolean) ;
...
procedure spCreateVerValVDImport (v_data_in in clob, v_data_out out clob, v_usr_id in varchar2, v_mode in varchar2);
...
function getVersionList (v_admin_item_typ_id number, v_item_id number) return varchar2;
procedure spSetLatestVersion (v_data_in in clob, v_data_out out clob, v_usr_id in varchar2, v_mode in varchar2);
END;
/
...
begin
   ...
   -- raise_application_error(-20000,hookinput.originalRowset.rowset.count);
    for i in 1..hookinput.originalRowset.rowset.count loop
        ...
        if (ihook.getColumnValue (row_ori, 'CTL_VAL_STUS') <> 'PROCESSED') then
            ...
            if (v_mode <> 'V' and ihook.getColumnValue(row_ori, 'CTL_VAL_STUS') <> 'VALIDATED') then
               ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', ihook.getColumnValue(row_ori,'CTL_VAL_MSG') || 'Please validate before create/update' || chr(13));
                v_val_ind := false;
            end if;


            if (v_mode = 'C' and ihook.getColumnValue(row_ori, 'DE_CONC_ITEM_ID') is not null) then
               ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', ihook.getColumnValue(row_ori,'CTL_VAL_MSG') || 'DEC specified. Please run Update DEC.' || chr(13));
                v_val_ind := false;
            end if;
          ...
          if (v_val_ind = true and ihook.getColumnValue(row_ori, 'CNCPT_CONCAT_STR_1') is not null and ihook.getColumnValue(row_ori, 'MOD_NM') = 'I') then
            ...
            if (v_mode  in ('U','N','V','NV') and ihook.getColumnValue(row_ori, 'DE_CONC_ITEM_ID') is null) then
                ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', ihook.getColumnValue(row_ori,'CTL_VAL_MSG') ||  'DEC ID/Ver not specified. Please check selection.' || chr(13));
                v_val_ind := false;
            end if;

            if (v_mode  in ('N','NV') and ihook.getColumnValue(row_ori, 'NEW_VER_NR') is null) then
                ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', ihook.getColumnValue(row_ori,'CTL_VAL_MSG') ||  'New version not specified. Please check selection.' || chr(13));
                v_val_ind := false;
            end if;

        -- create version output string
            versionStr := 'Existing versions: ' || getVersionList(2,ihook.getColumnValue(row_ori, 'DE_CONC_ITEM_ID'));

          if (v_mode in ('N','NV') and ihook.getColumnValue(row_ori, 'NEW_VER_NR') is not null and ihook.getColumnValue(row_ori, 'NEW_VER_NR') <= nvl(ihook.getColumnValue(row_ori, 'DE_CONC_VER_NR'),1)) then
              ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', ihook.getColumnValue(row_ori,'CTL_VAL_MSG') ||  'New version should be greater than current version. ' || versionStr || chr(13));
                v_val_ind := false;
            end if;

       -- check if the new version already exists
          if (v_mode in ('N','NV') and ihook.getColumnValue(row_ori, 'NEW_VER_NR') is not null ) then
          select count(*) into v_cnt from admin_item where item_id = ihook.getColumnValue(row_ori, 'DE_CONC_ITEM_ID') and ver_nr = ihook.getColumnValue(row_ori, 'NEW_VER_NR');
          if (v_cnt = 1) then
                ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', ihook.getColumnValue(row_ori,'CTL_VAL_MSG') ||  'New version already exists. ' || versionStr || chr(13));
                v_val_ind := false;
            end if;
            ...
            if (v_cnt < 1) then
                ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', ihook.getColumnValue(row_ori,'CTL_VAL_MSG') ||  'Specified version is not the latest version. ' || versionStr || chr(13));
                v_val_ind := false;
            end if;
             for cur in (Select max(ver_nr) max_ver_nr from admin_item where item_id = ihook.getColumnValue(row_ori, 'DE_CONC_ITEM_ID') ) loop
                if ( ihook.getColumnValue(row_ori, 'NEW_VER_NR') is  not null and ihook.getColumnValue(row_ori, 'NEW_VER_NR') <=cur.max_ver_nr) then
                v_val_ind := false;
                    --ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', 'WARNING: New Version Number is not the largest version. Max version: ' || cur.max_ver_nr || chr(13));
                    ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', 'ERROR: New Version Number is not the largest version. ' || versionStr || chr(13));
                end if;
            end loop;
            end if;
            ...
            if (v_mode in ('N','NV') and v_stus not like '%RELEASED%') then
                ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', ihook.getColumnValue(row_ori, 'CTL_VAL_MSG') || 'ERROR: Only RELEASED or RELEASED-NON-CMPLNT Items can be versioned.' || chr(13));
                v_val_ind := false;
            end if;
            -- parse concepts
    ...
    --jira 9250: if Generated name is longer than 255, validate with warning message and create substring
        if (length(ihook.getColumnValue(row_ori,'ITEM_1_NM') ||  ' ' || ihook.getColumnValue(row_ori,'ITEM_2_NM')) > 254) THEN
            ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', ihook.getColumnValue(row_ori, 'CTL_VAL_MSG') || 'WARNING: Generated DEC Name exceeds 255 character limit. If Optional DEC Long Name is not specified, resulting DEC Long Name will be truncated.' || chr(13));
            ihook.setColumnValue(row_ori, 'GEN_DE_CONC_NM', substr(ihook.getColumnValue(row_ori,'ITEM_1_NM') ||  ' ' || ihook.getColumnValue(row_ori,'ITEM_2_NM'),0,255) ) ;
        Else
        ...
        if (nvl(ihook.getColumnValue(row_ori, 'DEC_ITEM_NM'),'') = '' AND length(ihook.getColumnValue(row_ori,'ITEM_1_NM') ||  ' ' || ihook.getColumnValue(row_ori,'ITEM_2_NM')) > 254) then
            v_val_ind := false;
            ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', ihook.getColumnValue(row_ori, 'CTL_VAL_MSG') || 'ERROR: Generated DEC Name too long. Please specify DEC Long Name and re-validate.' || chr(13));
            ihook.setColumnValue(row_ori, 'GEN_DE_CONC_NM', substr(ihook.getColumnValue(row_ori,'ITEM_1_NM') ||  ' ' || ihook.getColumnValue(row_ori,'ITEM_2_NM'),0,254) ) ;
          --  raise_application_error(-20001, 'inside' );
        end if;
        ...
        ihook.setColumnValue(row_ori, 'CURRNT_VAL_ITEM_NM', nvl(ihook.getColumnValue(row_ori,'DEC_ITEM_NM') , ihook.getColumnValue(row_ori, 'GEN_DE_CONC_NM')));
        if (v_val_ind = false) then 
                ihook.setColumnValue(row_ori, 'CTL_VAL_STUS', 'ERRORS');
         else
                ihook.setColumnValue(row_ori, 'CTL_VAL_STUS', 'VALIDATED');
 ...
 --   nci_dec_mgmt.createValAIWithConcept(row_ori, 2,6,'V', 'STRING', actions) ;
    ...
    --jira 2183 - created new import validation message column. report post import errors there and do not remove. do not allow validate/create
      if (   ihook.getColumnValue(row_ori, 'CNCPT_1_ITEM_ID_1') is null or ihook.getColumnValue(row_ori, 'CNCPT_2_ITEM_ID_1') is null) then
            ...
            if (   ihook.getColumnValue(row_ori, 'CONC_DOM_ITEM_ID') is null and   ihook.getColumnValue(row_ori, 'IMP_TYP_NM') = 'DEC') then
                ihook.setColumnValue(row_ori, 'CTL_IMPORT_VAL_MSG', ihook.getColumnValue(row_ori, 'CTL_IMPORT_VAL_MSG') || 'ERROR: DEC Conceptual Domain is missing.' || chr(13));
                                  ihook.setColumnValue(row_ori,'CTL_VAL_STUS','ERRORS');
...
--raise_application_error(-2000, 'Concept Parse:' || v_str);
   cnt := nci_11179.getwordcount(v_str);
 -- raise_application_error(-20000, cnt);
  for i in 1..10 loop
                           ihook.setColumnValue(row_ori, 'CNCPT_' || idx  ||'_ITEM_ID_' || i,'');
    ...
    ihook.setColumnValue(row_ori,'CTL_VAL_MSG', '');
     ...
     --   if (ihook.getColumnValue(row_ori, 'VAL_DOM_ITEM_ID') is null and v_val_ind = true and v_mode = 'C') then -- only go thru creating new if not specified
      ...
      if ( v_val_ind = false and v_mode = 'V') then -- only go thru creating new if not specified
        ...
        ihook.setColumnValue(row_ori, 'CURRNT_VAL_ITEM_NM', nvl(ihook.getColumnValue(row_ori, 'VAL_DOM_NM'),ihook.getColumnValue(row_ori, 'ITEM_3_NM')));
    rows.extend; rows(rows.last) := row_ori;
...
as
v_cnt integer;
versionStr varchar2(4000) := '';
begin
  if ( ihook.getColumnValue(row_ori, 'NEW_VER_NR') is null) then
                v_val_ind := false;
                ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', 'ERROR: New Version Number not specified' || chr(13));
    else
    versionStr := 'Existing versions: ' || getVersionList(v_typ, v_item_id);
        end if;


           if ( ihook.getColumnValue(row_ori, 'NEW_VER_NR') is  not null 
           and ihook.getColumnValue(row_ori, 'NEW_VER_NR') <=ihook.getColumnValue(row_ori, 'VAL_DOM_VER_NR')
           and v_typ = 3) then
                v_val_ind := false;
                --ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', 'ERROR: New Version Number has to be greater than current version.' || chr(13) || 'Current Version: ' || ihook.getColumnValue(row_ori, 'VAL_DOM_VER_NR') || chr(13));
                ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', 'ERROR: New Version Number has to be greater than current version.' || chr(13) || versionStr || chr(13));
           end if;

           if ( ihook.getColumnValue(row_ori, 'NEW_VER_NR') is  not null 
           and ihook.getColumnValue(row_ori, 'NEW_VER_NR') <=ihook.getColumnValue(row_ori, 'IMP_CDE_VER_NR')
           and v_typ = 4) then
                v_val_ind := false;
               -- ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', 'ERROR: New Version Number has to be greater than current version.' || chr(13) || 'Current Version: ' || ihook.getColumnValue(row_ori, 'IMP_CDE_VER_NR') || chr(13));
                ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', 'ERROR: New Version Number has to be greater than current version.' || chr(13) || versionStr || chr(13));
           end if;

        for cur in (Select max(ver_nr) max_ver_nr from admin_item where item_id = v_item_id) loop
           if ( ihook.getColumnValue(row_ori, 'NEW_VER_NR') is  not null and ihook.getColumnValue(row_ori, 'NEW_VER_NR') <=cur.max_ver_nr) then
                v_val_ind := false;
              --  ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', 'WARNING: New Version Number is not the largest version. Max version: ' || cur.max_ver_nr || chr(13));
                ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', 'ERROR: New Version Number is not the largest version. ' || versionStr || chr(13));
            ...
            select count(*) into v_cnt from admin_item where item_id =v_item_id and 
            ver_nr =v_ver_nr and currnt_ver_ind = 1 and admin_item_typ_id = v_typ;
            --raise_application_error(-20000,to_char(v_typ) || ' ' || to_char(v_item_id) || ' ' || to_char(v_ver_nr) || ' ' || to_char(v_cnt));
        if (v_cnt = 0 ) then
            v_val_ind := false;
            ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', ihook.getColumnValue(row_ori, 'CTL_VAL_MSG')  || 'ERROR: Invalid Item/Ver combination or Item/Ver is not the latest version.' || chr(13) || versionStr);
        end if;


            if v_val_ind = true then
            select count(*) into v_cnt from admin_item where item_id =v_item_id and 
              ver_nr =v_ver_nr and currnt_ver_ind = 1 and admin_item_typ_id = v_typ and admin_stus_nm_dn like '%RELEASED%';
              if (v_cnt =0 ) then
            v_val_ind := false;
            ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', ihook.getColumnValue(row_ori, 'CTL_VAL_MSG')  || 'ERROR: Only RELEASED or RELEASED-NON-CMPLNT Items can be versioned.'|| chr(13));
            end if;
    ...
    if  ihook.getColumnValue(row_ori, 'NEW_VER_NR') is  not null then
        select count(*) into v_cnt from admin_item where item_id = v_item_id and 
            ver_nr =ihook.getColumnValue(row_ori, 'NEW_VER_NR') and admin_item_typ_id = v_typ;
        if (v_cnt =1 ) then
            v_val_ind := false;
            --ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', ihook.getColumnValue(row_ori, 'CTL_VAL_MSG')  || 'ERROR: Specified new version already exists.' || chr(13) || versionStr);
            ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', 'ERROR: Specified new version already exists.' || chr(13) || versionStr || chr(13));
        end if;
    end if;
end;
...
for i in 1..hookinput.originalRowset.rowset.count loop
    ...
    ihook.setColumnValue(row_ori,'CTL_VAL_MSG', '');
       ...
       validateVersionRow (row_ori,3,ihook.getColumnValue(row_ori, 'VAL_DOM_ITEM_ID'), ihook.getColumnValue(row_ori, 'VAL_DOM_VER_NR'), v_val_ind); 
     if ( v_val_ind = true and v_mode = 'V') then
            nci_vd.spCreateVerValVDImport(row_ori, 'V', actions, v_val_ind, v_usr_id);
    end if;
     --   if (ihook.getColumnValue(row_ori, 'VAL_DOM_ITEM_ID') is null and v_val_ind = true and v_mode = 'C') then -- only go thru creating new if not specified
         if (ihook.getColumnValue(row_ori, 'VAL_DOM_ITEM_ID') is not null and  v_mode = 'CV' and v_val_ind = true) then -- only go thru creating new if not specified
           nci_vd.spCreateVerValVDImport(row_ori, 'CV', actions, v_val_ind, v_usr_id);
            if (v_val_ind = true) then
                iHook.setColumnValue (row_ori, 'CTL_VAL_STUS', 'PROCESSED');
            end if;
        end if;
 ...
 action := t_actionrowset(rows, 'VD Bulk Create Version', 2,10,'update');
        actions.extend;
        actions(actions.last) := action;
        hookoutput.actions := actions;
    V_DATA_OUT := IHOOK.GETHOOKOUTPUT (HOOKOUTPUT);
...
--default vd version to latest if not specified
   ...
   --  raise_application_error(-20000, 'HErer');
        ...
        if (upper(cur.admin_stus_nm_dn) like '%RETIRED%') then
            ...
            if (upper(cur.admin_stus_nm_dn) not like '%RELEASED%') then
                ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', ihook.getColumnValue(row_ori, 'CTL_VAL_MSG') ||  'WARNING: VD is not RELEASED.' || chr(13));
            end if;
        end if;
    end loop;
    ...
    ---- END VD VALIDATION
 --raise_application_error(-20000,'HEre' || ihook.getColumnValue(row_ori, 'CTL_VAL_MSG'));

        if (v_val_ind = true ) then -- X
            for cur in (select * from admin_item where admin_item_typ_id = 4 and item_id = v_item_id and ver_nr = v_ver_nr) loop
                    ...
                    --jira 9500- let users update CDEs that are released but not retired. include a warning message in the CTL_VAL_MSG column
                    if (upper(cur.admin_stus_nm_dn) = 'RELEASED') then
                        --v_val_ind := false;
                        ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', ihook.getColumnValue(row_ori, 'CTL_VAL_MSG') || 'WARNING: CDE is RELEASED. Update may be performed with caution.' || chr(13));
                    end if;
                    /*if (upper(cur.admin_stus_nm_dn) = 'RELEASED') then
                        v_val_ind := false;
                    ...
                    v_admin_notes := nvl(cur.admin_notes,'');
        ...
        if (ihook.getColumnValue(row_ori, 'CDE_ITEM_LONG_NM') is not null) then
          ...
          select count(*) into v_cnt from admin_item where item_id = v_item_id and ver_nr = v_ver_nr and regexp_like(item_long_nm, '(.*)v(.*):(.*)v(.*)');
                if (v_cnt = 0 or cde_nm_ind = false ) then -- not changed
                for cur in (Select item_long_nm from admin_item where item_id = v_item_id and ver_nr = v_ver_nr) loop
                    ihook.setColumnValue(row_ori, 'ITEM_LONG_NM',cur.item_long_nm);
                end loop;
                end if;
                 if (v_cnt = 1 and cde_nm_ind = true ) then -- DEC/VD changed
            ...
            --jira 9374
            ihook.setColumnValue(row_ori, 'CURRNT_VAL_ITEM_NM', ihook.getColumnValue(row_ori, 'ITEM_NM'));
          ...
          --  ihook.setColumnValue(row_ori,'GEN_DE_NM', ihook.getColumnValue(row_ori,'ITEM_NM'));
            ihook.setColumnValue(row_ori,'GEN_DE_NM', substr(v_dec_item_nm || ' ' || v_vd_item_nm, 1, 255));
      ...
      --  raise_application_error(-20000, v_err_Str);
        ...
        if ( v_mode  in ('U') and v_val_ind = true) then  
            ihook.setColumnValue (row_ori, 'ADMIN_NOTES',  ihook.getColumnValue(row_ori,'ADMIN_NOTES') || ' ' || v_admin_notes);
...
for i in 1..hookinput.originalRowset.rowset.count loop
    ...
    if (ihook.getColumnValue(row_ori, 'CTL_VAL_STUS')<> 'PROCESSED') then -- A
        v_val_ind := true;
        ihook.setColumnValue(row_ori,'CTL_VAL_MSG', '');
        v_item_id := ihook.getColumnValue(row_ori, 'IMP_CDE_ITEM_ID');
        v_ver_nr := ihook.getColumnValue(row_ori, 'IMP_CDE_VER_NR'); --reusing existing column instead of creating new one
        cde_nm_ind := false;
        cde_long_nm_ind := false;
      ...
      -- START CDE ID/Version validation
            ...
            if (ihook.getColumnValue(row_ori, 'IMP_CDE_VER_NR') is null or ihook.getColumnValue(row_ori, 'IMP_CDE_VER_NR') = '') then
                if ( ihook.getColumnValue(row_ori, 'IMP_CDE_ITEM_ID') is not null) then
                 for x in (select * from admin_item where admin_item_typ_id = 4 and item_id = ihook.getColumnValue(row_ori, 'IMP_CDE_ITEM_ID') and CURRNT_VER_IND = 1) loop
                        ihook.setColumnValue(row_ori, 'IMP_CDE_VER_NR', x.ver_nr);
                        ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', ihook.getColumnValue(row_ori,'CTL_VAL_MSG') || 'WARNING: CDE Version not specified. Set to latest version: ' || x.ver_nr || chr(13));
                                   ihook.setColumnValue(row_ori, 'CDE_ITEM_ID', v_item_id);

                        ihook.setColumnValue(row_ori, 'CDE_VER_NR', x.ver_nr);
                        v_ver_nr := x.ver_nr;
                    end loop;
                end if;
                if (ihook.getColumnValue(row_ori, 'IMP_CDE_VER_NR') is null or ihook.getColumnValue(row_ori, 'IMP_CDE_VER_NR') = '') then --- not valid
                    ihook.setColumnValue(row_ori, 'CTL_VAL_STUS', 'ERRORS');
                    v_val_ind := false;
                    ihook.setColumnValue(row_ori, 'CTL_VAL_MSG',ihook.getColumnValue(row_ori,'CTL_VAL_MSG') || 'CDE ID or Version missing.' || chr(13));  
                 end if;
            else
                 select count(*) into v_cnt from  admin_item where admin_item_typ_id = 4 and item_id = ihook.getColumnValue(row_ori, 'IMP_CDE_ITEM_ID') and CURRNT_VER_IND = 1 and ver_nr = ihook.getColumnValue(row_ori, 'IMP_CDE_VER_NR') ;
                 if (v_cnt = 1) then 
                        ihook.setColumnValue(row_ori, 'CDE_ITEM_ID', ihook.getColumnValue(row_ori, 'IMP_CDE_ITEM_ID') );
                        ihook.setColumnValue(row_ori, 'CDE_VER_NR', ihook.getColumnValue(row_ori, 'IMP_CDE_VER_NR'));
                else 
                ihook.setColumnValue(row_ori, 'CTL_VAL_STUS', 'ERRORS');
                        v_val_ind := false;
                      ihook.setColumnValue(row_ori, 'CTL_VAL_MSG',ihook.getColumnValue(row_ori,'CTL_VAL_MSG') || 'ERROR: Invalid Item/Ver combination or Item/Ver is not the latest version.' || chr(13) || 'Valid versions: ' || getVersionList(4, v_item_id) || chr(13));  
                end if;
            end if;


        if (v_val_ind = true) then            
       validateVersionRow (row_ori,4,v_item_id,v_ver_nr, v_val_ind); 
       end if;
       ---- START DEC VALIDATION
...
--name validation
        if (v_val_ind = true) then 
            if (ihook.getColumnValue(row_ori, 'DE_CONC_ITEM_ID') != tmp_dec_id or ihook.getColumnValue(row_ori, 'DE_CONC_VER_NR') != tmp_dec_ver_nr 
                or ihook.getColumnValue(row_ori, 'VAL_DOM_ITEM_ID') != tmp_vd_id or ihook.getColumnValue(row_ori, 'VAL_DOM_VER_NR') != tmp_vd_ver_nr) then
                cde_nm_ind := true;
            end if;
            ...
            if (cde_nm_ind = true) then -- DEC or VD changed
                ...
                ihook.setColumnValue(row_ori, 'ITEM_DESC',nvl(ihook.getColumnValue(row_ori, 'CDE_ITEM_DESC'),substr(v_dec_item_def || ':' || v_vd_item_def,1,4000)));
                ihook.setColumnValue(row_ori, 'ITEM_NM',nvl(ihook.getColumnValue(row_ori, 'CDE_ITEM_NM'),substr(v_dec_item_nm || ' ' || v_vd_item_nm, 1, 255)));
                ihook.setColumnValue(row_ori, 'CURRNT_VAL_ITEM_NM',nvl(ihook.getColumnValue(row_ori, 'CDE_ITEM_NM'),substr(v_dec_item_nm || ' ' || v_vd_item_nm, 1, 255)));
            else --DEC/VD not updated, use specified cde details or, if not provided, existing cde details
                select item_nm, item_desc into v_dec_item_nm, v_dec_item_def from admin_item where item_id = v_item_id and ver_nr = v_ver_nr;

                ihook.setColumnValue(row_ori, 'ITEM_DESC',nvl(ihook.getColumnValue(row_ori, 'CDE_ITEM_DESC'),v_dec_item_def));
                ihook.setColumnValue(row_ori, 'CURRNT_VAL_ITEM_NM', nvl(ihook.getColumnValue(row_ori, 'CDE_ITEM_NM'),v_dec_item_nm ));
      ...
      --   ihook.setColumnValue(row_ori,'GEN_DE_NM', ihook.getColumnValue(row_ori,'ITEM_NM'));    
         --jira 9374
        ihook.setColumnValue(row_ori, 'CURRNT_VAL_ITEM_NM', nvl(ihook.getColumnValue(row_ori, 'CDE_ITEM_NM'),ihook.getColumnValue(row_ori, 'ITEM_NM')));
            ihook.setColumnValue(row_ori,'PROCESS_DT_CHAR',TO_CHAR(SYSDATE, 'MM/DD/YY HH24:MI:SS') );
    ...
    -- raise_application_error(-20000, ihook.getColumnValue(row_ori, 'CHNG_DESC_TXT'));
        --       ihook.setColumnValue (row_ori, 'ADMIN_NOTES',  ihook.getColumnValue(row_ori,'ADMIN_NOTES') || ' ' || nvl(cur.admin_notes,''));
            ...
            if (ihook.getColumNValue(row_ori, 'PREF_QUEST_TXT') is null) then
                for cur in (select * from de where item_id = v_item_id and ver_nr = v_ver_nr) loop
                    ihook.setColumnValue (row_ori, 'PREF_QUEST_TXT',  cur.PREF_QUEST_TXT);
                end loop;
            end if;
...
-- Returns a comma separated list of versions for the specified item id and type. Used for error messages when an invalid version is specified.
function getVersionList (v_admin_item_typ_id number, v_item_id number) return varchar2 is
    v_ret_str varchar2(4000);
    v_cnt number;
begin
    v_ret_str := '';
    for cur in (select ver_nr from admin_item where admin_item_typ_id = v_admin_item_typ_id and item_id = v_item_id and nvl(fld_delete, 1) = 0 order by ver_nr desc) loop
        v_ret_str := v_ret_str || cur.ver_nr || ', ';
    end loop;
    v_ret_str := substr(v_ret_str, 1, length(v_ret_str)-2);
    return v_ret_str;
end;

--called from the Bulk Version Imports (CDE,DEC,VD) to set the latest version of the item. The version number is specified in the "Set Latest Version" column of the import file.
procedure spSetLatestVersion (v_data_in in clob, v_data_out out clob, v_usr_id in varchar2, v_mode in varchar2) is
    hookInput           t_hookInput;
    hookOutput           t_hookOutput := t_hookOutput();
    rows      t_rows;
    rows_ori t_rows;
...
for i in 1..hookinput.originalRowset.rowset.count loop
    row_ori := hookInput.originalRowset.rowset(i);
    v_ver_nr := ihook.getColumnValue(row_ori, 'SET_CUR_VER');

    if (v_mode = 'CDE') then
        v_item_id := ihook.getColumnValue(row_ori, 'IMP_CDE_ITEM_ID');
        versionStr := getVersionList(4, v_item_id);
        v_obj := 'CDE Bulk Create Version';
    end if;
    if (v_mode = 'DEC') then
        v_item_id := ihook.getColumnValue(row_ori, 'DE_CONC_ITEM_ID');
        versionStr := getVersionList(2, v_item_id);
        v_obj := 'DEC Bulk Create Version';
    end if;
    if (v_mode = 'VD') then
        v_item_id := ihook.getColumnValue(row_ori, 'VAL_DOM_ITEM_ID');
        versionStr := getVersionList(3, v_item_id);
        v_obj := 'VD Bulk Create Version';
    end if;

    if (ihook.getColumnValue(row_ori, 'CTL_VAL_STUS')<> 'PROCESSED') then -- A
        if (v_ver_nr is null) then
            v_val_ind := false;
           -- raise_application_error(-20000, 'Version not specified. To set the latest version, specify the version number in the ''Set Latest Version'' column.');
            ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', 'ERROR: Version not specified.' || chr(13) || 'To set the latest version, specify the version number in the ''Set Latest Version'' field.' || chr(13) || 'Existing versions: ' || versionStr || chr(13));
        else
            select count(*) into v_cnt from admin_item where item_id = v_item_id and ver_nr = v_ver_nr and CURRNT_VER_IND = 1;
            if (v_cnt = 1) then
                v_val_ind := false;
                ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', 'ERROR: Specified version is already the latest version.' || chr(13) || 'Existing versions: ' || versionStr || chr(13));
            end if;
            select count(*) into v_cnt from admin_item where item_id = v_item_id and ver_nr = v_ver_nr;
            if (nvl(v_cnt,0) = 0) then
                v_val_ind := false;
                ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', 'ERROR: Specified version does not exist.' || chr(13) || 'Existing versions: ' || versionStr || chr(13));
                --raise_application_error(-20000, 'ERROR: Specified version does not exist. Existing versions: ' || versionStr);
            end if;

            -- version exists and is not the latest, set as the latest
            if (v_val_ind = true) then
                row := t_row();
                ihook.setColumnValue(row, 'ITEM_ID', v_item_id);
            ...
            --update the original row to indicate that the version was set as the latest
                ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', 'Latest version set to: ' || v_ver_nr || chr(13));
                ihook.setColumnValue(row_ori, 'CTL_VAL_STUS', 'PROCESSED');
                rows_ori.extend;
                rows_ori(rows_ori.last) := row_ori;

            --update existing latest version to indicate that it is no longer the latest
                row_cur := t_row();
                select item_id, ver_nr into v_item_id, v_ver_nr from admin_item where item_id = v_item_id and CURRNT_VER_IND = 1;
                ihook.setColumnValue(row_cur, 'ITEM_ID', v_item_id);
                ihook.setColumnValue(row_cur, 'VER_NR', v_ver_nr);
                ihook.setColumnValue(row_cur, 'CURRNT_VER_IND', 0);
                rows.extend;
                rows(rows.last) := row_cur;
            end if;
        end if;

       -- if (v_val_ind = true) then
       --     nci_chng_mgmt.spSetLatestVersion(row_ori, actions, v_val_ind, v_item_id, v_ver_nr, v_usr_id);
       -- end if;
    else
        ihook.setColumnValue(row_ori, 'CTL_VAL_MSG', 'Row already PROCESSED');
    end if;
...
action := t_actionrowset(rows, 'Administered Item (No Sequence)', 2,20,'update');
actions.extend;
actions(actions.last) := action;

action := t_actionrowset(rows_ori, v_obj, 2,20,'update');
actions.extend;
actions(actions.last) := action;
