...
procedure valIfEndif (v_item_id number, v_ver_nr number, v_grp_nm varchar2, v_strt_idx in out number, v_end_idx in out number,v_cur_idx in out number, v_err in out integer, rows in out t_rows);
 

procedure genPcodeIfEndif (v_item_id number, v_ver_nr number, v_grp_nm varchar2, v_first_strt_idx in number,v_strt_idx in out number, v_end_idx in number,v_cur_idx in out number, v_pcode in out varchar2, v_iter_nbr in out integer);

procedure    execDeriveTarget (v_item_id in number, v_ver_nr in number);
  procedure spGenerateMapIncrement ( v_data_in in clob, v_data_out out clob, v_user_id in varchar2, v_mode in varchar2);
...
procedure spProvenanceBulkUpdate (v_data_in in clob, v_data_out out clob, v_user_id in varchar2);
 ...
 procedure spPostHookUpdMM( v_data_in in clob, v_data_out out clob, v_user_id in varchar2);
  ...
  procedure    spModelMapFuncValidation(v_item_id in number, v_ver_nr in number, v_open_paren in integer, v_close_paren in integer, rows in out t_rows);
     procedure  spModelMapQueryFuncValidation(v_item_id in number, v_ver_nr in number, rows in out t_rows);
...
begin
    ...
    ihook.setColumnValue(row,'ITEM_ID',1);
      rows := t_rows(); rows.extend;          rows(rows.last) := row;
                 v_rowset := t_rowset(rows, 'Model Mapping (For Bulk Update)', 1, 'NCI_MDL_MAP');
        forms                  := t_forms();
        form1                  := t_form('Model Mapping (For Bulk Update)', 2,1);
        form1.rowset :=v_rowset;
        forms.extend;    forms(forms.last) := form1;
        hookoutput.forms := forms;
        hookoutput.question := nci_11179.getQuestionGeneric('Please specify values. Only attributes where value is selected will be used in bulk update.', 'Update');
    end if;
    if (hookinput.invocationnumber = 1) then   -- second invocation
        ...
        rows := t_rows();
         for i in 1..hookinput.originalrowset.rowset.count loop
                    ...
                    if (ihook.getColumnValue(rowform,'PROV_RVW_DT') is not null) then
                        ihook.setColumnValue(row, 'PROV_RVW_DT', ihook.getColumnValue(rowform,'PROV_RVW_DT'));
                    end if;
                    if (ihook.getColumnValue(rowform,'PROV_APRV_DT') is not null) then
                        ihook.setColumnValue(row, 'PROV_APRV_DT', ihook.getColumnValue(rowform,'PROV_APRV_DT'));
                    end if;
                    if (ihook.getColumnValue(rowform,'PROV_TYP_RVW_TXT') is not null) then
                        ihook.setColumnValue(row, 'PROV_TYP_RVW_TXT', ihook.getColumnValue(rowform,'PROV_TYP_RVW_TXT'));
                    end if;
                    if (ihook.getColumnValue(rowform,'PROV_CNTCT_ID') is not null) then
                        ihook.setColumnValue(row, 'PROV_CNTCT_ID', ihook.getColumnValue(rowform,'PROV_CNTCT_ID'));
                    end if;
                    if (ihook.getColumnValue(rowform,'PROV_ORG_ID') is not null) then
                        ihook.setColumnValue(row, 'PROV_ORG_ID', ihook.getColumnValue(rowform,'PROV_ORG_ID'));
                    end if;
                    if (ihook.getColumnValue(rowform,'PROV_RSN_TXT') is not null) then
                        ihook.setColumnValue(row, 'PROV_RSN_TXT', ihook.getColumnValue(rowform,'PROV_RSN_TXT'));
                    end if;
                        rows.extend;    rows(rows.last) := row;
                end loop;
                if (hookinput.originalrowset.tablename like '%Mapping%') then
                    action := t_actionrowset(rows, 'Model Mapping', 2,1,'update');
              --  raise_application_error(-20000,hookinput.originalrowset.tablename);
                else
                action := t_actionrowset(rows, 'Model', 2,1,'update');
                end if;
            actions.extend;    actions(actions.last) := action;
                    hookoutput.actions := actions;
                    hookoutput.message := 'Bulk Update action completed.';
    end if;
...
raise_application_error (-20000, 'Work in progress.');
 ...
 BEGIN
  ...
  if (hookinput.invocationnumber = 0 or (hookinput.invocationnumber >0 and hookinput.selectedrowset is null)) then
  -- showable rowset
    rows := t_rows();
  ...
  if (hookinput.invocationnumber !=0 and hookinput.selectedrowset is not null ) then
        --raise_application_error(-20001, 'something selected');
        ...
        nci_codemap.spUpdateModelCDEAssocSub (v_mdl_id,v_mdl_ver_nr, v_cnt,'F', v_msg, 0, hookinput, hookoutput);
  --  else
        --raise_application_error(-20001, 'nothing selected');
   --     hookoutput.message := 'No model was selected. Refresh not completed.';
...
BEGIN
 ...
 -- verify that this is either the only version or is not a current version
  ...
  if (hookinput.invocationnumber = 0) then 
        ...
        row := t_row();
        ihook.setColumnValue(row,'Option','Delete only Rules');
        ihook.setColumnValue(row,'ID',4);
        rows.extend; rows(rows.last) := row;
 ...
 if (hookinput.invocationnumber = 1 and hookinput.selectedRowset.rowset.count > 0) then -- only if an option is selected
     ...
     -- Delete value mapping in all cases except 4
     if (v_temp <4) then
...
-- update value map rules to null if option 4 selected
 if (v_temp  in (4)) then
        update nci_mec_val_map set MECM_ID_DESC= null where  mdl_map_item_id = v_item_id and mdl_map_ver_nr = v_ver_nr;
    end if;
...
hookInput        t_hookInput;
 ...
 -- raise_application_error(-20000,actions.count);
    ...
    if (actions.count > 0) then
      ...
      nci_post_hook.spUpdAdminItemAudit  ( ihook.getColumnValue(row_ori,'MDL_MAP_ITEM_ID'), 
      ihook.getColumnValue(row_ori,'MDL_MAP_VER_NR'), v_user_id , actions);
      hookoutput.actions := actions;
 V_DATA_OUT := IHOOK.GETHOOKOUTPUT (HOOKOUTPUT);
end;
...
if (upper(cur1.TGT_FUNC_PARAM)='SOURCE' and ihook.getColumnValue(row_ori, 'SRC_MEC_ID') is  null ) then
v_valid := false;
v_val_stus_msg := 'Parameter is SOURCE but no Source Characteristic specified. ';
end if;
...
begin
 ...
 --raise_application_error(-20000, v_grp_nm || v_strt_idx || ' '|| v_end_idx);
 
 i:= v_strt_idx+1;
 v_err_str := '';
 v_step := 1;
    while ( i <= v_end_idx and v_err = 0) loop
    
    for x in (select   flow_cntrl 
    from  nci_MEC_MAP map
    where   map.MAP_DEG in (86,87,120)
        and map.MDL_MAP_ITEM_ID = v_item_id
        and map.MDL_MAP_VER_NR = v_ver_nr
        and map.mec_map_nm = v_grp_nm and mec_sub_grp_nbr = i) loop
    v_flow_cntl := x.flow_cntrl;
    end loop;
  if (v_flow_cntl is not null) then
    if v_flow_cntl = v_then then
        if (v_step = 1) then
            v_step := 2;
        else 
  ...
  --raise_application_error(-20000,v_step);
    if (v_flow_cntl= v_if and v_step = 1 ) then
        v_err :=1;
        v_err_str :=v_err_str || 'Nested IF not allowed.';
    end if;
    if (v_flow_cntl= v_if and v_step > 1 ) then
       -- v_ret_idx := 0
        valIfEndif (v_item_id, v_ver_nr, v_grp_nm, i,v_end_idx, v_cur_idx, v_err, rows);
        i := v_cur_idx;
    end if;
 end if;  -- if v_flow_cntl is not null
 -- 
  i:= i + 1;
  /*if ( v_step = 1) then 
 ...
 ihook.setColumnValue(row, 'Characteristic Group', v_grp_nm);
...
i number;
 ...
 row t_row;
 v_flow_cntl integer;
 v_ifandor boolean := false;
 v_err integer :=0;
 cur_pcode varchar2(8000);
begin
 -- select obj_key_id into v_if from obj_key where obj_typ_id = 56 and upper(obj_key_Desc) = 'IF';
 --select obj_key_id into v_endif from obj_key where obj_typ_id = 56 and upper(obj_key_Desc) = 'ENDIF';
 --select obj_key_id into v_else from obj_key where obj_typ_id = 56 and upper(obj_key_Desc) = 'ELSE'; 
...
--raise_application_error (-20000, v_grp_nm ||v_end_idx);
 i:= v_strt_idx;
 v_pcode := '';
    while ( i <= v_end_idx ) loop
...
and MAPPING_GROUP_NAME = v_grp_nm and  map.DERIVATION_GROUP_ORDER = i order by DERIVATION_GROUP_ORDER) loop
--ret_from_nested_if := false;
  --  if (cur.flow_control = 'IF' and v_first_if = false) then
    --    v_first_if := true;
   -- elsif cur.flow_control = 'IF' and v_first_if = true then -- nested if
     if (cur.flow_control = 'IF' and i > v_first_strt_idx ) then
     cur_pcode := v_pcode;
       -- raise_application_error(-20000,v_pcode || ' '|| v_cur_idx || ' ' || i);
       v_iter_nbr := v_iter_nbr + 1;
        genPcodeIfEndif (v_item_id , v_ver_nr,v_grp_nm,i,i, v_end_idx,v_cur_idx, v_pcode, v_iter_nbr);
        v_pcode := cur_pcode || v_pcode;
     --   v_first_if := false;
        i:= v_cur_idx-1;
     --  raise_application_error(-20000,v_pcode || ' '|| v_cur_idx || ' ' || i);
        continue;
     --   goto nextidx;
          ...
          if (cur.FLOW_CONTROL ='IF' or cur.OPERATOR in ('AND','OR')) then
              v_pcode := v_pcode || chr(13) ||lpad(' ',v_iter_nbr*4, '  ') ||  nvl(cur.FLOW_CONTROL,'') || ' ' ||  nvl(cur.OPERATOR,'') || ' ' ;
              v_ifandor := true;
            end if;
           if (cur.parenthesis = '(') and cur.target_function is null and v_ifandor= true then
             v_pcode := v_pcode || nvl(cur.PARENTHESIS,'') ;
            end if;
               if (cur.FLOW_CONTROL = 'THEN' or cur.FLOW_CONTROL = 'ELSE') then 
                    v_pcode :=   v_pcode  ||chr(13) ||lpad(' ',v_iter_nbr*4, '  ') ||  nvl(cur.FLOW_CONTROL,'')  || ' ' ;
                    v_ifandor := false;
        ...
        -- if function
             if (cur.target_function is not null) then
              if ( cur.target_function ='EQUALS' and cur.tgt_mec_id is not null) then
                    v_pcode := v_pcode || chr(13) ||  lpad(' ',(v_iter_nbr+1)*4, '  ') || cur.TGT_PHY_NAME || ' = ' ||  nvl(cur.TARGET_FUNCTION_PARAM,cur.SMEC_NM);
                else
                  v_pcode :=v_pcode ||   case when cur.TGT_MEC_ID is not null then chr(13) ||  lpad(' ',(v_iter_nbr+1)*4, '  ') || cur.TGT_PHY_NAME  else '' end  || case when cur.TGT_MEC_ID is not null then ' = ' else '' end  || nvl(getFuncPcode (v_item_id, v_ver_nr, upper(v_grp_nm), cur.target_function_param, i, v_end_idx,
                    cur.target_function_id,v_open_paren, v_close_paren, v_err),'Error');
                end if;
                --    i := i-1;
            end if;
            --else
               v_pcode := v_pcode || ' '|| cur.left_operand || ' ' ;
          
           -- end if;
        
            if ( cur.operand_type is not null) then 
            v_pcode := v_pcode ||  ' ' || cur.OPERAND_TYPE || ' ' || cur.RIGHT_OPERAND || ' ' ;
            end if;
          if (cur.parenthesis = ')' and cur.target_function is null and cur.target_function_param is null and v_ifandor=true) then
             v_pcode := v_pcode || nvl(cur.PARENTHESIS,'') ;
            end if;
           
            if (cur.FLOW_CONTROL = 'ENDIF')  then 
              v_pcode := v_pcode  ||chr(13)  ||lpad(' ',v_iter_nbr*4, '  ') || 'ENDIF'  ; 
               v_cur_idx := i+1;
               v_iter_nbr := v_iter_nbr -1;
                return;
            end if;
...
-- Validation model map as a whole
 ...
 BEGIN
  ...
  v_valid := true;
  v_val_stus_msg := '';
  v_item_id := nvl(ihook.getColumnValue(row_ori,'ITEM_ID'), ihook.getColumnValue(row_ori, 'MDL_MAP_ITEM_ID'));
  v_ver_nr := nvl(ihook.getColumnValue(row_ori,'VER_NR'),ihook.getColumnValue(row_ori,'MDL_MAP_VER_NR')) ;
...
--execute immediate 'alter trigger tr_MEC_MAP_POST disable';
...
where   map.MAP_DEG in (86,87,120)
    and map.MDL_MAP_ITEM_ID = v_item_id
    and map.MDL_MAP_VER_NR = v_ver_nr
    and upper(map.TGT_FUNC_PARAM) ='SOURCE' and map.SRC_MEC_ID is null
 ) loop
 row := t_row();
 ihook.setColumnValue(row,'Rule Type','Consistency');
 ihook.setColumnValue(row, 'Rule Description','SOURCE parameter specified with no Source Characteristic.');
 --ihook.setColumnValue(row, 'Source Element', cur.src_item_phy_obj_nm);
 --ihook.setColumnValue(row, 'Source Characteristic', cur.src_mec_phy_nm);
 --ihook.setColumnValue(row, 'Target Element', cur.tgt_item_phy_obj_nm);
 ihook.setColumnValue(row, 'Rule ID', cur.mecm_id);
...
where   map.MAP_DEG in (86,87,120)
    and map.MDL_MAP_ITEM_ID = v_item_id
    and map.MDL_MAP_VER_NR = v_ver_nr
    and map.SRC_MEC_ID is not null and map.TGT_MEC_ID is not null
    and map.flow_cntrl is null and map.op_id is null and map.paren is null
 ) loop
 row := t_row();
 ihook.setColumnValue(row,'Rule Type','Consistency');
 ihook.setColumnValue(row, 'Rule Description','EQUALS function missing or provide IF/THEN/ELSE');
 --ihook.setColumnValue(row, 'Source Element', cur.src_item_phy_obj_nm);
 --ihook.setColumnValue(row, 'Source Characteristic', cur.src_mec_phy_nm);
 --ihook.setColumnValue(row, 'Target Element', cur.tgt_item_phy_obj_nm);
 ihook.setColumnValue(row, 'Rule ID', cur.mecm_id);
...
--all groups should have equal number of if/then/endif

for curgrp in (
select     map.mec_map_nm, sum(decode(map.flow_cntrl,v_if,1,0)) sum_if,
sum(decode(map.flow_cntrl,v_endif,1,0)) sum_endif,
sum(decode(map.flow_cntrl,v_then,1,0)) sum_then
from  nci_MEC_MAP map
where   map.MAP_DEG in (86,87,120)
    and map.MDL_MAP_ITEM_ID = v_item_id
    and map.MDL_MAP_VER_NR = v_ver_nr
    and map.FLOW_CNTRL is not null
    group by map.mec_map_nm
 ) loop
 if (curgrp.sum_if <> curgrp.sum_endif) or (curgrp.sum_if <> curgrp.sum_then) then
 row := t_row();
 ihook.setColumnValue(row,'Rule Type','Consistency');
 ihook.setColumnValue(row, 'Rule Description','IF/THEN/ENDIF count is incorrect for group.');
 --ihook.setColumnValue(row, 'Source Element', cur.src_item_phy_obj_nm);
 --ihook.setColumnValue(row, 'Source Characteristic', cur.src_mec_phy_nm);
 --ihook.setColumnValue(row, 'Target Element', cur.tgt_item_phy_obj_nm);
 --ihook.setColumnValue(row, 'Rule ID', cur.mecm_id);
...
for curgrp in (
select     map.mec_map_nm, min(mec_sub_grp_nbr) start_grp_nbr
from  nci_MEC_MAP map
where   map.MAP_DEG in (86,87,120)
    and map.MDL_MAP_ITEM_ID = v_item_id
    and map.MDL_MAP_VER_NR = v_ver_nr
    and map.FLOW_CNTRL =v_if
   -- and mec_map_nm = 'Patient.race.detailed.MULTIPLE-SUPPDM'
    group by map.mec_map_nm
 ) loop
 
 v_err := 0;
 
select    max(mec_sub_grp_nbr) into v_end_grp_nbr
from  nci_MEC_MAP map
where   map.MAP_DEG in (86,87,120)
    and map.MDL_MAP_ITEM_ID = v_item_id
    and map.MDL_MAP_VER_NR = v_ver_nr
    and mec_map_nm = curgrp.mec_map_nm;
   -- group by map.mec_map_nm
 
 
 valIfEndif(v_item_id, v_ver_nr, curgrp.mec_Map_nm, curgrp.start_grp_nbr,v_end_grp_nbr,curgrp.start_grp_nbr,v_err, rows);
...
for cur in (
select    sme.item_phy_obj_nm src_me_nm, tme.item_phy_obj_nm tgt_me_nm , sme.item_id, sme.ver_nr
from NCI_MDL_ELMNT_CHAR smec, NCI_MDL_ELMNT tme,NCI_MDL_ELMNT sme,
                NCI_MDL_ELMNT_CHAR tmec,   nci_MEC_MAP map
where  map.SRC_MEC_ID = smec.MEC_ID
    and smec.MDL_ELMNT_ITEM_ID = sme.item_id
    ...
    and map.MDL_MAP_ITEM_ID = v_item_id
    and map.MDL_MAP_VER_NR = v_ver_nr
group by sme.item_phy_obj_nm , tme.item_phy_obj_nm , sme.item_id, sme.ver_nr having sum (decode(smec.pk_ind,1,1,0) ) = 0
 ) loop
 row := t_row();
 temp_str := '';
 for x in (select listagg(MEC_PHY_NM, ',') WITHIN GROUP (ORDER by mec_id) as charlist from nci_mdl_elmnt_char y where MDL_ELMNT_ITEM_ID=cur.item_id and MDL_ELMNT_VER_NR=cur.ver_nr and y.pk_ind =1) loop
 temp_str := x.charlist;
 end loop;
 ihook.setColumnValue(row,'Rule Type','Code Generation');
 ihook.setColumnValue(row, 'Rule Description','Required key not mapped for source element: ' || cur.src_me_nm ||'. Please map one or more of the following source characteristics:' || temp_str);
...
--  Check if CDE has changed.
for cur in (select m.mec_Map_nm, m.mecm_id, m.src_cde_item_id || 'v' ||  m.src_cde_ver_nr src, mec.cde_item_id || 'v'|| mec.cde_ver_nr tgt 
from nci_mec_map m, nci_mdl_elmnt_char mec where m.src_mec_id is not null  and
m.src_mec_id = mec.mec_id
and m.mdl_map_item_id = ihook.getColumnValue(row_ori, 'ITEM_ID') and m.mdl_map_ver_nr = ihook.getColumnValue (row_ori,'VER_NR')
and (nvl(m.src_cde_item_id,0) <> nvl(mec.cde_item_id,0) or  nvl( m.src_cde_ver_nr,0) <>  nvl(mec.cde_ver_nr,0))) loop
 row := t_row();
        ihook.setColumnValue(row,'Rule Type','CDE out of sync ');
        ihook.setColumnValue(row, 'Rule Description','Source CDE on the rule: ' || cur.src ||' different from the CDE attached to the Source Characteristic: '|| cur.tgt );
      --  ihook.setColumnValue(row, 'Rule ID', cur.mecm_id);
        ihook.setColumnValue(row, 'Characteristic Group', cur.mec_map_nm);
        ihook.setColumnValue(row, 'Rule ID', cur.mecm_id);
...
--  Check if CDE has changed.
for cur in (select m.mec_Map_nm, m.mecm_id, m.tgt_cde_item_id || 'v' ||  m.tgt_cde_ver_nr src, mec.cde_item_id || 'v'|| mec.cde_ver_nr tgt from nci_mec_map m, nci_mdl_elmnt_char mec where m.src_mec_id is not null 
and m.mdl_map_item_id = ihook.getColumnValue(row_ori, 'ITEM_ID') and m.mdl_map_ver_nr = ihook.getColumnValue (row_ori,'VER_NR') and
m.tgt_mec_id = mec.mec_id
and (nvl(m.tgt_cde_item_id,0) <> nvl(mec.cde_item_id,0) or  nvl( m.tgt_cde_ver_nr,0) <>  nvl(mec.cde_ver_nr,0) )) loop
 row := t_row();
        ihook.setColumnValue(row,'Rule Type','CDE out of sync ');
        ihook.setColumnValue(row, 'Rule Description','Target CDE on the rule: ' || cur.src ||' different from the CDE attached to the Target Characteristic: ' || cur.tgt);
      --  ihook.setColumnValue(row, 'Rule ID', cur.mecm_id);
        ihook.setColumnValue(row, 'Characteristic Group', cur.mec_map_nm);
        ihook.setColumnValue(row, 'Rule ID', cur.mecm_id);
     ...
     --    else
      --   update nci_mec_map set ctl_val_msg =  ihook.getColumnValue (row_sel, 'Rule Type') || ': ' ||ihook.getColumnValue (row_sel, 'Rule Description')  where mdl_map_item_id =v_item_id and mdl_map_ver_nr = v_ver_nr and
       --  mec_map_nm = ihook.getColumnValue(row_sel, 'Characteristic Group' )  ;
         ...
         spModelMapQueryFuncValidation(v_item_id, v_ver_nr, rows);
      for i in 1..rows.count loop
       row_sel := rows(i);
       if (ihook.getColumnValue(row_sel, 'Rule ID' ) is not null) then
         update nci_mec_map set ctl_val_msg =   ihook.getColumnValue (row_sel, 'Rule Type') || ': ' ||ihook.getColumnValue (row_sel, 'Rule Description')  where mecm_id =ihook.getColumnValue(row_sel, 'Rule ID' )  ;
...
--jira 4041
if (rows.count > 0) then
showRowset := t_showableRowset(rows, 'Issues with Model Map Validation for: ' || ihook.getColumnValue(row_ori, 'ITEM_NM') || '   ' || v_item_id|| 'v' || v_ver_nr || ',  ' || to_char(systimestamp, 'yyyy-mm-dd HH:mm:ss'),4, 'unselectable');
...
--execute immediate 'alter trigger tr_MEC_MAP_POST enable';
...
begin
 ...
 --hookoutput.question := nci_form_curator.getProceedQuestion ('Proceed', 'All rules except Mapping Type "Derived From" and "Ignore" will be regenerated. Please confirm semantic map generation for: ' || v_id || 'v' || v_ver_nr);
  ...
  select obj_key_id into v_nmtyp from obj_key where obj_key_desc = 'Ref Term Short Name' and obj_typ_id = 11;
    rows := t_rows();
    if (hookinput.answerid = 2) then -- selected entity
...
--execute immediate 'alter trigger tr_MEC_MAP_POST disable';
  ...
  if (v_sel_id is null and v_mode = 'C') then
  ...
  if (v_sel_id is not null and v_mode = 'C') then
        delete from NCI_MEC_MAP where MDL_MAP_ITEM_ID = ihook.getColumnValue(row_ori, 'ITEM_ID') and MDL_MAP_VER_NR = ihook.getColumnValue(row_ori, 'VER_NR')
...
*/
        ...
        v_grp_suffix :='';
        if (v_mode = 'I') then
            v_grp_suffix := TO_CHAR(SYSDATE, 'YYYY-MM-DD HH24:MI');
        end if;
    -- Both source and target CDE are present
  k := 1;
  v_prev_grp_nm := v_grp_suffix;
...
and tgt_mec_id is not null) order by me1.ITEM_LONG_NM,me2.ITEM_LONG_NM ) loop
        ...
        v_cur_grp_nm := trim(cur.SRC_ME_ITEM_LONG_NM  ||'.'  || cur.TGT_ME_ITEM_LONG_NM || ' '|| v_grp_suffix);
        if (v_cur_grp_nm <> nvl(v_prev_grp_nm ,'XX') or v_prev_grp_nm = v_grp_suffix) then
...
and tgt_mec_id is not null)) loop
      ...
      --  ihook.setColumnValue(row,'MEC_MAP_NM',cur.SRC_ME_ITEM_LONG_NM  ||'.'  || cur.SRC_MEC_LONG_NM);
       ...
       v_grp_nm := trim(cur.SRC_ME_ITEM_LONG_NM  ||'.'  || cur.TGT_ME_ITEM_LONG_NM ||'.'|| cur.SRC_MEC_LONG_NM || '.'|| cur.TGT_MEC_LONG_NM || ' ' || v_grp_suffix);
    ...
    generateQueryRowSingle(v_grp_nm,'VALUE_MAP', v_func, v_open_paren ,1, row, rows);
     ...
     generateQueryRowSingle(v_grp_nm,'NCIT_CODE', null, null, 5, row, rows);
       ...
       generateQueryRowSingle(v_grp_nm,'SOURCE_TERMINOLOGY: NCIt', null, null, 7, row, rows);
         generateQueryRowSingle(v_grp_nm,'SOURCE_TERM', null, null, 8, row, rows);
...
MDL_MAP_ITEM_ID  = ihook.getColumnValue(row_ori, 'ITEM_ID') and MDL_MAP_VER_NR = ihook.getColumnValue (row_ori,'VER_NR') and tgt_mec_id is not null )) loop
        ...
        ihook.setColumnValue(row,'MAP_DEG',130); -- not mapped
         ihook.setColumnValue(row,'MEC_SUB_GRP_NBR',0);
        ihook.setColumnValue(row,'MEC_MAP_NM',trim(cur.SRC_ME_ITEM_LONG_NM  ||'.'  || cur.SRC_MEC_LONG_NM || ' ' || v_grp_suffix));
               getStdMMComment(row,row_ori);
                ihook.setColumnValue(row,'TGT_CDE_ITEM_ID',cur.TGT_CDE_ITEM_ID);
        ihook.setColumnValue(row,'TGT_CDE_VER_NR',cur.TGT_CDE_VER_NR);
          ihook.setColumnValue(row,'TGT_FUNC_ID','');
...
-- looking for open parenthesis first
   ...
   if (cur.tgt_func_id is not null) then -- another function found
      ...
      --  v_rtn_str := v_rtn_str || cur.TGT_FUNC_PARAM || ',';
         ...
         v_rtn_str := v_rtn_str  || case when cur.tgt_func_param is null then '' else ',' end || getFuncPcode (v_mm_id, v_mm_ver, v_grp_nm, cur.tgt_func_param, i, v_max_idx,
...
trim(map.SRC_ELMNT_PHY_NAME || '.' || map.SRC_PHY_NAME) SRC_PHY_NAME,trim(map.TGT_ELMNT_PHY_NAME || '.'|| map.TGT_PHY_NAME) TGT_PHY_NAME,
 ...
 from VW_MDL_MAP_IMP_TEMPLATE  map, obj_key ob
where map.MODEL_MAP_ID= v_item_id and map.MODEL_MAP_VERSION = v_ver_nr and map.target_function_id = ob.obj_key_id (+)
and MAPPING_GROUP_NAME = curouter.MAPPING_GROUP_NAME and  map.DERIVATION_GROUP_ORDER = j ) loop


--  if (cur.FLOW_CONTROL ='IF' or cur.OPERATOR in ('AND','OR')) then
    if (cur.FLOW_CONTROL ='IF') then
  --      inif := true;
  --raise_application_error(-20000,cur.FLOW_CONTROL || j || curouter.mapping_group_name || curouter.min_grp_ord);
       v_cur_idx := j;
        v_str := '';
        v_strt_idx := j;
        v_iter_nbr := 0;
        genPcodeIfEndif (v_item_id , v_ver_nr,curouter.mapping_group_Name,j, j, curouter.max_grp_ord,v_cur_idx, v_str,v_iter_nbr);
        ...
        and mec_sub_grp_nbr=v_strt_idx;
        commit;
        j := v_cur_idx + 1;
        v_str := '';
    else --- not in IF  
        if (  cur.target_function is not null and cur.tgt_mec_id is not null) then
      --  if (cur.parenthesis <> '(') then
        --    v_Str := v_str || 'Function found with no open parenthesis.';
         --   v_assign := true;
        --else
        v_strt_idx := j;
        --  if (cur.target_function = 'EQUALS') then
         --   v_str := v_str || cur.TGT_PHY_NAME || ' = ' ||
         -- else
...
BEGIN
 ...
 for curgrp in (select mec_map_nm, mecm_id, tgt_func_param, mec_sub_grp_nbr, tgt_func_id , flow_cntrl, left_op from nci_mec_map where mdl_map_item_id = v_item_id and mdl_map_ver_nr = v_ver_nr 
 and tgt_func_id = curfunc.tgt_func_id and paren = v_open_paren  )loop
 --and ctl_val_msg is null ) loop
 --and  mec_map_nm = 'CONDITION_OCCURRENCE.CONDITION.condition_type_concept_id.CONDITION_SOURCE') loop
 v_num_open := 1;
 v_cur_param :=0;
 v_num_close := 0;
 if (curgrp.tgt_func_param is not null or (curgrp.flow_cntrl is not null and curgrp.left_op is not null)) then -- function parameters specified in the same row as ( 
 v_cur_param :=1;
...
AS
row t_row;
v_tgt_vd_typ_id integer;
v_src_vd_typ_id integer;
v_max_word integer;
i integer;
j integer;
v_func_query integer;
v_func_Str varchar2(255);
v_str varchar2(255);
v_func varchar2(255);
v_open_paren integer;
v_close_paren integer;
...
BEGIN
 ...
 -- check to makes sure both source and target MEC have CDE attached.
 ...
 if curgrp.mec_sub_grp_nbr = cur.min_mec_sub_grp_nbr then
    ...
    v_tgt_vd_typ_id  := curgrp.tgt_vd_typ_id;
    v_str := '';
 if (v_src_vd_typ_id is not null and v_tgt_vd_typ_id is not null) then 
            --  src is enum, target is enum
        if (v_src_vd_typ_id =17 and v_tgt_vd_typ_id = 17) then
            v_func_str := 'QUERY';
            v_str := 'VALUE_MAP, SOURCE, SOURCE_PV, TARGET_PV';
        end if;
        --enum/non-enum
        if (v_src_vd_typ_id =17 and v_tgt_vd_typ_id = 18) then
            v_func_str := 'QUERY';
            v_str := 'VALUE_MAP, SOURCE, SOURCE_PV, SOURCE_LABEL';
        end if;
     -- non-enum/enum then 2 rows - use case 3
         if (v_src_vd_typ_id=18 and v_tgt_vd_typ_id = 17) then
            v_func_str := 'QUERY, QUERY';
            v_str := 'VALUE_MAP, NCIT, SOURCE, PREF_TERM_SYN, NCIT_CODE, TARGET_CODE, TARGET_PV';
        end if;
        -- non-enum/Enum by Ref - use case 4
         if (v_src_vd_typ_id =18 and v_tgt_vd_typ_id = 16) then
            v_func_str := 'QUERY, QUERY';
            v_str := 'XWALK, NCIT, SOURCE, PREF_TERM_SYN, NCIT_CODE, SOURCE_TERMINOLOGY*, SOURCE_TERM,TARGET_TERMINOLOGY*, TARGET_*';
        end if;

         -- enum/Enum by Ref
         if (v_src_vd_typ_id =17 and v_tgt_vd_typ_id = 16) then
            v_func_str := 'QUERY, QUERY';
            v_str := 'XWALK, VALUE_MAP, SOURCE, SOURCE_PV, NCIT_CODE, SOURCE_TERMINOLOGY*, SOURCE_CODE,TARGET_TERMINOLOGY*, TARGET_*';
        end if;
            -- Enum by Ref/Enum by Ref
         if (v_src_vd_typ_id =16 and v_tgt_vd_typ_id= 16) then
            v_func_str := 'QUERY';
     ...
     else -- source or target vd type is null
        row :=     t_row();
        ihook.setColumnValue(row,'Rule Type','Function Validation');
        ihook.setColumnValue(row, 'Rule Description','Function QUERY needs both Source and Target Characteristic attached to a CDE.');
        ihook.setColumnValue(row, 'Rule ID', curgrp.mecm_id);
        ihook.setColumnValue(row, 'Characteristic Group', curgrp.mec_map_nm);
        rows.extend;    rows(rows.last) := row;
     end if;
    ...
    if (v_Str <> '') then
        ...
        if (substr(v_sub_str, length(v_sub_Str) -1,1) = '*') then
            ...
            v_sub_str := substr(v_sub_str,1, length(v_sub_Str) -1);
        end if;
        if ( v_src_str <> v_sub_str) then
            row :=     t_row();
            ihook.setColumnValue(row,'Rule Type','Function Parameter');
            ihook.setColumnValue(row, 'Rule Description','Function QUERY  parameter is incorrect. Expected Parameter equals or begins with: ' || v_sub_str || ' Current Parameter: ' || curgrp.tgt_func_param );
            ihook.setColumnValue(row, 'Rule ID', curgrp.mecm_id);
            ihook.setColumnValue(row, 'Characteristic Group', curgrp.mec_map_nm);
            rows.extend;    rows(rows.last) := row;
        end if;
