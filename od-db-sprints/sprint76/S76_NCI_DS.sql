...
END;
/*
The following procedure is for testing CDE Match updates with impacting current code. Copy/paste spDSRun into this procedure.
...
EXCEPTION
    WHEN OTHERS THEN
    raise_application_error(-20001, 'There was an internal processing error. Please report this to your OneData Administrator.');
    --nci_util.debugHook('GENERAL', v_data_out);
end;
     ...
     --v_hdr_id := 194361;
            ...
            select seq_ds_prmtr_id.nextval into v_id from dual;
               ...
               if nvl(v_reg_stus_id,0) <> 0 then
                ...
                ihook.setColumnValue(row, 'MTCH_LMT', v_mtch_lmt);
                ihook.setColumnValue(row, 'BTCH_USR_NM', v_btch_usr_nm);
                ihook.setColumnValue(row, 'BTCH_NM', v_btch_nm);
...
--nci_util.debugHook('GENERAL', v_data_out);
        ...
        --Loop through all rows and insert into new object that triggers IS service on insert
           ...
           /*     if nvl(v_cde_var, 0) <> 0 then
                ...
                --for multi-select
                ihook.setColumnValue(row, 'VARIANT_1_LIST', v_var_str);
...
--raise_application_error(-20000,hookinput.ta
    ...
    if (hookinput.invocationnumber = 1) then
     ...
     if (ihook.getColumnValue(row_sel,'Deletion ID') = 1) then -- batch user
      ...
      if (ihook.getColumnValue(row_sel,'Deletion ID') = 3 or v_obj_nm = 'DS Batch Header') then -- batch user/name
