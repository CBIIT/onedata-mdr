create or replace PACKAGE            nci_codemap AS
 ...
 procedure spCompareAICDE (v_data_in in clob, v_data_out out clob);
  procedure getModelActionNew ( row_ori in out t_row, actions in out t_actions, v_mdl_hdr_id in number);
procedure spCompareModelCDE (v_data_in in clob, v_data_out out clob);
...
/*ihook.setColumnValue(row,'PROV_ORG_ID',ihook.getColumnvalue(row_ori, 'PROV_ORG_ID'));
ihook.setColumnValue(row,'PROV_CNTCT_ID',ihook.getColumnvalue(row_ori, 'PROV_CNTCT_ID'));
ihook.setColumnValue(row,'PROV_RSN_TXT',ihook.getColumnvalue(row_ori, 'PROV_RSN_TXT'));
ihook.setColumnValue(row,'PROV_TYP_RVW_TXT',ihook.getColumnvalue(row_ori, 'PROV_TYP_RVW_TXT'));
ihook.setColumnValue(row,'PROV_RVW_DT',ihook.getColumnvalue(row_ori, 'PROV_RVW_DT'));
ihook.setColumnValue(row,'PROV_APRV_DT',ihook.getColumnvalue(row_ori, 'PROV_APRV_DT'));*/
	
--raise_application_error(-20000, ihook.getColumnValue(row_ori,'PROV_TYP_RVW_TXT'));
...
if (ihook.getColumnvalue(row_ori, 'PROV_ORG_ID') is not null) then
ihook.setColumnValue(row,'PROV_ORG_ID',ihook.getColumnvalue(row_ori, 'PROV_ORG_ID'));
end if;
if (ihook.getColumnvalue(row_ori, 'PROV_CNTCT_ID') is not null) then
ihook.setColumnValue(row,'PROV_CNTCT_ID',ihook.getColumnvalue(row_ori, 'PROV_CNTCT_ID'));
end if;
if (ihook.getColumnvalue(row_ori, 'PROV_RSN_TXT') is not null) then
ihook.setColumnValue(row,'PROV_RSN_TXT',ihook.getColumnvalue(row_ori, 'PROV_RSN_TXT'));
end if;
if (ihook.getColumnvalue(row_ori, 'PROV_TYP_RVW_TXT') is not null) then
ihook.setColumnValue(row,'PROV_TYP_RVW_TXT',ihook.getColumnvalue(row_ori, 'PROV_TYP_RVW_TXT'));
end if;
if (ihook.getColumnvalue(row_ori, 'PROV_RVW_DT') is not null) then
ihook.setColumnValue(row,'PROV_RVW_DT',ihook.getColumnvalue(row_ori, 'PROV_RVW_DT'));
end if;
if (ihook.getColumnvalue(row_ori, 'PROV_APRV_DT') is not null) then
ihook.setColumnValue(row,'PROV_APRV_DT',ihook.getColumnvalue(row_ori, 'PROV_APRV_DT'));
end if;
ihook.setColumnValue(row, 'ITEM_ID', v_mdl_hdr_id);
     rows.extend;
...
ihook.setColumnValue(row,'PROV_ORG_ID',cur.PROV_ORG_ID);
 ...
 */
 ...
 item_id = nci_stg_mdl_elmnt_char.cde_item_id and ver_nr = nci_stg_mdl_elmnt_char.cde_ver_nr )
  ...
  getModelActionNew(row_ori,actions, v_mdl_hdr_id);
...
function getAssociateCDEQuestion  return t_question
is
  question t_question;
  answer t_answer;
  answers t_answers;
...
-- There is no way for a hook step to know what the previous selection was. I am using AnswerId to determine if 50+ then use Name. Else, use code.
 ANSWERS                    := T_ANSWERS();
    ANSWER                     := T_ANSWER(1, 1, 'Update selected.');
    ANSWERS.EXTEND;
    ANSWERS(ANSWERS.LAST) := ANSWER;
    
  QUESTION               := T_QUESTION('CDE/VD/DEC Associations have changed. Select characterisics to refresh. ', ANSWERS);
...
hookinput in t_hookinput, hookoutput in out t_hookoutput)
as
 v_mdl_hdr_id number;
  v_mdl_elmnt_id number;
-- v_cnt integer :=0;
 ...
 -- rows := t_rows();
  ...
  if (v_invocationnumber = 0) then
 showrows := t_rows();
  for curchar in (select mec_id, MEC_PHY_NM, me.ITEM_PHY_OBJ_NM me_phy_nm,me.ITEM_LONG_NM me_long_nm,  mec.cde_item_id , mec.cde_ver_nr
  , de_conc_item_id,de_conc_ver_nr, val_dom_item_id, val_dom_ver_nr 
  from  nci_mdl_elmnt me, nci_mdl_elmnt_char mec where me.MDL_ITEM_ID =v_mdl_id
  and me.MDL_ITEM_VER_NR= v_mdl_ver_nr and me.item_id = mec.MDL_ELMNT_ITEM_ID and me.ver_nr =mec.MDL_ELMNT_VER_NR
  ) loop
  ...
  and csi.cs_item_ver_nr = v_cs_ver_nr and csi.item_id = r.p_item_id and csi.ver_nr = r.p_item_ver_nr and r.rel_typ_id = 65);
    if (v_temp = 1) then
      ...
      --    if (curchar.cde_item_id != curcde.item_id and curchar.cde_item_id is not null and curcde.item_id is not null) then
          if ( curcde.item_id is not null and curchar.cde_item_id is not null and (curcde.item_id <> curchar.cde_item_id or curcde.ver_nr <> curchar.cde_ver_nr)) then -- prompt
          showrow := t_row();
          ihook.setColumnValue(showrow,'Element Physical Name', curchar.me_phy_nm);
          ihook.setColumnValue(showrow,'Characteristic Physical Name', curchar.mec_phy_nm);
  ...
  mec.cde_ver_nr = de.ver_nr and (mec.de_conc_item_id <> de.de_conc_item_id or mec.de_conc_ver_nr <> de.de_conc_ver_nr)) loop
    ...
    --(select cur.de_conc_ITEM_Id, cur.de_conc_VER_NR, cur.chng_desc_txt || 'Updated DEC to ' || cur.de_conc_item_id from dual)
    --where  mec_id = cur.mec_id;
    showrow := t_row();
          ihook.setColumnValue(showrow,'Element Physical Name', cur.me_phy_nm);
          ihook.setColumnValue(showrow,'Characteristic Physical Name', cur.mec_phy_nm);
...
as
 ...
 v_sql varchar2(4000);
    ...
    type t_item_ver is table of admin_item.ver_nr%type;
    type t_item_nm is table of admin_item.item_nm%type;
...
begin
 ...
 --jira 3978: add ability to CDE Match to match using Source VMs -- change column header to reflect update
  ...
  and mec.cde_item_id = ai.item_id and mec.cde_ver_nr = ai.ver_nr) loop

            v_found := false;
            for j in 1..v_tab_item_nm.count loop
                    v_found := v_found or v_tab_item_nm(j)=rec.cde_item_id || 'v' || rec.cde_ver_nr || ':' || rec.item_nm;
            end loop;
            if not v_found then
...
-- last column header: matched CDE id + cde item nm + associated VD id + vd item name
        ...
        v_tab_model_nm(v_tab_model_nm.count) :=  ihook.getColumnValue(row_cur,'ITEM_NM')||': ' || ihook.getColumnValue(row_cur,'ITEM_ID') || 'v' || ihook.getColumnValue(row_cur,'VER_NR');
    end loop;

--raise_application_error(-20000, v_tab_item_id.count);
    -- populating val means/perm vals
    rows := t_rows();
   --   row_cur_1 := hookInput.originalRowset.Rowset(1);
    --    row_cur_2 := hookInput.originalRowset.Rowset(2);
    ...
    for i in 1 .. v_tab_item_id.count loop
        ...
        for j in 1 .. hookInput.originalRowset.Rowset.count loop
           row_cur := hookInput.originalRowset.Rowset(j);
             v_nm := '';
           for rec in   (select     me.ITEM_PHY_OBJ_NM || '.' || MEC_PHY_NM phy_nm from nci_mdl_elmnt_char mec, nci_mdl_elmnt me 
           where me.MDL_ITEM_ID =ihook.getColumnValue(row_cur,'ITEM_ID')
           and me.MDL_ITEM_VER_NR= ihook.getColumnValue(row_cur,'VER_NR') and cde_item_id is not null and cde_item_id = v_tab_item_id(i) and cde_ver_nr = v_tab_item_ver(i)
           and mec.mdl_elmnt_item_id = me.item_id and mec.mdl_elmnt_ver_nr = me.ver_nr) loop 
         --  for rec in ('select ' || v_sql) loop
            v_nm :=  v_nm || '  |  ' || rec.phy_nm ;
            end loop;
         --   ihook.setColumnValue(row, v_tab_model_nm(j), v_nm);
            ihook.setColumnValue(row, v_tab_model_nm(j), substr(v_nm,5));
  /*          v_nm := '';
            for rec in   (select     me.ITEM_PHY_OBJ_NM || '.' || MEC_PHY_NM phy_nm from nci_mdl_elmnt_char mec, nci_mdl_elmnt me 
           where me.MDL_ITEM_ID =ihook.getColumnValue(row_cur_2,'ITEM_ID')
           and me.MDL_ITEM_VER_NR= ihook.getColumnValue(row_cur_2,'VER_NR') and cde_item_id is not null and cde_item_id = v_tab_item_id(i) and cde_ver_nr = v_tab_item_ver(i)
           and mec.mdl_elmnt_item_id = me.item_id and mec.mdl_elmnt_ver_nr = me.ver_nr) loop
            v_nm :=  v_nm || '  |  ' || rec.phy_nm ;
            end loop;
         --   ihook.setColumnValue(row, v_tab_model_nm(j), v_nm);
            ihook.setColumnValue(row, v_tab_model_nm(2), substr(v_nm,5));
            */
...
if (rows.count > 0) then
    ...
    showRowset := t_showableRowset(rows, 'Model Comparison',4, 'unselectable');
    hookOutput.showRowset := showRowset;
hookoutput.message := 'Model Comparison';
...
as
    ...
    type t_item_ver is table of admin_item.ver_nr%type;
    type t_item_nm is table of admin_item.item_nm%type;
    type t_result_nm is table of varchar2(4000);

    v_tab_item_id  t_item_id:= t_item_id();
    v_tab_item_ver  t_item_ver:= t_item_ver();
    v_tab_item_nm  t_item_nm:= t_item_nm();
   v_tab_nm  t_item_nm:= t_item_nm();
   v_tab_context_nm t_item_nm := t_item_nm();
  
    v_cde_item_id t_item_id;
    v_cde_ver_nr  t_item_ver;
    v_cde_item_nm t_item_nm;
   v_cde_cntxt_nm t_item_nm;
   v_result_nm t_result_nm;
...
begin
    ...
    row_ori := hookInput.originalRowset.rowset (1);
  -- AI Type is based on first row selected
    v_ai_typ :=  ihook.getColumnValue(row_ori,'ADMIN_ITEM_TYP_ID');
    ...
    for i in 1 .. hookInput.originalRowset.Rowset.count loop
        ...
        EXECUTE IMMEDIATE
         'select distinct cde_item_id, cde_ver_nr, ai.item_nm, ai.cntxt_nm_dn from nci_mdl_elmnt_char mec, nci_mdl_elmnt me, admin_item ai
        where me.MDL_ITEM_ID = ' || ihook.getColumnValue(row_cur,'ITEM_ID')
        || ' and me.MDL_ITEM_VER_NR= ' || ihook.getColumnValue(row_cur,'VER_NR') || ' and me.item_id = mec.MDL_ELMNT_ITEM_ID and me.ver_nr =mec.MDL_ELMNT_VER_NR 
        and mec.cde_item_id = ai.item_id and mec.cde_ver_nr = ai.ver_nr '
        BULK COLLECT INTO v_cde_item_id, v_cde_ver_nr,v_cde_item_nm, v_cde_cntxt_nm;

        when 54 then-- Form

        EXECUTE IMMEDIATE
         'select distinct de_item_id, de_ver_nr, DE_ITEM_NM, cntxt_nm_dn from VW_NCI_MODULE_DE
        where FRM_ITEM_ID = ' || ihook.getColumnValue(row_cur,'ITEM_ID')
        || ' and FRM_VER_NR= ' || ihook.getColumnValue(row_cur,'VER_NR')
        BULK COLLECT INTO v_cde_item_id, v_cde_ver_nr,v_cde_item_nm, v_cde_cntxt_nm;
        ...
        when 50 then -- protocol
         EXECUTE IMMEDIATE
         'select distinct de_item_id, de_ver_nr, DE_ITEM_NM, cntxt_nm_dn from VW_NCI_MODULE_DE v, nci_admin_item_rel r
        where r.P_ITEM_ID = ' || ihook.getColumnValue(row_cur,'ITEM_ID')
        || ' and r.P_ITEM_VER_NR= ' || ihook.getColumnValue(row_cur,'VER_NR')
        || ' and r.rel_typ_id = 60 and r.c_item_id = v.FRM_ITEM_ID and r.c_item_ver_nr = v.FRM_VER_NR '
        BULK COLLECT INTO v_cde_item_id, v_cde_ver_nr,v_cde_item_nm, v_cde_cntxt_nm;
        end case; 
        for indx in 1..v_cde_item_id.count loop
          v_found := false;
            for j in 1..v_tab_item_nm.count loop
                    v_found := v_found or v_tab_item_nm(j)=v_cde_item_id(indx) || 'v' || v_cde_ver_nr(indx) || ':' || v_CDE_item_nm(indx);
            end loop;
            if not v_found then
               v_tab_item_nm.extend();
               v_tab_item_id.extend();
               ...
               v_tab_context_nm.extend();
                  ...
                  v_tab_item_nm(v_tab_item_nm.count) := v_cde_item_id(indx) || 'v' || v_cde_ver_nr(indx) || ':' || v_cde_item_nm(indx);
                v_tab_context_nm(v_tab_context_nm.count) := v_cde_cntxt_nm(indx);
               -- v_tab_item_nm(v_tab_item_nm.count) := rec.cde_item_id || 'v' || rec.cde_ver_nr ;
               v_tab_item_id(v_tab_item_id.count) := v_cde_item_id(indx);
               v_tab_item_ver(v_tab_item_ver.count) := v_cde_ver_nr(indx);
...
-- last column header: matched CDE id + cde item nm + associated VD id + vd item name
        --v_tab_admin_item_nm(v_tab_admin_item_nm.count) := 'CDE:' || ihook.getColumnValue(row_cur,'ITEM_ID') || '-' || nvl(ihook.getColumnValue(row_cur,'ITEM_NM'), 'test mn') || chr(13) || 'VD:' ||  vd_id_nm;
        v_tab_nm(v_tab_nm.count) :=  ihook.getColumnValue(row_cur,'ITEM_NM')||': ' || ihook.getColumnValue(row_cur,'ITEM_ID') || 'v' || ihook.getColumnValue(row_cur,'VER_NR');
    end loop;
...
--raise_application_error(-20000, v_tab_item_id.count);
   ...
   --   row_cur_1 := hookInput.originalRowset.Rowset(1);
    ...
    for i in 1 .. v_tab_item_id.count loop
        ...
        EXECUTE IMMEDIATE
         'select     me.ITEM_PHY_OBJ_NM || ''.'' || MEC_PHY_NM result_nm from nci_mdl_elmnt_char mec, nci_mdl_elmnt me 
        where me.MDL_ITEM_ID = ' || ihook.getColumnValue(row_cur,'ITEM_ID')
        || ' and me.MDL_ITEM_VER_NR= ' || ihook.getColumnValue(row_cur,'VER_NR') || ' and cde_item_id is not null and cde_item_id = ' || v_tab_item_id(i) || 
        ' and cde_ver_nr = ' || v_tab_item_ver(i) || 
        '    and mec.mdl_elmnt_item_id = me.item_id and mec.mdl_elmnt_ver_nr = me.ver_nr '
         BULK COLLECT INTO v_result_nm;

        when 54 then-- Form

        EXECUTE IMMEDIATE
         'select 	MOD_ITEM_NM from VW_NCI_MODULE_DE
        where FRM_ITEM_ID = ' || ihook.getColumnValue(row_cur,'ITEM_ID')
        || ' and FRM_VER_NR= ' || ihook.getColumnValue(row_cur,'VER_NR') 
        || ' and de_item_id = ' || v_tab_item_id(i) || 
        ...
        EXECUTE IMMEDIATE
         'select 	distinct FRM_ITEM_NM from VW_NCI_MODULE_DE   v, nci_admin_item_rel r
        where r.P_ITEM_ID = ' || ihook.getColumnValue(row_cur,'ITEM_ID')
        || ' and r.P_ITEM_VER_NR= ' || ihook.getColumnValue(row_cur,'VER_NR')
        || ' and r.rel_typ_id = 60 and r.c_item_id = v.FRM_ITEM_ID and r.c_item_ver_nr = v.FRM_VER_NR '
        || ' and de_item_id = ' || v_tab_item_id(i) || 
       ...
       for indx in 1..v_result_nm.count loop
            v_nm :=  v_nm || '  |  ' || v_result_nm(indx) ;
            end loop;
         --   ihook.setColumnValue(row, v_tab_model_nm(j), v_nm);
            ihook.setColumnValue(row, v_tab_nm(j), substr(v_nm,5));
...
if (rows.count > 0) then
    --showRowset := t_showableRowset(rows, 'Permissible Value Comparison',4, 'unselectable');
    showRowset := t_showableRowset(rows, 'CDE Comparison',4, 'unselectable');
    hookOutput.showRowset := showRowset;
hookoutput.message := 'Model Comparison';
