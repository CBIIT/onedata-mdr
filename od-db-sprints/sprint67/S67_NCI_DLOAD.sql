...
if (v_err_str is not null) then
    ...
    if (ihook.getColumnValue(rowhdr, 'DLOAD_FMT_ID' ) in (90, 110, 223, 224, 230, 245, 246)) then
...
begin
    ...
    v_fmt_id := ihook.getColumnValue(row_ori, 'DLOAD_TYP_ID');
    if (hookinput.invocationnumber = 0) then
        if (v_usr_typ = 'L') then
     ...
     if (hookinput.invocationnumber = 1)  then
       forms              := hookInput.forms;
        ...
        if (ihook.getColumnValue(row_ori,'DLOAD_TYP_ID') = 92) then
            v_item_typ_id := 54;
        elsif (ihook.getColumnValue(row_ori, 'DLOAD_TYP_ID') in (229, 244)) then
            v_item_typ_id := 58;
