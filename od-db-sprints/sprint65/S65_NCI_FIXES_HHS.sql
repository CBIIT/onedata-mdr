...
update admin_item set item_desc = replace(item_desc,trim(v_str),'') where
(item_id, ver_nr) in (select c_item_id, c_item_ver_nr from nci_admin_item_rel where 
 p_item_id = 15910169)
and admin_item_typ_id = 4;
commit;
end;
...
-- RT
for cur in (select item_id, ver_nr from admin_item where admin_item_typ_id in (7) 
and (item_id,ver_nr) in (select item_id, ver_nr from cncpt_admin_item ac, temp_concept c where c.cncpt_item_id = ac.cncpt_item_id 
and c.cncpt_ver_nr = ac.cncpt_ver_nr)) loop
v_desc := '';
v_nm := '';

for cur1 in (select c.item_desc, c.item_nm from vw_cncpt c, cncpt_admin_item ac where c.item_id = ac.cncpt_item_id and c.ver_nr = ac.cncpt_ver_nr and c.item_id = cur.item_id
and c.ver_nr = cur.ver_nr and nvl(nci_prmry_ind,0) = 0 order by nci_ord desc) loop
v_desc := v_desc || '_' || cur1.item_desc;
v_nm := v_nm || ' ' || cur1.item_nm;
end loop;

update admin_item set item_desc = substr(v_desc,2), item_nm = v_nm where item_id = cur.item_id and ver_nr = cur.ver_nr;
end loop;
commit;
*/
end;
