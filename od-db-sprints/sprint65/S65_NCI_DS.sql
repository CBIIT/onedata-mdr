...
procedure spDSMatchEnum ( v_hdr_id in number, v_usr_id in varchar2,v_flt_str in varchar2, v_mtch_typ in varchar2, v_mtch_nm in varchar2, v_suffix in varchar2, v_pv_flt_str in varchar2);
procedure spPostCDEMatch (v_data_in in clob, v_data_out out clob, v_user_id in varchar2);
procedure spDSRunPV(v_data_in in clob, v_data_out out clob, v_user_id in varchar2, v_pv_ind varchar2);
...
---procedure spDSMatchNonEnum ( v_hdr_id in number, v_usr_id in varchar2,v_flt_str in varchar2, v_mtch_typ in varchar2, v_mtch_nm in varchar2);
            spDSMatchEnum(v_hdr_id, v_user_id, v_flt_str,'CDE_MATCH', nvl(ihook.getColumnValue(row_ori,'ENTTY_NM_USR'), ihook.getColumnValue(row_ori, 'ENTTY_NM')),
              v_suffix, getPVFilterString(v_hdr_id));
              
                select count(*) into v_temp from nci_ds_hdr where hdr_id = v_hdr_id and cde_item_id is not null and cde_ver_nr is not null;
                if (v_temp > 0) then
                    select num_pv_mtch into v_pv_mtch from nci_ds_rslt r, nci_ds_hdr d where r.item_id = d.cde_item_id and r.ver_nr = d.cde_ver_nr and d.hdr_id = v_hdr_id and d.hdr_id = r.hdr_id;
                    select count(*) into v_temp from nci_ds_dtl where hdr_id = v_hdr_id and excl_pv_ind = 0;
                    v_sys_msg := 'Source PVs ' || v_temp || ' - Matched PVs ' || v_pv_mtch;
                else
                    select count(*) into v_temp from nci_ds_dtl where hdr_id = v_hdr_id and excl_pv_ind = 0;
                    v_sys_msg := '';
                end if;
                ...
                update nci_ds_hdr set NUM_CDE_MTCH = (select count(*) from nci_ds_rslt where hdr_id = v_hdr_id and mtch_typ = 'CDE'),
                    NUM_PV = v_temp, DYN_PV_SEL = v_pv, SYS_MSG = v_sys_msg
                    where hdr_id = v_hdr_id;
                update nci_ds_rslt r set NUM_PV_IN_CDE = (select count(*) from vw_nci_de_pv_lean where de_item_id = r.item_id and de_ver_nr = r.ver_nr)
                    where hdr_id = v_hdr_id;
                    commit;
  ...
  if (v_temp = 1) then
  --raise_application_error(-20000, 'here');
    select rule_desc, item_id, ver_nr, nvl(num_pv_mtch,0) into v_str, tmp_item_id, tmp_ver_nr, v_temp from nci_ds_rslt where hdr_id = v_hdr_id;
    if (v_str like '%Exact Match%') then
        ...
        if (v_val_dom_typ = 17) then
           ...
           -- select num_pv_mtch into v_pv_mtch from nci_ds_rslt r, nci_ds_hdr d where r.item_id = d.cde_item_id and r.ver_nr = d.cde_ver_nr and d.hdr_id = v_hdr_id and d.hdr_id = r.hdr_id;
            v_sys_msg := v_sys_msg || '. Source PVs ' || v_num_src_pv || ' - Matched PVs ' || v_temp;
        end if;
        update nci_ds_hdr set CDE_ITEM_ID = tmp_item_id, CDE_VER_NR = tmp_ver_nr, SYS_MSG = v_sys_msg where hdr_id = v_hdr_id;
 ...
 -- v_sys_msg := v_sys_msg || '<BR>' || 'Last run: CDE Match';
  v_sys_msg := 'CDE Match executed successfully. <BR>' || v_sys_msg;
...
begin
  ...
  commit;
  v_sys_msg := 'DEC Match executed successfully. <BR>';
update nci_ds_hdr set SYS_MSG = v_sys_msg where hdr_id = v_hdr_id;
...
AS
    ...
    tmp_item_id number;
    tmp_ver_nr number(4,2);
    v_usr_cmnts varchar2(1000);
    v_temp integer;
    v_pv_out_str varchar2(1000);
begin
    hookinput := ihook.gethookinput (v_data_in);
    hookoutput.invocationnumber  := hookinput.invocationnumber;
    hookoutput.originalrowset    := hookinput.originalrowset;
...
--    end if;
    for i in 1..hookinput.originalrowset.rowset.count loop
        row_ori := hookInput.originalRowset.rowset(i);
        if (i = 1) then
            v_pv_flt_str := v_pv_flt_str || '''' || upper(ihook.getColumnValue(row_ori, 'PERM_VAL_NM')) || '''';
            v_hdr_id := ihook.getColumnValue(row_ori, 'HDR_ID');
            v_pv_out_str := ihook.getColumnValue(row_ori, 'PERM_VAL_NM');
        else
            v_pv_flt_str := v_pv_flt_str || ', ' || '''' || upper(ihook.getColumnValue(row_ori, 'PERM_VAL_NM')) || '''';
            v_pv_out_str := v_pv_out_str || ', ' || ihook.getColumnValue(row_ori, 'PERM_VAL_NM');
        end if;
    end loop;
    --for ui display
    --for dynamic sql
    if (v_pv_ind = 'E') then
        ...
        v_pv_flt_str := 'in (' || v_pv_flt_str || ')';
    end if;
    --v_pv_flt_str := v_pv_flt_str || ')';
   -- raise_application_error(-20000, v_pv_flt_str);
    
    select entty_nm_usr, usr_cmnts into v_entty_nm, v_usr_cmnts from nci_ds_hdr where hdr_id = v_hdr_id;
    if (v_entty_nm is not null) then
        v_suffix := 'User Tip';
    else
        v_suffix := 'Name';
        select entty_nm into v_entty_nm from nci_ds_hdr where hdr_id = v_hdr_id;
    end if;
    if (v_usr_cmnts is not null) then -- Context restriction
        v_cntxt_str := upper(trim(v_usr_cmnts));
        v_cnt := nci_11179.getwordcountdelim(v_cntxt_str,',');
        v_flt_str := ' upper(de.cntxt_nm_dn) in (';
        for i in 1..v_cnt loop
            if (i = v_cnt) then
 ...
 --   raise_application_error(-20000, v_suffix);
    
    --need v_hdr_id in number, v_usr_id in varchar2,v_flt_str in varchar2, v_mtch_typ in varchar2, v_mtch_nm in varchar2, v_suffix in varchar2
    spDSMatchEnum(v_hdr_id, v_user_id, v_flt_str,'CDE_MATCH', v_entty_nm, v_suffix, v_pv_flt_str);
    
    select count(*) into v_temp from nci_ds_dtl where hdr_id = v_hdr_id;
    v_temp := v_temp - hookinput.originalrowset.rowset.count;
    update nci_ds_hdr set NUM_CDE_MTCH = (select count(*) from nci_ds_rslt where hdr_id = v_hdr_id and mtch_typ = 'CDE'),
    NUM_PV = v_temp, DYN_PV_SEL = v_pv_out_str
    where hdr_id = v_hdr_id;
    update nci_ds_rslt r set NUM_PV_IN_CDE = (select count(*) from vw_nci_de_pv_lean where de_item_id = r.item_id and de_ver_nr = r.ver_nr)
    where hdr_id = v_hdr_id;
...
if (v_temp > 0) then
    for pv in (select perm_val_nm from nci_ds_dtl where hdr_id = v_hdr_id and excl_pv_ind = 1) loop
        if (v_pv_flt_str = '') then
            v_pv_flt_str := '''' || upper(pv.perm_val_nm) || '''' || ',';
        else
            v_pv_flt_str := v_pv_flt_str || '''' || upper(pv.perm_val_nm) || '''' || ',';
        end if;
    end loop;
...
if (v_temp > 0) then
    for pv in (select perm_val_nm from nci_ds_dtl where hdr_id = v_hdr_id and excl_pv_ind = 1) loop
        if (v_pv_flt_str = '') then        
            v_pv_flt_str := pv.perm_val_nm ||  ', ';
        else
            v_pv_flt_str := v_pv_flt_str || pv.perm_val_nm || ', ';
        end if;
    end loop;
    v_pv_flt_str := trim(TRAILING ' ' from v_pv_flt_str);
    v_pv_flt_str := trim(TRAILING ',' from v_pv_flt_str);
...
--jira 3904: all inclusion/exclusion of user select pvs
...
if  (sql%rowcount = 0) then
    v_sql := 'insert into nci_ds_rslt ( HDR_ID,ITEM_ID,VER_NR, NUM_PV_IN_SRC, NUM_PV_MTCH, RULE_DESC, MTCH_TYP) select ' || cur.hdr_id || ', de_item_id item_id, de_ver_nr ver_nr, ' || cur.cnt || ', count(*), dtl.rule_Desc, ' 
            || '''' || 'CDE' || '''' || ' from vw_nci_de_pv_lean v, admin_item ai, nci_ds_rslt_dtl dtl where nvl(ai.currnt_ver_ind,0) = 1 and v.de_item_id = ai.item_id and v.de_ver_nr = ai.ver_nr and ' ||
            'dtl.hdr_id = ' || cur.hdr_id || ' and dtl.item_id = de_item_id and dtl.ver_nr = de_ver_nr and upper(v.item_nm) in (select upper(perm_val_nm) from nci_ds_dtl where hdr_id = ' || cur.hdr_id || ' and ' ||
            v_pv_filter || ') ' ||
            'group by de_item_id, de_ver_nr, dtl.rule_Desc having count(*) = ' || cur.cnt;
    execute immediate v_sql;
end if;
...
row := t_row();
            ...
            ihook.setColumnValue(row, 'SYS_MSG', 'Preferred CDE updated. Source PVs ' || ihook.getColumnValue(row_ori, 'NUM_PV_IN_SRC') || ' - Matched PVs ' || ihook.getColumnValue(row_ori, 'NUM_PV_MTCH'));
         --   raise_application_error(-20000, ihook.getColumnValue(row_ori, 'NUM_PV_MTCH'));
            rows:= t_rows();
            rows.extend;
            rows(rows.last) := row;
