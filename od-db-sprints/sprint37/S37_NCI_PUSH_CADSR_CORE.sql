...
commit;
delete from sbrext.protocol_qc_ext where qc_idseq = cur.nci_idseq;
commit;

delete from sbrext.qc_recs_ext where p_qc_idseq = cur.nci_idseq or c_qc_idseq = cur.nci_idseq;
commit;

delete from quest_contents_ext where dn_crf_idseq = cur.nci_idseq;
commit;
...
-- Deleted blank Question and Valid Value instructions
...
and ai.lst_upd_dt >= sysdate -vHours/24 and admin_item_typ_id <> 8 and nvl(ai.fld_delete,0) = 0 order by ok.disp_ord, ai.creat_dt) loop
...
(select  ai.ADMIN_STUS_NM_DN, ai.EFF_DT, ai.chng_desc_txt, c.nci_idseq, ai.UNTL_DT, decode(ai.CURRNT_VER_IND,1,'Yes',0,'No'),
            ai.ITEM_NM, nvl(ai.ORIGIN_ID_DN, ai.ORIGIN),nci_cadsr_push.getShortDef(ai.ITEM_DESC),ai.ITEM_LONG_Nm,  ai.def_src,
...
select  ai.NCI_IDSEQ, ai.ADMIN_STUS_NM_DN,ai.EFF_DT, ai.chng_desc_txt, c.nci_idseq,ai.UNTL_DT, decode(ai.CURRNT_VER_IND,1,'Yes',0,'No'),
            ai.ITEM_NM, nvl(ai.ORIGIN_ID_DN, ai.ORIGIN),nci_cadsr_push.getShortDef(ai.ITEM_DESC),ai.ITEM_LONG_Nm,ai.ITEM_ID, ai.NCI_IDSEQ,ai.VER_NR,ai.def_src,
...
update sbr.administered_components set ( asl_name,begin_date,change_note,conte_idseq,end_date,latest_version_ind,
            ...
            date_modified, modified_by, deleted_ind) =
(select  ai.ADMIN_STUS_NM_DN,ai.EFF_DT, ai.chng_desc_txt, c.nci_idseq,ai.UNTL_DT, decode(ai.CURRNT_VER_IND,1,'Yes',0,'No'),
            ai.ITEM_NM, nvl(ai.ORIGIN_ID_DN, ai.ORIGIN),nci_cadsr_push.getShortDef (ai.ITEM_DESC),ai.ITEM_LONG_NM,
            ai.LST_UPD_DT,ai.LST_UPD_USR_ID, decode(nvl(ai.fld_delete,0),0,'No',1,'Yes')
    from admin_item ai, admin_item c
    where ai.cntxt_item_id = c.item_id and ai.cntxt_ver_nr = c.ver_nr
        and ai.nci_idseq = vIdseq)
...
select  ai.NCI_IDSEQ, ai.ADMIN_STUS_NM_DN,ai.EFF_DT, ai.chng_desc_txt, c.nci_idseq,ai.UNTL_DT, decode(ai.CURRNT_VER_IND,1,'Yes',0,'No'),
            ai.ITEM_NM, nvl(ai.ORIGIN_ID_DN, ai.ORIGIN),nci_cadsr_push.getShortDef (ai.ITEM_DESC),ai.ITEM_LONG_NM,ai.ITEM_ID,  ai.VER_NR,
...
(select  ai.ADMIN_STUS_NM_DN,ai.EFF_DT, ai.chng_desc_txt, c.nci_idseq,ai.UNTL_DT, decode(ai.CURRNT_VER_IND,1,'Yes',0,'No'),
            ai.ITEM_NM, nvl(ai.ORIGIN_ID_DN, ai.ORIGIN),nci_cadsr_push.getShortDef(ai.ITEM_DESC),ai.ITEM_LONG_NM,   dec.nci_idseq, vd.nci_idseq,
...
select  ai.NCI_IDSEQ, ai.ADMIN_STUS_NM_DN,ai.EFF_DT, ai.chng_desc_txt, c.nci_idseq,ai.UNTL_DT, decode(ai.CURRNT_VER_IND,1,'Yes',0,'No'),
            ai.ITEM_NM, nvl(ai.ORIGIN_ID_DN, ai.ORIGIN),nci_cadsr_push.getShortDef(ai.ITEM_DESC),ai.ITEM_LONG_NM,ai.ITEM_ID, ai.VER_NR, dec.nci_idseq, vd.nci_idseq,
...
update  sbr.data_element_concepts  set ( asl_name,begin_date,change_note,conte_idseq,end_date,latest_version_ind,
            ...
            date_modified, modified_by, deleted_ind) =
(select  ai.ADMIN_STUS_NM_DN,ai.EFF_DT, ai.chng_desc_txt, c.nci_idseq,ai.UNTL_DT, decode(ai.CURRNT_VER_IND,1,'Yes',0,'No'),
            ai.ITEM_NM, nvl(ai.ORIGIN_ID_DN, ai.ORIGIN),nci_cadsr_push.getShortDef(ai.ITEM_DESC),ai.ITEM_LONG_NM, oc.nci_idseq, prop.nci_idseq, cd.nci_idseq ,
            ai.LST_UPD_DT,ai.LST_UPD_USR_ID,decode(nvl(ai.fld_delete,0),0,'No',1,'Yes')
...
insert into sbr.data_element_concepts (dec_idseq, asl_name,begin_date,change_note,conte_idseq,end_date,latest_version_ind,
            ...
            created_by, date_created,date_modified, modified_by, deleted_ind)
select  ai.NCI_IDSEQ, ai.ADMIN_STUS_NM_DN,ai.EFF_DT, ai.chng_desc_txt, c.nci_idseq,ai.UNTL_DT, decode(ai.CURRNT_VER_IND,1,'Yes',0,'No'),
            ai.ITEM_NM, nvl(ai.ORIGIN_ID_DN, ai.ORIGIN),nci_cadsr_push.getShortDef(ai.ITEM_DESC),ai.ITEM_LONG_NM,ai.ITEM_ID, oc.nci_idseq, prop.nci_idseq, cd.nci_idseq,ai.VER_NR,
            ai.CREAT_USR_ID, ai.CREAT_DT, ai.LST_UPD_DT,ai.LST_UPD_USR_ID ,decode(nvl(ai.fld_delete,0),0,'No',1,'Yes')
        from admin_item ai, admin_item c, de_conc dec, admin_item oc, admin_item prop, admin_item cd
        where ai.cntxt_item_id = c.item_id and ai.cntxt_ver_nr = c.ver_nr
            and ai.nci_idseq = vIdseq
...
(select  ai.ADMIN_STUS_NM_DN,ai.EFF_DT, ai.chng_desc_txt, c.nci_idseq,ai.UNTL_DT, decode(ai.CURRNT_VER_IND,1,'Yes',0,'No'),
            ai.ITEM_NM, nvl(ai.ORIGIN_ID_DN, ai.ORIGIN),nci_cadsr_push.getShortDef(ai.ITEM_DESC),ai.ITEM_LONG_Nm, ok.NCI_CD,  ai.def_src,
...
insert into sbr.concepts_ext (con_idseq, asl_name,begin_date,change_note,conte_idseq,end_date,latest_version_ind,
            ...
            created_by, date_created,date_modified, modified_by, deleted_ind)
select  ai.NCI_IDSEQ, ai.ADMIN_STUS_NM_DN,ai.EFF_DT, ai.chng_desc_txt, c.nci_idseq,ai.UNTL_DT, decode(ai.CURRNT_VER_IND,1,'Yes',0,'No'),
            ai.ITEM_NM, nvl(ai.ORIGIN_ID_DN, ai.ORIGIN),nci_cadsr_push.getShortDef(ai.ITEM_DESC),ai.ITEM_LONG_Nm,ai.ITEM_ID, ok.nci_cd, ai.def_src,ai.VER_NR,
...
update sbr.value_domains set ( asl_name,begin_date,change_note,conte_idseq,end_date,latest_version_ind,
                ...
                date_modified, modified_by, deleted_ind) =
(select  ai.ADMIN_STUS_NM_DN,ai.EFF_DT, ai.chng_desc_txt, c.nci_idseq,ai.UNTL_DT, decode(ai.CURRNT_VER_IND,1,'Yes',0,'No'),
                ai.ITEM_NM, nvl(ai.ORIGIN_ID_DN, ai.ORIGIN),nci_cadsr_push.getShortDef(ai.ITEM_DESC),ai.ITEM_LONG_NM,  VAL_DOM_HIGH_VAL_NUM,VAL_DOM_LOW_VAL_NUM,VAL_DOM_MAX_CHAR,VAL_DOM_MIN_CHAR,
...
insert into sbr.value_domains (vd_idseq, asl_name,begin_date,change_note,conte_idseq,end_date,latest_version_ind,
            ...
            created_by, date_created,date_modified, modified_by, deleted_ind)
select  ai.NCI_IDSEQ, ai.ADMIN_STUS_NM_DN,ai.EFF_DT, ai.chng_desc_txt, c.nci_idseq,ai.UNTL_DT, decode(ai.CURRNT_VER_IND,1,'Yes',0,'No'),
            ai.ITEM_NM, nvl(ai.ORIGIN_ID_DN, ai.ORIGIN),nci_cadsr_push.getShortDef(ai.ITEM_DESC),ai.ITEM_LONG_NM,ai.ITEM_ID,
...
update sbr.conceptual_domains set ( asl_name,begin_date,change_note,conte_idseq,end_date,latest_version_ind,
            ...
            date_modified, modified_by, deleted_ind) =
(select  ai.ADMIN_STUS_NM_DN,ai.EFF_DT, ai.chng_desc_txt, c.nci_idseq,ai.UNTL_DT, decode(ai.CURRNT_VER_IND,1,'Yes',0,'No'),
            ai.ITEM_NM, nvl(ai.ORIGIN_ID_DN, ai.ORIGIN),nci_cadsr_push.getShortDef (ai.ITEM_DESC),ai.ITEM_LONG_Nm, DIMNSNLTY,
            ai.LST_UPD_DT,ai.LST_UPD_USR_ID ,decode(nvl(ai.fld_delete,0),0,'No',1,'Yes')
...
insert into sbr.conceptual_domains (cd_idseq, asl_name,begin_date,change_note,conte_idseq,end_date,latest_version_ind,
            ...
            created_by, date_created,date_modified, modified_by, deleted_ind)
select  ai.NCI_IDSEQ, ai.ADMIN_STUS_NM_DN,ai.EFF_DT, ai.chng_desc_txt, c.nci_idseq,ai.UNTL_DT, decode(ai.CURRNT_VER_IND,1,'Yes',0,'No'),
            ai.ITEM_NM, nvl(ai.ORIGIN_ID_DN, ai.ORIGIN),nci_cadsr_push.getShortDef(ai.ITEM_DESC),ai.ITEM_LONG_Nm,ai.ITEM_ID, ai.VER_NR,DIMNSNLTY,
            ai.CREAT_USR_ID, ai.CREAT_DT, ai.LST_UPD_DT,ai.LST_UPD_USR_ID ,decode(nvl(ai.fld_delete,0),0,'No',1,'Yes')
        from admin_item ai, admin_item c, conc_dom cd
        where ai.cntxt_item_id = c.item_id and ai.cntxt_ver_nr = c.ver_nr
        and c.admin_item_typ_id = 8
...
(select  nci_cadsr_push.getShortDef(frm.hdr_instr), frm.LST_UPD_DT,frm.LST_UPD_USR_ID
        ...
        where ai.item_id = frm.item_id and ai.ver_nr = frm.ver_nr
            and ai.nci_idseq= vIdseq)
            where qc_idseq =curtemp.qc_idseq ;
...
(select  ai.item_nm, nci_cadsr_push.getShortDef(frm.hdr_instr), frm.LST_UPD_DT,frm.LST_UPD_USR_ID
        ...
        where ai.item_id = frm.item_id and ai.ver_nr = frm.ver_nr
            and ai.nci_idseq= vIdseq)
            where ac_idseq = curtemp.qc_idseq;
end loop;
end if;
...
update sbrext.protocols_ext set ( asl_name,begin_date,change_note,conte_idseq,end_date,latest_version_ind,
            ...
            date_modified, modified_by, deleted_ind) =
(select  ai.ADMIN_STUS_NM_DN,ai.EFF_DT, ai.chng_desc_txt, c.nci_idseq,ai.UNTL_DT, decode(ai.CURRNT_VER_IND,1,'Yes',0,'No'),
            ai.ITEM_NM, nvl(ai.ORIGIN_ID_DN, ai.ORIGIN),nci_cadsr_push.getShortDef(ai.ITEM_DESC),ai.ITEM_LONG_Nm, CHNG_TYP,	CHNG_NBR,	RVWD_DT,	RVWD_USR_ID, APPRVD_DT,	APPRVD_USR_ID, ok.nci_cd,
            PROTCL_ID,    org.ORG_NM,                    SUBSTR(PROTCL_PHASE, 0, 3),
            ai.LST_UPD_DT,ai.LST_UPD_USR_ID ,decode(nvl(ai.fld_delete,0),0,'No',1,'Yes')
...
insert into sbrext.protocols_ext (proto_idseq, asl_name,begin_date,change_note,conte_idseq,end_date,latest_version_ind,
            ...
            created_by, date_created,date_modified, modified_by, deleted_ind)
select  ai.NCI_IDSEQ, ai.ADMIN_STUS_NM_DN,ai.EFF_DT, ai.chng_desc_txt, c.nci_idseq,ai.UNTL_DT, decode(ai.CURRNT_VER_IND,1,'Yes',0,'No'),
            ai.ITEM_NM, nvl(ai.ORIGIN_ID_DN, ai.ORIGIN),nci_cadsr_push.getShortDef(ai.ITEM_DESC),ai.ITEM_LONG_Nm,ai.ITEM_ID, ok.nci_cd, 	CHNG_TYP,	CHNG_NBR,	RVWD_DT,	RVWD_USR_ID, APPRVD_DT,	APPRVD_USR_ID,ai.VER_NR,
           PROTCL_ID,  org.ORG_NM,  SUBSTR(PROTCL_PHASE, 0, 3),
            ai.CREAT_USR_ID, ai.CREAT_DT, ai.LST_UPD_DT,ai.LST_UPD_USR_ID ,decode(nvl(ai.fld_delete,0),0,'No',1,'Yes')
...
update sbrext.value_meanings set ( asl_name,begin_date,change_note,conte_idseq,end_date,latest_version_ind,
            ...
            date_modified, modified_by, deleted_ind) =
(select  ai.ADMIN_STUS_NM_DN,ai.EFF_DT, ai.chng_desc_txt, c.nci_idseq,ai.UNTL_DT, decode(ai.CURRNT_VER_IND,1,'Yes',0,'No'),
            ai.ITEM_NM, nvl(ai.ORIGIN_ID_DN, ai.ORIGIN),nci_cadsr_push.getShortDef(ai.ITEM_DESC),ai.ITEM_LONG_Nm, nvl(vm_desc_txt, nci_cadsr_push.getShortDef(ai.ITEM_DESC)), vm_cmnts,ai.nci_idseq,ai.DEF_SRC,
            ai.LST_UPD_DT,ai.LST_UPD_USR_ID ,decode(nvl(ai.fld_delete,0),0,'No',1,'Yes')
...
select  ai.NCI_IDSEQ, ai.ADMIN_STUS_NM_DN,ai.EFF_DT, ai.chng_desc_txt, c.nci_idseq,ai.UNTL_DT, decode(ai.CURRNT_VER_IND,1,'Yes',0,'No'),
            ai.ITEM_NM, nvl(ai.ORIGIN_ID_DN, ai.ORIGIN),nci_cadsr_push.getShortDef(ai.ITEM_DESC),ai.ITEM_LONG_Nm,ai.ITEM_ID, nvl(vm_desc_txt, nci_cadsr_push.getShortDef(ai.ITEM_DESC)), vm_cmnts ,ai.VER_NR, ai.nci_idseq,ai.DEF_SRC,
...
(select  ai.ADMIN_STUS_NM_DN,ai.EFF_DT, ai.chng_desc_txt, c.nci_idseq,ai.UNTL_DT, decode(ai.CURRNT_VER_IND,1,'Yes',0,'No'),
            ai.ITEM_NM, nvl(ai.ORIGIN_ID_DN, ai.ORIGIN),nci_cadsr_push.getShortDef(ai.ITEM_DESC),ai.ITEM_LONG_Nm, ai.LST_UPD_DT,ai.LST_UPD_USR_ID,
...
if vActionType = 'U' then
update sbr.cs_items set ( csi_name, asl_name,begin_date,change_note,conte_idseq,end_date,latest_version_ind,
            long_name,origin,preferred_definition,preferred_name,csitl_name, description, comments,
            date_modified, modified_by, deleted_ind) =
(select  ai.item_nm, ai.ADMIN_STUS_NM_DN,ai.EFF_DT, ai.chng_desc_txt, c.nci_idseq,ai.UNTL_DT, decode(ai.CURRNT_VER_IND,1,'Yes',0,'No'),
            ai.ITEM_NM, nvl(ai.ORIGIN_ID_DN, ai.ORIGIN),nci_cadsr_push.getShortDef(ai.ITEM_DESC),ai.ITEM_LONG_Nm, ok.NCI_CD,  csi_desc_txt, csi_cmnts,
            ai.LST_UPD_DT,ai.LST_UPD_USR_ID ,decode(nvl(ai.fld_delete,0),0,'No',1,'Yes')
...
select  ai.NCI_IDSEQ, ai.ITEM_NM, ai.ADMIN_STUS_NM_DN,ai.EFF_DT, ai.chng_desc_txt, c.nci_idseq,ai.UNTL_DT, decode(ai.CURRNT_VER_IND,1,'Yes',0,'No'),
            ai.ITEM_NM, nvl(ai.ORIGIN_ID_DN, ai.ORIGIN),nci_cadsr_push.getShortDef(ai.ITEM_DESC),ai.ITEM_LONG_Nm,ai.ITEM_ID, ok.nci_cd, csi_desc_txt, csi_cmnts,ai.VER_NR,
            ai.CREAT_USR_ID, ai.CREAT_DT, ai.LST_UPD_DT,ai.LST_UPD_USR_ID ,decode(nvl(ai.fld_delete,0),0,'No',1,'Yes')
...
update sbr.classification_schemes set ( asl_name,begin_date,change_note,conte_idseq,end_date,latest_version_ind,
            ...
            date_modified, modified_by, deleted_ind) =
(select  ai.ADMIN_STUS_NM_DN,ai.EFF_DT, ai.chng_desc_txt, c.nci_idseq,ai.UNTL_DT, decode(ai.CURRNT_VER_IND,1,'Yes',0,'No'),
            ai.ITEM_NM, nvl(ai.ORIGIN_ID_DN, ai.ORIGIN),nci_cadsr_push.getShortDef(ai.ITEM_DESC),ai.ITEM_LONG_Nm,  ok.nci_cd, nvl(nci_label_typ_flg, 'A'),
            ai.LST_UPD_DT,ai.LST_UPD_USR_ID ,decode(nvl(ai.fld_delete,0),0,'No',1,'Yes')
...
insert into sbr.classification_schemes (cs_idseq, asl_name,begin_date,change_note,conte_idseq,end_date,latest_version_ind,
            ...
            created_by, date_created,date_modified, modified_by, deleted_ind)
select  ai.NCI_IDSEQ, ai.ADMIN_STUS_NM_DN,ai.EFF_DT, ai.chng_desc_txt, c.nci_idseq,ai.UNTL_DT, decode(ai.CURRNT_VER_IND,1,'Yes',0,'No'),
            ai.ITEM_NM, nvl(ai.ORIGIN_ID_DN, ai.ORIGIN),nci_cadsr_push.getShortDef(ai.ITEM_DESC),ai.ITEM_LONG_Nm,ai.ITEM_ID, ai.VER_NR, ok.nci_cd, nvl(cs.nci_label_typ_flg, 'A'),
            ai.CREAT_USR_ID, ai.CREAT_DT, ai.LST_UPD_DT,ai.LST_UPD_USR_ID ,decode(nvl(ai.fld_delete,0),0,'No',1,'Yes')
        from admin_item ai, admin_item c, clsfctn_schm cs, obj_key ok
        where ai.cntxt_item_id = c.item_id and ai.cntxt_ver_nr = c.ver_nr
        and c.admin_item_typ_id = 8
...
select vCondrIdseq, nvl(max(cai.creat_dt), sysdate),nvl(max(cai.creat_usr_id), 'ONEDATA'), decode(v_cnt,0, 'Simple Concept',1, 'Simple Concept','CONCATENATION') ,  
nvl(listagg(con.item_long_nm,':') within group (order by cai.NCI_ORD desc),'No concepts')
...
(select vCondrIdseq, decode(v_cnt,0, 'Simple Concept',1, 'Simple Concept','CONCATENATION') ,  nvl(listagg(con.item_long_nm,':') within group (order by cai.NCI_ORD desc),'No concepts')
from cncpt_admin_item cai, admin_item con where cai.item_id = vItemId and cai.ver_nr = vVerNr and cai.cncpt_item_id = con.item_id and cai.cncpt_ver_nr = con.ver_nr)
