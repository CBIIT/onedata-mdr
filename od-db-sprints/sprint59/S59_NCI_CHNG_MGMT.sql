...
procedure spCDEValUpdate (rowform in out t_row, v_op in varchar2, actions in out t_actions, v_val_ind in out boolean, v_item_id in number, v_ver_nr in number);
...
procedure spCDEValUpdate (rowform in out t_row, v_op in varchar2, actions in out t_actions, v_val_ind in out boolean, v_item_id in number, v_ver_nr in number) as
c_ver_suffix varchar2(5) := 'v1.00';
    row t_row;
    rows  t_rows;
...
BEGIN
    ...
    if (v_cnt < 1) then
        v_val_ind := false;
        ihook.setColumnValue(rowform, 'CTL_VAL_MSG', 'Invalid CDE Public ID or Version.');
    else
...
*/
 ...
 --       if (v_op = 'V') then  --- check if CDE is a duplicate
        ...
        if (ihook.getColumnValue(rowform, 'CDE_ITEM_LONG_NM') is not null ) then  --- check if CDE short name specified
       -- raise_application_Error(-20000, ihook.getColumnValue(rowform, 'CDE_ITEM_LONG_NM'));
            for cur in (select ai.item_id item_id from admin_item ai, de de
            where ai.item_id = de.item_id and ai.ver_nr = de.ver_nr
            and ai.ITEM_LONG_NM=ihook.getColumnValue(rowform,'CDE_ITEM_LONG_NM')
    ...
    if (v_op = 'C' and v_val_ind = true) then -- update
        row := t_row();
...
--     rows.extend;
--    rows(rows.last) := row;
--    action := t_actionrowset(rows, 'References (for hook insert)', 2,12,'update');
--    actions.extend;
--    actions(actions.last) := action;
--
    ...
    rows := t_rows();
    rows.extend; rows(rows.last) := rowform;
