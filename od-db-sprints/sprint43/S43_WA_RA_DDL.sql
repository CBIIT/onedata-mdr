...
update ADMIN_ITEM set ITEM_RPT_URL = (select PARAM_VAL || '/invoke/downloads.form/printerFriendly?item_id=' || item_id ||  chr(38) || 'version=' || ver_nr 
...
nvl(decode(trim(ADMIN_ITEM.DEF_SRC), 'NCI', '1-NCI', ADMIN_ITEM.DEF_SRC), 'No Def Source') DEF_SRC, e.CNCPT_CONCAT_WITH_INT,
  ...
  CREATE MATERIALIZED VIEW MVW_VAL_MEAN
  AS Select * from admin_item where admin_item_typ_id = 53;
...
ai.NCI_IDSEQ, ai.ADMIN_STUS_NM_DN, ai.CNTXT_NM_DN, ai.REGSTR_STUS_NM_DN, ai.ORIGIN_ID, ai.ORIGIN_ID_DN, c.evs_src_ori,
decode(e.CNCPT_CONCAT,e.cncpt_concat_nm, null, e.cncpt_concat)  cncpt_concat, e.CNCPT_CONCAT_NM, e.CNCPT_CONCAT_DEF
from admin_item ai, nci_admin_item_Ext e, vw_cncpt c where ai.item_id = e.item_id and ai.ver_nr = e.ver_nr  and
			       ai.item_id = c.item_Id (+) and ai.ver_nr = c.ver_nr (+);
			       

--Jira 2244 Reference Doc Import			      
alter table nci_stg_alt_nms disable all triggers;

alter table nci_stg_alt_nms add (
IMP_TYP_NM varchar2(10),
DOC_NM varchar2(255),
DOC_TYP varchar2(32),
DOC_CNTXT varchar2(32),
RD_DO varchar2(255),
RD_URL varchar2(255)
);
		
update nci_stg_alt_nms set imp_typ_nm = 'DES';
