...
BEGIN
 ...
 if hookInput.invocationNumber = 0 then
    ANSWERS                    := T_ANSWERS();
    ANSWER                     := T_ANSWER(1, 1, 'Specify From/To');
    ANSWERS.EXTEND;
    ANSWERS(ANSWERS.LAST) := ANSWER;
...
forms              := hookInput.forms;
   ...
   ihook.setColumnValue(row, 'Level', 'Question Default');
   ihook.setColumnValue(row, 'Sequence', 0);
ihook.setColumnValue(row, 'Default', ihook.getColumnValue(row_sel, 'VAL'));

            rows.extend;
            rows(rows.last) := row;
   for cur in (Select rep_seq  from nci_quest_vv_rep where QUEST_PUB_ID=ihook.getColumNValue(row_ori, 'NCI_PUB_ID')  and quest_ver_nr = ihook.getColumNValue(row_ori, 'NCI_VER_NR')) loop
   row := t_row();
       ...
       row_sel := hookinput.selectedRowset.rowset(i);
        ...
        if (rep = 0) then -- main question default
          ...
          rows0 := t_rows();
           ...
           ihook.setColumnValue(row, 'DEFLT_VAL', ihook.getColumnValue(row_sel,'Default'));
  --   ihook.setColumnValue(row, 'QUEST_REF_ID', ihook.getColumnValue(row_ori,'NCI_PUB_ID'));
    ...
    rows0.extend;
     ...
     -- raise_application_error(-20000,  ihook.getColumnValue(row_ori,'ITEM_ID'));
        ...
        and quest_ver_nr = ihook.getColumnValue(row_ori,'NCI_VER_NR') and rep_seq = rep;
          row := t_row();
            ihook.setColumnValue(row, 'QUEST_VV_REP_ID', v_temp);
            ihook.setColumnValue(row, 'VAL', ihook.getColumnValue(row_sel,'Default'));
            rows.extend;
            rows(rows.last) := row;
...
--raise_application_error(-20000, ihook.getColumnValue(row_sel,'ITEM_ID'));
    ...
    ihook.getColumnValue(row_ori, 'CNTXT_ITEM_ID'),ihook.getColumnValue(row_ori, 'CNTXT_VER_NR'));
