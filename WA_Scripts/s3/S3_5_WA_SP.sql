...
create or replace PROCEDURE spAddToCartGuest
  (
    v_data_in IN CLOB,
    v_data_out OUT CLOB)
AS
...
begin
 ...
 hookOutput.originalRowset := hookInput.originalRowset;
  ...
  v_tab_admin_item_nm(v_tab_admin_item_nm.count) := 'DE:' || ihook.getColumnValue(row_cur,'ITEM_ID') || '-' || ihook.getColumnValue(row_cur,'ITEM_NM') || chr(13) || 'VD:' ||  vd_id_nm ;
end loop;
