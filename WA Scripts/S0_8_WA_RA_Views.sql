...
ADMIN_ITEM_TYP_ID,
  '' CNTXT_AGG,
	...
	CNTXT.NCI_PRG_AREA_ID,
        ADMIN_ITEM.ITEM_NM || 	ADMIN_ITEM.ITEM_LONG_NM || nvl(ref.ref_desc, '')  SEARCH_STR 
from ADMIN_ITEM, 
CNTXT,
 --(SELECT item_id, ver_nr, LISTAGG(item_nm, ',') WITHIN GROUP (ORDER by ITEM_ID) AS CNTXT_AGG
--FROM (select distinct an.item_id,an.ver_nr, ai.item_nm from alt_nms an, admin_item ai where an.cntxt_item_id = ai.item_id ) 
--GROUP BY item_id, ver_nr)  CNTXT_AGG,
...
--and ADMIN_ITEM.ITEM_ID = CNTXT_AGG.ITEM_ID(+)
--and ADMIN_ITEM.VER_NR = CNTXT_AGG.VER_NR(+)
...
-- CUrrent 
 CREATE OR REPLACE FORCE EDITIONABLE VIEW VW_CNTXT as
  SELECT ADMIN_ITEM.ITEM_ID, ADMIN_ITEM.VER_NR, ADMIN_ITEM.ITEM_NM, ADMIN_ITEM.ITEM_LONG_NM, 
ADMIN_ITEM.ITEM_DESC,
ADMIN_ITEM.LST_UPD_DT, ADMIN_ITEM.S2P_TRN_DT, ADMIN_ITEM.LST_DEL_DT, ADMIN_ITEM.FLD_DELETE, 
ADMIN_ITEM.LST_UPD_USR_ID, ADMIN_ITEM.CREAT_USR_ID, 
ADMIN_ITEM.CREAT_DT, cntxt.NCI_PRG_AREA_ID
       FROM ADMIN_ITEM, CNTXT
       WHERE ADMIN_ITEM.ADMIN_ITEM_TYP_ID = 8
and ADMIN_ITEM.ITEM_ID = CNTXT.ITEM_ID
and ADMIN_ITEM.VER_NR = CNTXT.VER_NR;
...
DE.DERV_TYP_ID,
	...
	VALUE_DOM.UOM_ID, 
	decode(VALUE_DOM.VAL_DOM_TYP_ID,17,'Enumerated',18,'Non-enumerated') VAL_DOM_TYP_ID, 
...
CREATE OR REPLACE VIEW VW_NCI_DE_HORT_CNTXT as
  SELECT 
	ADMIN_ITEM.ITEM_ID ITEM_ID, 
	ADMIN_ITEM.VER_NR,
    ADMIN_ITEM.CREAT_USR_ID, 
...
select * from admin_item where admiN_item_typ_id = 53;
...
CREATE OR REPLACE VIEW VW_NCI_ALT_NMS AS
       ...
       (select a.nm_id, listagg(cs.item_long_nm ||  '/' || ai.item_long_nm, chr(10)) within group (order by a.item_id) CSI_ALT_NMS
        from alt_nms a , NCI_CSI_ALT_DEFNMS n, nci_admin_item_rel_alt_key x, admin_item ai , admin_item cs
       where n.nmdef_id = a.nm_id and n.nci_pub_id = x.nci_pub_id and x.c_item_id = ai.item_id 
...
CREATE OR REPLACE VIEW VW_NCI_ALT_DEF AS
      ...
      (select a.def_id, listagg(cs.item_long_nm ||  '/' || ai.item_long_nm, chr(10)) within group (order by a.item_id) CSI_ALT_DEFS
        from alt_def a , NCI_CSI_ALT_DEFNMS n, nci_admin_item_rel_alt_key x, admin_item ai , admin_item cs
       where n.nmdef_id = a.def_id and n.nci_pub_id = x.nci_pub_id and x.c_item_id = ai.item_id 
       and x.c_item_ver_nr = ai.ver_nr and x.cntxt_cs_item_id = cs.item_id and x.cntxt_cs_ver_nr = cs.ver_nr
...
CREATE OR REPLACE VIEW VW_NCI_DE_CNCPT AS
        SELECT  '2. Object Class' ALT_NMS_LVL, DE.ITEM_ID  DE_ITEM_ID, 
		DE.VER_NR DE_VER_NR,
		a.ITEM_ID, a.VER_NR, 
	...
	union
         SELECT '1. Property' ALT_NMS_LVL, DE.ITEM_ID  DE_ITEM_ID, 
		DE.VER_NR DE_VER_NR,
		a.ITEM_ID, a.VER_NR, 
		a.CNCPT_ITEM_ID, a.CNCPT_VER_NR, 
        	a.CREAT_DT, a.CREAT_USR_ID, a.LST_UPD_USR_ID, 
...
as
        SELECT PV.PERM_VAL_BEG_DT, PERM_VAL_END_DT, PERM_VAL_NM, PERM_VAL_DESC_TXT, DE.ITEM_ID  DE_ITEM_ID, 
		DE.VER_NR DE_VER_NR, VM.ITEM_NM, VM.ITEM_LONG_NM, NCI_VAL_MEAN_ITEM_ID, NCI_VAL_MEAN_VER_NR, 
...
select frm.item_long_nm frm_item_long_nm, frm.item_id frm_item_id, frm.ver_nr frm_ver_nr, 
air.p_item_id mod_item_id, air.p_item_ver_nr mod_ver_nr, ai.item_id de_item_id, ai.ver_nr de_ver_nr, air.disp_ord,
 ai.item_nm de_item_nm, ai.item_long_nm de_item_long_nm, ai.item_desc de_item_desc, aim.item_nm MOD_ITEM_NM,
...
from admin_item ai, nci_admin_item_rel_alt_key air, admin_item aim,
admin_item frm, nci_admin_item_rel frm_mod 
where ai.item_id = air.c_item_id and ai.ver_nr = air.c_item_ver_nr and air.rel_typ_id = 63
...
and frm.admin_item_typ_id in ( 54,55) ;
  ...
  ai.item_nm csi_ITEM_NM,cs.item_nm CS_ITEM_NM, cs.item_long_nm CS_ITEM_LONG_NM, cs.ITEM_ID CS_ITEM_ID, cs.VER_NR CS_VER_NR, cs.CNTXT_NM_DN CS_CNTXT_NM, aim.rel_typ_id,
 ai.item_long_nm  csi_item_long_nm, ai.item_desc csi_item_desc,
ai.CREAT_DT, ai.CREAT_USR_ID, ai.LST_UPD_USR_ID, ai.FLD_DELETE, ai.LST_DEL_DT, ai.S2P_TRN_DT, ai.LST_UPD_DT
from admin_item ai, admin_item cs, nci_admin_item_rel_alt_key air, nci_alt_key_admin_item_rel aim 
...
and cs.item_id = air.cntxt_cs_item_id and cs.ver_nr = air.cntxt_cs_ver_nr;


create or replace view vw_NCI_AI_CURRNT as
select * from admin_item where ADMIN_ITEM.CURRNT_VER_IND = '1';
...
UC.CNTCT_SECU_ID, UC.CREAT_DT, UC.CREAT_USR_ID, UC.LST_UPD_USR_ID, UC.FLD_DELETE, UC.LST_DEL_DT, UC.S2P_TRN_DT, UC.LST_UPD_DT
...
CREATE OR REPLACE VIEW VW_DE_CONC (LST_UPD_DT, S2P_TRN_DT, LST_DEL_DT, FLD_DELETE, LST_UPD_USR_ID, CREAT_USR_ID, CREAT_DT, OBJ_CLS_VER_NR, OBJ_CLS_ITEM_ID, PROP_ITEM_ID, PROP_VER_NR, CONC_DOM_ITEM_ID, CONC_DOM_VER_NR, ITEM_ID, VER_NR, ITEM_NM, ITEM_LONG_NM, ITEM_DESC, CNTXT_ITEM_ID, CNTXT_VER_NR, ADMIN_NOTES, CHNG_DESC_TXT, CREATION_DT, EFF_DT, DATA_ID_STR, ADMIN_ITEM_TYP_ID, CURRNT_VER_IND, REGSTR_STUS_ID, ADMIN_STUS_ID, STEWRD_CNTCT_ID, SUBMT_CNTCT_ID, REGISTRR_CNTCT_ID, STEWRD_ORG_ID, SUBMT_ORG_ID, DEC_CNTXT_NM, ALT_KEY, DE_CONC_CNTXT_STUS_VER_NM)  AS
       SELECT DE_CONC.LST_UPD_DT, DE_CONC.S2P_TRN_DT, DE_CONC.LST_DEL_DT, DE_CONC.FLD_DELETE, DE_CONC.LST_UPD_USR_ID, DE_CONC.CREAT_USR_ID, DE_CONC.CREAT_DT, DE_CONC.OBJ_CLS_VER_NR, DE_CONC.OBJ_CLS_ITEM_ID, DE_CONC.PROP_ITEM_ID, DE_CONC.PROP_VER_NR, DE_CONC.CONC_DOM_ITEM_ID, DE_CONC.CONC_DOM_VER_NR, ADMIN_ITEM.ITEM_ID, ADMIN_ITEM.VER_NR, ADMIN_ITEM.ITEM_NM, ADMIN_ITEM.ITEM_LONG_NM, ADMIN_ITEM.ITEM_DESC, ADMIN_ITEM.CNTXT_ITEM_ID, ADMIN_ITEM.CNTXT_VER_NR, ADMIN_ITEM.ADMIN_NOTES, ADMIN_ITEM.CHNG_DESC_TXT, ADMIN_ITEM.CREATION_DT, ADMIN_ITEM.EFF_DT, ADMIN_ITEM.DATA_ID_STR, ADMIN_ITEM.ADMIN_ITEM_TYP_ID, ADMIN_ITEM.CURRNT_VER_IND, ADMIN_ITEM.REGSTR_STUS_ID, ADMIN_ITEM.ADMIN_STUS_ID, ADMIN_ITEM.STEWRD_CNTCT_ID, ADMIN_ITEM.SUBMT_CNTCT_ID, ADMIN_ITEM.REGISTRR_CNTCT_ID, ADMIN_ITEM.STEWRD_ORG_ID, ADMIN_ITEM.SUBMT_ORG_ID, ADMIN_ITEM.ITEM_NM || ' ( ' || CNTXT_AI.ITEM_NM || ' )', ADMIN_ITEM.ALT_KEY,  '[' || nvl(STUS_MSTR.STUS_ACRO,STUS_MSTR.STUS_NM) || '] ' || CNTXT.CNTXT_ACRO || ' : ' ||
...
CREATE OR REPLACE VIEW VW_DATA_ELEM_RPT AS
  SELECT ADMIN_ITEM.ITEM_ID,  ADMIN_ITEM.VER_NR, ADMIN_ITEM.CNTXT_ITEM_ID, ADMIN_ITEM.CNTXT_VER_NR, 
ADMIN_ITEM.ITEM_NM, ADMIN_ITEM.ITEM_DESC, 
VALUE_DOM.DTTYPE_ID, 
VALUE_DOM.VAL_DOM_FMT_ID, 
VALUE_DOM.UOM_ID, VALUE_DOM.NCI_DEC_PREC, VALUE_DOM.NCI_STD_DTTYPE_ID,
 VALUE_DOM.VAL_DOM_MIN_CHAR,  
VALUE_DOM.VAL_DOM_MAX_CHAR,
 VALUE_DOM.VAL_DOM_LOW_VAL_NUM , VALUE_DOM.VAL_DOM_HIGH_VAL_NUM, 
 ADMIN_ITEM.EFF_DT, ADMIN_ITEM.UNTL_DT, ADMIN_ITEM.CREAT_DT, 
ADMIN_ITEM.LST_UPD_DT, ADMIN_ITEM.S2P_TRN_DT, ADMIN_ITEM.LST_DEL_DT, ADMIN_ITEM.FLD_DELETE, ADMIN_ITEM.LST_UPD_USR_ID, ADMIN_ITEM.CREAT_USR_ID, 
ADMIN_ITEM.ADMIN_STUS_NM_DN, ADMIN_ITEM.CURRNT_VER_IND, 
decode(VALUE_DOM.VAL_DOM_TYP_ID,17,'Enumerated',18,'Non-enumerated') VAL_DOM_TYP_ID, 
ADMIN_ITEM.ITEM_LONG_NM, ADMIN_ITEM.REGSTR_STUS_NM_DN, 
VALUE_DOM.NON_ENUM_VAL_DOM_DESC,  ADMIN_ITEM.ORIGIN, 
ADMIN_ITEM.REGSTR_AUTH_ID, 
DE.DERV_DE_IND, DE.DERV_MTHD, DE.DERV_RUL, DE.DERV_TYP_ID,
 VALUE_DOM.ITEM_ID VAL_DOM_ITEM_ID, VALUE_DOM.VER_NR VAL_DOM_VER_NR
...
and DE.VAL_DOM_VER_NR = VALUE_DOM.VER_NR;
  ...
  CREATE OR REPLACE  VIEW VW_PERM_VALUES AS
  SELECT PERM_VAL.PERM_VAL_NM, PERM_VAL.PERM_VAL_DESC_TXT, VAL_MEAN.ITEM_NM VAL_MEAN_DESC, 
VAL_MEAN.ITEM_LONG_NM VAL_MEAN_LONG_DESC, PERM_VAL.PERM_VAL_BEG_DT, PERM_VAL_END_DT, DE.ITEM_ID, DE.VER_NR, DE.CREAT_DT, DE.CREAT_USR_ID, 
DE.LST_UPD_USR_ID, DE.FLD_DELETE, DE.LST_DEL_DT, DE.S2P_TRN_DT, DE.LST_UPD_DT, PERM_VAL.VAL_ID
       FROM PERM_VAL, ADMIN_ITEM VAL_MEAN, VALUE_DOM, DE
       WHERE PERM_VAL.NCI_VAL_MEAN_ITEM_ID = VAL_MEAN.ITEM_ID
AND  PERM_VAL.NCI_VAL_MEAN_VER_NR = VAL_MEAN.VER_NR 
and PERM_VAL.VAL_DOM_ITEM_ID = VALUE_DOM.ITEM_ID
and PERM_VAL.VAL_DOM_VER_NR =  VALUE_DOM.VER_NR
and VALUE_DOM.ITEM_ID = DE.VAL_DOM_ITEM_ID
and VALUE_DOM.VER_NR = DE.VAL_DOM_VER_NR;
