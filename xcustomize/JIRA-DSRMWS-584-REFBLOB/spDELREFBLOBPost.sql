CREATE OR REPLACE PROCEDURE ONEDATA_WA.spUPREFBLOBPost
  (
    v_data_in IN CLOB,
    v_data_out OUT CLOB,
...
BEGIN
  ...
  hookoutput.originalrowset    := hookinput.originalrowset;
    ...
    select count(*) into v_temp from  ref_doc cl,ref ai,onedata_md.vw_usr_row_filter v where
       ...
       and upper(v.USR_ID) = upper(v_usr_id) and v.ACTION_TYP = 'U' ;
    if (v_temp = 0) then 
        select count(*) into v_temp from onedata_md.vw_usr_row_filter v where 
         v.CNTXT_ITEM_ID = 100 and upper(v.USR_ID) = upper(v_usr_id) and v.ACTION_TYP = 'U' ;
    end if;
     if (v_temp = 0) then
     raise_application_error(-20000, 'You are not authorized to update in this context. ');
        return;
     end if;
