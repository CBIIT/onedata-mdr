DROP VIEW ONEDATA_WA.ODCADSR_COUNTS_ALL_OCT24;

CREATE OR REPLACE FORCE VIEW ONEDATA_WA.ODCADSR_COUNTS_ALL_OCT24
...
UNION
select 'Module', 
(select count(*) from onedata_wa.NCI_ADMIN_ITEM_REL  where REL_TYP_ID=61 and nvl(fld_delete,0)=0) WA_CNT,
((select count(*) from onedata_wa.NCI_ADMIN_ITEM_REL where REL_TYP_ID=61 and nvl(fld_delete,0)=0)-(select count(*) from sbrext.quest_contents_ext 
...
GRANT SELECT ON ONEDATA_WA.ODCADSR_COUNTS_ALL_OCT24 TO ONEDATA_RA;

GRANT SELECT ON ONEDATA_WA.ODCADSR_COUNTS_ALL_OCT24 TO ONEDATA_RO;
