--
--
--CREATE OR REPLACE PACKAGE VD_PACK AS
--  function getVDCreateForm (v_rowset1 in t_rowset, v_rowset2 in t_rowset,v_rowset3 in t_rowset) return t_forms;
...
--  procedure spVDCommon ( v_init in t_rowset,  v_op  in varchar2, hookInput in t_hookInput, hookOutput in out t_hookOutput);
...
--END;
--/
CREATE OR REPLACE PACKAGE BODY VD_PACK AS
...
rows := t_rows();    rows.extend;    rows(rows.last) := row;
    ...
    spVDCommon(rowset, 'insert', hookinput, hookoutput);
...
--          rowset := t_rowset(rows, 'AI Creation With Concepts', 1, 'NCI_STG_AI_CNCPT_CREAT');      
 ...
 --          raise_application_error(-20000, 'Here 1');
   ...
   --     raise_application_error(-20000, 'Here');
        ...
        createValAIWithConcept(rowform , 1,7,'V','STRING',actions);
     
    end if;
    
    IF HOOKINPUT.ANSWERID = 2 or Hookinput.answerid = 4 THEN  -- Validate using drop-down
        
               nci_current.createValAIWithConcept(rowform , 1,7,'V','DROP-DOWN',actions);
    ...
    IF HOOKINPUT.ANSWERID in ( 3,4)  and is_valid = true THEN  -- Create
        createValAIWithConcept(rowform , 1,7,'C','DROP-DOWN',actions); -- OC
       -- Property
    
        nci_chng_mgmt.createDEC(rowform, actions, v_item_id);
        hookoutput.message := 'VDCreated Successfully with ID ' || v_item_id ;
