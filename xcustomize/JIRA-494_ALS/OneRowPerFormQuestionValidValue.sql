select FR.ITEM_NM "from Long Name", FR.ITEM_ID "Form  Public ID", FR.VER_NR "Form Version", PROTOCOL.PROTOCOL_AGR_NAME "PROTOCOLS NAMES",
...
DE.ITEM_ID DE_PUB_ID,DE.VER_NR DE_VERSION,DE.ITEM_LONG_NM "CDE Preferred Name",
...
VD.ITEM_id "VD ID",VD.VER_NR "VD Version"
...
(SELECT FORM.ITEM_ID FORM_ITEM_ID,FORM.VER_NR FORM_VER_NR,
 LISTAGG (PROTO.ITEM_NM , '||')  WITHIN GROUP (ORDER BY PROTO.ITEM_ID DESC,PROTO.VER_NR DESC) PROTOCOL_AGR_NAME
 FROM NCI_ADMIN_ITEM_REL PROTO_FORM,NCI_FORM FORM,ADMIN_ITEM PROTO
WHERE PROTO.ADMIN_ITEM_TYP_ID=50
and PROTO_FORM.P_ITEM_ID=PROTO.ITEM_ID
and PROTO_FORM.P_ITEM_VER_NR=PROTO.VER_NR
and PROTO_FORM.C_ITEM_ID=FORM.ITEM_ID
and PROTO.VER_NR = FORM.VER_NR
 GROUP BY FORM.ITEM_ID, FORM.ver_nr ) PROTOCOL
where 
...
and FORM.ITEM_ID in (2076012,3931472)
